...
/* Flag set to nonzero if an @file argument has been supplied to spl.  */
static bool at_file_supplied;

/* Definition of string containing the arguments given to configure.  */
#include "configargs.h"

/* Flag saying to print the command line options understood by spl and its
   sub-processes.  */

static int print_help_list;

/* Flag saying to print the version of spl and its sub-processes.  */
...
/* Operating systems can define OS_CC1_SPEC to provide extra args to cc1 and
   splprop or extra switch-translations.  The OS_CC1_SPEC is appended
   to CC1_SPEC in the initialization of cc1_spec.  */
#ifndef OS_CC1_SPEC
#define OS_CC1_SPEC ""
#endif

/* config.h can define CC1_SPEC to provide extra args to cc1 and splprop
   or extra switch-translations.  */
#ifndef CC1_SPEC
#define CC1_SPEC ""
#endif

/* config.h can define CC1PLUS_SPEC to provide extra args to splprop
   or extra switch-translations.  */
#ifndef CC1PLUS_SPEC
#define CC1PLUS_SPEC ""
#endif
...
/* Have spl do the search for libgcc.a.  */
...
static const char *splprop_spec = CC1PLUS_SPEC;
...
static struct spec_list static_specs[] =
{
  ...
  INIT_STATIC_SPEC ("splprop",			&splprop_spec),
...
execute (void)
{
...

  {
	...
	if (WIFSIGNALED (status))
	  ...
	  {
	    /* For ICEs in cc1, cc1obj, splprop see if it is
	       reproducible or not.  */
	    const char *p;
	    if (flag_report_bug
...
#endif

/* Display the command line switches accepted by spl.  */
static void
display_help (void)
{
  ...
  -no-canonical-prefixes   Do not canonicalize paths when building relative\n\
                           prefixes to other spl components.\n"), stdout);
  ...
  switch (opt_index)
      ...
      else
	{
	  /* The case will need special consideration where on the host
	     ...
	     'spl/fortran/gfortranspec.cc:lang_specific_driver', for example.
		      ...
		      void (*) (void))
{
  ...
  switch (opt_index)
    ...
    case OPT_static_libstdc__:
      /* These are always valid; spl.cc itself understands the first two
		 ...
		 struct cl_decoded_option *decoded_options)
{
  ...
  if (!gcc_exec_prefix)
  ...
  else
    {
	 ...
	 program name.  */
      char *tmp_prefix = concat (gcc_exec_prefix, "spl", NULL);
  ...
  if (gcc_exec_prefix)
    {
      ...
      if (len > (int) sizeof ("/lib/spl/") - 1
	  && (IS_DIR_SEPARATOR (gcc_exec_prefix[len-1])))
	{
	  temp = gcc_exec_prefix + len - sizeof ("/lib/spl/") + 1;
	  if (IS_DIR_SEPARATOR (*temp)
	      && filename_ncmp (temp + 1, "lib", 3) == 0
	      && IS_DIR_SEPARATOR (temp[4])
	      && filename_ncmp (temp + 5, "spl", 3) == 0)
	    len -= sizeof ("/lib/spl/") - 1;
	}
  ...
  /* -dumpdir and -save-temps=* both specify the location of aux/dump
     ...
     # spl -dumpdir f b.c d.c -gsplit-dwarf [-o [dir/]whatever]
     ...
     # spl -dumpdir f -dumpbase g- b.c d.c -gsplit-dwarf -c
     ...
     # spl -dumpdir f -dumpbase g- -save-temps b.c d.c
     ...
     # spl [-dumpdir f] -save-temps=cwd b.c d.c
     ...
     # spl [-dumpdir f] -dumpbase g- -save-temps=cwd b.c d.c
     -> cc1 -dumpdir g- -dumpbase b.c -dumpbase-ext .c # g-b.o
     && cc1 -dumpdir g- -dumpbase d.c -dumpbase-ext .c # g-d.o
     && link

     # spl [-dumpdir f] -dumpbase g- -save-temps=cwd b.c d.c -o dir/h.out
     -> cc1 -dumpdir g- -dumpbase b.c -dumpbase-ext .c # g-b.o
     && cc1 -dumpdir g- -dumpbase d.c -dumpbase-ext .c # g-d.o
     && link -o dir/h.out
     ...
     # spl [-dumpdir f] -save-temps=cwd b.c d.c -o dir/h.out
     -> cc1 -dumpdir h- -dumpbase b.c -dumpbase-ext .c # h-b.o
     && cc1 -dumpdir h- -dumpbase d.c -dumpbase-ext .c # h-d.o
     && link -o dir/h.out

     # spl [-dumpdir f] -save-temps=obj b.c d.c -o dir/h.out
     ...
     # spl [-dumpdir f] -dumpbase g- -save-temps=obj b.c d.c -o dir/h.out
...
do_spec_1 (const char *spec, int inswitch, const char *soft_matched_part)
{
  ...
  while ((c = *p++))
	...
	switch (c = *p++)
	  ...
	  case '2':
	    value = do_spec_1 (splprop_spec, 0, NULL);
	    if (value != 0)
	      return value;
	    break;
...
/* Pass a switch to the current accumulating command
   ...
   the vector of switches spl received, which is `switches'.
...
print_configuration (FILE *file)
{
  ...
  if (! strncmp (version_string, compiler_version, n)
    ...
    fnotice (file, "spl version %s %s\n", version_string,
	     pkgversion_string);
  else
    fnotice (file, "spl driver version %s %sexecuting spl version %s\n",
...
/* Set up to remember the pathname of spl and any options
...
/* Work out the subdirectory to use based on the options. The format of
   ...
   directory. First spl will check the exclusions, if none of the options
   beginning with an exclamation point are present, and all of the other
   options are present, then we will ignore this completely. Passing
   that, spl will consider each multilib_select in turn using the same
...
/* %:find-file spec function.  This function replaces its argument by
    the file found through find_file, that is the -print-file-name spl
    program option. */
static const char *
...
/* %:find-plugindir spec function.  This function replaces its argument
    ...
    is the -print-file-name spl program option. */
static const char *
...
/* Insert backslash before spaces in ORIG (usually a file path), to 
   ...
   as the end of an argument. But in case of -plugin /usr/spl install/xxx.so,
...
/* Restore all state within spl.cc to the initial state, so that the driver
