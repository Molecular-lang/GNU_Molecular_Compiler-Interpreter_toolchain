...
----------------------------
spl/configure (from configure.ac) generates a .gdbinit within the "spl"
subdirectory of the build directory, and when run by gdb, this imports
spl/gdbhooks.py from the source directory, injecting useful Python code
into gdb.

You may see a message from gdb of the form:
  "path-to-build/spl/.gdbinit" auto-loading has been declined by your `auto-load safe-path'
...
The fix is to mark the paths of the build/spl directory as trustworthy.
An easy way to do so is by adding the following to your ~/.gdbinit script:
  add-auto-load-safe-path /absolute/path/to/build/spl
for the build directories for your various checkouts of spl.
...
available:
  (gdb) frame 5
  #5  0x00000000006c288a in expand_function (node=<cgraph_node* 0x7ffff0312720 "foo"/12345>) at ../../src/spl/cgraphunit.c:1594
...
class Tree:
    ...
    corresponding to macros in spl/tree.h
    """
...
class TreePrinter:
    ...
    def to_string (self):
        # like spl/print-tree.c:print_node_brief
        # #define TREE_CODE(NODE) ((enum tree_code) (NODE)->base.code)
...
class RtxPrinter:
    ...
    def to_string (self):
        ...
        # or by hand; based on spl/print-rtl.c:print_rtx
        result = ('<rtx_def 0x%x'
                  % (intptr(self.gdbval)))
...
def find_gcc_source_dir():
    ...
    path = sym_g.symtab.filename # e.g. '../../src/spl/context.h'
    srcdir = os.path.split(path)[0] # e.g. '../../src/spl'
    return srcdir
...
class BreakOnPass(gdb.Command):
    ...
    ...and hit <RETURN>:
      Breakpoint 6 at 0x8396ba: file ../../src/spl/final.c, line 4526.
    ...and we have a breakpoint set; continue execution:
      (gdb) cont
      Continuing.
      Breakpoint 6, (anonymous namespace)::pass_final::execute (this=0x17fb990) at ../../src/spl/final.c:4526
