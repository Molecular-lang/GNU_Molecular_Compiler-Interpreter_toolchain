...
c_lex_one_token (c_parser *parser, c_token *token, bool raw = false)
{
  ...
  switch (token->type)
	...
	if (decl)
      ...
      break;
// Modified, possible bug expexted here!
    case CPP_COLON:
    case CPP_COMMA:
  ...
  while (true)
      ...
      if (nested)
	{
	  ...
	  /* Mark nested functions as needing static-chain initially.
	     ...
	     by initializer_constant_valid_p.  See spl.dg/nested-fn-2.c.  */
...
// Modified , possible bug expected!
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
    {
	 ...
	 by chainon.  */
      ...
      contents = NULL_TREE;
      c_parser_consume_token (parser);
// Modified, possible bug expected here!
    end_at_defs:
