...
/* Memregs are memory locations that spl treats like general
   ...
   order to try to persuade spl to try harder to use real registers.
...
/* Implements TARGET_CAN_CHANGE_MODE_CLASS.  Only r0 and r1 can change to
   ...
   registers (well, it does on a0/a1 but if we let spl do that, reload
...
/* Implements TARGET_LIBCALL_VALUE.  Most values are returned in $r0, or some
   ...
   general registers and spl often can't compile it.  So, instead, we
				...
				int type, int ind_levels ATTRIBUTE_UNUSED)
{
  ...
  /* At one point, this function tried to get $fb copied to an address
     register, which in theory would maximize sharing, but spl was
     *also* still trying to reload the whole address, and we'd run out
     of address registers.  So we let spl do the naive (but safe)
...
/* Here we try to describe when we use multiple opcodes for one RTX so
   that spl knows when to use them.  */
...
/* The m32c has a limited range of operations that work on PSImode
   ...
   PSI.  Yes, this is expensive, but hopefully spl will learn to avoid
   those cases.  */
void
