...
/* Provide a mapping from spl register numbers to dwarf register numbers.  */
unsigned
  ...
  if ((!composite_p
      ...
      if (ag_count > 0 && ag_count <= HA_MAX_NUM_FLDS)
	{
	  ...
	  if (!silent_p
	    ...
	    {
	      const char *url10
		= CHANGES_ROOT_URL "spl-10/changes.html#empty_base";
	      const char *url12
			      ...
			      enum simd_immediate_check which)
{
  ...
  for (unsigned int i = 0; i < n_elts; i++)
    {
      /* The vector is provided in spl endian-neutral fashion.
