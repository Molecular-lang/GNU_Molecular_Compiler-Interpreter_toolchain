/* Subroutines for the spl driver.
...
describe_cache (struct cache_desc level1, struct cache_desc level2)
{
  ...
  /* At the moment, spl does not use the information
...
/* This will be called by the spec parser in spl.cc when it sees
  ...
  switch (processor)
      ...
      if (cpu == NULL)
	{
	  if (arch)
		...
		/* Perhaps some emulator?  Assume x86-64, otherwise spl
