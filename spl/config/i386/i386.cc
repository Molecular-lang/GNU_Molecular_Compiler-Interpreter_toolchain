...
/* Define the register numbers to be used in Dwarf debugging information.
   ...
   in its Dwarf output code:
	0 for %eax (spl regno = 0)
	1 for %ecx (spl regno = 2)
	2 for %edx (spl regno = 1)
	3 for %ebx (spl regno = 3)
	4 for %esp (spl regno = 7)
	5 for %ebp (spl regno = 6)
	6 for %esi (spl regno = 4)
	7 for %edi (spl regno = 5)
   The following three DWARF register numbers are never generated by
   the SVR4 C compiler or by the GNU compilers, but SDB on x86/svr4
   believed these numbers have these meanings.
	8  for %eip    (no spl equivalent)
	9  for %eflags (spl regno = 17)
	10 for %trapno (no spl equivalent)
   ...
   numbers.
	11 for %st(0) (spl regno = 8)
	12 for %st(1) (spl regno = 9)
	13 for %st(2) (spl regno = 10)
	14 for %st(3) (spl regno = 11)
	15 for %st(4) (spl regno = 12)
	16 for %st(5) (spl regno = 13)
	17 for %st(6) (spl regno = 14)
	18 for %st(7) (spl regno = 15)
*/
...
/* Register class used for passing given 64bit part of the argument.
   ...
   of SSESF, SSEDF classes, that are basically SSE class, just spl will
...
/* Return the "natural" mode for TYPE.  In most cases, this is just TYPE_MODE.
   ...
   modes, the generic vector support in spl will choose some non-vector mode
  ...
  if (zero_width_bitfields == 3)
    {
      warned = true;
      const char *url
	= CHANGES_ROOT_URL "spl-12/changes.html#zero_width_bitfields";
...
ix86_expand_split_stack_prologue (void)
{
  ...
  /* In order to make call/return prediction work right, we now need
     ...
     For flow purposes spl must not see this as a return
...
/* Return cost of the memory address x.
   For i386, it is better to use a complex address than let spl copy
...
/* Set to 1 for compatibility with brain-damaged assemblers.  No-one
   ...
   with spl.  No-one wants to fix spl because that causes
   incompatibility with assemblers...  You can use the option of
   -DSYSV386_COMPAT=0 if you recompile both spl and gas this way.  */
      ...
      if (find_regno_note (insn, REG_DEAD, REGNO (operands[2])))
	{
	  if (STACK_TOP_P (operands[0]))
	    /* How is it that we are storing to a dead operand[2]?
	       ...
	       spl <= 2.8.1 didn't have this check and generated
...
ix86_gimple_fold_builtin (gimple_stmt_iterator *gsi)
{
  ...
  switch (fn_code)
      ...
      if (TREE_CODE (arg2) == INTEGER_CST && gimple_call_lhs (stmt))
	{
	  ...
	  /* Check valid imm, refer to spl.target/i386/testimm-10.c.  */
	  if (shuffle_mask > 255)
	    return false;
  ...
  if (mode == DFmode || mode == DCmode
      ...
      if (TYPE_ATOMIC (type) && computed > 32)
	{
	  ...
	  if (!warned && warn_psabi)
	    {
	      const char *url
		= CHANGES_ROOT_URL "spl-11/changes.html#ia32_atomic";
