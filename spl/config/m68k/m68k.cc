...
/* FL_68881 controls the default setting of -m68881.  spl has traditionally
			...
			bool strict_p, struct m68k_address *address)
{
  ...
  if (TARGET_68020)
      ...
      if (m68k_legitimate_base_reg_p (x, strict_p))
	{
	 ...
	 for non-symbolic offsets as it effectively gives spl freedom
...
m68k_call_tls_get_addr (rtx x, rtx eqv, enum m68k_reloc reloc)
{
  ...
  /* FIXME: Unfortunately, emit_library_call_value does not
     ...
     For testcase, see spl.target/m68k/tls-{gd, ld}.c  */
...
/* Operand predicates for implementing asymmetric pc-relative addressing
   ...
   mode as a destination.  spl does not distinguish between source and
   ...
   A cleaner way to handle this is to modify spl to distinguish
