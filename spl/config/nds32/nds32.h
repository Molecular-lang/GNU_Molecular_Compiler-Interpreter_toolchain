...
/* Determine whether we would like to have code generation strictly aligned.
   ...
   Check spl/common/config/nds32/nds32-common.cc for the optimizations that
   apply -malways-align.  */
...
/* The TARGET_BIG_ENDIAN_DEFAULT is defined if we
   configure spl with --target=nds32be-* setting.
   Check spl/config.spl for more information.  */
...
/* We have to undef it first because elfos.h formerly define it
   check spl/config.spl and spl/config/elfos.h for more information.  */
