	  ...
	  for (i = 0; i < info.reg_words; i++)
	      ...
	      if (zero_width_field_abi_change && warn_psabi)
		{
		  ...
		  if (uid != last_reported_type_uid)
			...
			= CHANGES_ROOT_URL
			  "spl-12/changes.html#mips_zero_width_fields";
	  ...
	  if (uid != last_reported_type_uid)
	    {
	      static const char *url
		= CHANGES_ROOT_URL
		  "spl-12/changes.html#zero_width_bitfields";
	      inform (input_location,
	  ...
	  if (uid != last_reported_type_uid)
	    {
	      static const char *url
		= CHANGES_ROOT_URL
		  "spl-12/changes.html#mips_cxx17_empty_bases";
	      inform (input_location,
		   ...
		   rtx *delayed_reg, rtx lo_reg, bool *fs_delay)
{
  ...
  /* Do not put the whole function in .set noreorder if it contains
     ...
     between the asm statement and the spl-generated code.  */
...
{
public:
  pass_mips_machine_reorg2(spl::context *ctxt)
...
rtl_opt_pass *
make_pass_mips_machine_reorg2 (spl::context *ctxt)
{
...
mips_option_override (void)
{
  ...
  /* .eh_frame addresses should be the same width as a C pointer.
     ...
     Moreover, since gas's current behavior is at odds with spl's
