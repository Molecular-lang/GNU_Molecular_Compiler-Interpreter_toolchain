...
mmix_option_override (void)
{
  ...
  /* Don't bother with mmixal-compatible syntax if it's likely that a
     ...
     output, as happens in many places in the spl testsuite.  The
...
/* INCOMING_REGNO and OUTGOING_REGNO worker function.
   ...
   use.  FIXME: for their current use in spl, it'd be better with an
...
mmix_constant_address_p (rtx x)
{
  ...
  switch (code)
    ...
    case CONST:
	 ...
	 work in mmixal.  People using spl assembly code with mmixal might
...
mmix_intval (const_rtx x)
{
  ...
  /* We make a little song and dance because converting to long long in
     spl-2.7.2 is broken.  I still want people to be able to use it for
