...
namespace
{
  ...
  {
  public:
    pass_rl78_devirt (spl::context *ctxt)
...
rtl_opt_pass *
make_pass_rl78_devirt (spl::context *ctxt)
{
...
namespace
{
  ...
  {
  public:
    pass_rl78_move_elim (spl::context *ctxt)
...
rtl_opt_pass *
make_pass_rl78_move_elim (spl::context *ctxt)
{
...
/* Most registers are 8 bits.  Some are 16 bits because, for example,
   spl doesn't like dealing with $FP as a register pair (the second
...
rl78_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
  ...
  if (s < 1)
    return false;
  /* These are not to be used by spl.  */
...
At least through register allocation and reload, spl is told that it
...
rl78_alloc_physical_registers_cmp (rtx_insn * insn)
{
  ...
  /* Surprisingly, spl can generate a comparison of a register with itself, but this
...
{
  /* During most of the compile, spl is dealing with virtual
  ...
  if (priority != DEFAULT_INIT_PRIORITY)
    {
	 ...
	 from: spl/varasm.cc:get_cdtor_priority_section().  */
      char buf[18];
