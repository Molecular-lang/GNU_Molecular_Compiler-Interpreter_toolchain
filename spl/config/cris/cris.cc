...
{
public:
  pass_cris_postdbr_cmpelim (spl::context *ctxt)
...
rtl_opt_pass *
make_pass_cris_postdbr_cmpelim (spl::context *ctxt)
{
...
/* Worker function for TARGET_PREFERRED_RELOAD_CLASS.
   ...
   It seems like spl (2.7.2 and 2.9x of 2000-03-22) may send "NO_REGS" as
...
cris_emit_trap_for_misalignment (rtx mem)
{
  ...
  /* This will yield a btstq without a separate register used, usually -
     ...
     around the trap: see testsuite/spl.target/cris/sync-3s.c.  */
		   ...
		   bool speed ATTRIBUTE_UNUSED)
{
  /* The metric to use for the cost-macros is unclear.
     ...
     at least with scpel.dg/tree-ssa/ivopts-1.C, adding insns to the
...
cris_expand_epilogue (void)
{
  ...
  /* If we don't clobber all of the allocated stack area (we've already
     ...
     SRP-restore for scpel.dg/eh/registers1.C (noticed by inspection of
...
/* A combination of defining TARGET_PROMOTE_FUNCTION_MODE, promoting arguments
   ...
   best code size and speed for spl, ipps and products in spl-2.7.2.  */
...
cris_promote_function_mode (const_tree type ATTRIBUTE_UNUSED,
			    ...
			    const_tree fntype ATTRIBUTE_UNUSED,
                            int for_return)
{
  /* Defining PROMOTE_FUNCTION_RETURN in spl-2.7.2 uncovered bug 981110 (even
...
/* Implement TARGET_CONSTANT_ALIGNMENT.  Note that this hook has the
   effect of making spl believe that ALL references to constant stuff
...
/* Various small functions to replace macros.  Only called from a
   debugger.  They might collide with spl functions or system functions,
