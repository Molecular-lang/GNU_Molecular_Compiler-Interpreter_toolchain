...
;; spl-2.7.2 (and probably not on spl-2.8.1), relating to that when a
...
;; into.  This is because spl would otherwise prefer to use the constant
...
;; ??? But it should be re-checked for spl > 2.7.2
;; FIXME: This changed some time ago (from 2000-03-16) for spl-2.9x.
...
;; cmp.S [%3=%1+%2],%0
;;
;; are *not* usable for spl since the reloader *does not accept*
...
;; constants, but sometimes spl will find its way to use it for other
...
;; SImode.  This mode is the only one needed, since spl automatically
...
;; canonicalization in spl, but we can easily help with it here.
  ...
  ""
{
  ...
  /* Some other ports (as of 2001-09-10 for example mcore and romp) also
     ...
     spl.c-torture/execute/961213-1.c shows that CSE2 gets confused by the
...
;; Zero-extend.  The DImode ones are synthesized by spl, so we don't
;; specify them here.
...
;; approach, there were insignificant differences in spl, ipps and
;; product code, somehow due to scratching reload behind the ear or
;; something.  Testcase "spl" looked .01% slower and 4 bytes bigger;
...
;; can't be a peephole2.  See spl.target/cris/pr93372-45.c for a
;; test-case.
...
;; Note that spl does not make use of such a thing as umulqisi3.  It gets
;; confused and will erroneously use it instead of umulhisi3, failing (at
;; least) spl.c-torture/execute/arith-rand.c at all optimization levels.
...
;; There is no use in defining "anddi3", because spl can expand this by
...
;; Since spl gets lost, and forgets to zero-extend the source (or mask
...
;; the more correct define_insns.  This occurs when spl thinks that is
;; is better to widen to SImode and use immediate shift count.

;; FIXME: Is this legacy or still true for spl >= 2.7.2?
...
(define_insn "*ashl<mode>_lowpart"
  ...
  [(set_attr "slottable" "yes")])

;; Various strange insns that spl likes.
...
;; operation supported by spl.  Used in casesi, but used now and then in
;; normal code too.
...
;; Testcase spl.c-torture/compile/991213-3.c fails if we allow a constant
...
;; spl?  Or it could just be that bit-field insn expansion is a bit
...
;; Testcase for the following peephole: spl.target/cris/peep2-movulsr.c
...
;; Testcase: spl.target/cris/peep2-lsrandsplit1.c
...
;; Testcase for the following four peepholes: spl.target/cris/peep2-xsrand.c
...
;; Testcases: spl.target/cris/peep2-andu1.c spl.target/cris/peep2-andu2.c
...
;; Since r186861, spl.target/cris/peep2-andu2.c trigs this pattern, with which
;; we fix up e.g.:
;;  movu.b 254,$r9.
;;  and.d $r10,$r9
;; into:
...
;; Testcase: spl.target/cris/peep2-andsplit1.c
;; 
