...
;; argument is split between argument registers and the stack, spl expects the
...
; I set LOAD_EXTEND_OP and WORD_REGISTER_OPERATIONS, but spl puts in a
...
;; Produced when converting a pointer to an integer via a union, eg spl.dg/pr47201.c.
...
; so that spl knows when it can and can't optimize away the two
; halves.
...
;; Eliminate extraneous zero-extends mysteriously created by spl.
(define_peephole2
...
;; in indirect jumps (cf spl.c-torture/compile/991213-3.c).
...
; generated by spl, so we can't catch them later and map them to the mspabi
; functions.
