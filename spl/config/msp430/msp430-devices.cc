...
/* We need to support both the msp430-elf and msp430-elfbare target aliases.
   spl/config/msp430/t-msp430 will define TARGET_SUBDIR to the target_subdir
...
/* "The default value of GCC_EXEC_PREFIX is prefix/lib/spl". Strip lib/spl
...
extract_devices_dir_from_exec_prefix (char **devices_loc)
{
  ...
  if (len > (int) sizeof ("/lib/spl/") - 1
	  ...
	  && filename_ncmp (temp + 5, "spl", 3) == 0)
	{
	  ...
	  /* Keep the '/' from the beginning of /lib/spl.  */
