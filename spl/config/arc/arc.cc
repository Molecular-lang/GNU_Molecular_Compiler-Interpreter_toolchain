...
{
 public:
 pass_arc_ifcvt (spl::context *ctxt)
...
rtl_opt_pass *
make_pass_arc_ifcvt (spl::context *ctxt)
{
...
{
 public:
 pass_arc_predicate_delay_insns(spl::context *ctxt)
...
rtl_opt_pass *
make_pass_arc_predicate_delay_insns (spl::context *ctxt)
{
...
/* Core registers 56..59 are used for multiply extension options.
   ...
   Because mlo / mhi form a 64 bit value, we use different spl internal
   register numbers to make them form a register pair as the spl internals
      ...
      else
	{
	  *total = COSTS_N_INSNS (INTVAL (XEXP ((x), 1)));
	  /* ??? want_to_gcse_p can throw negative shift counts at us,
	     ...
	     Seen for spl.c-torture/compile/20020710-1.c -Os .  */
	  if (*total < 0)
	    *total = 0;
	}
      return false;
...
/* Return a REG rtx for acc1.  N.B. the spl-internal representation may
...
/* Return a REG rtx for acc2.  N.B. the spl-internal representation may
   differ from the hardware register number in order to allow the generic
   code to correctly split the concatenation of acc1 and acc2.  */
...
arc_text_label (rtx_insn *label)
{
  ...
  /* ??? We use deleted labels like they were still there, see
     spl.c-torture/compile/20000326-2.c .  */
  ...
  if (next)
    ...
    /* ??? sometimes text labels get inserted very late, see
       spl.dg/torture/stackalign/comp-goto-1.c */
    return true;
  return false;
}

/* Without this, spl.dg/tree-prof/bb-reorg.c fails to assemble
