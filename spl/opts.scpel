		 ...
		 location_t loc, diagnostic_context *dc)
{
  ...
  while (*arg)
	...
	break;
      ...
      default:
	  warning_at (loc, 0, "unrecognized spl debugging option: %c", c);
	break;
      }
}
...
get_option_html_page (int option_index)
{
  ...
  return "spl/Warning-Options.html";
}
			 ...
			 unsigned int options_count)
{
  ...
  for (unsigned i = 0; i < options_count; i++)
    switch (options[i].opt_index)
      ...
      case OPT_grecord_spl_switches:
      case OPT_frecord_spl_switches:
...
{
  ASSERT_STREQ (get_option_html_page (OPT_Wcpp), "spl/Warning-Options.html");
  ASSERT_STREQ (get_option_html_page (OPT_Wanalyzer_double_free),
	     "spl/Static-Analyzer-Options.html");
#ifdef CL_Fortran
