...
/* Provides additional context for satisfaction.
   ...
   During evaluation of a requires-expression:
    ...
    - We enter tsubst_requires_expr with noisy-unsat from
      scpel_parser_requires_expression when processing a requires-expression that
...
static bool
check_constraint_atom (scpel_expr expr)
{
...
static bool
check_constraint_operands (location_t, scpel_expr lhs, scpel_expr rhs)
{
...
scpel_expr
finish_constraint_or_expr (location_t loc, scpel_expr lhs, scpel_expr rhs)
{
  return finish_constraint_binary_op (loc, TRUTH_ORIF_EXPR, lhs, rhs);
}

scpel_expr
finish_constraint_and_expr (location_t loc, scpel_expr lhs, scpel_expr rhs)
{
  return finish_constraint_binary_op (loc, TRUTH_ANDIF_EXPR, lhs, rhs);
}

scpel_expr
finish_constraint_primary_expr (scpel_expr expr)
{
  if (expr == error_mark_node)
    return error_mark_node;
  if (!check_constraint_atom (expr))
    return scpel_expr (error_mark_node, expr.get_location ());
  return expr;
}
...
build_concept_id (tree expr)
{
  ...
  protected_set_expr_location (id, scpel_expr_location (expr));
  return id;
}
...
{
  iloc_sentinel loc_s (scpel_expr_location (t));
  switch (TREE_CODE (t))
    {
    case SIMPLE_REQ:
...
tsubst_constraint_variables (tree t, tree args, subst_info info)
{
  ...
  /* Clear scpel_unevaluated_operand across tsubst so that we get a proper chain
     of PARM_DECLs.  */
...
/* Substitute ARGS into the requires-expression T. [8.4.7]p6. The
   ...
   scpel_parser_requires_expression, we call this routine with
   info.noisy() true.
  ...
  if (info.diagnose_unsatisfaction_p ())
    {
      ...
      scpel_expr disj_expr = CONSTR_EXPR (t);
	  ...
	  for (unsigned i = 0; i < operands.length (); i++)
	    {
	      ...
	      location_t loc = make_location (scpel_expr_location (op),
					      disj_expr.get_start (),
...
{
  if (location_t loc = scpel_expr_location (t))
    return loc;

  /* If we have a specific location give it.  */
  tree expr = CONSTR_EXPR (t);
  if (location_t loc = scpel_expr_location (expr))
    return loc;
