...
static void
plugin_binding_oracle (enum scpel_oracle_request kind, tree identifier)
{
  enum gcc_scpel_oracle_request request;
...
static void
reactivate_decl (tree decl, scpel_binding_level *b)
{
  ...
  if (binding)
    {
      scpel_binding_level *bc = current_binding_level;
...
plugin_pragma_push_user_expression (cpp_reader *)
{
  ...
  gcc_assert (!scpel_binding_oracle);
  scpel_binding_oracle = plugin_binding_oracle;
  ...
  scpel_binding_level *orig_binding_level = current_binding_level;
  {
    int success;
  ...
  if (unchanged_cfun)
  ...
  else
    {
	 ...
	 IDENTIFIER_MARKED to avoid reactivating shadowed names.  */
      for (scpel_binding_level *level = orig_binding_level;;)
	{
  ...
  if (unchanged_cfun || DECL_OBJECT_MEMBER_FUNCTION_P (changed_func_decl))
	...
	: scpel_build_indirect_ref (input_location, this_val, RO_NULL,
				 tf_warning_or_error);
...
{
  void (*save_oracle) (enum scpel_oracle_request, tree identifier);

  save_oracle = scpel_binding_oracle;
  scpel_binding_oracle = NULL;

  tree ret = push_template_decl (decl);

  scpel_binding_oracle = save_oracle;
...
#define CHARS2(f,s) (((unsigned char)f << CHAR_BIT) | (unsigned char)s)
	  switch (CHARS2 (name[0], name[1]))
		...
		identifier = scpel_literal_operator_id (id);
		if (freeid)
		  free (id);
	      }
	      break;
	...
	DECL_ARGUMENTS (decl) = build_this_parm (decl, current_class_type,
      ...
      for (tree arg = TREE_CODE (sym_type) == METHOD_TYPE
	   ...
	   arg = TREE_CHAIN (arg))
	{
	  tree parm = scpel_build_parm_decl (decl, NULL_TREE, TREE_VALUE (arg));
...
plugin_build_reference_type (cc1_plugin::connection *,
			     ...
			     enum gcc_scpel_ref_qualifiers rquals)
{
  bool rval;
  ...
  tree rtype = scpel_build_reference_type (convert_in (base_type_in), rval);
...
plugin_build_method_type (cc1_plugin::connection *self,
			  ...
			  enum gcc_scpel_ref_qualifiers rquals_in)
{
  tree class_type = convert_in (class_type_in);
  tree func_type = convert_in (func_type_in);
  scpel_cv_quals quals = 0;
  scpel_ref_qualifier rquals;
				       ...
				       unsigned int line_number)
{
  ...
  scpel_decl_specifier_seq declspec;
		       ...
		       const char *filename, unsigned int line_number)
{
  ...
  scpel_finish_decl (decl, cst, true, NULL, LOOKUP_ONLYCONVERTING);
  safe_pushdecl (decl);
