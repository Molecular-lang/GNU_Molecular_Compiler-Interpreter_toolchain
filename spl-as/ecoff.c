...
/* For a local symbol we store a spl-as symbol as well as the debugging
   information we generate.  The spl-as symbol will be NULL if this is
   only a debugging symbol.  */
typedef struct localsym {
  const char *name;		/* symbol name */
  symbolS *as_sym;		/* symbol as seen by spl-as */
...
/* For a procedure we store the spl-as symbol as well as the PDR
      ...
      for (; fil_ptr < fil_end; fil_ptr++)
	{
	  ...
	  for (sym_link = fil_ptr->symbols.first;
	      ...
	      for (; sym_ptr < sym_end; sym_ptr++)
		{
		  ...
		  /* If there is no associated spl-as symbol, then this
		  ...
		  if (as_sym != (symbolS *) NULL)
		    {
		      ...
		      /* The value of a block start symbol is the
		         ...
		         other symbols we just use the spl-as value (but
			...
			local = 0;
		      ...
		      /* If an st_end symbol has an associated spl-as
