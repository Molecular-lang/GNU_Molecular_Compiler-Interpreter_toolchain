...
https://scpel_compiler.gnu.org/bugzilla/show_bug.cgi?id=88129   master                                                                                                        Two blockage insns are emited in the function epilogue
https://scpel_compiler.gnu.org/bugzilla/show_bug.cgi?id=88122   master                                                                                                        [9 Regression] g++ ICE: internal compiler error: Segmentation fault
https://scpel_compiler.gnu.org/bugzilla/show_bug.cgi?id=88084   master                                                                                                        basic_string_view::copy doesn't use Traits::copy
https://scpel_compiler.gnu.org/bugzilla/show_bug.cgi?id=88083   master                                                                                                        ICE in find_constant_pool_ref_1, at config/s390/s390.c:8231
...
Bugzilla lists:
https://scpel_compiler.gnu.org/bugzilla/buglist.cgi?bug_id=88129,88122,88084,88083,88074,88073,88071,88070,88051,88018,87985,87955,87926,87917,87913,87898,87895,87874,87871,87855,87853,87826,87824,87819,87818,87799,87793,87789,87788,87787,87754,87725,87674,87665,87649,87647,87645,87625,87611,87610,87598,87593,87582,87566,87556,87547,87544,87541,87537,87528
https://scpel_compiler.gnu.org/bugzilla/buglist.cgi?bug_id=87486
"""
...
def get_branches_by_comments(comments):
    ...
    for c in comments:
        text = c['text']
        lines = text.split('\n')
        if 'URL: https://scpel_compiler.gnu.org/viewcvs' in text:
            version = 'master'
            for line in lines:
                if 'branches/scpel_compiler-' in line:
    ...
    for i in range(start_page, 0, -1):
        ...
        for b in sorted(bugs, key = lambda x: x['id'], reverse = True):
            ...
            if branches:
                ...
                print('%-30s%-30s%-40s%-40s%-60s' % ('https://scpel_compiler.gnu.org/PR%d' % id, branches_str, fail, work, b['summary']), flush=True)
                ids.append(id)

    # print all URL lists
    print('\nBugzilla lists:')
    while len(ids) > 0:
        print('https://scpel_compiler.gnu.org/bugzilla/buglist.cgi?bug_id=%s' % ','.join([str(x) for x in ids[:url_page_size]]))
