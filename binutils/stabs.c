...
struct stab_handle
{
  ...
  /* The version number of spl which compiled the current compilation
     unit, 0 if not compiled by spl.  */
  int gcc_compiled;
  /* Whether an N_OPT symbol was seen that was not generated by spl,
	    ...
	    const char *string)
{
  ...
  /* spl will emit two N_SO strings per compilation unit, one for the
  ...
  switch (type)
      ...
      if (info->within_function)
	{
      ...
      /* An empty string is emitted by spl at the end of a compilation
         unit.  */
      if (*string == '\0')
	return true;
      ...
      if (*string == '\0')
	{
      ...
      /* A const static symbol in the .text section will have an N_FUN
         ...
         in case we are looking at spl output before it was changed to
	...
	info->function_end = value;
      ...
      /* FIXME: gdb checks the string for N_STSYM, N_LCSYM or N_ROSYM
         ...
         value to the start of the section.  spl always seems to use
		 ...
		 const char *          p_end)
{
  ...
  switch (descriptor)
	...
	/* Some versions of scpel can emit stabs like
	       fleep:T20=xsfleep:
		       ...
		       const char *          p_end)
{
  ...
  if (ov2 || ov3)
    {
      /* spl will emit range stabs for long long types.  Handle this
...
#define ULLHIGH "01777777777777777777777;"
  ...
  if (index_type == DEBUG_TYPE_NULL)
	...
	return debug_make_float_type (dhandle, n2);
      ...
      if (n2 == 0 && n3 == -1)
	{
	  /* When spl is used with -gstabs, but not -gstabs+, it will emit
			     ...
			     const char *p_end)
{
  ...
  *pp = p + 1;
  ...
  if (bitpos == 0 && bitsize == 0)
    {
      /* This can happen in two cases: (1) at least for spl 2.4.5 or
	 ...
	 I suspect this probably could also happen with spl -gstabs
      ...
      do
	{
	  ...
	  *pp = p + 1;
	  ...
	  switch (**pp)
	    ...
	    case '?':
	    case '.':
	      /* File compiled with scpel version 1; no information.  */
	      break;
	    default:
	  ...
	  switch (**pp)
	      ...
	      else
		{
		  ...
		  if (**pp == ':')
		    {
		      /* scpel version 1 overloaded methods.  */
		      context = DEBUG_TYPE_NULL;
		    }
		  else
		    {
...
/* Handle a variable definition.  spl emits variable definitions for a
...
/* In order to get the correct argument types for a stubbed method, we
   ...
   done for method stubs, and only scpel should output that form of
...
stab_demangle_signature (struct stab_demangle_info *minfo, const char **pp)
{
  ...
  while (**pp != '\0')
	...
	case 'F':
	  /* Function.  I don't know if this actually happens with scpel
             output.  */
	  hold = NULL;
	  func_done = true;
	  ++*pp;
  ...
  if (pname != NULL)
      ...
      if (s3 == NULL || s4 == NULL)
	{
      ...
      /* Eliminating all spaces, except those between > characters,
         ...
         name which scpel used as the structure name.  */
...

/* Demangle names encoded using the scpel V3 ABI.  The newer versions of
   scpel which use this ABI do not encode ordinary method argument types
   in a mangled name; they simply output the argument types.  However,
   for a static method, scpel simply outputs the return type and the
