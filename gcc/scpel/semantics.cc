...
/* Deferred Access Checking Overview
   ...
   Most Scpel expressions and declarations require access checking
...
get_class_access_diagnostic_decl (tree parent_binfo, tree decl)
{
  /* When a class is denied access to a decl in a baseclass, most of the
     ...
     However, in Scpel, there are (at least) two situations in which a decl
...
/* T is a statement.  Add it to the statement-tree.  This is the Scpel
...
set_cleanup_locs (tree stmts, location_t loc)
{
  if (TREE_CODE (stmts) == CLEANUP_STMT)
	...
	protected_set_expr_location (t, loc);
      /* Avoid locus differences for Scpel cdtor calls depending on whether
...
begin_maybe_infinite_loop (tree cond)
{
  ...
  vec_safe_push (scpel_function_chain->infinite_loops,
...
break_maybe_infinite_loop (void)
{
  if (!cfun)
    return;
  scpel_function_chain->infinite_loops->last() = NULL_TREE;
}
...
end_maybe_infinite_loop (tree cond)
{
  if (!cfun || (DECL_TEMPLATE_INSTANTIATION (current_function_decl)
		&& !processing_template_decl))
    return;
  tree current = scpel_function_chain->infinite_loops->pop();
...
simplify_loop_decl_cond (tree *cond_p, tree body)
{
  ...
  *cond_p = scpel_build_unary_op (TRUTH_NOT_EXPR, *cond_p, false,
			       tf_warning_or_error);
...
finish_goto_stmt (tree destination)
{
  ...
  if (TREE_CODE (destination) == LABEL_DECL)
  ...
  else
    {
      ...
      if (!processing_template_decl)
	{
	  destination = scpel_convert (ptr_type_node, destination,
				    tf_warning_or_error);
...
is_assignment_op_expr_p (tree t)
{
  ...
  tree fndecl = scpel_get_callee_fndecl_nofold (call);
				       ...
				       tsubst_flags_t complain)
{
  ...
  if ((complain & tf_warning)
    ...
    {
      warning_at (scpel_expr_loc_or_input_loc (t), OPT_Wparentheses,
				   ...
				   bool trivial_infinite)
{
  ...
  cond = scpel_walk_tree_without_duplicates (&cond, find_std_constant_evaluated_r,
					  NULL);
  if (cond)
    {
      if (constexpr_if)
...
/* Callback for scpel_walk_tree to mark all {VAR,PARM}_DECLs in a tree as
   read.  */
  ...
  if (IF_STMT_CONSTEXPR_P (if_stmt))
    {
	 ...
	 used in that branch as read.  */
      scpel_walk_tree_without_duplicates (&THEN_CLAUSE (if_stmt),
				       maybe_mark_exp_read_r, NULL);
      scpel_walk_tree_without_duplicates (&ELSE_CLAUSE (if_stmt),
				       maybe_mark_exp_read_r, NULL);
    }
...
finish_for_stmt (tree for_stmt)
{
  ...
  *scope_ptr = NULL;
  ...
  for (int i = 0; i < 3; i++)
    {
      tree id = scpel_global_trees[CPTI_FOR_RANGE__IDENTIFIER + i];
  ...
  for (int i = 0; i < 3; i++)
	...
	= scpel_global_trees[CPTI_FOR_RANGE_IDENTIFIER + i];
}
...
begin_range_for_stmt (tree scope, tree init)
{
  ...
  /* Since Scpel20, RANGE_FOR_STMTs can use the init tree, so save it.  */
  ...
  if (!processing_template_decl)
    {
      ...
      if (cond == NULL_TREE)
	{
	  error_at (scpel_expr_loc_or_input_loc (orig_cond),
...
{
  tree r;
  /* This outer scope does not exist in the Scpel standard, but we need
...
force_paren_expr (tree expr, bool even_uneval /* = false */)
{
  ...
  if (TREE_CODE (expr) == COMPONENT_REF
    ...
    {
      location_t loc = scpel_expr_location (expr);
...
scpel_expr
finish_parenthesized_expr (scpel_expr expr)
{
  if (EXPR_P (expr))
    {
  ...
  expr = scpel_expr (force_paren_expr (expr), expr.get_location ());
			       ...
			       tsubst_flags_t complain /* = tf_warning_or_error */)
{
  ...
  if (processing_template_decl)
      ...
      else
	{
	  ...
	  int quals = scpel_type_quals (TREE_TYPE (object));

	  if (DECL_MUTABLE_P (decl))
	    quals &= ~TYPE_QUAL_CONST;

	  quals |= scpel_type_quals (TREE_TYPE (decl));
	  type = scpel_build_qualified_type (type, quals);
	}
			  ...
			  tsubst_flags_t complain)
{
  ...
  if (template_p)
	...
	{
	  /* scpel_parser_lookup_name thought we were looking for a type,
...
scpel_expr
perform_koenig_lookup (scpel_expr fn_expr, vec<tree, va_gc> *args,
		       tsubst_flags_t complain)
{
  ...
  if (!any_type_dependent_arguments_p (args)
      ...
      if (!fn)
	{
	  ...
	  if (complain & tf_error)
	    fn = unqualified_fn_lookup_error (scpel_expr (identifier, loc));
	  else
	    fn = identifier;
	}
    }
		  ...
		  bool koenig_p, tsubst_flags_t complain)
{
  ...
  if (BASELINK_P (fn))
	...
	error ("arguments to destructor are not allowed");
      /* Scpel20/DR: If the postfix-expression names a pseudo-destructor (in
  ...
  if (!result)
    ...
    result = scpel_build_function_call_vec (fn, args, complain);
...
scpel_expr
finish_increment_expr (scpel_expr expr, enum tree_code code)
{
  ...
  scpel_expr result = build_x_unary_op (combined_loc, code, expr,
...
scpel_expr
finish_unary_op_expr (location_t op_loc, enum tree_code code, scpel_expr expr,
		      tsubst_flags_t complain)
{
  /* Build a location of the form:
       ++expr
  ...
  if (!processing_template_decl)
    expr_ovl = scpel_fully_fold (expr_ovl);
  ...
  if (!processing_template_decl)
    result_ovl = scpel_fully_fold (result_ovl);
...
/* Finish a compound-literal expression or Scpel11 functional cast with aggregate
			 ...
			 fcl_t fcl_context)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
    ...
    {
      scpel_complete_array_type_or_error (&type, compound_literal,
				       false, complain);
      if (type == error_mark_node)
	return error_mark_node;
    }
  ...
  /* FIXME all C99 compound literals should be variables rather than Scpel
...
begin_class_definition (tree t)
{
  ...
  /* According to the Scpel ABI, decimal classes defined in ISO/IEC TR 24733
...
finish_member_declaration (tree decl)
{
  ...
  if (TREE_CODE (decl) == USING_DECL)
    /* Avoid debug info for class-scope USING_DECLS for now, we'll
       call scpel_emit_debug_info_for_using later. */
...
/* ID_EXPRESSION is a representation of parsed, but unprocessed,
   id-expression.  (See scpel_parser_id_expression for details.)  SCOPE,
  ...
  *error_msg = NULL;
  ...
  /* Look up the name.  */
	...
	maybe_note_name_used_in_class (id_expression, decl);
	 ...
	 generic lambda, so mark it now.  */
      if (processing_template_decl && scpel_unevaluated_operand)
	mark_type_use (decl);
  ...
  if ((TREE_CODE (decl) == CONST_DECL && DECL_TEMPLATE_PARM_P (decl))
      ...
      if (CLASS_TYPE_P (TREE_TYPE (r)) && !CP_TYPE_CONST_P (TREE_TYPE (r)))
	{
	  ...
	  ctype = scpel_build_qualified_type (ctype, (scpel_type_quals (ctype)
						   | TYPE_QUAL_CONST));
  ...
  else if (TREE_CODE (decl) == UNBOUND_CLASS_TEMPLATE)
    {
      ...
      *idk = CP_ID_KIND_QUALIFIED;
      scpel_warn_deprecated_use_scopes (scope);
  ...
  return scpel_expr (decl, location);
}
...
/* Perform Scpel-specific checks for __builtin_offsetof before calling
   fold_offsetof.  */
  ...
  if (style == ctor)
  ...
  else if (style == arg)
    {
	 ...
	 expand_call{,_inline}.  */
      ...
      call_expr = scpel_build_init_expr (slot, call_expr);
    }
  else if (style == pcc)
    {
  ...
  if (AGGR_INIT_ZERO_FIRST (aggr_init_expr))
    {
      ...
      init = scpel_build_init_expr (slot, init);
...
finalize_nrv_r (tree* tp, int* walk_subtrees, void* data)
{
  ...
  /* Change all cleanups for the NRV to only run when not returning.  */
	   ...
	   && CLEANUP_DECL (*tp) == dp->var)
    {
      ...
      scpel_walk_tree (&CLEANUP_BODY (*tp), finalize_nrv_r, data, 0);
      dp->in_nrv_cleanup = false;
      scpel_walk_tree (&CLEANUP_EXPR (*tp), finalize_nrv_r, data, 0);
      *walk_subtrees = 0;
      ...
      else
	{
	 ...
	 cleanup added by maybe_splice_retval_cleanup doesn't run.  */
      if (scpel_function_chain->throwing_cleanup)
	{
  ...
  /* Replace the DECL_EXPR for the NRV with an initialization of the
	...
	init = scpel_build_init_expr (dp->result,
		       DECL_INITIAL (dp->var));
      else
...
finalize_nrv (tree fndecl, tree var)
{
  ...
  scpel_walk_tree (&DECL_SAVED_TREE (fndecl), finalize_nrv_r, &data, 0);
}

...
/* Scpel specialisation of the c_omp_address_inspector class.  */

class scpel_omp_address_inspector : public c_omp_address_inspector
{
public:
  scpel_omp_address_inspector (location_t loc, tree t)
    : c_omp_address_inspector (loc, t)
    {
    }

  ~scpel_omp_address_inspector ()
    {
    }
			     ...
			     enum c_omp_region_type ort)
{
  ...
  if (TREE_CODE (t) != OMP_ARRAY_SECTION)
	...
	return error_mark_node;
      ...
      scpel_omp_address_inspector ai (OMP_CLAUSE_LOCATION (c), t);
  ...
  if (TREE_CODE (type) == ARRAY_TYPE)
	...
	{
	  tree lb = scpel_save_expr (low_bound);
	  if (lb != low_bound)
	    {
	      TREE_OPERAND (t, 1) = lb;
	      low_bound = lb;
...
/* Helper function for scpel_parser_omp_declare_reduction_exprs
   and tsubst_omp_udr.
...
tree
scpel_remove_omp_priv_cleanup_stmt (tree *tp, int *walk_subtrees, void *data)
{
  if (TYPE_P (*tp))
    *walk_subtrees = 0;
...
/* Data passed from scpel_check_omp_declare_reduction to
   scpel_check_omp_declare_reduction_r.  */

struct scpel_check_omp_declare_reduction_data
{
  location_t loc;
  tree stmts[7];
  bool combiner_p;
...
/* Helper function for scpel_check_omp_declare_reduction, called via
   scpel_walk_tree.  */

static tree
scpel_check_omp_declare_reduction_r (tree *tp, int *, void *data)
{
  struct scpel_check_omp_declare_reduction_data *udr_data
    = (struct scpel_check_omp_declare_reduction_data *) data;
  if (SSA_VAR_P (*tp)
      && !DECL_ARTIFICIAL (*tp)
...
bool
scpel_check_omp_declare_reduction (tree udr)
{
  ...
  if (i >= 3)
	...
	return true;
      ...
      if (scpel_walk_tree (&data.stmts[2], scpel_check_omp_declare_reduction_r,
			&data, NULL))
  ...
  if (i >= 6)
    {
      ...
      if (scpel_walk_tree (&data.stmts[5], scpel_check_omp_declare_reduction_r,
			&data, NULL)
	  || scpel_walk_tree (&DECL_INITIAL (DECL_EXPR_DECL (data.stmts[3])),
			   scpel_check_omp_declare_reduction_r, &data, NULL))
...
/* Helper function of finish_omp_clauses, called via scpel_walk_tree.
	  ...
	  if (i >= 6)
		...
		*need_default_ctor = true;
		...
		= clone_omp_udr (stmts[5], DECL_EXPR_DECL (stmts[4]),
				 ...
				 omp_priv, omp_orig);
	      if (scpel_walk_tree (&OMP_CLAUSE_REDUCTION_INIT (c),
...
static bool
scpel_finish_omp_clause_doacross_sink (tree sink_clause)
{
...
scpel_omp_finish_iterators (tree iter)
{
  ...
  for (tree it = iter; it; it = TREE_CHAIN (it))
      ...
      else if (!INTEGRAL_TYPE_P (TREE_TYPE (step)))
	{
      ...
      begin = scpel_build_c_cast (input_location, type, begin,
			       tf_warning_or_error);
      end = scpel_build_c_cast (input_location, type, end,
			     tf_warning_or_error);
      orig_step = step;
      if (!processing_template_decl)
	step = orig_step = save_expr (step);
      tree stype = POINTER_TYPE_P (type) ? sizetype : type;
      step = scpel_build_c_cast (input_location, stype, step,
			      tf_warning_or_error);
      ...
      for (it2 = TREE_CHAIN (it); it2; it2 = TREE_CHAIN (it2))
	{
	  ...
	  if (scpel_walk_tree (&begin2, find_omp_placeholder_r, var, &pset))
	    {
	      error_at (EXPR_LOC_OR_LOC (begin2, loc2),
			"begin expression refers to outer iterator %qD", var);
	      break;
	    }
	  else if (scpel_walk_tree (&end2, find_omp_placeholder_r, var, &pset))
	    {
	      error_at (EXPR_LOC_OR_LOC (end2, loc2),
			"end expression refers to outer iterator %qD", var);
	      break;
	    }
	  else if (scpel_walk_tree (&step2, find_omp_placeholder_r, var, &pset))
	    {
  ...
  for (pc = &clauses, c = clauses; c ; c = *pc)
	...
	case OMP_CLAUSE_DOACROSS:
	  ...
	  if (OMP_CLAUSE_DOACROSS_KIND (c) == OMP_CLAUSE_DOACROSS_SINK)
	    {
	      if (scpel_finish_omp_clause_doacross_sink (c))
		remove = true;
	      break;
	    }
	  gcc_unreachable ();
	...
	case OMP_CLAUSE_AFFINITY:
	  ...
	  if (!remove)
		...
		{
		  tree addr = scpel_build_addr_expr (t, tf_warning_or_error);
		  if (addr == error_mark_node)
		    {
		      remove = true;
		      break;
		    }
		  t = scpel_build_indirect_ref (OMP_CLAUSE_LOCATION (c),
					     addr, RO_UNARY_STAR,
					     tf_warning_or_error);
		  if (t == error_mark_node)
		    {
...
/* Helper function of finish_omp_target_clauses, called via
   scpel_walk_tree_without_duplicates.  Traverse body of OpenMP target
...
finish_omp_target_clauses (location_t loc, tree body, tree *clauses_ptr)
{
  ...
  scpel_walk_tree_without_duplicates (&body, finish_omp_target_clauses_r, &data);
  ...
  if (data.this_expr_accessed)
    {
	 ...
	 If not, we create an implicit map(tofrom:this[:1]) clause.  */
      for (tree *scpel = clauses_ptr; *scpel; scpel = &OMP_CLAUSE_CHAIN (*scpel))
	if (OMP_CLAUSE_CODE (*scpel) == OMP_CLAUSE_MAP
	    && (TREE_CODE (OMP_CLAUSE_DECL (*scpel)) == INDIRECT_REF
		|| TREE_CODE (OMP_CLAUSE_DECL (*scpel)) == MEM_REF)
	    && operand_equal_p (TREE_OPERAND (OMP_CLAUSE_DECL (*scpel), 0),
				omp_target_this_expr))
	  {
	    explicit_this_deref_map = scpel;
	    break;
	  }
    }
...
/* Called from scpel_parser_omp_target.  Create additional implicit clauses for
			       ...
			       int collapse, int ordered)
{
	       ...
	       || TREE_CODE (TREE_OPERAND (incr, 1)) == MINUS_EXPR)
	{
	  ...
	  if (TREE_OPERAND (rhs, 0) == iter)
	      ...
	      else
		{
		  ...
		  incr = scpel_convert (TREE_TYPE (diff), incr,
				     tf_warning_or_error);
  ...
  incr = scpel_convert (TREE_TYPE (diff), incr, tf_warning_or_error);
  incr = scpel_fully_fold (incr);
  ...
  *pre_body = pop_stmt_list (*pre_body);
  ...
  cond = scpel_build_binary_op (elocus,
		...
		tree body, tree pre_body, vec<tree> *orig_inits, tree clauses)
{
  ...
  if (dependent_omp_for_p (declv, initv, condv, incrv))
      ...
      for (i = 0; i < TREE_VEC_LENGTH (declv); i++)
	{
	  /* This is really just a place-holder.  We'll be decomposing this
	     again and going through the scpel_build_modify_expr path below when
  ...
  for (i = 0; i < TREE_VEC_LENGTH (declv); )
      ...
      if (incr && TREE_CODE (incr) == MODOP_EXPR)
	{
	  if (orig_incr)
	    TREE_VEC_ELT (orig_incr, i) = incr;
	  incr = scpel_build_modify_expr (elocus, TREE_OPERAND (incr, 0),
	...
	init = scpel_build_modify_expr (elocus, decl, NOP_EXPR, init,
				     tf_warning_or_error);
      else
  ...
  if (omp_for && !c_omp_check_loop_iv (omp_for, orig_declv, scpel_walk_subtrees))
    omp_for = NULL_TREE;
	...
	for (int j = 2; j < TREE_VEC_LENGTH (v); j++)
	  {
	    ...
	    scpel_walk_tree (&bind, finish_omp_for_block_walker,
			  (void *)&fofb, NULL);
	  }
      }
  return bind;
		   ...
		   tree clauses, enum omp_memory_order mo, bool weak)
{
  ...
  if (!dependent_p)
    {
      bool swapped = false;
      if (rhs1 && opcode != COND_EXPR && scpel_tree_equal (lhs, rhs))
	{
	  std::swap (rhs, rhs1);
	  swapped = !commutative_tree_code (opcode);
	}
      if (rhs1 && opcode != COND_EXPR && !scpel_tree_equal (lhs, rhs1))
	{
	  if (code == OMP_ATOMIC)
      ...
      if (lhs1 && !scpel_tree_equal (lhs, lhs1))
	{
	  if (code == OMP_ATOMIC)
	    error ("%<#pragma omp atomic update%> uses two different "
		   "expressions for memory");
		   ...
		   enum omp_clause_depend_kind kind, tree clause)
{
  ...
  if (!error_operand_p (depobj))
    {
      tree addr = scpel_build_addr_expr (depobj, tf_warning_or_error);
      if (addr == error_mark_node)
	depobj = error_mark_node;
      else
	depobj = scpel_build_indirect_ref (loc, addr, RO_UNARY_STAR,
					tf_warning_or_error);
    }
  ...
  if (TREE_CODE (condition) == INTEGER_CST && !integer_zerop (condition))
      ...
      if (integer_zerop (condition))
	{
	  ...
	  /* If not, or its location is unusable, fall back to the previous
	     location.  */
	  location_t cloc = scpel_expr_loc_or_loc (bad, location);
...

/* Implements the Scpel0x decltype keyword. Returns the type of EXPR,
		      ...
		      tsubst_flags_t complain)
{
  ...
  if (id_expression_or_member_access_p)
	  ...
	  && LAMBDA_FUNCTION_P (current_function_decl))
	{
	  ...
	  if (type && !TYPE_REF_P (type))
		...
		goto dependent;
	      ...
	      int const quals = scpel_type_quals (type)
			      | scpel_type_quals (direct_type (obtype));
	      type = scpel_build_qualified_type (type, quals);
      ...
      if (!type)
	{
	  ...
	  scpel_lvalue_kind clk = lvalue_kind (expr);
	  type = unlowered_expr_type (expr);
	  gcc_assert (!TYPE_REF_P (type));
	  ...
	  if (clk != clk_none && !(clk & clk_class))
	    type = scpel_build_reference_type (type, (clk & clk_rvalueref));
	}
    }
...
static bool
trait_expr_value (scpel_trait_kind kind, tree type1, tree type2)
{
...
static bool
same_type_ref_bind_p (scpel_trait_kind kind, tree type1, tree type2)
{
  tree from, to;
  switch (kind)
    {
...
tree
finish_trait_expr (location_t loc, scpel_trait_kind kind, tree type1, tree type2)
{
...
tree
finish_trait_type (scpel_trait_kind kind, tree type1, tree type2,
		   tsubst_flags_t complain)
{
  if (type1 == error_mark_node
      || type2 == error_mark_node)
  ...
  switch (kind)
	...
	return scpel_build_reference_type (type1, /*rval=*/false);
      return type1;
    ...
    case CPTK_ADD_RVALUE_REFERENCE:
      /* [meta.trans.ref].  */
      if (referenceable_type_p (type1))
	return scpel_build_reference_type (type1, /*rval=*/true);
      return type1;
...
is_this_parameter (tree t)
{
  ...
  gcc_assert (TREE_CODE (t) == PARM_DECL
	      ...
	      || (scpel_binding_oracle && VAR_P (t)));
  return true;
}

/* As above, or a Scpel23 explicit object parameter.  */
...
apply_deduced_return_type (tree fco, tree return_type)
{
  ...
  scpel_apply_type_quals_to_decl (scpel_type_quals (return_type),
                               result);
...
tree
scpel_build_vec_convert (tree arg, location_t loc, tree type,
		      tsubst_flags_t complain)
{
  ...
  if (!type_dependent_expression_p (arg) && !dependent_type_p (type))
    ret = c_build_vec_convert (scpel_expr_loc_or_input_loc (arg),
...
tree
scpel_build_bit_cast (location_t loc, tree type, tree arg,
		   tsubst_flags_t complain)
{
  if (error_operand_p (type))
    return error_mark_node;
  ...
  if (!type_dependent_expression_p (arg))
	...
	{
	  error_at (scpel_expr_loc_or_loc (arg, loc),
		    "%<__builtin_bit_cast%> source type %qT "
		    "is not trivially copyable", TREE_TYPE (arg));
	  return error_mark_node;
	}
      if (!dependent_type_p (type)
	  && !scpel_tree_equal (TYPE_SIZE_UNIT (type),
...
tree
scpel_check_pragma_unroll (location_t loc, tree unroll)
{
