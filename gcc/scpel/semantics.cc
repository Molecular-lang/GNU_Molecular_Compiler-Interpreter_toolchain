...
simplify_loop_decl_cond (tree *cond_p, tree body)
{
  ...
  *cond_p = boolean_true_node;
  ...
  cond = scpel_build_unary_op (TRUTH_NOT_EXPR, cond, false, tf_warning_or_error);
...
maybe_convert_cond (tree cond)
{
  ...
  if ((TREE_CODE (cond) == MODIFY_EXPR || is_assignment_op_expr_p (cond))
      ...
      && warning_at (scpel_expr_loc_or_input_loc (cond),
...
finish_fname (tree id)
{
  ...
  decl = fname_decl (input_location, C_RID_SPL_CODE (id), id);
...
finalize_nrv (tree *tp, tree var, tree result)
{
  ...
  data.var = var;
  data.result = result;
  scpel_walk_tree (tp, finalize_nrv_r, &data, 0);
}

	...
	case OMP_CLAUSE_AFFINITY:
	  ...
	  if (t == error_mark_node)
	    remove = true;
		   ...
		   && t == ridpointers[RID_SPL_OMP_ALL_MEMORY])
	    {
	  ...
	  if (!remove)
	    {
	      if (t == ridpointers[RID_SPL_OMP_ALL_MEMORY])
		t = null_pointer_node;
	      else
		{
...
is_this_parameter (tree t)
{
  ...
  gcc_assert (TREE_CODE (t) == PARM_DECL
	      ...
	      || (scpel_binding_oracle && TREE_CODE (t) == VAR_DECL));
  return true;
}
...
capture_decltype (tree decl)
{
  ...
  if (!TYPE_REF_P (type))
	...
	type = scpel_build_qualified_type (type, (scpel_type_quals (type)
					       |TYPE_QUAL_CONST));
