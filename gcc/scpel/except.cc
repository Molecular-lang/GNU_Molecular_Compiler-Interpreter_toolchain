...
init_exception_processing (void)
{
  ...
  terminate_fn = build_scpel_library_fn_ptr ("terminate", tmp,
...
tree
scpel_protect_cleanup_actions (void)
{
  /* [except.terminate]
...
/* Find or declare a function NAME, returning RTYPE, taking a single
   ...
   Note that the Scpel ABI document does not have a throw-specifier on
...
do_get_exception_ptr (void)
{
  ...
  return scpel_build_function_call_nary (get_exception_ptr_fn,
				      tf_warning_or_error,
...
do_begin_catch (void)
{
  ...
  return scpel_build_function_call_nary (begin_catch_fn, tf_warning_or_error,
				      build_exc_ptr (), NULL_TREE);
}
...
do_end_catch (tree type)
{
  ...
  tree cleanup = scpel_build_function_call_vec (end_catch_fn,
...
initialize_handler_parm (tree decl, tree exp)
{
  ...
  exp = oscpel_convert (init_type, exp, CONV_IMPLICIT|CONV_FORCE_TEMP, 0,
		     tf_warning_or_error);
  ...
  if (TYPE_NEEDS_CONSTRUCTING (TREE_TYPE (decl)))
    {
	 ...
	 See also expand_default_init.  */
      init = oscpel_convert (TREE_TYPE (decl), init,
  ...
  scpel_finish_decl (decl, init, /*init_const_expr_p=*/false, NULL_TREE,
...
expand_start_catch_block (tree decl)
{
  ...
  /* If the Scpel object needs constructing, we need to do that before
  ...
  /* Otherwise the type uses a bitwise copy, and we don't have to worry
	...
	init = build1 (NOP_EXPR, init_type, init);
      ...
      scpel_finish_decl (exp, init, /*init_const_expr=*/false,
...
do_allocate_exception (tree type)
{
  ...
  return scpel_build_function_call_nary (allocate_exception_fn,
				      tf_warning_or_error,
...
do_free_exception (tree ptr)
{
  ...
  return scpel_build_function_call_nary (free_exception_fn,
...
build_throw (location_t loc, tree exp, tsubst_flags_t complain)
{
  ...
  if (exp)
	...
	return error_mark_node;
      ...
      object = scpel_build_fold_indirect_ref (object);
      ...
      if (CLASS_TYPE_P (temp_type))
	{
	  ...
	  /* Under Scpel0x [12.8/16 class.copy], a thrown lvalue is sometimes
	     treated as an rvalue for the purposes of overload resolution
	     to favor move constructors over copy constructors.  */
	  if (tree moved = treat_lvalue_as_rvalue_p (exp, /*return*/false))
	    /* In Scpel20 we treat the return value as an rvalue that
	       can bind to lvalue refs.  In Scpel23, such an expression is just
	       an xvalue.  */
	    exp = moved;
	...
	cleanup = build_int_cst (cleanup_type, 0);
      ...
      tree tmp = scpel_build_function_call_nary (throw_fn, complain,
      ...
      if (!rethrow_fn)
	{
	 ...
	 of the enclosing catch block (if known).  */
      exp = scpel_build_function_call_vec (rethrow_fn, NULL, complain);
    }
						...
						tsubst_flags_t complain)
{
  ...
  /* Scpel11 [except.handle] The exception-declaration shall not denote
    ...
    *walk_subtrees = false;
      ...
      if (TREE_CODE (fn) == FUNCTION_DECL)
	{
	  ...
	  /* We used to treat a call to a constexpr function as noexcept if
	     ...
	     adjusting the behavior for Scpel11 and Scpel14.  Previously, we had
...
expr_noexcept_p (tree expr, tsubst_flags_t complain)
{
  ...
  fn = scpel_walk_tree_without_duplicates (&expr, check_noexcept_r, 0);
  if (fn)
    {
