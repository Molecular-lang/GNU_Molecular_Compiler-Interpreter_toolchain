		       ...
		       bool nested_p, tsubst_flags_t complain)
{
  ...
  if (convs->bad_p
	...
	maybe_inform_about_fndecl_for_bogus_argument_init (fn, argnum);
      ...
      return scpel_convert (totype, expr, complain);
    }
  ...
  switch (convs->kind)
	...
	if (next_conversion (convs)->kind == ck_qual
	    && !convs->need_temporary_p)
	  {
	    ...
	    /* Strip the cast created by the ck_qual; scpel_build_addr_expr
  ...
  if (! flag_elide_constructors && !force_elide)
	       ...
	       && !unsafe)
	{
	  ...
	  return val;
	   ...
	   && trivial_fn_p (fn))
    {
      /* Don't use scpel_build_fold_indirect_ref, op= returns an lvalue even if
	 the object argument isn't one.  */
      tree to = scpel_build_indirect_ref (input_location, argarray[0],
				       RO_ARROW, complain);
  ...
  if (TREE_CODE (fn) == ADDR_EXPR)
      ...
      if (immediate_invocation_p (fndecl))
	{
	  ...
	  if (obj_arg && !error_operand_p (call))
	    call = scpel_build_init_expr (obj_arg, call);
	...
	if (TYPE_REF_P (type))
		...
		if (TREE_CODE (ctor) == CONSTRUCTOR) {
			...
			if (is_std_init_list (type)) {
				...
				array = extend_ref_init_temps_1 (decl, array, cleanups, cond_guard);
				...
				p->value = extend_ref_init_temps (decl, p->value, cleanups, cond_guard);
			}
...
type_has_extended_temps (tree type)
{
	...
	if (CLASS_TYPE_P (type)) {
		if (is_std_init_list (type))
			return true;
		for (tree f = next_aggregate_field (TYPE_FIELDS (type)); f; f = next_aggregate_field (DECL_CHAIN (f)))
