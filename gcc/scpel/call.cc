...
set_flags_from_callee (tree call)
{
  ...
  tree decl = scpel_get_callee_fndecl_nofold (call);

  /* We check both the decl and the type; a function may be known not to
     throw without being declared throw().  */
  bool nothrow = decl && TREE_NOTHROW (decl);
  tree callee = scpel_get_callee (call);
  if (callee)
  ...
  if (cfun && scpel_function_chain && !scpel_unevaluated_operand)
    {
      if (!nothrow && at_function_scope_p ())
	scpel_function_chain->can_throw = 1;
...
strip_top_quals (tree t)
{
  if (TREE_CODE (t) == ARRAY_TYPE)
    return t;
  return scpel_build_qualified_type (t, 0);
}
		     ...
		     int flags, tsubst_flags_t complain)
{
  ...
  if ((tcode == POINTER_TYPE || TYPE_PTRMEM_P (to)
	       ...
	       && TREE_CODE (from_pointee) != FUNCTION_TYPE)
	{
	  ...
	  int quals = scpel_type_quals (nfrom) & ~TYPE_QUAL_RESTRICT;
	  from_pointee = scpel_build_qualified_type (void_type_node, quals);
	...
	{
	  from_pointee
	    = scpel_build_qualified_type (to_pointee,
				       scpel_type_quals (from_pointee));
	  from = build_pointer_type (from_pointee);
	  conv = build_conv (ck_ptr, from, conv);
	  conv->base_p = true;
	}
  ...
  /* As an extension, allow conversion to complex type.  */
	...
	conv->rank = cr_promotion;
	 ...
	 conversion rank or from has higher conversion rank.  */
	  ...
	  && scpel_compare_floating_point_conversion_ranks (from, to) >= 2)
	conv->bad_p = true;
    }
		   ...
		   tsubst_flags_t complain)
{
  ...
  if (related_p && c_cast_p
    ...
    to = scpel_build_qualified_type (to, scpel_type_quals (tfrom));
  ...
  if ((related_p || compatible_p) && gl_kind)
	...
	conv->bad_p = true;
      ...
      /* Nor the reverse.  */
      if (!is_lvalue && !TYPE_REF_IS_RVALUE (rto)
	  /* Unless it's really a Scpel20 lvalue being treated as an xvalue.
	     But in Scpel23, such an expression is just an xvalue, not a special
		       ...
		       int flags, tsubst_flags_t complain)
{
  ...
  parmtype = scpel_build_qualified_type (ctype,
				      scpel_type_quals (TREE_TYPE (parmtype)));
  bool this_p = true;
  if (FUNCTION_REF_QUALIFIED (TREE_TYPE (fn)))
    {
	 ...
	 object parameter has reference type.  */
      ...
      parmtype = scpel_build_reference_type (parmtype, rv);
...
build_this (tree obj)
{
  ...
  return scpel_build_addr_expr (obj, tf_warning_or_error);
}
...
{
  auto_vec<tree*> temps;
  scpel_walk_tree_without_duplicates (&expr, find_temps_r, &temps);
  for (tree *p : temps)
    {
      tree t = TREE_TYPE (*p);
...
maybe_init_list_as_array (tree elttype, tree init)
{
  ...
  TREE_VEC_ELT (copy_argtypes, 0)
    = scpel_build_qualified_type (elttype, TYPE_QUAL_CONST);
  if (!is_xible (INIT_EXPR, elttype, copy_argtypes))
    return NULL_TREE;

  init_elttype = scpel_build_qualified_type (init_elttype, TYPE_QUAL_CONST);
...
maybe_init_list_as_range (tree fn, tree expr)
{
  if (!processing_template_decl
      ...
      if (tree init = maybe_init_list_as_array (to, expr))
	{
	  ...
	  tree end = scpel_build_binary_op (input_location, PLUS_EXPR, begin,
					 nelts, tf_none);
	  begin = scpel_build_compound_expr (init, begin, tf_none);
...
resolve_args (vec<tree, va_gc> *args, tsubst_flags_t complain)
{
  ...
  FOR_EACH_VEC_SAFE_ELT (args, ix, arg)
	...
	{
	  if (complain & tf_error)
	    error_at (scpel_expr_loc_or_input_loc (arg),
			 ...
			 tsubst_flags_t complain)
{
  ...
  if (!cand)
      ...
      if (complain & tf_error)
	{
	  ...
	  // let the error be diagnosed by scpel_build_function_call_vec.
	  if (!any_viable_p && candidates && ! candidates->next
	    ...
	    return scpel_build_function_call_vec (candidates->fn, args, complain);
...
keep_unused_object_arg (tree result, tree obj, tree fn)
{
  ...
  if (TREE_SIDE_EFFECTS (a))
    return scpel_build_compound_expr (a, result, tf_error);
  return result;
}
...
build_op_call (tree obj, vec<tree, va_gc> **args, tsubst_flags_t complain)
{
  ...
  if (!any_viable_p)
      ...
      else
	{
	  ...
	  result = scpel_build_function_call_vec (obj, args, complain);
	}
    }
...
conditional_conversion (tree e1, tree e2, tsubst_flags_t complain)
{
  ...
  if (glvalue_p (e2))
    {
      tree rtype = scpel_build_reference_type (t2, !lvalue_p (e2));
			...
			tsubst_flags_t complain)
{
  ...
  if (!arg2)
	...
	pedwarn (loc, OPT_Wpedantic,
		 "ISO Scpel forbids omitting the middle term of "
		 "a %<?:%> expression");
	...
	arg2 = arg1 = build1 (EXCESS_PRECISION_EXPR, TREE_TYPE (arg1),
			      scpel_save_expr (TREE_OPERAND (arg1, 0)));
      else
	arg2 = arg1 = scpel_save_expr (arg1);
    }
  ...
  if (gnu_vector_type_p (TREE_TYPE (arg1))
	  ...
	  && !VECTOR_TYPE_P (arg3_type))
	{
	  ...
	  arg2 = scpel_convert (stype, arg2, complain);
	  arg2 = save_expr (arg2);
	  arg2 = build_vector_from_val (vtype, arg2);
	  arg2_type = vtype;
	  arg3 = scpel_convert (stype, arg3, complain);
	  arg3 = save_expr (arg3);
          ...
          if (complain & tf_error)
            {
              if (VOID_TYPE_P (arg2_type))
                error_at (scpel_expr_loc_or_loc (arg3, loc),
			  ...
			  "neither a throw-expression nor of type %<void%>");
                ...
                error_at (scpel_expr_loc_or_loc (arg2, loc),
  ...
  /* [expr.cond]
	...
	return result;
	 ...
	 the type of the other.  */
	  ...
	  && scpel_type_quals (arg2_type) != scpel_type_quals (arg3_type))
	arg2_type = arg3_type =
	  scpel_build_qualified_type (arg2_type,
				   scpel_type_quals (arg2_type)
				   | scpel_type_quals (arg3_type));
    }
  ...
  if (processing_template_decl && is_glvalue)
    {
      ...
      result_type = scpel_build_reference_type (result_type, xvalue_p (arg));
    }
...
/* Subroutine of build_new_op: Add to CANDIDATES all candidates for the
   ...
   handle Scpel20 rewritten comparison operator candidates.  Returns NULL_TREE
  ...
  if (!rewritten)
    {
	 ...
	 but there's no point.  */
      ...
      /* Maybe add Scpel20 rewritten comparison candidates.  */
	      ...
	      tree *overload, tsubst_flags_t complain)
{
  ...
  if (!any_viable_p)
	...
	case POSTDECREMENT_EXPR:
	  ...
	  /* Look for an `operator++ (int)'. Pre-1985 Scpel didn't
      ...
      else if (TREE_CODE (cand->fn) == FUNCTION_DECL)
	{
	  ...
	  if (resolve_args (arglist, complain) == NULL)
	      ...
	      if (cand->reversed ())
		{
		  ...
		  if (cand->fn == current_function_decl)
		    warning_at (loc, 0, "in Scpel20 this comparison calls the "
	  ...
	  /* If this was a Scpel20 rewritten comparison, adjust the result.  */
	  if (cand->rewritten ())
	    {
 ...
 builtin:
  switch (code)
    {
    case MODIFY_EXPR:
      return scpel_build_modify_expr (loc, arg1, code2, arg2, complain);

    case INDIRECT_REF:
      return scpel_build_indirect_ref (loc, arg1, RO_UNARY_STAR, complain);
    ...
    case BIT_IOR_EXPR:
    case BIT_XOR_EXPR:
      return scpel_build_binary_op (loc, code, arg1, arg2, complain);
    ...
    case ABS_EXPR:
    case CO_AWAIT_EXPR:
      return scpel_build_unary_op (code, arg1, false, complain);

    case ARRAY_REF:
      return scpel_build_array_ref (input_location, arg1, arg2, complain);

    case MEMBER_REF:
      return build_m_component_ref (scpel_build_indirect_ref (loc, arg1,
							   RO_ARROW_STAR,
                                                           complain),
                                    arg2, complain);
...
extract_call_expr (tree call)
{
  ...
  if (cxx_dialect >= cxx20)
    switch (TREE_CODE (call))
      {
	/* Scpel20 rewritten comparison operators.  */
      case TRUTH_NOT_EXPR:
	call = TREE_OPERAND (call, 0);
	break;
      case LT_EXPR:
		      ...
		      tree alloc_fn, tsubst_flags_t complain)
{
  ...
  tree oaddr = addr;
  addr = scpel_convert (ptr_type_node, addr, complain);
      ...
      if (second_parm_is_size_t (fn))
	{
	  ...
	  const char *const msg2
	    = G_("%qD is a usual (non-placement) deallocation "
		 "function in Scpel14 (or with %<-fsized-deallocation%>)");
	  ...
	  /* Before Scpel14 a two-parameter global deallocation function is
	  ...
	  if (complain & tf_warning_or_error)
		...
		{
		  /* Only mention Scpel14 for namespace-scope delete.  */
  ...
  if (fn)
      ...
      else
	{
	  ...
	  if (destroying)
	    {
		 ...
		 object.  */
	      ...
	      addr = scpel_convert (rtype, oaddr, complain);
	  ...
	  ret = scpel_build_function_call_vec (fn, &args, complain);
	}
...
/* Maybe warn about Scpel20 Conversions to arrays of unknown bound.  C is
		       ...
		       bool nested_p, tsubst_flags_t complain)
{
  ...
  if (convs->bad_p
	...
	switch (scpel_compare_floating_point_conversion_ranks (TREE_TYPE (expr),
							    totype))
	  {
	  case 2:
	    if (pedwarn (loc, OPT_Wnarrowing, "ISO Scpel does not allow "
	...
	expr = scpel_convert (totype, expr, complain);
      if (complained == 1)
  ...
  switch (convs->kind)
	...
	if (DECL_NONCONVERTING_P (convfn) && DECL_CONSTRUCTOR_P (convfn)
	    ...
	    /* And in Scpel98 a default constructor can't be explicit.  */
	    && cxx_dialect >= cxx11)
	  {
	    if (!(complain & tf_error))
	      return error_mark_node;
		...
		if (pedwarn (loc, 0, "converting to %qT from initializer list "
			     ...
			     totype, convfn))
		  {
		    inform (DECL_SOURCE_LOCATION (convfn), "%qD declared here",
			    convfn);
		    inform (loc, "in Scpel11 and above a default constructor "
			    "can be explicit");
		  }
	      }
	    else
	...
	expr = scpel_convert (totype, TREE_OPERAND (expr, 0), complain);
      if (expr == null_node
	...
	if (tree init = maybe_init_list_as_array (elttype, expr))
	  {
	    elttype = scpel_build_qualified_type
	      (elttype, scpel_type_quals (elttype) | TYPE_QUAL_CONST);
	    array = build_array_of_n_type (elttype, len);
	    array = build_vec_init_expr (array, init, complain);
	    array = get_target_expr (array);
	    array = scpel_build_addr_expr (array, complain);
	  }
	else if (len)
	  {
	...
	array = scpel_convert (build_pointer_type (elttype), array, complain);
  ...
  switch (convs->kind)
	...
	/* If necessary, create a temporary. 
           ...
           upcoming call to scpel_build_addr_expr doesn't fail.  */
	if (convs->need_temporary_p
	    ...
	    || TREE_CODE (expr) == VA_ARG_EXPR)
	  {
	    ...
	    scpel_lvalue_kind lvalue = lvalue_kind (expr);
	...
	expr = scpel_convert (build_pointer_type (TREE_TYPE (ref_type)),
			   expr, complain);
  ...
  if (issue_conversion_warnings)
    expr = scpel_convert_and_check (totype, expr, complain);
  else
    {
...
convert_arg_to_ellipsis (tree arg, tsubst_flags_t complain)
{
  ...
  if (SCALAR_FLOAT_TYPE_P (arg_type)
      ...
      if (TREE_SIDE_EFFECTS (arg))
	{
	  ...
	  arg = scpel_build_compound_expr (arg, null_pointer_node, complain);
	}
      else
	...
	{
	  tree prom = scpel_convert (ENUM_UNDERLYING_TYPE (arg_type), arg,
				  complain);
	  prom = scpel_perform_integral_promotions (prom, complain);
  ...
  if (type_has_nontrivial_copy_init (type)
      || TYPE_HAS_NONTRIVIAL_DESTRUCTOR (type))
    {
	 ...
	 it as pass by invisible reference.  */
      ...
      tree ref = scpel_build_reference_type (type, false);
...
convert_for_arg_passing (tree type, tree val, tsubst_flags_t complain)
{
  ...
  /* If VAL is a bitfield, then -- since it has already been converted
     ...
     often have type "int", even though the Scpel type for the field is
  ...
  /* Pass classes with copy ctors by invisible reference.  */
    ...
    val = build1 (ADDR_EXPR, build_reference_type (type), val);
	   ...
	   && COMPLETE_TYPE_P (type)
	   && tree_int_cst_lt (TYPE_SIZE (type), TYPE_SIZE (integer_type_node)))
    val = scpel_perform_integral_promotions (val, complain);
  if (complain & tf_warning)
    {
      if (warn_suggest_attribute_format)
	{
      ...
      maybe_warn_parm_abi (type, scpel_expr_loc_or_input_loc (val));
    }
...
make_base_init_ok (tree exp)
{
  ...
  if (!AGGR_INIT_VIA_CTOR_P (exp))
    ...
    return true;
  tree fn = scpel_get_callee_fndecl_nofold (exp);
...
/* Return 2 if T refers to a base, 1 if a potentially-overlapping field,
   ...
   doing Scpel17 mandatory copy elision for either of these cases.
...
build_trivial_dtor_call (tree instance, bool no_ptr_deref)
{
  ...
  if (INDIRECT_TYPE_P (TREE_TYPE (instance))
	...
	goto no_clobber;
      instance = scpel_build_fold_indirect_ref (instance);
    }
...
{
  return (scpel_unevaluated_operand != 0
...
build_over_call (struct z_candidate *cand, int flags, tsubst_flags_t complain)
{
 ...
 not_really_used:
  ...
  if (!force_elide
	...
	arg = scpel_build_fold_indirect_ref (arg);

      /* In Scpel17 we shouldn't be copying a TARGET_EXPR except into a
	...
	scpel_handle_deprecated_or_unavailable (fn, complain);
      ...
      if (is_dummy_object (fa))
	{
	  if (TREE_CODE (arg) == TARGET_EXPR)
	    ...
	    return force_target_expr (DECL_CONTEXT (fn), arg, complain);
	       ...
	       && !unsafe)
	{
	  tree to = scpel_build_fold_indirect_ref (fa);
	  val = scpel_build_init_expr (to, arg);
	  return val;
	}
    }
  else if (DECL_ASSIGNMENT_OPERATOR_P (fn)
	   ...
	   && trivial_fn_p (fn))
    {
      tree to = scpel_build_fold_indirect_ref (argarray[0]);
      ...
      if (is_really_empty_class (type, /*ignore_vptr*/true))
	{
	  ...
	  to = scpel_build_addr_expr (to, tf_none);
	  to = scpel_build_indirect_ref (input_location, to, RO_ARROW, complain);
      ...
      else
	{
	  ...
	  arg0 = scpel_build_addr_expr (to, complain);
      ...
      else if (default_ctor_p (fn))
	{
	  if (is_dummy_object (argarray[0]))
	    ...
	    return scpel_build_fold_indirect_ref (argarray[0]);
	}
    }
		...
		tsubst_flags_t complain, tree orig_fndecl)
{
  ...
  location_t loc = scpel_expr_loc_or_input_loc (fn);
...
check_self_delegation (tree ret)
{
  ...
  tree fn = scpel_get_callee_fndecl_nofold (ret);
			   ...
			   tree binfo, int flags, tsubst_flags_t complain)
{
  ...
  /* In Scpel17, "If the initializer expression is a prvalue and the
  ...
  if (cxx_dialect >= cxx17
	      ...
	      (class_type, TREE_TYPE (arg))))
	{
	  ...
	  return scpel_build_init_expr (instance, arg);
	}
    }
		       ...
		       tree *fn_p, tsubst_flags_t complain)
{
  ...
  if (DECL_CONSTRUCTOR_P (fn)
      ...
      if (init)
	{
	  ...
	  return scpel_build_init_expr (instance, init);
	}
  ...
  if (!any_viable_p)
	  ...
	  && !vec_safe_is_empty (user_args))
	{
	  ...
	  if (is_dummy_object (instance))
		...
		return error_mark_node;
	      return scpel_build_init_expr (instance, ctor);
	    }
	}
      if (complain & tf_error)
...
compare_ics (conversion *ics1, conversion *ics2)
{
...

  {
	...
	if (SCALAR_FLOAT_TYPE_P (fp2))
	  {
	    /* scpel_compare_floating_point_conversion_ranks returns -1, 0 or 1
	       if the conversion rank is equal (-1 or 1 if the subrank is
	       different).  */
	    if (IN_RANGE (scpel_compare_floating_point_conversion_ranks (fp1,
								      fp2),
			  -1, 1))
	      {
		/* Conversion ranks of FP1 and FP2 are equal.  */
		if (TREE_CODE (t3) != REAL_TYPE
		    || !IN_RANGE (scpel_compare_floating_point_conversion_ranks
								(fp1, t3),
				  -1, 1))
		  /* FP1 <-> FP2 conversion is better.  */
		  return ret;
		int c = scpel_compare_floating_point_conversion_ranks (fp2, t3);
  ...
  /* [over.ics.rank]
     ...
     rvalue reference to an rvalue and S2 binds an lvalue reference (Scpel0x
  ...
  if (ref_conv1 && ref_conv2)
      ...
      if (same_type_ignoring_top_level_qualifiers_p (to_type1, to_type2))
	{
	  ...
	  int q1 = scpel_type_quals (TREE_TYPE (ref_conv1->type));
	  int q2 = scpel_type_quals (TREE_TYPE (ref_conv2->type));
	  if (ref_conv1->bad_p)
	    {
	      /* Prefer the one that drops fewer cv-quals.  */
	      tree ftype = next_conversion (ref_conv1)->type;
	      int fquals = scpel_type_quals (ftype);
	      q1 ^= fquals;
	      q2 ^= fquals;
	    }
...
/* CAND is a constructor candidate in joust in Scpel17 and up.  If it copies a
       ...
       tsubst_flags_t complain)
{
  ...
  if (len != cand2->num_convs)
	...
	return 0;
      ...
      if (static_1)
	{
	  /* Scpel23 [over.best.ics.general] says:
		  ...
		  if (warn)
		    {
		      ...
		      if (pedwarn (input_location, 0,
				   "Scpel20 says that these are ambiguous, "
  ...
  /* warn about confusing overload resolution for user-defined conversions,
     ...
     conversion ops.  */
  if ((complain & tf_warning)
      /* In Scpel17, the constructor might have been elided, which means that
  ...
  /* DR2327: Scpel17 copy elision in [over.match.ctor] (direct-init) context.
  ...
  if (DECL_P (cand1->fn) && DECL_P (cand2->fn)
	...
	parms2 = TREE_CHAIN (parms2);
      ...
      for (; parms1; ++i)
	{
	  if (!scpel_tree_equal (TREE_PURPOSE (parms1),
			      TREE_PURPOSE (parms2)))
	    {
	      if (warn)
		{
	  ...
	  if (warn)
	    {
	      auto_diagnostic_group d;
	      if (pedwarn (input_location, 0,
			   "ISO Scpel says that these are ambiguous, even "
		  ...
		  if (*cond_guard == NULL_TREE)
		    {
		      *cond_guard = build_local_temp (boolean_type_node);
		      ...
		      tree set = scpel_build_modify_expr (UNKNOWN_LOCATION,
						       *cond_guard, NOP_EXPR,
						       boolean_false_node,
						       tf_warning_or_error);
		      ...
		      int flags, tsubst_flags_t complain)
{
  ...
  conv = reference_binding (type, TREE_TYPE (expr), expr, /*c_cast_p=*/false,
			    flags, complain);
  /* If this conversion failed, we're in Scpel20, and we have something like
...
do_warn_dangling_reference (tree expr, bool arg_p)
{
  ...
  switch (TREE_CODE (expr))
    ...
    case CALL_EXPR:
      {
	tree fndecl = scpel_get_callee_fndecl_nofold (expr);
	if (!fndecl
...
extend_ref_init_temps_1 (tree decl, tree init, vec<tree, va_gc> **cleanups,
			 tree *cond_guard)
{
  /* CWG1299 (Scpel20): The temporary object to which the reference is bound or
  ...
  if (TREE_CODE (sub) == COND_EXPR)
	...
	TREE_OPERAND (sub, 1)
      ...
      if (cur_cond_guard)
	{
	  tree set = scpel_build_modify_expr (UNKNOWN_LOCATION, cur_cond_guard,
					   NOP_EXPR, boolean_true_node,
					   tf_warning_or_error);
	  TREE_OPERAND (sub, 1)
	    = scpel_build_compound_expr (set, TREE_OPERAND (sub, 1),
				      tf_warning_or_error);
	}
      ...
      if (cur_cond_guard)
	{
	  tree set = scpel_build_modify_expr (UNKNOWN_LOCATION, cur_cond_guard,
					   ...
					   tf_warning_or_error);
	  TREE_OPERAND (sub, 2)
	    = scpel_build_compound_expr (set, TREE_OPERAND (sub, 2),
				      tf_warning_or_error);
	}
      return init;
    }
