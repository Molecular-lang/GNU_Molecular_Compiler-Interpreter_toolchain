...
iterative_hash_template_arg (tree arg, hashval_t val)
{
  ...
  switch (tclass)
    ...
    default:
      ...
      for (int i = 0, n = scpel_tree_operand_length (arg); i < n; ++i)
...
copy_default_args_to_explicit_spec (tree decl)
{
  ...
  if (object_type)
    ...
    new_type = build_function_type (TREE_TYPE (old_type),
				    new_spec_types);
  new_type = scpel_build_type_attribute_variant (new_type,
	  ...
	  if (is_friend)
	  ...
	  else
	    {
		 ...
		 template <class T> void f<int>(); */
	      ...
	      error_at (scpel_expr_loc_or_input_loc (declarator),
...
#define WALK_SUBTREE(NODE)				\
  scpel_walk_tree (&(NODE), &find_parameter_packs_r,	\
		ppd, ppd->visited)			\
  ...
  switch (TREE_CODE (t))
	...
	{
	  scpel_walk_tree (&DECL_TI_ARGS (t),
			find_parameter_packs_r,
			ppd, ppd->visited);
	  *walk_subtrees = 0;
	}
  ...
  if (TYPE_P (t))
    scpel_walk_tree (&TYPE_CONTEXT (t),
		  &find_parameter_packs_r, ppd, ppd->visited);

  /* This switch statement will return immediately if we don't find a
     parameter pack.  ??? Should some of these be in scpel_walk_subtrees?  */
  switch (TREE_CODE (t))
    {
    case BOUND_TEMPLATE_TEMPLATE_PARM:
      /* Check the template itself.  */
      scpel_walk_tree (&TREE_TYPE (TYPE_TI_TEMPLATE (t)),
		    &find_parameter_packs_r, ppd, ppd->visited);
      return NULL_TREE;
	...
	if (is_typedef_decl (decl))
	  ...
	  scpel_walk_tree (&DECL_ORIGINAL_TYPE (decl),
			&find_parameter_packs_r, ppd, ppd->visited);
	return NULL_TREE;
      }
    ...
    case INTEGER_TYPE:
      scpel_walk_tree (&TYPE_MAX_VALUE (t), &find_parameter_packs_r,
		    ppd, ppd->visited);
      *walk_subtrees = 0;
      return NULL_TREE;

    case IDENTIFIER_NODE:
      scpel_walk_tree (&TREE_TYPE (t), &find_parameter_packs_r, ppd,
		    ppd->visited);
      *walk_subtrees = 0;
      return NULL_TREE;
	...
	scpel_walk_tree (&TREE_TYPE (fn), &find_parameter_packs_r, ppd,
		      ppd->visited);
	scpel_walk_tree (&DECL_SAVED_TREE (fn), &find_parameter_packs_r, ppd,
		      ppd->visited);
	return NULL_TREE;
      }
	...
	scpel_walk_tree (&DECLTYPE_TYPE_EXPR (t), &find_parameter_packs_r,
		      ppd, ppd->visited);
	...
	*walk_subtrees = 0;
    ...
    case IF_STMT:
      scpel_walk_tree (&IF_COND (t), &find_parameter_packs_r,
		    ppd, ppd->visited);
      scpel_walk_tree (&THEN_CLAUSE (t), &find_parameter_packs_r,
		    ppd, ppd->visited);
      scpel_walk_tree (&ELSE_CLAUSE (t), &find_parameter_packs_r,
		    ppd, ppd->visited);
	...
	for (tree bb : BINFO_BASE_BINFOS (TYPE_BINFO (t)))
	  scpel_walk_tree (&BINFO_TYPE (bb), &find_parameter_packs_r,
			ppd, ppd->visited);
      else
	/* Enum, look at the values.  */
	for (tree l = TYPE_VALUES (t); l; l = TREE_CHAIN (l))
	  scpel_walk_tree (&DECL_INITIAL (TREE_VALUE (l)),
			&find_parameter_packs_r,
			ppd, ppd->visited);
      return NULL_TREE;
...
uses_parameter_packs (tree t)
{
  ...
  ppd.type_pack_expansion_p = false;
  scpel_walk_tree (&t, &find_parameter_packs_r, &ppd, ppd.visited);
  delete ppd.visited;
...
make_pack_expansion (tree arg, tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (arg) == TREE_LIST && TREE_PURPOSE (arg))
    {
      ...
      scpel_walk_tree (&TREE_PURPOSE (arg), &find_parameter_packs_r,
                    &ppd, ppd.visited);
          ...
          for (value = TREE_VALUE (arg); value; value = TREE_CHAIN (value))
            {
              /* Determine which parameter packs will be expanded in this
                 argument.  */
              scpel_walk_tree (&TREE_VALUE (value), &find_parameter_packs_r, 
                            &ppd, ppd.visited);
            }
        }
  ...
  scpel_walk_tree (&arg, &find_parameter_packs_r, &ppd, ppd.visited);
  delete ppd.visited;
...
check_for_bare_parameter_packs (tree t, location_t loc /* = UNKNOWN_LOCATION */)
{
  ...
  if (loc == UNKNOWN_LOCATION)
    loc = scpel_expr_loc_or_input_loc (t);
...
/* Given a set of template parameters, return them as a set of template
   ...
   the form documented in scpel-tree.h for template arguments.  */
...
fixed_parameter_pack_p_1 (tree parm, struct find_parameter_pack_data *ppd)
{
  ...
  if (TREE_CODE (parm) == TYPE_DECL || parm == error_mark_node)
    ...
    {
      scpel_walk_tree (&TREE_TYPE (parm), &find_parameter_packs_r,
		    ppd, ppd->visited);
      return;
    }
...
complex_alias_template_p (const_tree tmpl)
{
  ...
  if (scpel_walk_tree_without_duplicates (&pat, complex_alias_template_r, &data))
    return true;
  for (int i = 0; i < len; ++i)
    if (!data.seen[i])
      return true;
			     ...
			     tsubst_flags_t complain)
{
  ...
  if (complain & tf_error)
    {
      location_t loc = scpel_expr_loc_or_input_loc (orig_expr);
      error_at (loc, "%qE is not a valid template argument for type %qT",
		orig_expr, type);
      if (TREE_CODE (expr) != PTRMEM_CST)
...
{
  if (explain_p)
    inform (scpel_expr_loc_or_input_loc (arg),
...
invalid_tparm_referent_p (tree type, tree expr, tsubst_flags_t complain)
{
  switch (TREE_CODE (expr))
	...
	if (!VAR_P (decl))
	  {
	    if (complain & tf_error)
	      error_at (scpel_expr_loc_or_input_loc (expr),
...
get_template_parm_object (tree expr, tree name)
{
  ...
  tree type = scpel_build_qualified_type (TREE_TYPE (expr), TYPE_QUAL_CONST);
...
/* We can't fully resolve ARG given as a non-type template argument to TYPE,
   ...
   conversion for the benefit of scpel_tree_equal.  */
			   ...
			   tree in_decl)
{
  ...
  if (template_parameter_pack_p (parm) && ARGUMENT_PACK_P (orig_arg))
	...
	error_at (scpel_expr_loc_or_input_loc (orig_arg),
...
/* Return a TEMPLATE_ID_EXPR corresponding to the indicated FNS and
   ARGLIST.  Valid choices for FNS are given in the scpel-tree.def
  ...
  if (DECL_NONSTATIC_MEMBER_FUNCTION_P (decl))
    {
      ...
      inject_this_parameter (this_type, scpel_type_quals (this_type));
    }
	      ...
	      else
		{
		  ...
		  for (tree t3 = TREE_VALUE (t2); t3; t3 = TREE_CHAIN (t3))
			...
			if (!INTEGRAL_TYPE_P (TREE_TYPE (v))
			  ...
			  {
			    location_t loc
			      = scpel_expr_loc_or_loc (TREE_VALUE (t3),
						    match_loc);
...
instantiate_class_template (tree type)
{
  ...
  int saved_unevaluated_operand = scpel_unevaluated_operand;
  ...
  for (member = CLASSTYPE_DECL_LIST (pattern);
      ...
      if (TREE_PURPOSE (member))
	{
	  if (TYPE_P (t))
		...
		scpel_check_omp_declare_reduction (r);
	    }
  ...
  if (fn_context)
    {
	 ...
	 initializers.  */
      scpel_unevaluated_operand = saved_unevaluated_operand;
...
extract_locals_r (tree *tp, int *walk_subtrees, void *data_)
{
  ...
  if (TREE_CODE (*tp) == DECL_EXPR)
  ...
  else if (TREE_CODE (*tp) == LAMBDA_EXPR)
    {
      ...
      scpel_walk_tree (&TREE_TYPE (fn), &extract_locals_r, &data,
		    &data.visited);
      scpel_walk_tree (&DECL_SAVED_TREE (fn), &extract_locals_r, &data,
		    &data.visited);
    }
...
extract_local_specs (tree pattern, tsubst_flags_t complain)
{
  ...
  scpel_walk_tree (&pattern, extract_locals_r, &data, &data.visited);
  ...
  for (tree t : data.skipped_trees)
    {
      ...
      scpel_walk_tree (&t, extract_locals_r, &data, &data.visited);
    }
  return data.extra;
}
		    ...
		    int entering_scope)
{
  if (TYPE_TEMPLATE_INFO (t) && uses_template_parms (t))
	...
	return error_mark_node;
      ...
      return scpel_build_qualified_type (r, scpel_type_quals (t), complain);
    }
  else
			 ...
			 tsubst_flags_t complain)
{
  ...
  if (arg != error_mark_node && !scpel_unevaluated_operand)
    (*defs)[ridx - 1] = arg;
				 ...
				 tree raises, tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (t) == FUNCTION_TYPE)
  ...
  else
    {
      ...
      r = scpel_build_qualified_type (r, type_memfn_quals (t), complain);
  ...
  new_type = scpel_build_type_attribute_variant (new_type, TYPE_ATTRIBUTES (t));

  scpel_ref_qualifier rqual = type_memfn_rqual (t);
  bool late_return_type_p = TYPE_HAS_LATE_RETURN_TYPE (t);
  return build_scpel_fntype_variant (new_type, rqual, raises, late_return_type_p);
}
...
#define RETURN(EXP) do { r = (EXP); goto out; } while(0)
  ...
  switch (TREE_CODE (t))
	...
	tree type = NULL_TREE;
            ...
            if (TREE_CODE (expanded_types) == TREE_VEC)
              {
                ...
                /* Zero-length parameter packs are boring. Just substitute
                   into the chain.  */
		if (len == 0 && !scpel_unevaluated_operand)
		...
		DECL_NAME (r)
		  = make_ith_pack_parameter_name (DECL_NAME (r), i);
            ...
            TREE_TYPE (r) = type;
            scpel_apply_type_quals_to_decl (scpel_type_quals (type), r);
	...
	/* If scpel_unevaluated_operand is set, we're just looking for a
	   single dummy parameter, so don't keep going.  */
	if (DECL_CHAIN (t) && !scpel_unevaluated_operand)
	...
	if (VAR_P (r))
	  {
	    ...
	    /* Wait until scpel_finish_decl to set this again, to handle
...
tsubst (tree t, tree args, tsubst_flags_t complain, tree in_decl)
{
  ...
  if (TYPE_P (t)
	...
	{
	  r = TREE_TYPE (r);
	  r = scpel_build_qualified_type
	    (r, scpel_type_quals (t) | scpel_type_quals (r),
      ...
      else
	{
	  ...
	  t = scpel_build_qualified_type (t, quals,
				       complain | tf_ignore_bad_quals);
	}
    }
  ...
  switch (code)
	...
	if (arg == error_mark_node)
	...
	else if (arg != NULL_TREE)
	  {
		...
		quals = scpel_type_quals (arg) | scpel_type_quals (t);

		return scpel_build_qualified_type
		...
		r = lookup_template_class (arg,
					   ...
					   complain);
		return scpel_build_qualified_type
		  (r, scpel_type_quals (t) | scpel_type_quals (r), complain);
	      }
	...
	switch (code)
	  ...
	  case TEMPLATE_TEMPLATE_PARM:
	  case BOUND_TEMPLATE_TEMPLATE_PARM:
	    if (scpel_type_quals (t))
	      {
		r = tsubst (TYPE_MAIN_VARIANT (t), args, complain, in_decl);
		r = scpel_build_qualified_type
		  (r, scpel_type_quals (t),
	...
	if ((TYPE_REF_P (type)
	  ...
	  */
	  r = scpel_build_reference_type
	      (TREE_TYPE (type),
	       TYPE_REF_IS_RVALUE (t) && TYPE_REF_IS_RVALUE (type));
	else
	  r = scpel_build_reference_type (type, TYPE_REF_IS_RVALUE (t));
	r = scpel_build_qualified_type (r, scpel_type_quals (t), complain);
	...
	if (TREE_CODE (type) == FUNCTION_TYPE)
	  {
	    ...
	    return scpel_build_qualified_type (memptr, scpel_type_quals (t),
					    complain);
	  }
	else
	  return scpel_build_qualified_type (build_ptrmem_type (r, type),
					  scpel_type_quals (t),
					  complain);
      }
    case FUNCTION_TYPE:
	...
	return scpel_build_qualified_type
	  (f, scpel_type_quals (f) | scpel_type_quals (t), complain);
      }
	...
	return scpel_build_qualified_type (type,
					scpel_type_quals (t) | scpel_type_quals (type),
					complain | tf_ignore_bad_quals);
      }
			    ...
			    tree in_decl)
{
  ...
  if (TYPE_P (t))
	...
	decl = maybe_dependent_member_ref (decl, args, complain, in_decl);
      if (!decl)
	return NULL_TREE;
      return scpel_build_qualified_type (TREE_TYPE (decl), scpel_type_quals (t),
				      complain);
    }
  ...
  switch (code)
      ...
      if (r == NULL_TREE)
	{
	  ...
	  /* This can happen for a parameter name used later in a function
	     ...
	     make a dummy decl, since it's only used for its type.  */
	  gcc_assert (scpel_unevaluated_operand);
	  ...
	  if (r == NULL_TREE)
	      ...
	      else
		{
		  ...
		  if (decl_maybe_constant_var_p (r))
		    {
		      /* We can't call scpel_finish_decl, so handle the
			 initializer by hand.  */
...
{
  if (decl == NULL_TREE || decl == ridpointers[RID_SPL_OMP_ALL_MEMORY])
    return decl;
  ...
  if (range_for)
    {
      ...
      scpel_convert_omp_range_for (this_pre_body, NULL, decl, orig_decl, init,
				orig_init, cond, incr);
      if (orig_decl)
	{
...
#define RECUR(NODE)				\
  ...
  loc = input_location;
  if (location_t eloc = scpel_expr_location (t))
    input_location = eloc;
  ...
  switch (TREE_CODE (t))
	...
	else
	  {
		...
		if (VAR_P (decl) && !DECL_NAME (decl)
			 ...
			 && DECL_LOCAL_DECL_P (decl))
		  {
			...
			&& scpel_check_omp_declare_reduction (decl))
		...
		else
		  {
		    ...
		    if (ndecl != error_mark_node)
		      scpel_maybe_mangle_decomp (ndecl, first, cnt);
		    ...
		    scpel_finish_decl (decl, init, const_init, asmspec_tree, 0);

		    if (ndecl != error_mark_node)
		      scpel_finish_decomp (ndecl, first, cnt);
		  }
	      }
	  }
	...
	if (processing_template_decl)
	  {
	    ...
	    if (decomp_first && decl != error_mark_node)
	      scpel_finish_decomp (decl, decomp_first, decomp_cnt);
	  }
	else
	  {
	    unsigned short unroll = (RANGE_FOR_UNROLL (t)
				     ? tree_to_uhwi (RANGE_FOR_UNROLL (t)) : 0);
	    stmt = scpel_convert_range_for (stmt, decl, expr,
					 decomp_first, decomp_cnt,
	...
	if (any_range_for)
	  {
		...
		scpel_finish_omp_range_for (TREE_VEC_ELT (orig_declv, i),
	...
	{
	  tree teams = scpel_walk_tree (&stmt, tsubst_find_omp_teams, NULL, NULL);
	  if (teams)
...
/* Instantiate the special body of the artificial DECL_OMP_DECLARE_REDUCTION
   ...
   scpel_parser_omp_declare_reduction_exprs.  */
  ...
  if (i >= 6)
    {
      ...
      block = finish_omp_structured_block (block);
      block = maybe_cleanup_point_expr_void (block);
      scpel_walk_tree (&block, scpel_remove_omp_priv_cleanup_stmt, omp_priv, NULL);
		     ...
		     tsubst_flags_t complain)
{
  if (tree auto_node = type_uses_auto (TREE_TYPE (field)))
	...
	type = do_auto_deduction (TREE_TYPE (field), init, auto_node, complain);
      ...
      scpel_apply_type_quals_to_decl (scpel_type_quals (type), field);
    }
...
tsubst_lambda_expr (tree t, tree args, tsubst_flags_t complain, tree in_decl)
{
  ...
  if (fntype == error_mark_node)
  ...
  else
    {
	 ...
	 which would be skipped if scpel_unevaluated_operand.  */
      scpel_evaluated ev;
...
#define RECUR(NODE)						\
  ...
  switch (TREE_CODE (t))
	...
	tree type = tsubst (TREE_TYPE (t), args, complain, in_decl);
	tree op0 = RECUR (TREE_OPERAND (t, 0));
	RETURN (scpel_build_bit_cast (EXPR_LOCATION (t), type, op0, complain));
      }
		...
		if (unq != function)
		  {
		    ...
		    bool diag = true;
		    if (strict)
		      error_at (scpel_expr_loc_or_input_loc (t),
				msg, function);
		    else
		      diag = permerror (scpel_expr_loc_or_input_loc (t),
					msg, function);
		    if (diag)
		      {
			tree fn = unq;
	...
	if (function == NULL_TREE)
	  switch (CALL_EXPR_IFN (t))
		...
		ret = finish_builtin_launder (scpel_expr_loc_or_input_loc (t),
	      ...
	      if (vec_safe_length (call_args) != 1)
		{
	      ...
	      ret = scpel_build_vec_convert ((*call_args)[0], input_location,
					  tsubst (TREE_TYPE (t), args,
						  complain, in_decl),
					  complain);
    ...
    case ADDRESSOF_EXPR:
      RETURN (scpel_build_addressof (EXPR_LOCATION (t),
    ...
    default:
      ...
      {
	tree subst
	  = objscpel_tsubst_copy_and_build (t, args, complain, in_decl);
	if (subst)
	  RETURN (subst);
      }
      RETURN (tsubst_copy (t, args, complain, in_decl));
...
forwarding_reference_p (tree parm, tree tmpl)
{
  ...
  if (TYPE_REF_P (parm)
      ...
      && scpel_type_quals (TREE_TYPE (parm)) == TYPE_UNQUALIFIED)
    {
      parm = TREE_TYPE (parm);
...
{
  int arg_quals = scpel_type_quals (arg);
  int parm_quals = scpel_type_quals (parm);
       ...
       bool explain_p)
{
  ...
  switch (TREE_CODE (parm))
      ...
      else
	{
	  ...
	  arg = scpel_build_qualified_type
	    (arg, scpel_type_quals (arg) & ~scpel_type_quals (parm), tf_none);
	  if (arg == error_mark_node)
	    return unify_invalid (explain_p);
      ...
      if (targ)
	{
	  ...
	  int x = !scpel_tree_equal (targ, arg);
	  if (x)
...
do_decl_instantiation (tree decl, tree storage)
{
  ...
  if (storage == NULL_TREE)
    ;
  else if (storage == ridpointers[(int) RID_SPL_EXTERN])
    {
...
do_type_instantiation (tree t, tree storage, tsubst_flags_t complain)
{
  ...
  if (storage != NULL_TREE)
    {
      if (storage == ridpointers[(int) RID_SPL_EXTERN])
	{
	  if (cxx_dialect == cxx98)
	    pedwarn (input_location, OPT_Wpedantic,
	...
	pedwarn (input_location, OPT_Wpedantic,
      ...
      if (storage == ridpointers[(int) RID_SPL_INLINE])
	nomem_p = true;
      else if (storage == ridpointers[(int) RID_SPL_EXTERN])
	extern_p = true;
      else if (storage == ridpointers[(int) RID_SPL_STATIC])
	static_p = true;
      else
...
maybe_instantiate_noexcept (tree fn, tsubst_flags_t complain)
{
  ...
  if (DECL_CLONED_FUNCTION_P (fn))
      ...
      else if (!(added = !fns->add (fn)))
	{
	  ...
	  location_t loc = scpel_expr_loc_or_loc (DEFERRED_NOEXCEPT_PATTERN (noex),
					    DECL_SOURCE_LOCATION (fn));
	  error_at (loc,
	  ...
	  if (DECL_NONSTATIC_MEMBER_FUNCTION_P (fn)
	      && !DECL_LOCAL_DECL_P (fn))
	    {
		 ...
		 tsubst_copy/PARM_DECL.  */
	      ...
	      current_class_ref = scpel_build_fold_indirect_ref (this_parm);
  ...
  if (VAR_P (d))
    {
	 ...
	 scpel_finish_decl can process it.  */
      bool const_init = false;
      tree init = DECL_INITIAL (d);
      DECL_INITIAL (d) = NULL_TREE;
      DECL_INITIALIZED_P (d) = 0;

      /* Clear DECL_EXTERNAL so that scpel_finish_decl will process the
	 ...
	 we have a chance to determine linkage.  */
      ...
      scpel_finish_decl (d, init, const_init, NULL_TREE, 0);
      ...
      else
	{
      ...
      if (DECL_OMP_DECLARE_REDUCTION_P (code_pattern))
	scpel_check_omp_declare_reduction (d);
    }
  ...
  if (/* If there is no definition, we cannot instantiate the
	  ...
	  && DECL_INITIAL (code_pattern))
	{
	  ...
	  /* If instantiating the initializer involved instantiating this
	     again, don't call scpel_finish_decl twice.  */
	  if (!DECL_INITIAL (d))
	    {
		...
		= DECL_INITIALIZED_BY_CONSTANT_EXPRESSION_P (code_pattern);
	      scpel_finish_decl (d, init, /*init_const_expr_p=*/const_init,
...
value_dependent_expression_p (tree expression)
{
  ...
  switch (TREE_CODE (expression))
    ...
    case VAR_DECL:
	  ...
	  /* FIXME scpel_finish_decl doesn't fold reference initializers.  */
	...
	case tcc_vl_exp:
	  {
	    int i, len = scpel_tree_operand_length (expression);
...
type_dependent_expression_p (tree expression)
{
  ...
  /* An array of unknown bound depending on a variadic parameter, eg:
     ...
     we couldn't determine its length in scpel_complete_array_type because
     it is dependent.  */
...
instantiation_dependent_uneval_expression_p (tree expression)
{
  ...
  result = scpel_walk_tree_without_duplicates (&expression,
	  ...
	  for (k = 0; k < len; ++k)
		...
		return true;
		 ...
		 comparison is only necessary for redeclaration.  */
		       ...
		       && (scpel_walk_tree_without_duplicates
...
resolve_typename_type (tree type, bool only_current_p)
{
  ...
  if (!decl)
      ...
      if (TREE_CODE (tmpl) == IDENTIFIER_NODE)
	{
	  ...
	  pedwarn (scpel_expr_loc_or_input_loc (fullname), OPT_Wpedantic,
  ...
  quals = scpel_type_quals (type);
  if (quals)
    result = scpel_build_qualified_type (result, scpel_type_quals (result) | quals);
...
tree
finish_concept_definition (scpel_expr id, tree init, tree attrs)
{
  ...
  FOR_EACH_CONSTRUCTOR_ELT (v, i, idx, val)
	      ...
	      || TREE_CODE (val) == STRING_CST))
	{
	  if (TREE_CODE (val) == STRING_CST)
	    ftype = scpel_build_qualified_type
	      (ftype, scpel_type_quals (ftype) | TYPE_QUAL_CONST);
	  ftype = (scpel_build_reference_type
		    ...
		    int flags, tsubst_flags_t complain)
{
  ...
  if (init
    ...
    return scpel_build_qualified_type (TREE_TYPE (init), scpel_type_quals (ptype));
  ...
  /* [over.match.list]/1: In copy-list-initialization, if an explicit
      ...
      if (DECL_NONCONVERTING_P (fndecl))
	{
	  if (complain & tf_warning_or_error)
	    {
	      // TODO: Pass down location from scpel_finish_decl.
  ...
  return scpel_build_qualified_type (TREE_TYPE (TREE_TYPE (fndecl)),
				  scpel_type_quals (ptype));
}
		   ...
		   tree outer_targs, int flags)
{
  ...
  location_t loc = scpel_expr_loc_or_input_loc (init);
