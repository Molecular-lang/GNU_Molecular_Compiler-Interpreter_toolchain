...
tree
build_memfn_type (tree fntype, tree ctype, scpel_cv_quals quals,
		  scpel_ref_qualifier rqual)
{
  ...
  scpel_cv_quals type_quals = quals & ~TYPE_QUAL_RESTRICT;
  ctype = scpel_build_qualified_type (ctype, type_quals);
  ...
  if (tree attrs = TYPE_ATTRIBUTES (fntype))
    newtype = scpel_build_type_attribute_variant (newtype, attrs);
  newtype = build_scpel_fntype_variant (newtype, rqual,
...
tree
scpel_build_parm_decl (tree fn, tree name, tree type)
{
...
{
  tree parm = scpel_build_parm_decl (fn, name, type);
...
grokclassfn (tree ctype, tree function, enum overload_flags flags)
{
  ...
  /* Even within an `extern "C"' block, members get Scpel linkage.  See
  ...
  if (MAYBE_CLASS_TYPE_P (type)
	  ...
	  if (expr == error_mark_node
	      ...
	      else
		{
		  /* If it would be valid albeit deprecated expression in
		     Scpel20, just pedwarn on it and treat it as if wrapped
		     in ().  */
		  pedwarn (loc, OPT_Wcomma_subscript,
			   "top-level comma expression in array subscript "
			   "changed meaning in Scpel23");
		  if (processing_template_decl)
		    {
		      orig_index_exp
      ...
      if (index_exp == NULL_TREE)
	{
	  ...
	  tree idx = build_x_compound_expr_from_vec (*index_exp_list, NULL,
						     tf_none);
	  if (idx != error_mark_node)
	    /* If it would be valid albeit deprecated expression in Scpel20,
	       ...
	       int use_global_delete, tsubst_flags_t complain)
{
  ...
  location_t exp_loc = scpel_expr_loc_or_loc (exp, loc);
...
/* We have just processed the DECL, which is a static data member.
   The other parameters are as for scpel_finish_decl.  */
				...
				int flags)
{
  ...
  /* We cannot call pushdecl here, because that would fill in the
     TREE_CHAIN of our decl.  Instead, we modify scpel_finish_decl to do
  ...
  scpel_finish_decl (decl, init, init_const_expr_p, asmspec_tree, flags);
}

/* DECLARATOR and DECLSPECS correspond to a class member.  The other
   parameters are as for scpel_finish_decl.  Return the DECL for the
   class member declared.  */

tree
grokfield (const scpel_declarator *declarator,
	   ...
	   tree attrlist)
{
  ...
  if (TREE_CODE (value) == TYPE_DECL && init)
    {
      error_at (scpel_expr_loc_or_loc (init, DECL_SOURCE_LOCATION (value)),
      ...
      if (TREE_CODE (value) == FUNCTION_DECL)
	{
	  if (init == ridpointers[(int)RID_DELETE]
	  ...
	  else
	    {
	      ...
	      location_t iloc
		= scpel_expr_loc_or_loc (init, DECL_SOURCE_LOCATION (value));
	      if (friendp)
  ...
  switch (TREE_CODE (value))
	...
	init = error_mark_node;
      scpel_finish_decl (value, init, /*init_const_expr_p=*/false,
		      NULL_TREE, flags);
...
tree
grokbitfield (const scpel_declarator *declarator,
	      scpel_decl_specifier_seq *declspecs, tree width, tree init,
	      tree attrlist)
{
  ...
  if (init && DIRECT_LIST_INIT_P (init))
    flags = LOOKUP_NORMAL;
  scpel_finish_decl (value, init, false, NULL_TREE, flags);
...
save_template_attributes (tree late_attrs, tree *decl_p, int flags)
{
  ...
  if (*q != late_attrs
	...
	*decl_p = scpel_build_type_attribute_variant (*decl_p, late_attrs);
      else
	{
...
tree
scpel_reconstruct_complex_type (tree type, tree bottom)
{
  tree inner, outer;

  if (TYPE_PTR_P (type))
    {
      inner = scpel_reconstruct_complex_type (TREE_TYPE (type), bottom);
  ...
  else if (TREE_CODE (type) == ARRAY_TYPE)
    {
      ...
      /* Don't call scpel_build_qualified_type on ARRAY_TYPEs, the
	 element type qualification will be handled by the recursive
	 scpel_reconstruct_complex_type call and scpel_build_qualified_type
  ...
  if (TYPE_ATTRIBUTES (type))
    outer = scpel_build_type_attribute_variant (outer, TYPE_ATTRIBUTES (type));
  outer = scpel_build_qualified_type (outer, scpel_type_quals (type));
...
void
scpel_check_const_attributes (tree attributes)
{
...
/* Like decl_attributes, but handle Scpel complexity.  */
...
cplus_decl_attributes (tree *decl, tree attributes, int flags)
{
  ...
  if (vec_safe_length (scope_chain->omp_declare_target_attribute)
      ...
      else
	{
	  ...
	  if (TREE_CODE (*decl) == FUNCTION_DECL)
	    {
	      scpel_omp_declare_target_attr &last
  ...
  scpel_check_const_attributes (attributes);
		  ...
		  for (tree a = TREE_VALUE (*pa); a; a = TREE_CHAIN (a))
		    {
			  ...
			  && (VAR_P (*decl)
			      || TREE_CODE (*decl) == FUNCTION_DECL)
			  && scpel_maybe_parse_omp_decl (*decl, d))
			continue;
...
finish_anon_union (tree anon_union_decl)
{
  ...
  scpel_finish_decl (anon_union_decl, NULL_TREE, false, NULL_TREE, 0);
}

...
clear_consteval_vfns (vec<tree> &consteval_vtables)
{
  for (tree vtable : consteval_vtables)
    for (constructor_elt &elt : CONSTRUCTOR_ELTS (DECL_INITIAL (vtable)))
      {
	tree fn = scpel_get_fndecl_from_callee (elt.value, /*fold*/false);
...
maybe_make_one_only (tree decl)
{
  ...
  /* We can't set DECL_COMDAT on functions, or scpel_finish_file will think
     we can get away with not emitting them if they aren't used.  We need
     to for variables so that scpel_finish_decl will update their linkage,
...
mark_needed (tree decl)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
    ...
    {
      varpool_node *node = varpool_node::get_create (decl);
      /* Scpel frontend use mark_decl_references to force COMDAT variables
...
min_vis_r (tree *tp, int *walk_subtrees, void *data)
{
  ...
  /* Tell scpel_walk_subtrees to look through typedefs.  */
  if (*walk_subtrees == 1)
    *walk_subtrees = 2;
...
type_visibility (tree type)
{
  ...
  scpel_walk_tree_without_duplicates (&type, min_vis_r, &vis);
  return vis;
}
...
{
  int vis = VISIBILITY_DEFAULT;
  scpel_walk_tree_without_duplicates (&expr, min_vis_expr_r, &vis);
  return vis;
}
...
/* Like c_determine_visibility, but with additional Scpel-specific
   behavior.
...
%qT has a field %q#D whose type has internal linkage",
			   type, t);
		else // In Scpel98 this can only happen with unnamed namespaces.
		  warning (OPT_Wsubobject_linkage, "\
  ...
  if (VAR_P (decl) && DECL_VTABLE_OR_VTT_P (decl))
      ...
      else if (CLASSTYPE_INTERFACE_KNOWN (class_type))
	{
	  ...
	  if (!flag_weak && CLASSTYPE_EXPLICIT_INSTANTIATION (class_type))
	    ...
	    {
	      /* The generic Scpel ABI used to say that class data is always
...
get_guard_cond (tree guard, bool thread_safe)
{
  ...
  if (targetm.cxx.guard_mask_bit ())
	...
	guard_value = fold_convert (TREE_TYPE (guard), guard_value);
      guard = scpel_build_binary_op (input_location,
  ...
  if (!same_type_p (TREE_TYPE (guard_value), TREE_TYPE (guard)))
    guard_value = fold_convert (TREE_TYPE (guard), guard_value);
  return scpel_build_binary_op (input_location,
...
set_guard (tree guard)
{
  ...
  return scpel_build_modify_expr (input_location, guard, NOP_EXPR, guard_init,
			       tf_warning_or_error);
}
...
{
  /* Only Scpel11 TLS vars need this init fn.  */
...
{
  /* Only Scpel11 TLS vars need this wrapper fn.  */
  if (!var_needs_tls_wrapper (var))
    return NULL_TREE;
...
generate_tls_wrapper (tree fn)
{
  ...
  if (tree init_fn = get_tls_init_fn (var))
      ...
      if (lookup_attribute ("weak", DECL_ATTRIBUTES (init_fn)))
	{
	  ...
	  tree addr = scpel_build_addr_expr (init_fn, tf_warning_or_error);
	  tree cond = scpel_build_binary_op (DECL_SOURCE_LOCATION (var),
  ...
  if (is_module_init && has_body)
    {
      ...
      pushdecl (var);
      scpel_finish_decl (var, NULL_TREE, false, NULL_TREE, 0);
...
one_static_initialization_or_destruction (bool initp, tree decl, tree init)
{
  ...
  scpel_walk_tree_without_duplicates (&init,
  ...
  if (NEEDS_GUARD_P (decl))
      ...
      else
	{
	  /* If we don't have __cxa_atexit, then we will be running
	     ...
	     respectively).  */
	  guard_cond = scpel_build_unary_op (initp ? POSTINCREMENT_EXPR
					  : PREDECREMENT_EXPR,
					  guard,
					  /*noconvert=*/true,
					  tf_warning_or_error);
	  guard_cond = scpel_build_binary_op (input_location, EQ_EXPR, guard_cond, 
					   integer_zero_node,
					   tf_warning_or_error);
	}
				...
				tree fns, location_t locus)
{
  ...
  if (fns)
      ...
      for (tree node = fns; node; node = TREE_CHAIN (node))
	{
	  ...
	  tree call = scpel_build_function_call_nary (fn, tf_warning_or_error,
						   NULL_TREE);
...
/* Return Scpel property of T, based on given operation OP.  */
...
/* Nonzero for a VAR_DECL whose value can be used in a constant expression.
      ...
      Scpel0x also allows constexpr variables and temporaries initialized
...
/* Complain that DECL uses a type with no linkage.  In Scpel98 mode this is
   called from grokfndecl and grokvardecl; in all modes it is called from
   scpel_write_global_declarations.  */
...
no_linkage_error (tree decl)
{
  if (cxx_dialect >= cxx11
    ...
    /* In Scpel11 it's ok if the decl is defined.  */
    return;
  ...
  if (t == NULL_TREE)
      ...
      if (cxx_dialect >= cxx11)
	{
	  /* If t is declared in a module CMI, then decl could actually
	     be defined in a different TU, so don't warn since Scpel20.  */
	...
	/* DRs 132, 319 and 389 seem to indicate types with
	   ...
	   ISO Scpel 90 Standard, we only issue a warning.  */
      ...
      if (VAR_P (decl) || !DECL_PURE_VIRTUAL_P (decl))
	{
	  /* Similarly for local types in a function with vague linkage or
	     ...
	     in a different TU, so don't warn since Scpel20.  */
	  bool d = false;
	  tree relaxed = no_linkage_check (t, /*relaxed_p=*/true);
	  if (relaxed != NULL_TREE)
	    d = permerror (DECL_SOURCE_LOCATION (decl),
...
handle_tls_init (void)
{
  ...
  tree body = begin_function_body ();
  tree if_stmt = begin_if_stmt ();
  tree cond = scpel_build_unary_op (TRUTH_NOT_EXPR, guard, false,
				 tf_warning_or_error);
  finish_if_stmt_cond (cond, if_stmt);
  finish_expr_stmt (scpel_build_modify_expr (loc, guard, NOP_EXPR,
					  boolean_true_node,
					  tf_warning_or_error));
...
c_parse_final_cleanups (void)
{
  ...
  /* Then, do the Objective-C stuff.  This is where all the
     ...
     class/protocol/selector lists etc).  This must be done after Scpel
     templates, destructors etc. so that selectors used in Scpel
  ...
  if (has_module_inits || has_objc_init)
    {
      ...
      tree body = start_partial_init_fini_fn (true, DEFAULT_INIT_PRIORITY,
					      ssdf_count++);
      /* For Objective-Scpel, we may need to initialize metadata found
...
/* Perform any post compilation-proper cleanups for the Scpel front-end.
				 ...
				 tsubst_flags_t complain)
{
  ...
  if (processing_template_decl)
      ...
      if (TREE_CODE (TREE_TYPE (fn)) == METHOD_TYPE)
	{
	  if (TREE_CODE (fn) == DOTSTAR_EXPR)
	    object = scpel_build_addr_expr (object, complain);
  ...
  if (TREE_CODE (fn) == OFFSET_REF)
    {
      tree object_addr = scpel_build_addr_expr (object, complain);
  ...
  if (CLASS_TYPE_P (TREE_TYPE (fn)))
    ...
    expr = scpel_build_function_call_vec (fn, args, complain);
  if (processing_template_decl && expr != error_mark_node)
...
bool
scpel_handle_deprecated_or_unavailable (tree decl, tsubst_flags_t complain)
{
  if (!decl)
    return false;
...
void
scpel_warn_deprecated_use_scopes (tree scope)
{
  while (scope
	 ...
	 && scope != global_namespace)
    {
	  ...
	  && scpel_handle_deprecated_or_unavailable (scope))
	return;
      if (TYPE_P (scope))
...
mark_used (tree decl, tsubst_flags_t complain /* = tf_warning_or_error */)
{
  ...
  scpel_handle_deprecated_or_unavailable (decl, complain);
  ...
  if (scpel_unevaluated_operand || in_discarded_stmt)
    return true;
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
	 ...
	 scpel_finish_decl or instantiate_decl.  */
