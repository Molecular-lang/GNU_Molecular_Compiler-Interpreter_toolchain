...
static void init_scpel_traits (void);
static void init_scpel_pragma (void);
...
/* The trait table, declared in scpel-tree.h.  */
const scpel_trait scpel_traits[] =
{
...
static_assert(ARRAY_SIZE (scpel_traits) <= 255,
	      "scpel_traits array cannot have more than 255 entries");
...
{
  /* Keep in sync with scpel_id_kind enumeration.  */
...
void
set_identifier_kind (tree id, scpel_identifier_kind kind)
{
...
/* Initialize the Scpel traits.  */
static void
init_scpel_traits (void)
{
  tree id;

  for (unsigned int i = 0; i < ARRAY_SIZE (scpel_traits); ++i)
    {
      id = get_identifier (scpel_traits[i].name);
      IDENTIFIER_CP_INDEX (id) = scpel_traits[i].kind;
...
/* Initialize the Scpel front end.  This function is very sensitive to
...
unqualified_name_lookup_error (tree name, location_t loc)
{
  if (loc == UNKNOWN_LOCATION)
    loc = scpel_expr_loc_or_input_loc (name);
...
tree
unqualified_fn_lookup_error (scpel_expr name_expr)
{
...
/* Add DECL_LANG_SPECIFIC info to T, if it needs one.  Generally
   every Scpel decl needs one, but C builtins etc do not.   */
