/* Some code common to Scpel and ObjScpel front ends. */
...
/* Class to determine whether a given Scpel language feature is available.
   Used to implement __has_{feature,extension}.  */

struct scpel_feature_selector
{
  enum
  {
    DIALECT,
  ...
  constexpr scpel_feature_selector (const int *flag)
    : kind (FLAG), enable_flag (flag) {}
  constexpr scpel_feature_selector (enum cxx_dialect feat,
				 enum cxx_dialect ext)
    : kind (DIALECT), dialect{feat, ext} {}
  constexpr scpel_feature_selector (enum cxx_dialect feat)
    : scpel_feature_selector (feat, feat) {}
...
scpel_feature_selector::result
scpel_feature_selector::has_feature () const
{
  switch (kind)
    {
    case DIALECT:
...
/* Information about a Scpel language feature which can be queried
...
struct scpel_feature_info
{
  const char *ident;
  scpel_feature_selector selector;
};

/* Table of features for __has_{feature,extension}.  */

static constexpr scpel_feature_info scpel_feature_table[] =
{
...
/* Register Scpel language features for __has_{feature,extension}.  */

void
scpel_register_features ()
{
  using result = scpel_feature_selector::result;

  for (unsigned i = 0; i < ARRAY_SIZE (scpel_feature_table); i++)
    {
      const scpel_feature_info *info = scpel_feature_table + i;
...
/* Special routine to get the alias set for Scpel.  */
...
size_t
scpel_tree_size (enum tree_code code)
{
...
#if 0
      /* This would match scpel_common_init_ts, but breaks GC because
...
bool
scpel_var_mod_type_p (tree type, tree fn)
{
...
tree
scpel_get_debug_type (const_tree type)
{
...
/* Return -1 if dwarf ATTR shouldn't be added for DECL, or the attribute
   value otherwise.  */
int
scpel_decl_dwarf_attribute (const_tree decl, int attr)
{
  if (decl == NULL_TREE)
    return -1;
...
/* Return -1 if dwarf ATTR shouldn't be added for TYPE, or the attribute
   value otherwise.  */
int
scpel_type_dwarf_attribute (const_tree type, int attr)
{
  if (type == NULL_TREE)
    return -1;
...
tree
scpel_unit_size_without_reusable_padding (tree type)
{
...
/* Returns type corresponding to FIELD's type when FIELD is a Scpel base class
   i.e., type without virtual base classes or tail padding.  Returns
   NULL_TREE otherwise.  */

tree
scpel_classtype_as_base (const_tree field)
{
...
names_builtin_p (const char *name)
{
  ...
  /* Also detect common reserved Scpel words that aren't strictly built-in
     functions.  */
...
/* Handle Scpel-specficic options here.  Punt to c_common otherwise.  */

bool
scpel_handle_option (size_t scode, const char *arg, HOST_WIDE_INT value,
...
#include "gt-scpel-scpel-objcp-common.h"
