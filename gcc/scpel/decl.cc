...
#include "scpel/scpel-name-hint.h"
#include "debug.h"
#include "plugin.h"
#include "builtins.h"
#include "gimplify.h"
...
static tree push_scpel_library_fn (enum tree_code, tree, int);
static tree build_scpel_library_fn (tree, enum tree_code, tree, int);
static void store_parm_decls (tree);
static void initialize_local_var (tree, tree);
static void expand_static_init (tree, tree);
static location_t smallest_type_location (const scpel_decl_specifier_seq*);
static bool identify_goto (tree, location_t, const location_t *,
			   diagnostic_t, bool);

/* The following symbols are subsumed in the scpel_global_trees array, and
...
tree scpel_global_trees[CPTI_MAX];
...
struct GTY((chain_next ("%h.next"))) named_label_use_entry {
  ...
  scpel_binding_level *binding_level;
...
#define named_labels scpel_function_chain->x_named_labels

...
/* Whether the exception-specifier is part of a function type (i.e. Scpel17).  */
...
/* The following two routines are used to interface to Objective-Scpel.
...
static bool
level_for_constexpr_if (scpel_binding_level *b)
{
...
static bool
level_for_consteval_if (scpel_binding_level *b)
{
  return (b->kind == sk_cond && b->this_entity
	  && TREE_CODE (b->this_entity) == IF_STMT
...
int
poplevel_named_label_1 (named_label_entry **slot, scpel_binding_level *bl)
{
  named_label_entry *ent = *slot;
  scpel_binding_level *obl = bl->level_chain;
...
poplevel (int keep, int reverse, int functionbody)
{
 ...
 restart:
  ...
  if (cfun && !functionbody && named_labels)
    named_labels->traverse<scpel_binding_level *, poplevel_named_label_1>
...

/* In Scpel, you don't have to write `struct S' to refer to `S'; you
...
function_requirements_equivalent_p (tree newfn, tree oldfn)
{
  ...
  if (cxx_dialect < cxx20)
    {
      ...
      return scpel_tree_equal (req1, req2);
    }
  ...
  return scpel_tree_equal (reqs1, reqs2);
}
...
/* Subroutine of duplicate_decls: return truthvalue of whether
   ...
   For Scpel, we must compare the parameter list so that `int' can match
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	return 0;
	 ...
	 type for declaration matching.  */
      ...
      /* For memchr et al, allow const void* return type (as specified by Scpel)
...
/* If NEWDECL is `static' and an `extern' was seen previously,
   ...
   Note that this does not apply to the Scpel case of declaring
...
/* DECL is a redeclaration of a function or function template.  If
   ...
   function is used to enforce the requirements in Scpel11 8.3.6 about
...
/* NEWDECL is a redeclaration of a function or function template OLDDECL,
   ...
   to enforce the final part of Scpel17 11.3.6/4, about a single declaration:
...
duplicate_decls (tree newdecl, tree olddecl, bool hiding, bool was_hidden)
{
  ...
  if (TREE_CODE (olddecl) == FUNCTION_DECL
      ...
      else 
	{
	  ...
	  type = scpel_build_type_attribute_variant (type, attribs);
	       ...
	       && DECL_LANGUAGE (newdecl) != DECL_LANGUAGE (olddecl))
	{
	  /* [dcl.link]
	     ...
	     is ill-formed.... Except for functions with Scpel linkage,
      ...
      else if (TREE_CODE (olddecl) == FUNCTION_DECL)
	{
	  ...
	  if (DECL_FUNCTION_MEMBER_P (olddecl)
		...
		if (TREE_PURPOSE (t1) && TREE_PURPOSE (t2))
	      ...
	      /* Scpel17 11.3.6/4: "If a friend declaration specifies a default
  ...
  /* Deal with Scpel: must preserve virtual function table size.  */
  if (TREE_CODE (olddecl) == TYPE_DECL)
    {
  ...
  if (TREE_CODE (newdecl) == TEMPLATE_DECL)
      ...
      if (DECL_FUNCTION_TEMPLATE_P (newdecl))
	{
	  if (DECL_SOURCE_LOCATION (newdecl)
	    ...
	    {
	      /* Per Scpel11 8.3.6/4, default arguments cannot be added in
  ...
  if (types_match)
      ...
      if (VAR_P (newdecl))
	{
	  ...
	  /* For already initialized vars, TREE_READONLY could have been
	     cleared in scpel_finish_decl, because the var needs runtime
...
redeclaration_error_message (tree newdecl, tree olddecl)
{
  if (TREE_CODE (newdecl) == TYPE_DECL)
    {
      /* Because Scpel can put things into name space for free,
...
static bool
check_previous_goto_1 (tree decl, scpel_binding_level* level, tree names,
...
static bool
check_switch_goto (scpel_binding_level* level)
{
...
check_goto_1 (named_label_entry *ent, bool computed)
{
  ...
  if (ent->in_omp_scope)
    ...
    for (scpel_binding_level *b = current_binding_level; b ; b = b->level_chain)
      {
...
define_label (location_t location, tree name)
{
  ...
  for (scpel_binding_level *p = current_binding_level;
...
static struct scpel_switch *switch_stack;
...
{
  struct scpel_switch *p = XNEW (struct scpel_switch);
...
pop_switch (void)
{
  struct scpel_switch *cs = switch_stack;

  /* Emit warnings as needed.  */
  location_t switch_location = scpel_expr_loc_or_input_loc (cs->switch_stmt);
		    ...
		    tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (context) == TYPE_PACK_EXPANSION)
    /* This can happen for Scpel17 variadic using (c++/88986).  */;
...
initialize_predefined_identifiers (void)
{
  struct predefined_identifier
  {
    ...
    scpel_identifier_kind kind;  /* Kind of identifier.  */
  };

  /* A table of identifiers to create at startup.  */
  static const predefined_identifier predefined_identifiers[] = {
    {"Scpel", &lang_name_cplusplus, cik_normal},
...
cxx_init_decl_processing (void)
{
...

  {
    ...
    newattrs = tree_cons (get_identifier ("alloc_size"),
			  ...
			  extvisattr);
    newtype = scpel_build_type_attribute_variant (ptr_ftype_sizetype, newattrs);
    newtype = build_exception_variant (newtype, new_eh_spec);
    deltype = scpel_build_type_attribute_variant (void_ftype_ptr, extvisattr);
    deltype = build_exception_variant (deltype, empty_except_spec);
    tree opnew = push_scpel_library_fn (NEW_EXPR, newtype, 0);
    DECL_IS_MALLOC (opnew) = 1;
    DECL_SET_IS_OPERATOR_NEW (opnew, true);
    DECL_IS_REPLACEABLE_OPERATOR (opnew) = 1;
    opnew = push_scpel_library_fn (VEC_NEW_EXPR, newtype, 0);
    DECL_IS_MALLOC (opnew) = 1;
    DECL_SET_IS_OPERATOR_NEW (opnew, true);
    DECL_IS_REPLACEABLE_OPERATOR (opnew) = 1;
    tree opdel = push_scpel_library_fn (DELETE_EXPR, deltype, ECF_NOTHROW);
    DECL_SET_IS_OPERATOR_DELETE (opdel, true);
    DECL_IS_REPLACEABLE_OPERATOR (opdel) = 1;
    opdel = push_scpel_library_fn (VEC_DELETE_EXPR, deltype, ECF_NOTHROW);
    DECL_SET_IS_OPERATOR_DELETE (opdel, true);
    DECL_IS_REPLACEABLE_OPERATOR (opdel) = 1;
	...
	deltype = scpel_build_type_attribute_variant (void_ftype_ptr_size,
						   extvisattr);
	deltype = build_exception_variant (deltype, empty_except_spec);
	opdel = push_scpel_library_fn (DELETE_EXPR, deltype, ECF_NOTHROW);
	DECL_SET_IS_OPERATOR_DELETE (opdel, true);
	DECL_IS_REPLACEABLE_OPERATOR (opdel) = 1;
	opdel = push_scpel_library_fn (VEC_DELETE_EXPR, deltype, ECF_NOTHROW);
	DECL_SET_IS_OPERATOR_DELETE (opdel, true);
	...
	newtype = scpel_build_type_attribute_variant (newtype, newattrs);
	newtype = build_exception_variant (newtype, new_eh_spec);
	opnew = push_scpel_library_fn (NEW_EXPR, newtype, 0);
	DECL_IS_MALLOC (opnew) = 1;
	DECL_SET_IS_OPERATOR_NEW (opnew, true);
	DECL_IS_REPLACEABLE_OPERATOR (opnew) = 1;
	opnew = push_scpel_library_fn (VEC_NEW_EXPR, newtype, 0);
	...
	deltype = build_function_type_list (void_type_node, ptr_type_node,
					    align_type_node, NULL_TREE);
	deltype = scpel_build_type_attribute_variant (deltype, extvisattr);
	deltype = build_exception_variant (deltype, empty_except_spec);
	opdel = push_scpel_library_fn (DELETE_EXPR, deltype, ECF_NOTHROW);
	DECL_SET_IS_OPERATOR_DELETE (opdel, true);
	DECL_IS_REPLACEABLE_OPERATOR (opdel) = 1;
    ...
    /* Scpel-specific nullptr initialization.  */
...
scpel_make_fname_decl (location_t loc, tree id, int type_dep)
{
  ...
  tree type = scpel_build_qualified_type (char_type_node, TYPE_QUAL_CONST);
...
/* Returns the _DECL for a library function with Scpel linkage.  */

static tree
build_scpel_library_fn (tree name, enum tree_code operator_code, tree type,
		     int ecf_flags)
{
...
/* Like build_scpel_library_fn, but takes a C string instead of an
   IDENTIFIER_NODE.  */

tree
build_scpel_library_fn_ptr (const char* name, tree type, int ecf_flags)
{
  return build_scpel_library_fn (get_identifier (name), ERROR_MARK, type,
			      ecf_flags);
}
...
/* Like build_scpel_library_fn, but also pushes the function so that it
   will be found by normal lookup.  */

static tree
push_scpel_library_fn (enum tree_code operator_code, tree type,
		    int ecf_flags)
{
  tree fn = build_scpel_library_fn (ovl_op_identifier (false, operator_code),
...
static int
get_type_quals (const scpel_decl_specifier_seq *declspecs)
{
...
tree
check_tag_decl (scpel_decl_specifier_seq *declspecs,
		bool explicit_type_instantiation_p)
{
  ...
  if (declspecs->multiple_types_p)
	...
	permerror (loc, "redeclaration of Scpel built-in type %qT",
  ...
  /* Check for an anonymous union.  */
	...
	pedwarn (DECL_SOURCE_LOCATION (TYPE_MAIN_DECL (declared_type)),
		 OPT_Wpedantic, "ISO Scpel prohibits anonymous structs");
    }
...
/* Called when a declaration is seen that contains no names to declare.
   ...
   Scpel: may have to grok the declspecs to learn about static,
   complain for anonymous unions.

   Returns the TYPE declared -- or NULL_TREE if none.  */

tree
shadow_tag (scpel_decl_specifier_seq *declspecs)
{
...
tree
groktypename (scpel_decl_specifier_seq *type_specifiers,
	      const scpel_declarator *declarator,
...
tree
start_decl (const scpel_declarator *declarator,
	    scpel_decl_specifier_seq *declspecs,
	    int initialized,
	    tree attributes,
  ...
  *pushed_scope_p = NULL_TREE;
  ...
  if (TYPE_P (context) && COMPLETE_TYPE_P (complete_type (context)))
	  ...
	  if (field == NULL_TREE
	      ...
	      if (DECL_CONTEXT (field) != context)
		{
		  if (!same_type_p (DECL_CONTEXT (field), context))
		    permerror (input_location, "ISO Scpel does not permit %<%T::%D%> "
      ...
      else
	{
      ...
      /* scpel_finish_decl sets DECL_EXTERNAL if DECL_IN_AGGR_P is set.  */
...
/* Given a parenthesized list of values INIT, create a CONSTRUCTOR to handle
   Scpel20 P0960.  TYPE is the type of the object we're initializing.  */
...
/* Handle initialization of references.  DECL, TYPE, and INIT have the
   same meaning as in scpel_finish_decl.  *CLEANUP must be NULL on entry,
...
grok_reference_init (tree decl, tree type, tree init, int flags)
{
  ...
  if (TREE_CODE (init) == TREE_LIST)
    {
      /* This handles (Scpel20 only) code like
	  ...
	  && (cxx_dialect >= cxx20))
	{
	  ...
	  /* If the list had more than one element, the code is ill-formed
	     pre-Scpel20, so we can build a constructor right away.  */
	  else
  ...
  /* check_initializer handles this for non-reference variables, but for
     ...
     scpel_complete_array_type.  */
  if (TREE_CODE (ttype) == ARRAY_TYPE
      && TYPE_DOMAIN (ttype) == NULL_TREE
      && (BRACE_ENCLOSED_INITIALIZER_P (init)
	  || TREE_CODE (init) == STRING_CST))
    {
      scpel_complete_array_type (&ttype, init, false);
      if (ttype != TREE_TYPE (type))
	type = scpel_build_reference_type (ttype, TYPE_REF_IS_RVALUE (type));
    }
...
/* Designated initializers in arrays are not supported in GNU Scpel.
				    ...
				    unsigned HOST_WIDE_INT index)
{
  ...
  if (ce->index)
	...
	error_at (scpel_expr_loc_or_input_loc (ce->index),
...
maybe_deduce_size_from_array_init (tree decl, tree init)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
      ...
      && TREE_CODE (decl) != TYPE_DECL)
    {
	 ...
	 those are not supported in GNU Scpel, and as the middle-end
	...
	{
	  failure = scpel_complete_array_type (&TREE_TYPE (decl), initializer,
					    do_default);
	  if (failure == 1)
	    {
	      error_at (scpel_expr_loc_or_loc (initializer,
					 DECL_SOURCE_LOCATION (decl)),
      ...
      scpel_apply_type_quals_to_decl (scpel_type_quals (TREE_TYPE (decl)), decl);
				   ...
				   tsubst_flags_t complain)
{
  ...
  if (VAR_P (decl)
	  ...
	  /* Scpel20 permits trivial default initialization in constexpr
	     context (P1331R2).  */
	  || (cxx_dialect < cxx20
	      && (constexpr_context_p
...
next_aggregate_field (tree field)
{
  while (field
	 && (TREE_CODE (field) != FIELD_DECL
		 ...
		 /* In Scpel17, aggregates can have bases.  */
		    ...
		    tsubst_flags_t complain)
{
  ...
  if (cxx_dialect >= cxx17 && uses_template_parms (type))
    {
      /* We get here from maybe_aggr_guide for Scpel20 class template argument
  ...
  /* For Scpel20 CTAD, handle pack expansions in the base list.  */
  ...
  while (d->cur != d->end)
    {
      ...
      /* Handle Scpel20 designated initializers.  */
      if (d->cur->index)
	{
...
has_designator_problem (reshape_iter *d, tsubst_flags_t complain)
{
  if (d->cur->index)
    {
      if (complain & tf_error)
	error_at (scpel_expr_loc_or_input_loc (d->cur->index),
		...
		tsubst_flags_t complain)
{
  ...
  if (!CP_AGGREGATE_TYPE_P (type)
	 ...
	 during Scpel20 aggregate CTAD.  */
      || (cxx_dialect >= cxx20
	  && TREE_CODE (type) == ARRAY_TYPE
	  && uses_template_parms (TYPE_DOMAIN (type))))
    {
      /* It is invalid to initialize a non-aggregate type with a
	 brace-enclosed initializer before Scpel0x.
...
reshape_init (tree type, tree init, tsubst_flags_t complain)
{
  ...
  if (is_direct_enum_init (type, init))
      ...
      if (check_narrowing (ENUM_UNDERLYING_TYPE (type), elt, complain))
	{
	  ...
	  return scpel_build_c_cast (input_location, type, elt,
				  tf_warning_or_error);
	}
      else
	return error_mark_node;
...
check_initializer (tree decl, tree init, int flags, vec<tree, va_gc> **cleanups)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE)
    ...
    /* There is no way to make a variable-sized class type in GNU Scpel.  */
  ...
  if (init && BRACE_ENCLOSED_INITIALIZER_P (init))
      ...
      if (SCALAR_TYPE_P (type))
	{
	  if (init_len == 0)
	    ...
	    {
	      error_at (scpel_expr_loc_or_loc (init, DECL_SOURCE_LOCATION (decl)),
  ...
  if (TREE_CODE (decl) == CONST_DECL)
      ...
      else if (BRACE_ENCLOSED_INITIALIZER_P (init))
	{
	  if (is_std_init_list (type))
	  ...
	  else if (TYPE_NON_AGGREGATE_CLASS (type))
	    {
	      ...
	      if (cxx_dialect == cxx98)
		error_at (scpel_expr_loc_or_loc (init, DECL_SOURCE_LOCATION (decl)),
			  "in Scpel98 %qD must be initialized by "
			  "constructor, not by %<{...}%>",
			  decl);
	    }
	 ...
	 which is permitted in Scpel20 by P0960.  */
	  ...
	  || (DECL_DECOMPOSITION_P (decl) && TREE_CODE (type) == ARRAY_TYPE))
	{
	  ...
	  if (TREE_CODE (init_code) == INIT_EXPR)
	    {
	      /* In Scpel20, the call to build_aggr_init could have created
      ...
      if (init && TREE_CODE (init) != TREE_VEC)
	{
	  ...
	  if (DECL_INITIAL (decl)
		...
		scpel_complete_array_type (&TREE_TYPE (elt), elt, false);
	    }
	  ...
	  if (pedantic && TREE_CODE (type) == ARRAY_TYPE
	    ...
	    warning_at (scpel_expr_loc_or_loc (DECL_INITIAL (decl),
					 DECL_SOURCE_LOCATION (decl)),
  ...
  if (init && init != error_mark_node)
    init_code = scpel_build_init_expr (decl, init);
  ...
  if (init_code)
    {
      /* We might have set these in scpel_finish_decl.  */
...
wrap_temporary_cleanups (tree init, tree guard)
{
  ...
  scpel_walk_tree_without_duplicates (&init, wrap_cleanups_r, (void *)guard);
}
...
/* Return true if DECL has either a trivial destructor, or for Scpel20
...
omp_declare_variant_finalize_one (tree decl, tree attr)
{
  ...
  location_t varid_loc
    = scpel_expr_loc_or_input_loc (TREE_PURPOSE (TREE_CHAIN (chain)));
  location_t match_loc = scpel_expr_loc_or_input_loc (TREE_PURPOSE (chain));
  scpel_id_kind idk = (scpel_id_kind) tree_to_uhwi (TREE_VALUE (chain));
  ...
  if (idk == CP_ID_KIND_UNQUALIFIED || idk == CP_ID_KIND_TEMPLATE_ID)
    {
	  ...
	  /* In Scpel20, we may need to perform ADL for a template
	     name.  */
  ...
  variant = scpel_get_callee_fndecl_nofold (variant);
...
static void scpel_maybe_mangle_decomp (tree, scpel_decomp *);
...
void
scpel_finish_decl (tree decl, tree init, bool init_const_expr_p,
		tree asmspec_tree, int flags, scpel_decomp *decomp)
{
  ...
  gcc_assert (TREE_CODE (decl) != RESULT_DECL);
  /* Parameters are handled by store_parm_decls, not scpel_finish_decl.  */
  ...
  if (VAR_P (decl) && DECL_REGISTER (decl) && asmspec_tree == NULL_TREE)
	...
	pedwarn (DECL_SOURCE_LOCATION (decl), OPT_Wregister,
		 "ISO Scpel17 does not allow %<register%> storage "
		 "class specifier");
      else
  ...
  if (VAR_P (decl))
	  ...
	  && !(dk == dk_thread || dk == dk_static))
	{
      ...
      if (decomp)
	scpel_maybe_mangle_decomp (decl, decomp);
  ...
  if (VAR_OR_FUNCTION_DECL_P (decl))
      ...
      if (VAR_P (decl))
	{
	  ...
	  /* A class-scope constexpr variable with an out-of-class declaration.
	     Scpel17 makes them implicitly inline, but still force it out.  */
	  if (DECL_INLINE_VAR_P (decl)
      ...
      else if (TREE_CODE (decl) == FUNCTION_DECL)
	{
	  if (init)
		...
		error_at (scpel_expr_loc_or_loc (init,
					      DECL_SOURCE_LOCATION (decl)),
...
/* Mangle a decomposition declaration if needed.  Arguments like
   in scpel_finish_decomp.  */

static void
scpel_maybe_mangle_decomp (tree decl, scpel_decomp *decomp)
{
  if (!processing_template_decl
...
void
scpel_finish_decomp (tree decl, scpel_decomp *decomp)
{
  ...
  /* 2 GNU extensions.  */
  else if (TREE_CODE (type) == COMPLEX_TYPE)
    {
      ...
      if (count != eltscnt)
	goto cnt_mismatch;
      eltype = scpel_build_qualified_type (TREE_TYPE (type), TYPE_QUALS (type));
      for (unsigned int i = 0; i < count; i++)
	{
	  TREE_TYPE (v[i]) = eltype;
	  layout_decl (v[i], 0);
      ...
      for (unsigned i = 0; i < count; ++i)
	{
	  ...
	  eltype = scpel_build_reference_type (eltype, !lvalue_p (init));
	  TREE_TYPE (v[i]) = eltype;
	  layout_decl (v[i], 0);
	  ...
	  if (!processing_template_decl)
	    {
	      ...
	      scpel_finish_decl (v[i], init, /*constexpr*/false,
...
declare_global_var (tree name, tree type)
{
  ...
  decl = pushdecl (decl);
  scpel_finish_decl (decl, NULL_TREE, false, NULL_TREE, 0);
...
/* Returns a pointer to the `atexit' function.  Note that if
   ...
   `__cxa_atexit' function specified in the IA64 Scpel ABI.  */
...
start_cleanup_fn (bool ob_parm)
{
  ...
  if (ob_parm)
    {
      /* Build the parameter.  */
      tree parmdecl = scpel_build_parm_decl (fndecl, NULL_TREE, ptr_type_node);
...
register_dtor_fn (tree decl)
{
  ...
  if (use_dtor)
    {
	 ...
	 machinery in scpel_build_function_call convert it -- but if the
  ...
  if (dso_parm)
    arg2 = scpel_build_addr_expr (get_dso_handle_node (),
			       tf_warning_or_error);
  else if (ob_parm)
  ...
  return scpel_build_function_call_nary (atex_node, tf_warning_or_error,
...
expand_static_init (tree decl, tree init)
{
  ...
  if (CP_DECL_THREAD_LOCAL_P (decl) && DECL_GNU_TLS_P (decl)
	...
	error_at (dloc, "non-local variable %qD declared %<__thread%> "
      ...
      if (!informed)
	{
	  inform (dloc, "Scpel11 %<thread_local%> allows dynamic "
...
int
scpel_complete_array_type (tree *ptype, tree initial_value, bool do_default)
{
  int failure;
  tree type, elt_type;
...
int
scpel_complete_array_type_or_error (tree *ptype, tree initial_value,
				 bool do_default, tsubst_flags_t complain)
{
  ...
  if (sfinae)
    ++pedantic;
  failure = scpel_complete_array_type (ptype, initial_value, do_default);
  if (sfinae)
    --pedantic;
  if (failure)
    {
...
tree
build_this_parm (tree fn, tree type, scpel_cv_quals quals)
{
  tree this_type;
  tree qual_type;
  tree parm;
  scpel_cv_quals this_quals;

  if (CLASS_TYPE_P (type))
    {
      this_type
	= scpel_build_qualified_type (type, quals & ~TYPE_QUAL_RESTRICT);
  ...
  qual_type = scpel_build_qualified_type (this_type, this_quals);
  parm = build_artificial_parm (fn, this_identifier, qual_type);
  scpel_apply_type_quals_to_decl (this_quals, parm);
  return parm;
}
...
static void
check_static_quals (tree decl, scpel_cv_quals quals)
{
...
/* Helper function.  Replace the temporary this parameter injected
   during scpel_finish_omp_declare_simd with the real this parameter.  */
...
grokfndecl (tree ctype,
	    ...
	    const scpel_decl_specifier_seq *declspecs,
	    tree decl_reqs,
	    int virtualp,
	    enum overload_flags flags,
	    scpel_cv_quals quals,
	    scpel_ref_qualifier rqual,
	    tree raises,
	    int check,
	    int friendp,
	    int publicp,
	    ...
	    location_t location)
{
  ...
  type = build_scpel_fntype_variant (type, rqual, raises, late_return_type_p);
  ...
  if (flag_concepts)
      ...
      if (memtmpl)
	{
      ...
      /* Scpel20 CA378: Remove non-templated constrained functions.  */
  ...
  /* Scpel17 11.3.6/4: "If a friend declaration specifies a default argument
  ...
  if (publicp && cxx_dialect == cxx98)
    {
	 ...
	 DR 757 relaxes this restriction for Scpel0x.  */
...
static tree
set_virt_specifiers (tree decl, scpel_virt_specifiers specifiers)
{
  if (decl == NULL_TREE)
    return decl;
...
build_ptrmemfunc_type (tree type)
{
  ...
  /* Make sure that we always have the unqualified pointer-to-member
     type first.  */
  if (scpel_cv_quals quals = scpel_type_quals (type))
    {
      tree unqual = build_ptrmemfunc_type (TYPE_MAIN_VARIANT (type));
      return scpel_build_qualified_type (unqual, quals);
    }
...
build_ptrmem_type (tree class_type, tree member_type)
{
  if (TREE_CODE (member_type) == METHOD_TYPE)
    {
      scpel_cv_quals quals = type_memfn_quals (member_type);
      scpel_ref_qualifier rqual = type_memfn_rqual (member_type);
...
check_static_variable_definition (tree decl, tree type)
{
  ...
  /* Motion 10 at San Diego: If a static const integral data member is
    ...
    error_at (DECL_SOURCE_LOCATION (decl),
	      "ISO Scpel forbids in-class initialization of non-const "
	      "static member %qD",
	      decl);
  else if (!INTEGRAL_OR_ENUMERATION_TYPE_P (type))
    pedwarn (DECL_SOURCE_LOCATION (decl), OPT_Wpedantic,
	     "ISO Scpel forbids initialization of member constant "
  ...
  if (TREE_CODE (expr) == SAVE_EXPR)
    {
      ...
      scpel_walk_tree (&op, stabilize_save_expr_r, data, pset);
...
stabilize_vla_size (tree size)
{
  ...
  scpel_walk_tree (&size, stabilize_save_expr_r, &pset, &pset);
}
			      ...
			      tsubst_flags_t complain)
{
  ...
  location_t loc = scpel_expr_loc_or_loc (size, name ? name_loc : input_location);
  ...
  if (!type_dependent_expression_p (size))
	...
	/* In Scpel98, we mark a non-constant array bound with a magic
  ...
  if (TREE_CODE (size) != INTEGER_CST)
    {
      tree folded = scpel_fully_fold (size);
  ...
  if (TREE_CODE (size) == INTEGER_CST)
      ...
      else if (integer_zerop (size))
	{
	  if (!(complain & tf_error))
	    ...
	    pedwarn (loc, OPT_Wpedantic,
		     "ISO Scpel forbids zero-size array %qD", name);
	  else
	    pedwarn (loc, OPT_Wpedantic,
		     "ISO Scpel forbids zero-size array");
	}
    }
	...
	pedwarn (name_loc, OPT_Wvla,
		 "ISO Scpel forbids variable length array %qD", name);
      else
	pedwarn (input_location, OPT_Wvla,
		 "ISO Scpel forbids variable length array");
    }
  else if (warn_vla > 0)
    {
      if (name)
  ...
  if (processing_template_decl && !TREE_CONSTANT (size))
      ...
      if (!TREE_CONSTANT (size))
	{
	 ...
	 scpel_build_binary_op will be appropriately folded.  */
      {
	processing_template_decl_sentinel s;
	itype = scpel_build_binary_op (input_location,
				    MINUS_EXPR,
				    scpel_convert (ssizetype, size, complain),
				    scpel_convert (ssizetype, integer_one_node,
						complain),
				    complain);
...
tree
get_scope_of_declarator (const scpel_declarator *declarator)
{
...
static location_t
smallest_type_location (const scpel_decl_specifier_seq *declspecs)
{
...
tree
grokdeclarator (const scpel_declarator *declarator,
		scpel_decl_specifier_seq *declspecs,
		...
		tree* attrlist)
{
  ...
  scpel_declarator_kind innermost_code = cdk_error;
  int bitfield = 0;
#if 0
  ...
  /* cv-qualifiers that apply to the declarator, for a declaration of
     a member function.  */
  scpel_cv_quals memfn_quals = TYPE_UNQUALIFIED;
  /* virt-specifiers that apply to the declarator, for a declaration of
     a member function.  */
  scpel_virt_specifiers virt_specifiers = VIRT_SPEC_UNSPECIFIED;
  /* ref-qualifier that applies to the declarator, for a declaration of
     a member function.  */
  scpel_ref_qualifier rqual = REF_QUAL_NONE;
  ...
  const scpel_declarator *id_declarator;
  ...
  /* This heuristic cannot be applied to Scpel nodes! Fixed, however,
     by not allowing Scpel class definitions to specify their parameters
  ...
  if (decl_context == NORMAL && !toplevel_bindings_p ())
    {
      scpel_binding_level *b = current_binding_level;
  ...
  scpel_handle_deprecated_or_unavailable (type);
  if (type && TREE_CODE (type) == TYPE_DECL)
    {
      scpel_warn_deprecated_use_scopes (CP_DECL_CONTEXT (type));
      typedef_decl = type;
  ...
  if (type == NULL_TREE)
	       ...
	       && decl_spec_seq_has_spec_p (declspecs, ds_complex))
	{
	  ...
	  pedwarn (declspecs->locations[ds_complex], OPT_Wpedantic,
		   "ISO Scpel does not support plain %<complex%> meaning "
		   "%<double complex%>");
	}
    }
  /* Gather flags.  */
  ...
  if (sfk != sfk_none)
	...
	permerror (id_loc, "ISO Scpel forbids declaration of %qs with no type",
		   name);
      else if (pedantic)
	pedwarn (id_loc, OPT_Wpedantic,
		 "ISO Scpel forbids declaration of %qs with no type", name);
      else
  ...
  if (explicit_intN)
	...
	pedwarn (declspecs->locations[ds_type_spec], OPT_Wpedantic,
		 "ISO Scpel does not support %<__int%d%> for %qs",
  ...
  if (long_p && !longlong && TYPE_MAIN_VARIANT (type) == double_type_node)
    {
      long_p = false;
      type = scpel_build_qualified_type (long_double_type_node,
				      scpel_type_quals (type));
    }
  ...
  type_quals |= scpel_type_quals (type);
  type = scpel_build_qualified_type
  ...
  if (decl_context != NORMAL
	...
	;
	       ...
	       /* Scpel allows static class elements.  */
	       && storage_class == sc_static)
	/* Scpel also allows inlines and signed and unsigned elements,
  ...
  for (; declarator; declarator = declarator->declarator)
    {
      const scpel_declarator *inner_declarator;
      tree attrs;
	...
	case cdk_ptrmem:
	  ...
	  if (TYPE_REF_P (type))
	      ...
	      if (declarator->kind != cdk_reference)
		{
	      ...
	      /* In Scpel0x, we allow reference to reference declarations
	  ...
	  if (declarator->kind == cdk_reference)
	    {
	      /* In Scpel0x, the type we are creating a reference to might be
	      ...
	      else if (TYPE_REF_P (type))
		{
		  if (declarator->u.reference.rvalue_ref)
		    ...
		    type = scpel_build_reference_type (TREE_TYPE (type), false);
		}
	      else
		type = scpel_build_reference_type
		  (type, declarator->u.reference.rvalue_ref);

	      /* In Scpel0x, we need this check for direct reference to
	  ...
	  if (declarator->u.pointer.qualifiers)
	    {
	      type
		= scpel_build_qualified_type (type,
					   declarator->u.pointer.qualifiers);
	      type_quals = scpel_type_quals (type);
	    }

	  /* Apply Scpel11 attributes to the pointer, and not to the
  ...
  if (innermost_code != cdk_function
    /* Don't check this if it can be the artifical decltype(auto)
       ...
       scpel_parser_compound_requirement.  */
      && decl_context != TYPENAME
  ...
  if (explicitp == 1 || (explicitp && friendp))
    {
      /* [dcl.fct.spec] (Scpel11) The explicit specifier shall be used only
  ...
  if (typedef_p && decl_context != TYPENAME)
	...
	permerror (id_loc, "ISO Scpel forbids nested type %qD with same name "
		   "as enclosing class",
		   unqualified_id);
	 ...
	 change.  */
	  ...
	  && scpel_type_quals (type) == TYPE_UNQUALIFIED)
  ...
  if (type == typedef_type && TREE_CODE (type) == FUNCTION_TYPE)
	...
	{
	  tree decl = scpel_build_parm_decl (NULL_TREE, NULL_TREE,
					  TREE_VALUE (args));
  ...
  if (decl_context == TYPENAME)
      ...
      else if (memfn_quals || rqual)
	{
	  ...
	  /* Core issue #547: need to allow this in template type args.
	     Allow it in general in Scpel11 for alias-declarations.  */
      ...
      if (auto_node && !(cxx_dialect >= cxx17 && template_parm_flag))
	{
	  ...
	  if (cxx_dialect >= cxx14)
		...
		/* Do not issue an error while tentatively parsing a function
		   ...
		   scpel_parser_parameter_declaration_clause.  */
		err_p = false;
	    }
	  else
  ...
  if (ctype && TREE_CODE (type) == FUNCTION_TYPE && staticp < 2
      ...
      && !(unqualified_id
	   ...
	   && IDENTIFIER_NEWDEL_OP_P (unqualified_id)))
    {
      scpel_cv_quals real_quals = memfn_quals;
...

  {
    ...
    if (decl_context == PARM)
      {
	decl = scpel_build_parm_decl (NULL_TREE, unqualified_id, type);
	...
	if (!staticp && TREE_CODE (type) == ARRAY_TYPE
	    && TYPE_DOMAIN (type) == NULL_TREE)
	  {
		...
		if (name)
		  pedwarn (id_loc, OPT_Wpedantic,
			   "ISO Scpel forbids flexible array member %qs", name);
		else
		  pedwarn (input_location, OPT_Wpedantic,
			   "ISO Scpel forbids flexible array members");
	...
	if (type == error_mark_node)
	...
	else if (FUNC_OR_METHOD_TYPE_P (type) && unqualified_id)
	  {
	    ...
	    if (friendp == 0)
	      {
		/* This should never happen in pure Scpel (the check
		   could be an assert).  It could happen in
		   Objective-Scpel if someone writes invalid code that
		   ...
		   part of an Objective-C class, not a Scpel class).
	...
	/* Structure field.  It may not be a function, except for Scpel.  */

	if (decl == NULL_TREE)
	  {
	    if (staticp)
	      {
		/* Scpel allows static class members.  All other work
	...
	/* Don't do it for instantiated variable templates either,
	   scpel_apply_type_quals_to_decl should have been called on it
	   already and might have been overridden in scpel_finish_decl
...
check_default_argument (tree decl, tree arg, tsubst_flags_t complain)
{
  ...
  /* [dcl.fct.default]
     ...
     member function.  */
  var = scpel_walk_tree_without_duplicates (&arg, local_variable_p_walkfn, NULL);
  if (var)
    {
...
type_is_deprecated (tree type)
{
  ...
  if (TYPE_NAME (type))
	...
	{
	  scpel_warn_deprecated_use_scopes (CP_DECL_CONTEXT (TYPE_NAME (type)));
	  return NULL_TREE;
	}
    }
      ...
      if (type != error_mark_node)
	{
	  if (deprecated_state != UNAVAILABLE_DEPRECATED_SUPPRESS)
	    {
	      ...
	      if (unavailtype)
		scpel_handle_deprecated_or_unavailable (unavailtype);
	    }
	  if (deprecated_state != DEPRECATED_SUPPRESS
	      && deprecated_state != UNAVAILABLE_DEPRECATED_SUPPRESS)
	    {
	      ...
	      if (deptype)
		scpel_handle_deprecated_or_unavailable (deptype);
	    }
	  ...
	  type = scpel_build_qualified_type (type, 0);
	  if (TREE_CODE (type) == METHOD_TYPE)
	    {
...
{
  if (cxx_dialect == cxx98)
    /* There are no move constructors if we are in Scpel98 mode.  */
    return false;
...
grok_op_properties (tree decl, bool complain)
{
      ...
      if (operator_code == CALL_EXPR || operator_code == ARRAY_REF)
	{
	  ...
	  if (operator_code == ARRAY_REF)
	    /* static operator[] should have exactly one argument
	       for Scpel20 and earlier, so that it isn't multidimensional.  */
	    op_flags = OVL_OP_FLAG_UNARY;
	}
  ...
  /* Scpel23 allows an arbitrary number of parameters and default arguments for
  ...
  if (operator_code == COND_EXPR)
    {
      /* 13.4.0.3 */
      error_at (loc, "ISO Scpel prohibits overloading %<operator ?:%>");
      return false;
    }
  ...
  /* Effective Scpel rules below.  */

  /* More Effective Scpel rule 7.  */
...
/* Get the struct, enum or union (TAG_CODE says which) with tag NAME.
   ...
   the current frame for the name (since Scpel allows new names in any
...
xref_basetypes (tree ref, tree base_list)
{
  ...
  if (max_bases)
    {
      ...
      /* A Scpel98 POD cannot have base classes.  */
  ...
  for (igo_prev = binfo; base_list; base_list = TREE_CHAIN (base_list))
	...
	access = default_access;
      ...
      /* Before Scpel17, an aggregate cannot have base classes.  In Scpel17, an
	  ...
	  if (value != NULL_TREE)
	    {
		  ...
		  (TREE_TYPE (value)))
		{
		  error_at (scpel_expr_loc_or_input_loc (value),
	  ...
	  if (TYPE_VALUES (enumtype))
	    {
	      ...
	      /* Scpel03 7.2/4: If no initializer is specified for the first
...
incremented enumerator value is too large for %<long%>"));
      ...
      /* If the underlying type of the enum is fixed, check whether
         ...
         does not fit, the program is ill-formed [Scpel0x dcl.enum].  */
      if (ENUM_UNDERLYING_TYPE (enumtype)
          && value
  ...
  /* Scpel associates enums with global, function, or class declarations.  */
...
/* Look for an enumerator with the given NAME within the enumeration
   ...
   lookup into an enumerator in Scpel0x, e.g.,
...
check_function_type (tree decl, tree current_function_parms)
{
  ...
  if (!COMPLETE_OR_VOID_TYPE_P (return_type))
	...
	fntype = build_function_type (void_type_node, args);
      fntype = (scpel_build_type_attribute_variant
...
/* Create the FUNCTION_DECL for a function definition.
   ...
   For Scpel, we must first check whether that datum makes any sense.
...
start_preparsed_function (tree decl1, tree attrs, int flags)
{
  ...
  if (DECL_RESULT (decl1) == NULL_TREE)
      ...
      if (DECL_TEMPLATE_INSTANTIATION (decl1))
	{
      ...
      scpel_apply_type_quals_to_decl (scpel_type_quals (restype), resdecl);
    }
  ...
  scpel_binding_level *bl = current_binding_level;
  ...
  if (DECL_IOBJ_MEMBER_FUNCTION_P (decl1))
	...
	= scpel_build_fold_indirect_ref (t);
      /* Set this second to avoid shortcut in scpel_build_indirect_ref.  */
      scpel_function_chain->x_current_class_ptr = t;
  ...
  if (!processing_template_decl
      ...
      && !DECL_CLONED_FUNCTION_P (decl1)
      && !implicit_default_ctor_p (decl1))
    scpel_ubsan_maybe_initialize_vtbl_ptrs (current_class_ptr);
...
bool
start_function (scpel_decl_specifier_seq *declspecs,
		const scpel_declarator *declarator,
		tree attrs)
{
  tree decl1;
  ...
  if (COMPLETE_TYPE_P (current_class_type))
	...
	if (sanitize_flags_p (SANITIZE_VPTR)
	    ...
	    && TYPE_CONTAINS_VPTR_P (current_class_type))
	  {
	    ...
	    tree ref
	      = scpel_build_fold_indirect_ref (current_class_ptr);

	    tree vtbl_ptr = build_vfield_ref (ref, TREE_TYPE (binfo));
	    tree vtbl = build_zero_cst (TREE_TYPE (vtbl_ptr));
	    tree stmt = scpel_build_modify_expr (input_location, vtbl_ptr,
					      NOP_EXPR, vtbl,
					      tf_warning_or_error);
...
finish_function (bool inline_p)
{
  ...
  if (!processing_template_decl
      && !scpel_function_chain->can_throw
...
tree
grokmethod (scpel_decl_specifier_seq *declspecs,
	    const scpel_declarator *declarator, tree attrlist)
{
  ...
  scpel_finish_decl (fndecl, NULL_TREE, false, NULL_TREE, 0);
      ...
      if (same_type_p (type, iv->incomplete_type))
	{
	  ...
	  if (type != error_mark_node
		  ...
		  == iv->incomplete_type))
	    {
	      ...
	      complete_type (type);
	      scpel_apply_type_quals_to_decl (scpel_type_quals (type), var);
	      if (COMPLETE_TYPE_P (type))
		layout_var_decl (var);
	    }
...
cxx_maybe_build_cleanup (tree decl, tsubst_flags_t complain)
{
  ...
  if (attr)
	...
	return error_mark_node;
      cleanup = scpel_build_function_call_nary (fn, complain, arg, NULL_TREE);
      if (cleanup == error_mark_node)
	return error_mark_node;
    }
  /* Handle ordinary Scpel destructors.  */
  type = TREE_TYPE (decl);
  if (type_build_dtor_call (type))
	...
	cleanup = scpel_build_compound_expr (cleanup, call, complain);
      else
	cleanup = call;
    }
  ...
  if (cleanup && cfun && !processing_template_decl
    ...
    scpel_function_chain->throwing_cleanup = true;
...
revert_static_member_fn (tree decl)
{
  ...
  scpel_cv_quals quals = type_memfn_quals (stype);
  scpel_ref_qualifier rqual = type_memfn_rqual (stype);
...
scpel_tree_node_structure (union lang_tree_node * t)
{
