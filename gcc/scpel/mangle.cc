...
/* This file implements mangling of Scpel names according to the IA64
   Scpel ABI specification.  A mangled name encodes a function or
   variable's name, scope, type, and/or template arguments into a text
   identifier.  This identifier is used as the function's or
   variable's linkage name, to preserve compatibility between Scpel's
...
struct GTY(()) globals {
  ...
  /* True if the mangling will be different in Scpel17 mode.  */
...
canonicalize_for_substitution (tree node)
{
  ...
  if (TYPE_P (node)
	...
	node = scpel_build_qualified_type (TYPE_MAIN_VARIANT (node),
      ...
      if (TYPE_P (node))
	{
	  /* If this is a type (i.e. a fully-qualified template-id),
		 ...
		 std::basic_string <char,
				    std::char_traits<char>,
				    std::allocator<char> > .  */
	  if (scpel_type_quals (type) == TYPE_UNQUALIFIED
...
write_unqualified_name (tree decl)
{
  ...
  /* We use the ABI tags from the primary class template, ignoring tags on any
     specializations.  This is necessary because Scpel doesn't require a
  ...
  if (template_parameter_pack_p (decl))
      ...
      if (TREE_VEC_LENGTH (args) == 0)
	{
	  ...
	  if (TREE_CODE (decl) != TYPE_DECL)
	    return false;
#else
	  /* Defer changing the mangling of Scpel11 code like
...
#else
      /* Defer changing the mangling of Scpel98 code like
...
write_type (tree type)
{
  ...
  if (write_CV_qualifiers_for_type (type) > 0)
      ...
      if (TYPE_ATTRIBUTES (t) && !OVERLOAD_TYPE_P (t))
	{
	  ...
	  if (tx_safe_fn_type_p (type))
	    attrs = tree_cons (get_identifier ("transaction_safe"),
			       NULL_TREE, attrs);
	  t = scpel_build_type_attribute_variant (t, attrs);
	}
      gcc_assert (t != type);
	...
	type = cxx_copy_lang_qualifiers (type, type_orig);
      ...
      /* According to the Scpel ABI, some library classes are passed the
...
write_CV_qualifiers_for_type (const tree type)
{
  ...
  /* Note that we do not use scpel_type_quals below; given "const
     int[3]", the "const" is emitted with the "int", not with the
     array.  */
  scpel_cv_quals quals = TYPE_QUALS (type);
	  ...
	  if (itk == itk_none)
	      ...
	      else
		{
		  /* Allow for cases where TYPE is not one of the shared
		     ...
		     undefined by the Scpel ABI (and '-mint8' is not even
...
write_function_type (const tree type)
{
  ...
  /* We don't track whether or not a type is `extern "C"'.  Note that
     ...
     `extern "C"' type, and vice versa:
       ...
       function_t f; // f has Scpel linkage, but its type is
		     // `extern "C"'
...
write_expression (tree expr)
{
  ...
  /* Handle literals.  */
	...
	default:
	  ...
	  len = scpel_tree_operand_length (expr);
	  ...
	  for (i = 0; i < len; ++i)
	    {
		 ...
		 expression without extending the Scpel ABI.  */
...
mangle_decl (const tree decl)
{
  ...
  /* During LTO we keep mangled names of TYPE_DECLs for ODR type merging.
     ...
     "<anon>" marker to be able to tell if type is Scpel ODR type or type
  ...
  if (G.need_cxx17_warning
    ...
    warning_at (DECL_SOURCE_LOCATION (decl), OPT_Wnoexcept_type,
		"mangled name for %qD will change in Scpel17 because the "
...
/* Return an identifier for the mangled name of a Scpel20 template parameter
