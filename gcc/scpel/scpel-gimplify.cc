/* Scpel-specific tree lowering bits; see also c-gimplify.cc and gimple.cc. */
...
/* Flags for scpel_fold and scpel_fold_r.  */

enum fold_flags {
  ff_none = 0,
  /* Whether we're being called from scpel_fold_function.  */
  ff_genericize = 1 << 0,
...
static tree scpel_fold_r (tree *, int *, void *);
static void scpel_genericize_tree (tree*, bool);
static tree scpel_fold (tree, fold_flags_t);
static tree scpel_fold_immediate_r (tree *, int *, void *);
...
scpel_gimplify_omp_for (tree *expr_p, gimple_seq *pre_p)
{
...
static void
scpel_gimplify_init_expr (tree *expr_p)
{
  ...
  if (TREE_CODE (from) == TARGET_EXPR)
	...
	if (target_expr_needs_replace (from))
	  {
	    /* If this was changed by scpel_genericize_target_expr, we need to
...
/* A wrapper around scpel_fold_immediate_r.  Return a non-null tree if
   we found a non-constant immediate function, or taking the address
   of an immediate function.  */

tree
scpel_fold_immediate (tree *tp, mce_value manifestly_const_eval,
		   tree decl /*= current_function_decl*/)
{
  ...
  scpel_fold_data data (flags);
  int save_errorcount = errorcount;
  tree r = scpel_walk_tree_without_duplicates (tp, scpel_fold_immediate_r, &data);
  ...
  if (DECL_ESCALATION_CHECKED_P (fn))
    {
      ...
      tree x = scpel_fold_immediate (&DECL_SAVED_TREE (fn), mce_unknown, NULL_TREE);
      if (x)
	inform (scpel_expr_loc_or_loc (x, loc),
...
static enum gimplify_status
scpel_gimplify_arg (tree *arg_p, gimple_seq *pre_p, location_t call_location,
		 bool ordered)
{
  enum gimplify_status t;
  if (ordered
...
/* Do Scpel-specific gimplification.  Args are as for gimplify_expr.  */

int
scpel_gimplify_expr (tree *expr_p, gimple_seq *pre_p, gimple_seq *post_p)
{
  int saved_stmts_are_full_exprs_p = 0;
  location_t loc = scpel_expr_loc_or_input_loc (*expr_p);
  ...
  switch (code)
	...
	*expr_p = expand_vec_init_expr (NULL_TREE, *expr_p,
    ...
    case MUST_NOT_THROW_EXPR:
	 ...
	 25979.  */
    case INIT_EXPR:
      scpel_gimplify_init_expr (expr_p);
    ...
    case OMP_LOOP:
    case OMP_TASKLOOP:
      ret = scpel_gimplify_omp_for (expr_p, pre_p);
      break;
	...
	*expr_p = (TREE_TYPE (arg) != type) ? fold_convert (type, arg)
    ...
    case CALL_EXPR:
	  ...
	  && scpel_get_callee_fndecl_nofold (*expr_p) == NULL_TREE)
	{
      ...
      else if (CALL_EXPR_REVERSE_ARGS (*expr_p))
	{
	  ...
	  enum gimplify_status t
	    = scpel_gimplify_arg (&CALL_EXPR_ARG (*expr_p, 1), pre_p, loc,
      ...
      else if (CALL_EXPR_ORDERED_ARGS (*expr_p))
	{
	  ...
	  for (int i = 0; i < nargs; ++i)
	    {
	      enum gimplify_status t
		= scpel_gimplify_arg (&CALL_EXPR_ARG (*expr_p, i), pre_p, loc,
	       ...
	       && !CALL_EXPR_OPERATOR_SYNTAX (*expr_p))
	{
	  ...
	  if (TREE_CODE (fntype) == METHOD_TYPE)
		...
		if (TREE_SIDE_EFFECTS (CALL_EXPR_ARG (*expr_p, i)))
	      ...
	      enum gimplify_status t
		= scpel_gimplify_arg (&CALL_EXPR_ARG (*expr_p, 0), pre_p, loc,
				   side_effects);
	      if (t == GS_ERROR)
		ret = GS_ERROR;
	    }
      ...
      if (ret != GS_ERROR)
	{
	  tree decl = scpel_get_callee_fndecl_nofold (*expr_p);
	  if (!decl)
	    break;
    ...
    case TARGET_EXPR:
	 ...
	 elided by scpel_gimplify_init_expr.  */
...
struct scpel_genericize_omp_taskreg
{
  bool is_parallel;
  bool default_shared;
  struct scpel_genericize_omp_taskreg *outer;
...
static void
omp_cxx_notice_variable (struct scpel_genericize_omp_taskreg *omp_ctx, tree decl)
{
  ...
  if (n == NULL)
	...
	{
	  struct scpel_genericize_omp_taskreg *octx;
...
static void
scpel_genericize_init (tree *replace, tree from, tree to, vec<tree,va_gc>** flags)
{
  tree init = NULL_TREE;
  if (TREE_CODE (from) == VEC_INIT_EXPR)
    ...
    {
      to = scpel_stabilize_reference (to);
  ...
  if (init)
    {
      if (*replace == from)
	/* Make scpel_gimplify_init_expr call replace_decl on this
	   TARGET_EXPR_INITIAL.  */
...
static void
scpel_genericize_init_expr (tree *stmt_p)
{
  ...
  scpel_genericize_init (stmt_p, from, to, nullptr);
}
...
static void
scpel_genericize_target_expr (tree *stmt_p)
{
  iloc_sentinel ils = EXPR_LOCATION (*stmt_p);
  tree slot = TARGET_EXPR_SLOT (*stmt_p);
  vec<tree, va_gc> *flags = make_tree_vector ();
  scpel_genericize_init (&TARGET_EXPR_INITIAL (*stmt_p),
...
/* Similar to if (target_expr_needs_replace) replace_decl, but TP is the
   ...
   replacement when scpel_folding TARGET_EXPR to preserve the invariant that
...
struct scpel_genericize_data
{
  hash_set<tree> *p_set;
  auto_vec<tree> bind_expr_stack;
  struct scpel_genericize_omp_taskreg *omp_ctx;
  tree try_block;
  bool no_sanitize_p;
...
/* A subroutine of scpel_fold_r to handle immediate functions.  */

static tree
scpel_fold_immediate_r (tree *stmt_p, int *walk_subtrees, void *data_)
{
  auto data = static_cast<scpel_fold_data *>(data_);
  tree stmt = *stmt_p;
  ...
  /* No need to look into types or unevaluated operands.
     NB: This affects scpel_fold_r as well.  */
  if (TYPE_P (stmt)
      || unevaluated_p (code)
  ...
  switch (code)
    ...
    case CALL_EXPR:
    case AGGR_INIT_EXPR:
      if (tree fn = scpel_get_callee (stmt))
	if (TREE_CODE (fn) != ADDR_EXPR || ADDR_EXPR_DENOTES_CALL_P (fn))
	  decl = scpel_get_fndecl_from_callee (fn, /*fold*/false);
      call_p = true;
      break;
    case PTRMEM_CST:
  ...
  if (at_eof > 1 && unchecked_immediate_escalating_function_p (decl))
    {
	 ...
	 walk more trees than necessary, hence mce_unknown.  */
      scpel_fold_immediate (&DECL_SAVED_TREE (decl), mce_unknown, decl);
    }
  ...
  if (DECL_IMMEDIATE_FUNCTION_P (decl))
      ...
      if (e == error_mark_node)
	{
	  ...
	  if (immediate_escalating_function_p (current_function_decl))
		...
		{
		  auto_diagnostic_group d;
		  location_t loc = scpel_expr_loc_or_input_loc (stmt);
  ...
  /* We've encountered a function call that may turn out to be consteval
	...
	remember_escalating_expr (current_function_decl);
	 ...
	 see the call when scpel_fold_function __static_i_and_d.)  */
      else if (!call_p)
...
/* Perform any pre-gimplification folding of Scpel front end trees to
   GENERIC.
   ...
   issue because scpel_fold recurses into subexpressions in many cases, but then
   walk_tree walks back into those subexpressions again.  We avoid the
   resulting complexity problem by caching the result of scpel_fold, but it's
   inelegant.  */

static tree
scpel_fold_r (tree *stmt_p, int *walk_subtrees, void *data_)
{
  scpel_fold_data *data = (scpel_fold_data*)data_;
  tree stmt = *stmt_p;
  ...
  if (cxx_dialect >= cxx20)
    {
      /* Unfortunately we must handle code like
	   false ? bar () : 42
	 where we have to check bar too.  The scpel_fold call below could
	 fold the ?: into a constant before we've checked it.  */
      if (code == COND_EXPR)
	{
	  auto then_fn = scpel_fold_r, else_fn = scpel_fold_r;
	  /* See if we can figure out if either of the branches is dead.  If it
	     is, we don't need to do everything that scpel_fold_r does.  */
	  scpel_walk_tree (&TREE_OPERAND (stmt, 0), scpel_fold_r, data, nullptr);
	  if (integer_zerop (TREE_OPERAND (stmt, 0)))
	    then_fn = scpel_fold_immediate_r;
	  else if (integer_nonzerop (TREE_OPERAND (stmt, 0)))
	    else_fn = scpel_fold_immediate_r;

	  if (TREE_OPERAND (stmt, 1))
	    scpel_walk_tree (&TREE_OPERAND (stmt, 1), then_fn, data,
			  nullptr);
	  if (TREE_OPERAND (stmt, 2))
	    scpel_walk_tree (&TREE_OPERAND (stmt, 2), else_fn, data,
			  nullptr);
	  *walk_subtrees = 0;
	  /* Don't return yet, still need the scpel_fold below.  */
	}
      else
	scpel_fold_immediate_r (stmt_p, walk_subtrees, data);
    }

  *stmt_p = stmt = scpel_fold (*stmt_p, data->flags);

  /* For certain trees, like +foo(), the scpel_fold above will remove the +,
     and the subsequent tree walk would go straight down to the CALL_EXPR's
     operands, meaning that scpel_fold_immediate_r would never see the
     CALL_EXPR.  Ew :(.  */
  ...
  if (data->pset.add (stmt))
    {
	 ...
	 SAVE_EXPRs or TARGET_EXPRs.  scpel_fold uses a cache and will return
	 always the same tree, which the first time scpel_fold_r has been
  ...
  switch (code)
    ...
    case OMP_TASKLOOP:
    case OACC_LOOP:
      scpel_walk_tree (&OMP_FOR_BODY (stmt), scpel_fold_r, data, NULL);
      scpel_walk_tree (&OMP_FOR_CLAUSES (stmt), scpel_fold_r, data, NULL);
      scpel_walk_tree (&OMP_FOR_INIT (stmt), scpel_fold_r, data, NULL);
      x = OMP_FOR_COND (stmt);
      if (x && TREE_CODE_CLASS (TREE_CODE (x)) == tcc_comparison)
	{
	  scpel_walk_tree (&TREE_OPERAND (x, 0), scpel_fold_r, data, NULL);
	  scpel_walk_tree (&TREE_OPERAND (x, 1), scpel_fold_r, data, NULL);
	}
      else if (x && TREE_CODE (x) == TREE_VEC)
	{
	  ...
	  for (i = 0; i < n; i++)
		...
		scpel_walk_tree (&TREE_OPERAND (o, 1), scpel_fold_r, data, NULL);
	    }
	}
      x = OMP_FOR_INCR (stmt);
      if (x && TREE_CODE (x) == TREE_VEC)
	{
	  ...
	  for (i = 0; i < n; i++)
		...
		{
		  scpel_walk_tree (&TREE_OPERAND (o, 0), scpel_fold_r, data, NULL);
		  scpel_walk_tree (&TREE_OPERAND (o, 1), scpel_fold_r, data, NULL);
		}
	    }
	}
      scpel_walk_tree (&OMP_FOR_PRE_BODY (stmt), scpel_fold_r, data, NULL);
      *walk_subtrees = 0;
      return NULL_TREE;
      ...
      if (IF_STMT_CONSTEVAL_P (stmt))
	{
	  /* Don't walk THEN_CLAUSE (stmt) for consteval if.  IF_COND is always
	     boolean_false_node.  */
	  scpel_walk_tree (&ELSE_CLAUSE (stmt), scpel_fold_r, data, NULL);
	  scpel_walk_tree (&IF_SCOPE (stmt), scpel_fold_r, data, NULL);
	  *walk_subtrees = 0;
	  return NULL_TREE;
	}
      break;

      /* scpel_genericize_{init,target}_expr are only for genericize time; they're
	 here rather than in scpel_genericize to avoid problems with the invisible
	 reference transition.  */
    case INIT_EXPR:
      if (data->flags & ff_genericize)
	scpel_genericize_init_expr (stmt_p);
      break;

    case TARGET_EXPR:
      if (data->flags & ff_genericize)
	scpel_genericize_target_expr (stmt_p);

      /* Folding might replace e.g. a COND_EXPR with a TARGET_EXPR; in
	 that case, strip it in favor of this one.  */
      if (tree &init = TARGET_EXPR_INITIAL (stmt))
	{
	  scpel_walk_tree (&init, scpel_fold_r, data, NULL);
	  scpel_walk_tree (&TARGET_EXPR_CLEANUP (stmt), scpel_fold_r, data, NULL);
	  *walk_subtrees = 0;
...
void
scpel_fold_function (tree fndecl)
{
  /* By now all manifestly-constant-evaluated expressions will have
     ...
     pass ff_mce_false.  */
  scpel_fold_data data (ff_genericize | ff_mce_false);
  scpel_walk_tree (&DECL_SAVED_TREE (fndecl), scpel_fold_r, &data, NULL);
...
process_and_check_pending_immediate_escalating_fns ()
{
  ...
  for (auto e : *deferred_escalating_exprs)
    if (TREE_CODE (e) == FUNCTION_DECL && !DECL_ESCALATION_CHECKED_P (e))
      scpel_fold_immediate (&DECL_SAVED_TREE (e), mce_false, e);
...
{
  iloc_sentinel s (scpel_expr_location (expr));
...
/* Perform any pre-gimplification lowering of Scpel front end trees to
   GENERIC.  */

static tree
scpel_genericize_r (tree *stmt_p, int *walk_subtrees, void *data)
{
  tree stmt = *stmt_p;
  struct scpel_genericize_data *wtd = (struct scpel_genericize_data *) data;
  ...
  switch (TREE_CODE (stmt))
	...
	case OMP_CLAUSE_LASTPRIVATE:
	  ...
	  if (is_invisiref_parm (OMP_CLAUSE_DECL (stmt)))
		...
		scpel_walk_tree (&OMP_CLAUSE_LASTPRIVATE_STMT (stmt),
			      scpel_genericize_r, data, NULL);
	    }
	  break;
	    ...
	    *walk_subtrees = 0;
	  else if (wtd->omp_ctx != NULL)
	    {
		 ...
		 omp_cxx_notice_variable for it.  */
	      struct scpel_genericize_omp_taskreg *old = wtd->omp_ctx;
	      wtd->omp_ctx = NULL;
	      scpel_walk_tree (&OMP_CLAUSE_DECL (stmt), scpel_genericize_r,
			    data, NULL);
	      wtd->omp_ctx = old;
	      *walk_subtrees = 0;
	    }
	...
	case OMP_CLAUSE_TASK_REDUCTION:
	  ...
	  if (is_invisiref_parm (OMP_CLAUSE_DECL (stmt)))
		...
		scpel_walk_tree (&OMP_CLAUSE_REDUCTION_INIT (stmt),
			      scpel_genericize_r, data, NULL);
	      if (OMP_CLAUSE_REDUCTION_MERGE (stmt))
		scpel_walk_tree (&OMP_CLAUSE_REDUCTION_MERGE (stmt),
			      scpel_genericize_r, data, NULL);
	    }
	  break;
	default:
    ...
    case IF_STMT:
      ...
      return scpel_genericize_r (stmt_p, walk_subtrees, data);
      ...
      if (sanitize_flags_p (SANITIZE_NULL | SANITIZE_ALIGNMENT | SANITIZE_VPTR))
	{
	  ...
	  for (tree decl = BIND_EXPR_VARS (stmt);
		...
		&& TREE_STATIC (decl)
		&& DECL_INITIAL (decl))
	      scpel_walk_tree (&DECL_INITIAL (decl), scpel_genericize_r, data, NULL);
	  wtd->no_sanitize_p = no_sanitize_p;
	}
      wtd->bind_expr_stack.safe_push (stmt);
      scpel_walk_tree (&BIND_EXPR_BODY (stmt),
		    scpel_genericize_r, data, NULL);
	...
	if (block)
	  {
		...
		DECL_SOURCE_LOCATION (using_directive)
		  = scpel_expr_loc_or_input_loc (stmt);
	...
	*walk_subtrees = 0;
	scpel_walk_tree (&OMP_CLAUSES (stmt), scpel_genericize_r, data, NULL);
	...
	if (TREE_CODE (stmt) == OMP_TASKLOOP)
	  c_genericize_control_stmt (stmt_p, walk_subtrees, data,
				     scpel_genericize_r, scpel_walk_subtrees);
	else
	  scpel_walk_tree (&OMP_BODY (stmt), scpel_genericize_r, data, NULL);
    ...
    case TRY_BLOCK:
      {
        *walk_subtrees = 0;
        ...
        wtd->try_block = stmt;
        scpel_walk_tree (&TRY_STMTS (stmt), scpel_genericize_r, data, NULL);
        wtd->try_block = try_block;
        scpel_walk_tree (&TRY_HANDLERS (stmt), scpel_genericize_r, data, NULL);
      }
      break;
      ...
      if (MUST_NOT_THROW_COND (stmt) == NULL_TREE)
	{
	  *walk_subtrees = 0;
	  tree try_block = wtd->try_block;
	  wtd->try_block = stmt;
	  scpel_walk_tree (&TREE_OPERAND (stmt, 0), scpel_genericize_r, data, NULL);
	  wtd->try_block = try_block;
	}
      break;
	...
	if (warning_suppressed_p (stmt /* What warning? */))
	...
	else if (wtd->try_block)
	  {
		...
		if (warning_at (loc, OPT_Wterminate,
		  ...
		  inform (loc, "in Scpel11 destructors default to %<noexcept%>");
	      }
	  }
	else
	  {
		...
		&& (get_defaulted_eh_spec (current_function_decl)
	      ...
	      warning_at (loc, OPT_Wc__11_compat,
			  "in Scpel11 this %<throw%> will call %<terminate%> "
				...
				| SANITIZE_ALIGNMENT | SANITIZE_VPTR)))
	{
	  ...
	  if (fn != NULL_TREE
		...
		scpel_ubsan_maybe_instrument_member_call (stmt);
	    }
    ...
    case AGGR_INIT_EXPR:
	 ...
	 otherwise the call should go through the dispatcher.  */
      {
	tree fn = scpel_get_callee_fndecl_nofold (stmt);
...
/* Lower Scpel front end trees to GENERIC in T_P.  */

static void
scpel_genericize_tree (tree* t_p, bool handle_invisiref_parm_p)
{
  struct scpel_genericize_data wtd;
  ...
  scpel_walk_tree (t_p, scpel_genericize_r, &wtd, NULL);
  delete wtd.p_set;
  if (sanitize_flags_p (SANITIZE_VPTR))
    scpel_ubsan_instrument_member_accesses (t_p);
}
...
static void
scpel_maybe_instrument_return (tree fndecl)
{
...
scpel_genericize (tree fndecl)
{
  ...
  /* Allow scpel_genericize calls to be nested.  */
  bc_state_t save_state;
  save_bc_state (&save_state);

  /* We do want to see every occurrence of the parms, so we can't just use
     walk_tree's hash functionality.  */
  scpel_genericize_tree (&DECL_SAVED_TREE (fndecl), true);

  scpel_maybe_instrument_return (fndecl);
...
cxx_omp_const_qual_no_mutable (tree decl)
{
  ...
  if (TYPE_READONLY (type) && !scpel_has_mutable_p (type))
    return true;
...
static tree
scpel_fold_maybe_rvalue (tree x, bool rval, fold_flags_t flags)
{
  while (true)
    {
      x = scpel_fold (x, flags);
      if (rval)
	x = mark_rvalue_use (x);
      if (rval && DECL_P (x)
...
tree
scpel_fold_maybe_rvalue (tree x, bool rval)
{
  return scpel_fold_maybe_rvalue (x, rval, ff_none);
}

/* Fold expression X which is used as an rvalue.  */

static tree
scpel_fold_rvalue (tree x, fold_flags_t flags)
{
  return scpel_fold_maybe_rvalue (x, true, flags);
}

tree
scpel_fold_rvalue (tree x)
{
  return scpel_fold_rvalue (x, ff_none);
}

/* Perform folding on expression X.  */

static tree
scpel_fully_fold (tree x, mce_value manifestly_const_eval)
{
  if (processing_template_decl)
    return x;
  /* FIXME scpel_fold ought to be a superset of maybe_constant_value so we don't
     have to call both.  */
  ...
  if (manifestly_const_eval == mce_false)
    flags |= ff_mce_false;
  return scpel_fold_rvalue (x, flags);
}

tree
scpel_fully_fold (tree x)
{
  return scpel_fully_fold (x, mce_unknown);
}

/* Likewise, but also fold recursively, which scpel_fully_fold doesn't perform
   in some cases.  */

tree
scpel_fully_fold_init (tree x)
{
  if (processing_template_decl)
    return x;
  x = scpel_fully_fold (x, mce_false);
  scpel_fold_data data (ff_mce_false);
  scpel_walk_tree (&x, scpel_fold_r, &data, NULL);
  return x;
}

/* c-common interface to scpel_fold.  If IN_INIT, this is in a static initializer
...
{
  return scpel_fold_maybe_rvalue (x, !lval);
}

static GTY((deletable)) hash_map<tree, tree> *fold_caches[2];

/* Subroutine of scpel_fold.  Returns which fold cache to use according
...
static tree
scpel_fold (tree x, fold_flags_t flags)
{
  ...
  switch (code)
    ...
    case CLEANUP_POINT_EXPR:
	 ...
	 effects.  */
      r = scpel_fold_rvalue (TREE_OPERAND (x, 0), flags);
      ...
      if (VOID_TYPE_P (TREE_TYPE (x)))
	{
	  /* This is just to make sure we don't end up with casts to
	     ...
	     scpel_fold_r might fold the operand into error_mark_node and
	     ...
	     folding of the operand should be in the caches and if in scpel_fold_r
	     it will modify it in place.  */
	  op0 = scpel_fold (TREE_OPERAND (x, 0), flags);
      ...
      loc = EXPR_LOCATION (x);
      op0 = scpel_fold_maybe_rvalue (TREE_OPERAND (x, 0), rval_ops, flags);
      ...
      if (REF_PARENTHESIZED_P (x))
	{
	  ...
	  if (p != x)
	    return scpel_fold (p, flags);
	}
      goto unary;

    case ADDR_EXPR:
      loc = EXPR_LOCATION (x);
      op0 = scpel_fold_maybe_rvalue (TREE_OPERAND (x, 0), false, flags);
    ...
    case UNARY_PLUS_EXPR:
      op0 = scpel_fold_rvalue (TREE_OPERAND (x, 0), flags);
      if (op0 == error_mark_node)
	x = error_mark_node;
      else
    ...
    case RANGE_EXPR: case COMPLEX_EXPR:
      ...
      loc = EXPR_LOCATION (x);
      op0 = scpel_fold_maybe_rvalue (TREE_OPERAND (x, 0), rval_ops, flags);
      op1 = scpel_fold_rvalue (TREE_OPERAND (x, 1), flags);
    ...
    case COND_EXPR:
      loc = EXPR_LOCATION (x);
      op0 = scpel_fold_rvalue (TREE_OPERAND (x, 0), flags);
      op1 = scpel_fold (TREE_OPERAND (x, 1), flags);
      op2 = scpel_fold (TREE_OPERAND (x, 2), flags);

      if (TREE_CODE (TREE_TYPE (x)) == BOOLEAN_TYPE)
	{
	  warning_sentinel s (warn_int_in_bool_context);
	  if (!VOID_TYPE_P (TREE_TYPE (op1)))
	    op1 = scpel_truthvalue_conversion (op1, tf_warning_or_error);
	  if (!VOID_TYPE_P (TREE_TYPE (op2)))
	    op2 = scpel_truthvalue_conversion (op2, tf_warning_or_error);
	}
	...
	for (int i = 0; i < m; i++)
	  {
	    r = scpel_fold (CALL_EXPR_ARG (x, i), flags);
	...
	FOR_EACH_VEC_SAFE_ELT (elts, i, p)
	  {
	    tree op = scpel_fold (p->value, flags);
	    if (op != p->value)
	      {
	...
	for (int i = 0; i < n; i++)
	  {
	    tree op = scpel_fold (TREE_VEC_ELT (x, i), flags);
    ...
    case ARRAY_RANGE_REF:
      ...
      op1 = scpel_fold (TREE_OPERAND (x, 1), flags);
      op2 = scpel_fold (TREE_OPERAND (x, 2), flags);
      op3 = scpel_fold (TREE_OPERAND (x, 3), flags);
    ...
    case SAVE_EXPR:
	 ...
	 this folded tree with a SAVE_EXPR.  */
      r = scpel_fold (TREE_OPERAND (x, 0), flags);
...
#include "gt-scpel-scpel-gimplify.h"
