...
/* GCC Scpel coroutines implementation.
  ...
  The Scpel coroutine design makes use of some helper functions that are
...
instantiate_coro_traits (tree fndecl, location_t kw)
{
  ...
  while (arg_node != NULL_TREE && !VOID_TYPE_P (TREE_VALUE (arg_node)))
	  ...
	  || DECL_NAME (arg) == closure_identifier)
	{
	  ...
	  TREE_VEC_ELT (argtypes, p++) = scpel_build_reference_type (ct, false);
	}
      else
...
coro_diagnose_throwing_fn (tree fndecl)
{
  if (!TYPE_NOTHROW_P (TREE_TYPE (fndecl)))
    {
      location_t f_loc = scpel_expr_loc_or_loc (fndecl,
...
expand_one_await_expression (tree *stmt, tree *await_expr, void *d)
{
  ...
  if (TREE_CODE (TREE_TYPE (ready_cond)) != BOOLEAN_TYPE)
    ready_cond = scpel_convert (boolean_type_node, ready_cond,
			     tf_warning_or_error);
  ...
  if (VOID_TYPE_P (susp_type))
	...
	r = build1_loc (loc, VIEW_CONVERT_EXPR, void_coro_handle_type, r);
      r = scpel_build_init_expr (loc, data->conthand, r);
  ...
  r = build_call_expr_internal_loc (loc, IFN_CO_YIELD, integer_type_node, 5,
				    ...
				    data->coro_fp);
  r = scpel_build_init_expr (cond, r);
  ...
  *stmt = stmt_list;
  ...
  tree *aw_expr_ptr;
  if (await_init &&
      scpel_walk_tree (await_init, co_await_find_in_subtree, &aw_expr_ptr, NULL))
    expand_one_await_expression (await_init, aw_expr_ptr, d);

  /* Expand any more await expressions in the original statement.  */
  if (scpel_walk_tree (revised, co_await_find_in_subtree, &aw_expr_ptr, NULL))
...
{
  tree *aw_expr_ptr;
  if (scpel_walk_tree (stmt, co_await_find_in_subtree, &aw_expr_ptr, NULL))
...
await_statement_expander (tree *stmt, int *do_subtree, void *d)
{
  ...
  if (STATEMENT_CLASS_P (*stmt) || TREE_CODE (*stmt) == BIND_EXPR)
	...
	{
	  res = scpel_walk_tree (&s, await_statement_expander,
			      d, NULL);
	  if (res)
	    return res;
	}
  ...
  if (si->await_field_id)
    {
	 ...
	 computed.  */
      ...
      data.to = as;
      scpel_walk_tree (&await_expr, replace_proxy, &data, NULL);
...
transform_local_var_uses (tree *stmt, int *do_subtree, void *d)
{
  ...
  if (TREE_CODE (*stmt) == BIND_EXPR)
	   ...
	   lvar = DECL_CHAIN (lvar))
	{
      ...
      scpel_walk_tree (&BIND_EXPR_BODY (*stmt), transform_local_var_uses, d, NULL);
		...
		tree resume_idx_var, unsigned body_count, tree frame_size)
{
  ...
  scpel_walk_tree (&fnbody, transform_local_var_uses, &xform_vars_data, NULL);
  ...
  tree hfa = build_new_method_call (ash, hfa_m, &args, NULL_TREE, LOOKUP_NORMAL,
				    NULL, tf_warning_or_error);
  r = scpel_build_init_expr (ash, hfa);
  r = coro_build_cvt_void_expr_stmt (r, loc);
  add_stmt (r);
  release_tree_vector (args);
  ...
  /* Transform the await expressions in the function body.  Only do each
     await tree once!  */
  hash_set<tree> pset;
  scpel_walk_tree (&fnbody, transform_await_wrapper, &xform, &pset);
  ...
  scpel_walk_tree (&actor_body, await_statement_expander, &data, NULL);
		    ...
		    hash_set<tree> *temps_used, tree *replace_in)
{
  ...
  switch (TREE_CODE (n->init))
	  ...
	  if (!VOID_TYPE_P (TREE_TYPE (then_cl)))
	    {
	      ...
	      if (init_expr)
		then_cl = scpel_build_init_expr (var, then_cl);
	      else
	  ...
	  if (!VOID_TYPE_P (TREE_TYPE (else_cl)))
	    {
	      ...
	      if (init_expr)
		else_cl = scpel_build_init_expr (var, else_cl);
	      else
      ...
      case COND_EXPR:
	{
	  ...
	  if (scpel_walk_tree (&cond, find_any_await, &found, NULL))
	    {
	      ...
	      cond = scpel_build_init_expr (cond_var, cond);
	      var_nest_node *ins
  ...
  tree t = scpel_walk_tree (&n->init, find_interesting_subtree, (void *)&v, NULL);
  if (!t)
    return;
  switch (TREE_CODE (t))
    {
      ...
      case TARGET_EXPR:
	{
	  ...
	  init = scpel_build_modify_expr (input_location, var, INIT_EXPR, init,
				       tf_warning_or_error);
	  ...
	  if (t == n->init && n->var == NULL_TREE)
	    {
	      ...
	      scpel_walk_tree (&init, replace_proxy, &pr, NULL);
	      n->init = init;
	      if (replace_in)
		scpel_walk_tree (replace_in, replace_proxy, &pr, NULL);
	      flatten_await_stmt (n, promoted, temps_used, NULL);
	    }
	  else
	    {
		...
		= new var_nest_node (var, init, n->prev, n);
	      ...
	      *v.entry = var;
	      ...
	      proxy_replace pr = {TREE_OPERAND (t, 0), var};
	      scpel_walk_tree (&n->init, replace_proxy, &pr, NULL);
      ...
      if (n->var && DECL_NAME (n->var))
	{
	  ...
	  if (TYPE_HAS_NONTRIVIAL_DESTRUCTOR (TREE_TYPE (n->var)))
	      ...
	      if (!existed)
		{
		 ...
		 exit.  */
	      tree set_flag = scpel_build_init_expr (flag, boolean_true_node);
	      n->init
...
maybe_promote_temps (tree *stmt, void *d)
{
  ...
  *stmt = await_bind;
  ...
  return scpel_walk_tree (stmt, register_awaits, d, &visited);
}
	...
	*stmt = TREE_OPERAND (*stmt, 1);
      ...
      case TRUTH_ORIF_EXPR:
	{
	  /* We don't need special action for awaits in the always-executed
	     arm of a TRUTH_IF.  */
	  if (tree res = scpel_walk_tree (&TREE_OPERAND (*stmt, 0),
	  ...
	  if (tree res = scpel_walk_tree (&TREE_OPERAND (*stmt, 1),
				       analyze_expression_awaits, d, NULL))
	    return res;
      ...
      case TRUTH_ANDIF_EXPR:
	{
	  ...
	  tree cond_expr
	    = build3_loc (sloc, COND_EXPR, boolean_type_node,
			  cond, test1, fixed);
	  *expr = cond_expr;
	  if (tree res = scpel_walk_tree (&COND_EXPR_COND (*expr),
				       expand_one_truth_if, d, NULL))
	    return res;
	  if (tree res = scpel_walk_tree (&COND_EXPR_THEN (*expr),
				       expand_one_truth_if, d, NULL))
	    return res;
...
await_statement_walker (tree *stmt, int *do_subtree, void *d)
{
  ...
  if (TREE_CODE (*stmt) == BIND_EXPR)
    {
	 ...
	 into the correct scope, so keep a note of the current block scope.  */
      ...
      res = scpel_walk_tree (&BIND_EXPR_BODY (*stmt), await_statement_walker,
			  d, NULL);
  ...
  else if (TREE_CODE (*stmt) == STATEMENT_LIST)
    {
      for (tree &s : tsi_range (*stmt))
	{
	  res = scpel_walk_tree (&s, await_statement_walker,
			      d, NULL);
	  if (res)
	    return res;
	}
  ...
  if (STATEMENT_CLASS_P (expr))
	...
	case IF_STMT:
	  {
	    ...
	    /* We treat the condition as if it was a stand-alone statement,
	       ...
	       and registered.  */
	    if (!(scpel_walk_tree (&IF_COND (if_stmt),
	    ...
	    tree new_s = scpel_build_init_expr (sloc, newvar, cond_inner);
	    finish_expr_stmt (new_s);
	    IF_COND (if_stmt) = newvar;
	    add_stmt (if_stmt);
	    *stmt = pop_stmt_list (insert_list);
	    /* So now walk the new statement list.  */
	    res = scpel_walk_tree (stmt, await_statement_walker, d, NULL);
	    *do_subtree = 0; /* Done subtrees.  */
	    return res;
	  }
	  break;
	case FOR_STMT:
	  {
	    ...
	    gcc_checking_assert
	      (!(scpel_walk_tree (&FOR_INIT_STMT (for_stmt), find_any_await,
			       &await_ptr, &visited)));

	    visited.empty ();
	    bool for_cond_await
	      = scpel_walk_tree (&FOR_COND (for_stmt), find_any_await,
			      &await_ptr, &visited);

	    visited.empty ();
	    bool for_expr_await
	      = scpel_walk_tree (&FOR_EXPR (for_stmt), find_any_await,
			      &await_ptr, &visited);
	    ...
	    */
		...
		if ((res = scpel_walk_tree (&FOR_BODY (for_stmt),
	    ...
	    res = scpel_walk_tree (&FOR_BODY (for_stmt),
	...
	case WHILE_STMT:
	  {
	    ...
	    if (!(scpel_walk_tree (&WHILE_COND (while_stmt),
		  find_any_await, &await_ptr, &visited)))
	      return NULL_TREE; /* Nothing special to do here.  */
	    ...
	    /* So now walk the new statement list.  */
	    res = scpel_walk_tree (&WHILE_BODY (while_stmt),
				await_statement_walker, d, NULL);
	    *do_subtree = 0; /* Done subtrees.  */
	    return res;
	  }
	...
	case DO_STMT:
	  {
	    ...
	    tree *await_ptr;
	    hash_set<tree> visited;
	    if (!(scpel_walk_tree (&DO_COND (do_stmt),
		  find_any_await, &await_ptr, &visited)))
	      return NULL_TREE; /* Nothing special to do here.  */
	    ...
	    /* So now walk the new statement list.  */
	    res = scpel_walk_tree (&DO_BODY (do_stmt), await_statement_walker,
				d, NULL);
	    *do_subtree = 0; /* Done subtrees.  */
	    return res;
	  }
	...
	case SWITCH_STMT:
	  {
	    ...
	    tree *await_ptr;
	    hash_set<tree> visited;
	    if (!(scpel_walk_tree (&SWITCH_STMT_COND (sw_stmt),
		  find_any_await, &await_ptr, &visited)))
	      return NULL_TREE; /* Nothing special to do here.  */
	    ...
	    tree new_s = scpel_build_init_expr (sloc, newvar,
				     cond_inner);
	    finish_expr_stmt (new_s);
	    ...
	    *stmt = pop_stmt_list (insert_list);
	    ...
	    res = scpel_walk_tree (stmt, await_statement_walker,
				d, NULL);
	    *do_subtree = 0; /* Done subtrees.  */
	    return res;
	  }
	...
	case HANDLER:
	  {
	    ...
	    tree *await_ptr;
	    hash_set<tree> visited;
	    if (!(scpel_walk_tree (&HANDLER_BODY (expr), find_any_await,
		  &await_ptr, &visited)))
  ...
  else if (EXPR_P (expr))
    {
      hash_set<tree> visited;
      tree *await_ptr;
      if (!(scpel_walk_tree (stmt, find_any_await, &await_ptr, &visited)))
	return NULL_TREE; /* Nothing special to do here.  */
      ...
      if ((res = scpel_walk_tree (stmt, analyze_expression_awaits, d, &visited)))
	return res;
      *do_subtree = 0; /* Done subtrees.  */
      if (!awpts->saw_awaits)
	return NULL_TREE; /* Nothing special to do here.  */
      ...
      if (awpts->needs_truth_if_exp)
	{
	  ...
	  if ((res = scpel_walk_tree (stmt, expand_one_truth_if, &xf, NULL)))
	    return res;
	}
...
register_local_var_uses (tree *stmt, int *do_subtree, void *d)
{
  ...
  if (TREE_CODE (*stmt) == BIND_EXPR)
	   ...
	   lvar = DECL_CHAIN (lvar))
	{
      ...
      lvd->bind_indx++;
      lvd->nest_depth++;
      scpel_walk_tree (&BIND_EXPR_BODY (*stmt), register_local_var_uses, d, NULL);
  ...
  if (param_uses)
	   ...
	   arg = DECL_CHAIN (arg))
	{
	 ...
	 arrange to visit each containing expression only once.  */
      ...
      scpel_walk_tree (&fnbody, rewrite_param_uses, &param_data, NULL);
    }
  ...
  if (flag_exceptions)
      ...
      if (initial_await != error_mark_node)
	{
	  ...
	  tree update = build2 (MODIFY_EXPR, boolean_type_node, i_a_r_c,
				boolean_true_node);
	  aw_r = scpel_build_compound_expr (update, aw_r, tf_warning_or_error);
...
morph_fn_to_coro (tree orig, tree *resumer, tree *destroyer)
{
  ...
  *destroyer = error_mark_node;
  ...
  scpel_function_chain->throwing_cleanup = false;
  ...
  scpel_walk_tree (&fnbody, await_statement_walker, &body_aw_points, NULL);
  ...
  scpel_walk_tree (&fnbody, register_local_var_uses, &local_vars_data, NULL);
	  ...
	  if (parm_i->this_ptr || parm_i->lambda_cobj)
	    {
	      ...
	      tt = scpel_build_reference_type (tt, false);
  ...
  tree r = scpel_build_init_expr (coro_fp, allocated);
  ...
  tree fnf_x = build_class_member_access_expr (deref_fp, fnf_m, NULL_TREE,
					       false, tf_warning_or_error);
  r = scpel_build_init_expr (fnf_x, boolean_true_node);
  r = coro_build_cvt_void_expr_stmt (r, fn_start);
  add_stmt (r);
  ...
  tree resume_x = build_class_member_access_expr (deref_fp, resume_m, NULL_TREE,
						  false, tf_warning_or_error);
  r = scpel_build_init_expr (fn_start, resume_x, actor_addr);
  finish_expr_stmt (r);
  ...
  tree destroy_x
    = build_class_member_access_expr (deref_fp, destroy_m, NULL_TREE, false,
				      tf_warning_or_error);
  r = scpel_build_init_expr (fn_start, destroy_x, destroy_addr);
  finish_expr_stmt (r);
  ...
  if (DECL_ARGUMENTS (orig))
	   ...
	   arg = DECL_CHAIN (arg))
	{
	  ...
	  if (parm.this_ptr || parm.lambda_cobj)
	    {
	      ...
	      tree rt = scpel_build_reference_type (ct, false);
	  ...
	  if (parm.rv_ref || parm.pt_ref)
	    ...
	    r = scpel_build_modify_expr (fn_start, TREE_OPERAND (fld_idx, 0),
				      INIT_EXPR, arg, tf_warning_or_error);
	  else
	    {
	      r = forward_parm (arg);
	      r = scpel_build_modify_expr (fn_start, fld_idx, INIT_EXPR, r,
					tf_warning_or_error);
	    }
	  finish_expr_stmt (r);
  ...
  if (gro_is_void_p)
	...
	r = scpel_build_init_expr (fn_start, DECL_RESULT (orig), get_ro);
	...
	param to the CTOR for the return object.  */
      ...
      add_decl_expr (gro);
      gro_bind_vars = gro;
      r = scpel_build_modify_expr (input_location, gro, INIT_EXPR, get_ro,
				tf_warning_or_error);
  ...
  r = scpel_build_init_expr (fn_start, resume_idx, r);
  r = coro_build_cvt_void_expr_stmt (r, fn_start);
  add_stmt (r);
