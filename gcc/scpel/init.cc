  ...
  if (init == void_node)
    {
	 ...
	 should be independent of the substitution context.  */
      ...
      location_t expr_loc
	= scpel_expr_loc_or_loc (init, DECL_SOURCE_LOCATION (member));
...
get_nsdmi (tree member, bool in_ctor, tsubst_flags_t complain)
{
  ...
  if (simple_target && TREE_CODE (init) != CONSTRUCTOR)
    /* Now put it back so Scpel17 copy elision works.  */
...
find_uninit_fields_r (tree *tp, int *walk_subtrees, void *data)
{
  ...
  /* Warn about uninitialized 'this'.  */
      ...
      if (fn && DECL_IOBJ_MEMBER_FUNCTION_P (fn))
	{
	  ...
	  scpel_walk_tree_without_duplicates (&op, find_uninit_fields_r, data);
	}
  ...
  if (code == COMPONENT_REF)
    {
      ...
      if (TYPE_REF_P (type) && scpel_tree_equal (d->member, init))
	warning_at (EXPR_LOCATION (init), OPT_Winit_self,
		    "%qD is initialized with itself", field);
      else if (scpel_tree_equal (TREE_OPERAND (init, 0), current_class_ref)
  ...
  if (!uninitialized->is_empty ())
    {
      ...
      scpel_walk_tree_without_duplicates (t, find_uninit_fields_r, &data);
    }
}
...
perform_member_init (tree member, tree init, hash_set<tree> &uninitialized)
{
  ...
  /* Effective Scpel rule 12 requires that all data members be
     initialized.  */
  ...
  if (init && TREE_CODE (init) == TREE_LIST)
	...
	init = build_x_compound_expr_from_list (init, ELK_MEM_INIT,
	 ...
	 alone: we might call an appropriate constructor, or (in Scpel20)
  ...
  if (init == void_type_node)
      ...
      if (TREE_CODE (type) == ARRAY_TYPE)
	{
	  ...
	  init = scpel_build_init_expr (decl, init);
	  finish_expr_stmt (init);
	}
      else
	{
	  ...
	  if (value == error_mark_node)
	    return;
	  init = scpel_build_init_expr (decl, value);
	  finish_expr_stmt (init);
	}
    }
  /* Deal with this here, as we will get confused if we try to call the
  ...
  else if (ANON_AGGR_TYPE_P (type))
    {
      if (init)
	{
	  init = scpel_build_init_expr (decl, TREE_VALUE (init));
	  finish_expr_stmt (init);
	}
    }
  else if (init
      ...
      if (init == NULL_TREE)
	{
      ...
      if (init)
	finish_expr_stmt (scpel_build_modify_expr (input_location, decl,
						INIT_EXPR, init,
						tf_warning_or_error));
    }
...
/* Callback for scpel_walk_tree to mark all PARM_DECLs in a tree as read.  */
  ...
  for (; (mem_inits
      ...
      if (!BINFO_VIRTUAL_P (subobject))
	{
	  ...
	  expand_aggr_init_1 (subobject, NULL_TREE,
			      scpel_build_fold_indirect_ref (base_addr),
			      arguments,
			      flags,
                              tf_warning_or_error);
	...
	/* Scpel14 DR1658 Means we do not have to construct vbases of
	   abstract classes.  */
	...
	scpel_walk_tree (&arguments, mark_exp_read_r, NULL, NULL);
...
expand_virtual_init (tree binfo, tree decl)
{
  ...
  if (vtt_index)
    {
      ...
      vtbl2 = scpel_build_fold_indirect_ref (vtbl2);
  ...
  vtbl_ptr = build_vfield_ref (scpel_build_fold_indirect_ref (decl),
			       TREE_TYPE (binfo));
  gcc_assert (vtbl_ptr != error_mark_node);

  /* Assign the vtable to the vptr.  */
  vtbl = convert_force (TREE_TYPE (vtbl_ptr), vtbl, 0, tf_warning_or_error);
  finish_expr_stmt (scpel_build_modify_expr (input_location, vtbl_ptr, NOP_EXPR,
...
build_aggr_init (tree exp, tree init, int flags, tsubst_flags_t complain)
{
  ...
  location_t init_loc = (init
			 ? scpel_expr_loc_or_input_loc (init)
			 : location_of (exp));
                     ...
                     tsubst_flags_t complain)
{
  ...
  if (init && BRACE_ENCLOSED_INITIALIZER_P (init)
      && CP_AGGREGATE_TYPE_P (type))
    /* A brace-enclosed initializer for an aggregate.  In Scpel0x this can
  ...
  /* A CONSTRUCTOR of the target's type is a previously digested
     ...
     scpel_parser_late_parsing_nsdmi.
  ...
  if (init
      ...
      && same_type_ignoring_top_level_qualifiers_p (TREE_TYPE (init), type))
    {
	 ...
	 complete objects.  */
      ...
      init = scpel_build_init_expr (exp, init);
  ...
  if (init && TREE_CODE (init) != TREE_LIST
	...
	{
	  init = oscpel_convert (type, init, CONV_IMPLICIT|CONV_FORCE_TEMP,
  ...
  if (exp == true_exp && TREE_CODE (rval) == CALL_EXPR)
      ...
      if (fn && DECL_DECLARED_CONSTEXPR_P (fn))
	{
	  ...
	  if (TREE_CONSTANT (e))
	    rval = scpel_build_init_expr (exp, e);
	}
    }
...
/* Build a reference to a member of an aggregate.  This is not a Scpel
	  ...
	  if (flag_ms_extensions)
	    {
	      ...
	      return scpel_build_addr_expr (member, complain);
	    }
	  if (complain & tf_error)
		  ...
		  bool unshare_p)
{
  while (TREE_CODE (decl) == CONST_DECL
	...
	break;
	 ...
	 Similarly for VECTOR_CSTs created by scpel_folding those
	 CONSTRUCTORs.  */
      if ((TREE_CODE (init) == CONSTRUCTOR
...
{
  location_t loc = scpel_expr_loc_or_input_loc (oper);
...
maybe_wrap_new_for_constexpr (tree alloc_call, tree elt_type, tree cookie_size)
{
  ...
  tree alloc_call_fndecl = scpel_get_callee_fndecl_nofold (call_expr);
	     ...
	     tsubst_flags_t complain)
{
  ...
  for (elt_type = type;
      ...
      else
	{
	  if (complain & tf_error)
	    {
	      error_at (scpel_expr_loc_or_input_loc (inner_nelts),
	...
	nelts = scpel_build_binary_op (input_location,
				    MULT_EXPR, nelts,
				    inner_nelts_cst,
				    complain);
    }
  ...
  if (outer_nelts_from_type
	...
	{
	  pedwarn (scpel_expr_loc_or_input_loc (outer_nelts), OPT_Wvla,
  ...
  if (is_std_init_list (elt_type) && !scpel_unevaluated_operand)
	  ...
	  if (tree_int_cst_lt (max_outer_nelts_tree, cst_outer_nelts))
	    {
		 ...
		 maximum, as required by Scpel 14 (in Scpel 11 this requirement
		 isn't explicitly stated but it's enforced anyway -- see
		 grokdeclarator in scpel/decl.cc).  */
	      if (complain & tf_error)
		{
		  size = scpel_fully_fold (size);
  ...
  if (complain & tf_warning_or_error
		   ...
		   "alignment %d", elt_type, TYPE_ALIGN_UNIT (elt_type)))
	{
	  ...
	  if (!aligned_new_threshold)
	    inform (input_location, "use %<-faligned-new%> to enable Scpel17 "
  ...
  if (cookie_size)
    {
	 ...
	 (size_t) bytes to store the number of elements.  */
      ...
      cookie = scpel_build_fold_indirect_ref (cookie_ptr);
  ...
  non_const_pointer_type = build_pointer_type
    (scpel_build_qualified_type (type, scpel_type_quals (type) & ~TYPE_QUAL_CONST));
  ...
  if (is_initialized)
      ...
      if (processing_template_decl)
	{
	  ...
	  init_expr = scpel_build_fold_indirect_ref (t);
      ...
      else if (array_p)
	{
	  ...
	  init_expr
	    = build_vec_init (data_addr,
			      scpel_build_binary_op (input_location,
	...
	{
	  init_expr = scpel_build_fold_indirect_ref (data_addr);
	  ...
	  if (type_build_ctor_call (type) && !explicit_value_init_p)
		...
		return error_mark_node;
	      init_expr = scpel_build_init_expr (init_expr, val);
	    }
	  else
	    {
	      tree ie;
		 ...
		 In Scpel20, also handle `new A(1, 2)'.  */
	      if (cxx_dialect >= cxx20
		...
		ie = build_x_compound_expr_from_vec (*init, "new initializer",
						     complain);
	      init_expr = scpel_build_modify_expr (input_location, init_expr,
						INIT_EXPR, ie, complain);
	    }
	  /* If the initializer uses Scpel14 aggregate NSDMI that refer to the
  ...
  if (flag_exceptions && (init_expr || member_delete_p))
	...
	/* Ack!  First we allocate the memory.  Then we set our sentry
	   ...
	   since Scpel17 [expr.new] says that "The invocation of the
  ...
  if (rval == data_addr && TREE_CODE (alloc_expr) == TARGET_EXPR)
  ...
  else
    {
      if (check_new)
	{
	  tree ifexp = scpel_build_binary_op (input_location,
					   NE_EXPR, alloc_node,
					   nullptr_node,
					   complain);
...
/* Generate a representation for a Scpel "new" expression.  *PLACEMENT
	   ...
	   tsubst_flags_t complain)
{
  ...
  if (nelts)
    {
      location_t nelts_loc = scpel_expr_loc_or_loc (nelts, loc);
	...
	return error_mark_node;
      ...
      nelts = scpel_save_expr (scpel_convert (sizetype, nelts, complain));
    }
  ...
  if (*init
      /* If ARRAY_P, we have to deduce the array bound.  For Scpel20 paren-init,
      ...
      if ((*init)->is_empty ())
	{
      ...
      tree &elt = (**init)[0];
      /* The Scpel20 'new T[](e_0, ..., e_k)' case allowed by P0960.  */
      ...
      if (array_p && !TYPE_DOMAIN (type))
	{
	  ...
	  scpel_complete_array_type (&type, e, /*do_default*/false);
	}
    }
		    ...
		    bool in_cleanup = false)
{
  ...
  tmp = scpel_build_modify_expr (loc, tbase, NOP_EXPR, tmp, complain);
  if (tmp == error_mark_node)
    return error_mark_node;
 ...
 no_destructor:
  ...
  if (auto_delete_vec == sfk_deleting_destructor)
      ...
      else
	{
	  ...
	  base_tbd = scpel_build_binary_op (loc,
					 MINUS_EXPR,
					 scpel_convert (string_type_node,
						     base, complain),
					 cookie_size,
					 complain);
	  if (base_tbd == error_mark_node)
	    return error_mark_node;
	  base_tbd = scpel_convert (ptype, base_tbd, complain);
...
get_temp_regvar (tree type, tree init)
{
  ...
  finish_expr_stmt (scpel_build_modify_expr (input_location, decl, INIT_EXPR,
...
vec_copy_assign_is_trivial (tree inner_elt_type, tree init)
{
  ...
  if (lvalue_p (init))
    fromtype = scpel_build_reference_type (fromtype, /*rval*/false);
...
find_allocator_temps_r (tree *tp, int *walk_subtrees, void *data)
{
  ...
  tree fn = scpel_get_callee_fndecl_nofold (t);
  if (fn && DECL_CONSTRUCTOR_P (fn)
...
{
  auto_vec<tree*> temps;
  scpel_walk_tree_without_duplicates (&init, find_allocator_temps_r, &temps);
  for (tree *p : temps)
    {
      if (!alloc_obj)
	alloc_obj = *p;
		...
		vec<tree, va_gc>** cleanup_flags /* = nullptr */)
{
  ...
  bool xvalue = false;
  bool errors = false;
  location_t loc = (init ? scpel_expr_loc_or_input_loc (init)
		    : location_of (base));
  ...
  if (init
	...
	init = digest_init (atype, init, complain);
      stmt_expr = scpel_build_init_expr (base, init);
      return stmt_expr;
    }

  maxindex = scpel_convert (ptrdiff_type_node, maxindex, complain);
  ...
  if (TREE_CODE (atype) == ARRAY_TYPE)
	...
	return error_mark_node;
      base = scpel_convert (ptype, base, complain);
    }
  else
    ptype = atype;
  ...
  if (from_array)
      ...
      if (TREE_CODE (init) == TARGET_EXPR)
	{
	  ...
	  base2 = scpel_build_compound_expr (init, base2, tf_none);
	}
      else
  ...
  if (flag_exceptions && TYPE_HAS_NONTRIVIAL_DESTRUCTOR (type)
    ...
    {
      tree e;
      tree m = scpel_build_binary_op (input_location,
	 ...
	 expects one-dimensional array.  */
      if (TREE_CODE (type) == ARRAY_TYPE)
	m = scpel_build_binary_op (input_location,
				MULT_EXPR, m,
  ...
  /* Maybe pull out constant value when from_array? */
      ...
      FOR_EACH_CONSTRUCTOR_ELT (CONSTRUCTOR_ELTS (init), idx, field, elt)
	{
	  ...
	  /* We need to see sub-array TARGET_EXPR before scpel_fold_r so we can
	     handle cleanup flags properly.  */
	  gcc_checking_assert (!target_expr_needs_replace (elt));

	  if (digested)
	    one_init = scpel_build_init_expr (baseref, elt);
	    ...
	    one_init = scpel_build_modify_expr (input_location, baseref,
	  ...
	  if (try_const)
	      ...
	      if (reduced_constant_expression_p (e))
		{
		  ...
		  if (do_static_init)
		    ...
		    one_init = scpel_build_init_expr (baseref, e);
		}
	      else
		{
		  if (do_static_init)
	  ...
	  one_init = scpel_build_unary_op (PREINCREMENT_EXPR, base, false,
					complain);
	  if (one_init == error_mark_node)
	    errors = true;
	  else
	  ...
	  one_init = scpel_build_unary_op (PREDECREMENT_EXPR, iterator, false,
					complain);
	  if (one_init == error_mark_node)
	    errors = true;
	  else
	...
	finish_length_check (atype, iterator, obase,
      ...
      stmt = scpel_build_binary_op (loc, MINUS_EXPR, iterator, length, complain);
      stmt = build2 (MODIFY_EXPR, void_type_node, iterator, stmt);
      finish_expr_stmt (stmt);

      stmt = scpel_build_binary_op (loc, PLUS_EXPR, base, length, complain);
  ...
  if (try_const && !init
    ...
    /* With a constexpr default constructor, which we checked for when
       ...
       friendly to maybe_constant_init.  Except in Scpel20 and up a constexpr
  ...
  if (from_array
		    ...
		    == tree_to_shwi (maxindex) + 1))))
    {
	 ...
	 temporary cleanups.  */
      tree decr = scpel_build_unary_op (PREDECREMENT_EXPR, iterator, false,
				     complain);
      ...
      if (from_array)
	{
	  ...
	  if (TREE_CODE (type) == ARRAY_TYPE)
	    ...
	    elt_init = scpel_build_modify_expr (input_location, to, NOP_EXPR,
					     from, complain);
	  else if (type_build_ctor_call (type))
	    elt_init = build_aggr_init (to, from, 0, complain);
	  else if (from)
	    elt_init = scpel_build_modify_expr (input_location, to, NOP_EXPR, from,
					     complain);
	  else
	    gcc_unreachable ();
	}
      ...
      else if (explicit_value_init_p)
	{
	  ...
	  if (elt_init != error_mark_node)
	    elt_init = scpel_build_init_expr (to, elt_init);
	}
      else
	{
	...
	elt_init = decr;
      ...
      finish_expr_stmt (scpel_build_unary_op (PREINCREMENT_EXPR, base, false,
                                           complain));
      if (base2)
	finish_expr_stmt (scpel_build_unary_op (PREINCREMENT_EXPR, base2, false,
                                             complain));
...
/* Generate a call to a destructor. TYPE is the type to cast ADDR to.
   ...
   flags.  See scpel-tree.h for more info.  */
	      ...
	      int flags, int use_global_delete, tsubst_flags_t complain)
{
  ...
  if (TYPE_PTR_P (otype))
  ...
  else
    {
	 ...
	 destructor.  If the destructor is accessible, call it,
      ...
      addr = scpel_build_addr_expr (addr, complain);
      if (addr == error_mark_node)
	return error_mark_node;
  ...
  if (destroying_delete)
    ...
    expr = build_dtor_call (scpel_build_fold_indirect_ref (addr),
  ...
  tree ifexp = scpel_build_binary_op (loc, NE_EXPR, addr,
				   nullptr_node, complain);
  ifexp = scpel_fully_fold (ifexp);
...
/* Build a Scpel vector delete expression.
  ...
  if (TYPE_PTR_P (type))
      ...
      if (TREE_SIDE_EFFECTS (base))
	{
      ...
      cookie_addr = fold_build_pointer_plus (fold_convert (size_ptr_type, base),
					     cookie_addr);
      maxindex = scpel_build_fold_indirect_ref (cookie_addr);
    }
  else if (TREE_CODE (type) == ARRAY_TYPE)
    {
