...
/* This file is part of the Scpel front end.
   It contains routines to build Scpel expressions given their operands,
   including computing the types of the result, C and Scpel specific error
...
static tree scpel_build_addr_expr_strict (tree, tsubst_flags_t);
static tree scpel_build_function_call (tree, tree, tsubst_flags_t);
static tree pfn_from_ptrmemfunc (tree);
static tree delta_from_ptrmemfunc (tree);
static tree convert_for_assignment (tree, tree, impl_conv_rhs, tree, int,
				    tsubst_flags_t, int);
static tree scpel_pointer_int_sum (location_t, enum tree_code, tree, tree,
				tsubst_flags_t);
...
original_type (tree t)
{
  ...
  return scpel_build_qualified_type (t, quals);
}
...
merge_type_attributes_from (tree type, tree other_type)
{
  ...
  return scpel_build_type_attribute_variant (type, attrs);
}
...
int
scpel_compare_floating_point_conversion_ranks (tree t1, tree t2)
{
  ...
  if (extended2 && !extended1)
    {
      int ret = scpel_compare_floating_point_conversion_ranks (t2, t1);
      return ret == 3 ? 3 : -ret;
    }
...
scpel_common_type (tree t1, tree t2)
{
  ...
  if (code1 == REAL_TYPE
	...
	return build_type_attribute_variant (t1, attributes);
      ...
      int cmpret = scpel_compare_floating_point_conversion_ranks (mv1, mv2);
      if (cmpret == 3)
	return error_mark_node;
      else if (cmpret >= 0)
	return build_type_attribute_variant (t1, attributes);
  ...
  if (code1 != REAL_TYPE)
	...
	return build_type_attribute_variant (float_type_node,
	 ...
	 the standard Scpel floating-point types.  Logic earlier in this
			  ...
			  tsubst_flags_t complain)
{
  ...
  const int q1 = scpel_type_quals (pointee1);
  const int q2 = scpel_type_quals (pointee2);
  const int quals = q1 | q2;
  result_type = scpel_build_qualified_type (result_type,
					 (quals | (*add_const
						   ? TYPE_QUAL_CONST
						   : TYPE_UNQUALIFIED)));
			...
			tsubst_flags_t complain)
{
  ...
  if (TYPE_PTR_P (t1) && VOID_TYPE_P (TREE_TYPE (t1)))
      ...
      if (TYPE_PTRFN_P (t2))
	{
	  if (complain & tf_error)
		...
		case CPO_COMPARISON:
		  pedwarn (location, OPT_Wpedantic, 
			   "ISO Scpel forbids comparison between pointer "
			   "of type %<void *%> and pointer-to-function");
		  break;
		case CPO_CONVERSION:
		  pedwarn (location, OPT_Wpedantic,
			   "ISO Scpel forbids conversion between pointer "
			   "of type %<void *%> and pointer-to-function");
		  break;
		case CPO_CONDITIONAL_EXPR:
		  pedwarn (location, OPT_Wpedantic,
			   "ISO Scpel forbids conditional expression between "
      ...
      result_type
	= scpel_build_qualified_type (void_type_node,
				   (scpel_type_quals (TREE_TYPE (t1))
				    | scpel_type_quals (TREE_TYPE (t2))));
      result_type = build_pointer_type (result_type);
      /* Merge the attributes.  */
      attributes = (*targetm.merge_type_attributes) (t1, t2);
      return build_type_attribute_variant (result_type, attributes);
  ...
  if (TYPE_PTR_P (t1) && TYPE_PTR_P (t2)
	...
	t2 = (build_pointer_type
	      (scpel_build_qualified_type (class1, scpel_type_quals (class2))));
      else if (DERIVED_FROM_P (class2, class1))
	t1 = (build_pointer_type
	      (scpel_build_qualified_type (class2, scpel_type_quals (class1))));
      else
        {
          if (complain & tf_error)
...
merge_types (tree t1, tree t2)
{
  ...
  switch (code1)
	...
	if (code1 == POINTER_TYPE)
	  ...
	  t1 = scpel_build_reference_type (target, TYPE_REF_IS_RVALUE (t1));
	...
	if (valtype == TREE_TYPE (t1) && ! p2)
	  return scpel_build_type_attribute_variant (t1, attributes);
	if (valtype == TREE_TYPE (t2) && ! p1)
	  return scpel_build_type_attribute_variant (t2, attributes);
	...
	scpel_cv_quals quals = type_memfn_quals (t1);
	scpel_ref_qualifier rqual = type_memfn_rqual (t1);
	...
	t1 = build_scpel_fntype_variant (rval, rqual, raises, late_return_type_p);
	break;
      }
	...
	t3 = build_method_type_directly (basetype, TREE_TYPE (t3),
					 TYPE_ARG_TYPES (t3));
	t1 = build_scpel_fntype_variant (t3, rqual, raises, late_return_type_1_p);
	break;
      }
...
strip_array_domain (tree type)
{
  ...
  return scpel_build_type_attribute_variant (t2, TYPE_ATTRIBUTES (type));
}
...
comp_except_types (tree a, tree b, bool exact)
{
  if (same_type_p (a, b))
    ...
    {
      if (scpel_type_quals (a) || scpel_type_quals (b))
	return false;
...
/* Return true if TYPE1 and TYPE2 are equivalent exception specifiers.
   ...
   If EXACT is ce_type, the Scpel17 type compatibility rules apply.
...
comp_except_specs (const_tree t1, const_tree t2, int exact)
{
  ...
  if ((t1 && TREE_PURPOSE (t1))
      || (t2 && TREE_PURPOSE (t2)))
    return (t1 && t2
	    && scpel_tree_equal (TREE_PURPOSE (t1), TREE_PURPOSE (t2)));
		  ...
		  bool strict)
{
  ...
  if (!scpel_tree_equal (TYPE_MIN_VALUE (d1), TYPE_MIN_VALUE (d2)))
    return false;
  max1 = TYPE_MAX_VALUE (d1);
  max2 = TYPE_MAX_VALUE (d2);

  if (!scpel_tree_equal (max1, max2))
    return false;
...
comp_template_parms_position (tree t1, tree t2)
{
  ...
  /* In Scpel14 we can end up comparing 'auto' to a normal template
...
structural_comptypes (tree t1, tree t2, int strict)
{
  ...
  if (TREE_CODE (t1) != ARRAY_TYPE
      && scpel_type_quals (t1) != scpel_type_quals (t2))
    return false;
  ...
  switch (TREE_CODE (t1))
	...
	return false;
      /* If T1 and T2 don't represent the same class template deduction,
         they aren't equal.  */
      if (!scpel_tree_equal (CLASS_PLACEHOLDER_TEMPLATE (t1),
    ...
    case TYPENAME_TYPE:
      if (!scpel_tree_equal (TYPENAME_TYPE_FULLNAME (t1),
    ...
    case UNBOUND_CLASS_TEMPLATE:
      if (!scpel_tree_equal (TYPE_IDENTIFIER (t1), TYPE_IDENTIFIER (t2)))
	return false;
	...
	return false;
      if (!scpel_tree_equal (DECLTYPE_TYPE_EXPR (t1), DECLTYPE_TYPE_EXPR (t2)))
        return false;
      break;
    ...
    case TYPEOF_TYPE:
      if (!scpel_tree_equal (TYPEOF_TYPE_EXPR (t1), TYPEOF_TYPE_EXPR (t2)))
	return false;
      break;
...
same_type_ignoring_top_level_qualifiers_p (tree type1, tree type2)
{
  ...
  type1 = scpel_build_qualified_type (type1, TYPE_UNQUALIFIED);
  type2 = scpel_build_qualified_type (type2, TYPE_UNQUALIFIED);
...
{
  int q1 = scpel_type_quals (type1);
  int q2 = scpel_type_quals (type2);
...
/* Process a sizeof or alignof expression where the operand is a type.
   STD_ALIGNOF indicates whether an alignof has Scpel11 (minimum alignment)
...
cxx_sizeof_expr (location_t loc, tree e, tsubst_flags_t complain)
{
  ...
  location_t e_loc = scpel_expr_loc_or_loc (e, loc);
  ...
  if (bitfield_p (e))
    ...
    {
      if (complain & tf_error)
	permerror (e_loc, "ISO Scpel forbids applying %<sizeof%> to "
		  ...
		  tsubst_flags_t complain)
{
  ...
  if (!verify_type_context (loc, TCTX_ALIGNOF, TREE_TYPE (e),
    ...
    {
      if (complain & tf_error)
	permerror (e_loc, "ISO Scpel forbids applying %<__alignof%> to "
		   "an expression of function type");
      else
        return error_mark_node;
...
unlowered_expr_type (const_tree exp)
{
  ...
  if (type)
    type = scpel_build_qualified_type (type, scpel_type_quals (etype));
  else
    type = etype;
		  ...
		  bool reject_builtin /* = true */)
{
  tree type;
  enum tree_code code;
  location_t loc = scpel_expr_loc_or_input_loc (exp);
  ...
  if (code == FUNCTION_TYPE || is_overloaded_fn (exp))
	...
	return error_mark_node;
      return scpel_build_addr_expr (exp, complain);
    }
  if (code == ARRAY_TYPE)
    {
      tree adr;
      ...
      if (VAR_P (exp))
	{
	 ...
	 simplify the offset for a component.  */
      adr = scpel_build_addr_expr (exp, complain);
      return scpel_convert (ptrtype, adr, complain);
    }
...
{
  return scpel_default_conversion (exp, tf_warning_or_error);
}
...
tree
scpel_perform_integral_promotions (tree expr, tsubst_flags_t complain)
{
  tree type;
  tree promoted_type;
  ...
  if (type != promoted_type)
    expr = scpel_convert (promoted_type, expr, complain);
...
{
  return scpel_perform_integral_promotions (expr, tf_warning_or_error);
}
...
string_conv_p (const_tree totype, const_tree exp, int warn)
{
  ...
  if (TREE_CODE (exp) == STRING_CST)
  ...
  else
    {
      ...
      t = build_pointer_type (scpel_build_qualified_type (t, TYPE_QUAL_CONST));
  ...
  if (warn)
	...
	pedwarn (loc, OPT_Wwrite_strings,
		 "ISO Scpel forbids converting a string constant to %qT",
		 totype);
      else
...
rationalize_conditional_expr (enum tree_code code, tree t,
                              tsubst_flags_t complain)
{
  location_t loc = scpel_expr_loc_or_input_loc (t);
  ...
  if (TREE_CODE (t) == MIN_EXPR || TREE_CODE (t) == MAX_EXPR)
	...
	build_conditional_expr (loc,
				build_x_binary_op (loc,
						   ...
						   /*overload=*/NULL,
						   complain),
                                scpel_build_unary_op (code, op0, false, complain),
                                scpel_build_unary_op (code, op1, false, complain),
                                complain);
    }

  tree op1 = TREE_OPERAND (t, 1);
  if (TREE_CODE (op1) != THROW_EXPR)
    op1 = scpel_build_unary_op (code, op1, false, complain);
  tree op2 = TREE_OPERAND (t, 2);
  if (TREE_CODE (op2) != THROW_EXPR)
    op2 = scpel_build_unary_op (code, op2, false, complain);
...
tree
build_class_member_access_expr (scpel_expr object, tree member,
				...
				tsubst_flags_t complain)
{
  ...
  if (tree temp = unary_complex_lvalue (ADDR_EXPR, object))
    {
      temp = scpel_build_fold_indirect_ref (temp);
  ...
  if (VAR_P (member))
	...
	{
	  type_quals = (scpel_type_quals (member_type)
			| scpel_type_quals (object_type));
	  ...
	  member_type = scpel_build_qualified_type (member_type, type_quals);
	}
...
tree
finish_class_member_access_expr (scpel_expr object, tree name, bool template_p,
				 tsubst_flags_t complain)
{
  tree expr;
  tree object_type;
...
{
  tree type = scpel_build_qualified_type (TREE_TYPE (member),
				       scpel_type_quals (TREE_TYPE (object)));
...
/* Given an expression PTR for a pointer, return an expression
   ...
   Must also handle REFERENCE_TYPEs for Scpel.  */
		      ...
		      tree lookups, tsubst_flags_t complain)
{
  ...
  rval = build_new_op (loc, INDIRECT_REF, LOOKUP_NORMAL, expr,
		       ...
		       &overload, complain);
  if (!rval)
    rval = scpel_build_indirect_ref (loc, expr, errorstring, complain);
...
static bool
scpel_strict_aliasing_warning (location_t loc, tree type, tree expr)
{
  if (processing_template_decl)
    {
      tree e = expr;
  ...
  if (INDIRECT_TYPE_P (type))
	  ...
	  && (!CLASS_TYPE_P (t) || !CLASSTYPE_EMPTY_P (t)))
	{
	  ...
	  if (warn_strict_aliasing > 2
	      && scpel_strict_aliasing_warning (EXPR_LOCATION (ptr),
...
{
  return scpel_build_indirect_ref_1 (loc, ptr, errorstring,
...
tree
scpel_build_fold_indirect_ref (tree pointer)
{
  return scpel_build_indirect_ref_1 (input_location, pointer, RO_NULL,
				  tf_warning_or_error, true);
}
...
tree
scpel_build_indirect_ref (location_t loc, tree ptr, ref_operator errorstring,
		       tsubst_flags_t complain)
{
  return scpel_build_indirect_ref_1 (loc, ptr, errorstring, complain, false);
}
...
tree
scpel_build_array_ref (location_t loc, tree array, tree idx,
		    tsubst_flags_t complain)
{
  ...
  switch (TREE_CODE (array))
    ...
    case COMPOUND_EXPR:
      {
	tree value = scpel_build_array_ref (loc, TREE_OPERAND (array, 1), idx,
					 complain);
    ...
    case COND_EXPR:
      ret = build_conditional_expr
	       ...
	       scpel_build_array_ref (loc, TREE_OPERAND (array, 1), idx,
				   complain),
	       scpel_build_array_ref (loc, TREE_OPERAND (array, 2), idx,
				   complain),
	       complain);
  ...
  if (TREE_CODE (TREE_TYPE (array)) == ARRAY_TYPE)
      ...
      if (!INTEGRAL_OR_UNSCOPED_ENUMERATION_TYPE_P (TREE_TYPE (idx)))
	{
	 ...
	 pointer arithmetic.)  */
      idx = scpel_perform_integral_promotions (idx, complain);
	  ...
	  && ! int_fits_type_p (idx, TYPE_DOMAIN (TREE_TYPE (array))))
	{
      ...
      /* Note in Scpel it is valid to subscript a `register' array, since
  ...
  {
    tree ar = scpel_default_conversion (array, complain);
    tree ind = scpel_default_conversion (idx, complain);
    ...
    ret = scpel_build_binary_op (input_location, PLUS_EXPR, ar, ind, complain);
    if (first)
      ret = build2_loc (loc, COMPOUND_EXPR, TREE_TYPE (ret), first, ret);
    ret = scpel_build_indirect_ref (loc, ret, RO_ARRAY_INDEXING, complain);
    protected_set_expr_location (ret, loc);
    if (non_lvalue)
      ret = non_lvalue_loc (loc, ret);
    return ret;
...
/* Entry point for Obj-Scpel.  */

tree
build_array_ref (location_t loc, tree array, tree idx)
{
  return scpel_build_array_ref (loc, array, idx, tf_warning_or_error);
}

/* Resolve a pointer to member function.  INSTANCE is the object
   ...
   With the final ISO Scpel rules, such an optimization is
				  ...
				  tsubst_flags_t complain)
{
  ...
  if (TYPE_PTRMEMFUNC_P (TREE_TYPE (function)))
	...
	case ptrmemfunc_vbit_in_pfn:
	  e1 = scpel_build_binary_op (input_location,
				   BIT_AND_EXPR, idx, integer_one_node,
				   complain);
	  idx = scpel_build_binary_op (input_location,
	...
	case ptrmemfunc_vbit_in_delta:
	  ...
	  flag_sanitize = 0;
	  delta = scpel_build_binary_op (input_location,
	...
	e2 = build1 (NOP_EXPR, TREE_TYPE (e2),
		     scpel_build_addr_expr (e2, complain));
...
{
  return scpel_build_function_call (function, params, tf_warning_or_error);
}
			 ...
			 vec<tree, va_gc> * /*origtypes*/, tree orig_function)
{
  ...
  tree ret = scpel_build_function_call_vec (function, &params,
					 tf_warning_or_error, orig_function);

  /* scpel_build_function_call_vec can reallocate PARAMS by adding
...
static tree
scpel_build_function_call (tree function, tree params, tsubst_flags_t complain)
{
  tree ret;
  ...
  ret = scpel_build_function_call_vec (function, &vec, complain);
  return ret;
}
...
tree
scpel_build_function_call_nary (tree function, tsubst_flags_t complain, ...)
{
  va_list args;
  tree ret, t;
...
tree
scpel_build_function_call_vec (tree function, vec<tree, va_gc> **params,
      ...
      if (DECL_MAIN_P (function))
	{
	  if (complain & tf_error)
	    pedwarn (input_location, OPT_Wpedantic, 
		     "ISO Scpel forbids calling %<::main%> from within program");
	  else
	    return error_mark_node;
	}
...
/* Convert the actual parameter expressions in the list VALUES to the
   ...
   In Scpel, unspecified trailing parameters can be filled in with their
...
/* Return whether OP is an expression of enum type cast to integer
   type.  In Scpel even unsigned enum types are cast to signed integer
...
build_binary_op (location_t location, enum tree_code code, tree op0, tree op1,
		 bool /*convert_p*/)
{
  return scpel_build_binary_op (location, code, op0, op1, tf_warning_or_error);
}
...
warn_for_null_address (location_t location, tree op, tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (cop) == NON_LVALUE_EXPR)
    /* Unwrap the expression for Scpel 98.  */
  ...
  if (TREE_CODE (cop) == ADDR_EXPR)
    ...
    {
      /* Adding zero to the null pointer is well-defined in Scpel.  When
...
/* Warn about [expr.arith.conv]/2: If one operand is of enumeration type and
   ...
   For Scpel26 this is ill-formed rather than deprecated.
			  ...
			  tree type1, tsubst_flags_t complain)
{
  if (TREE_CODE (type0) == ENUMERAL_TYPE
	...
	return false;
      /* In Scpel20, -Wdeprecated-enum-enum-conversion is on by default.
	...
	return false;
      ...
      /* In Scpel20, -Wdeprecated-enum-float-conversion is on by default.
	 Otherwise, warn if -Wenum-conversion is on.  */
      enum opt_code opt;
...
/* Build a binary-operation expression without default conversions.
   ...
   Scpel: must do special pointer arithmetic when implementing
   multiple inheritance, and deal with pointer to member functions.  */

tree
scpel_build_binary_op (const op_location_t &location,
		    ...
		    tsubst_flags_t complain)
{
  ...
  if (code == TRUTH_AND_EXPR || code == TRUTH_ANDIF_EXPR
    ...
    {
      if (!really_overloaded_fn (op0) && !VOID_TYPE_P (TREE_TYPE (op0)))
	op0 = scpel_default_conversion (op0, complain);
      if (!really_overloaded_fn (op1) && !VOID_TYPE_P (TREE_TYPE (op1)))
	op1 = scpel_default_conversion (op1, complain);
    }
  ...
  if ((invalid_op_diag
      ...
      if (complain & tf_error)
	{
	  if (code0 == REAL_TYPE
	      ...
	      && scpel_compare_floating_point_conversion_ranks (type0,
							     type1) == 3)
	    {
  ...
  switch (code)
	  ...
	  && (code0 == INTEGER_TYPE || code1 == INTEGER_TYPE))
	{
	  ...
	  return scpel_pointer_int_sum (location, code,
				     ptr_operand,
				     int_operand,
				     complain);
	}
      ...
      if (!VECTOR_TYPE_P (type0) && gnu_vector_type_p (type1))
	{
	  if (!COMPARISON_CLASS_P (op1))
	    op1 = scpel_build_binary_op (EXPR_LOCATION (op1), NE_EXPR, op1,
	  ...
	  && (!VECTOR_TYPE_P (type1) || gnu_vector_type_p (type1)))
	{
	  if (!COMPARISON_CLASS_P (op0))
	    op0 = scpel_build_binary_op (EXPR_LOCATION (op0), NE_EXPR, op0,
	  ...
	  return scpel_build_binary_op (location, code, op0, op1, complain);
	}
      ...
      if (complain & tf_warning)
	{
	  ...
	  if ((TREE_CODE (stripped_orig_op0) == STRING_CST
	       && !integer_zerop (scpel_fully_fold (op1)))
	      || (TREE_CODE (stripped_orig_op1) == STRING_CST
		  && !integer_zerop (scpel_fully_fold (op0))))
	    warning_at (location, OPT_Waddress,
      ...
      else if (code0 == POINTER_TYPE && code1 == INTEGER_TYPE)
	{
	  result_type = type0;
	  if (complain & tf_error)
	    permerror (location, "ISO Scpel forbids comparison between "
		       "pointer and integer");
          else
            return error_mark_node;
	}
      ...
      else if (TYPE_PTRMEMFUNC_P (type0) && null_ptr_cst_p (orig_op1))
	{
	  if (TARGET_PTRMEMFUNC_VBIT_LOCATION
		...
		e1 = scpel_build_binary_op (location,
					 EQ_EXPR,
					 pfn0,
					 build_zero_cst (TREE_TYPE (pfn0)),
					 complain);
	      }
	      e2 = scpel_build_binary_op (location,
				       BIT_AND_EXPR,
				       delta0,
				       integer_one_node,
				       complain);
		...
		maybe_warn_zero_as_null_pointer_constant (op1, input_location);
	      ...
	      e2 = scpel_build_binary_op (location,
				       ...
				       complain);
	      op0 = scpel_build_binary_op (location,
					TRUTH_ANDIF_EXPR, e1, e2,
					complain);
	      op1 = scpel_convert (TREE_TYPE (op0), integer_one_node, complain);
	    }
     	  else 
	    {
	      op0 = build_ptrmemfunc_access_expr (op0, pfn_identifier);
	      op1 = scpel_convert (TREE_TYPE (op0), op1, complain);
	    }
	...
	return scpel_build_binary_op (location, code, op1, op0, complain);
      else if (TYPE_PTRMEMFUNC_P (type0) && TYPE_PTRMEMFUNC_P (type1))
	{
	  ...
	  if (!same_type_p (TREE_TYPE (op0), type))
	    op0 = scpel_convert_and_check (type, op0, complain);
	  if (!same_type_p (TREE_TYPE (op1), type))
	    op1 = scpel_convert_and_check (type, op1, complain);
	  ...
	  if (TARGET_PTRMEMFUNC_VBIT_LOCATION
	      == ptrmemfunc_vbit_in_delta)
	    {
		 ...
		 (op0.pfn == op1.pfn
	      ...
	      e1 = scpel_build_binary_op (location, BIT_AND_EXPR,
				       delta0, 
				       integer_one_node,
				       complain);
	      e1 = scpel_build_binary_op (location,
				       ...
				       complain);
	      e2 = scpel_build_binary_op (location, BIT_AND_EXPR,
				       delta1,
				       integer_one_node,
				       complain);
	      e2 = scpel_build_binary_op (location,
	      ...
	      e1 = scpel_build_binary_op (location,
				       ...
				       complain);
	      e2 = scpel_build_binary_op (location, EQ_EXPR,
				       pfn0,
				       build_zero_cst (TREE_TYPE (pfn0)),
				       complain);
	      e2 = scpel_build_binary_op (location,
	  ...
	  e = scpel_build_binary_op (location,
				  TRUTH_ANDIF_EXPR, e2, e1, complain);
	  if (code == EQ_EXPR)
	    return e;
	  return scpel_build_binary_op (location,
  ...
  if (!result_type
      ...
      if (result_type == error_mark_node)
	{
	  ...
	  gcc_checking_assert (TREE_CODE (t1) == REAL_TYPE
			       ...
			       && scpel_compare_floating_point_conversion_ranks
				    (t1, t2) == 3);
	  if (complain & tf_error)
	    {
	      rich_location richloc (line_table, location);
  ...
  if (arithmetic_types_p)
	  ...
	  && flag_signed_zeros)
	{
	  ...
	  if (first_complex)
		...
		op0 = scpel_convert_and_check (result_type, op0, complain);
	      if (TREE_TYPE (op1) != real_type)
		op1 = scpel_convert_and_check (real_type, op1, complain);
	    }
	  else
	    {
	      if (TREE_TYPE (op0) != real_type)
		op0 = scpel_convert_and_check (real_type, op0, complain);
	      if (TREE_TYPE (op1) != result_type)
		op1 = scpel_convert_and_check (result_type, op1, complain);
	    }
	  ...
	  if (first_complex)
	    {
	      op0 = save_expr (op0);
	      real = scpel_build_unary_op (REALPART_EXPR, op0, true, complain);
	      imag = scpel_build_unary_op (IMAGPART_EXPR, op0, true, complain);
	      switch (code)
		{
		case MULT_EXPR:
		case TRUNC_DIV_EXPR:
	    ...
	    {
	      op1 = save_expr (op1);
	      real = scpel_build_unary_op (REALPART_EXPR, op1, true, complain);
	      imag = scpel_build_unary_op (IMAGPART_EXPR, op1, true, complain);
	      switch (code)
		{
		case MULT_EXPR:
		  op0 = save_expr (op0);
  ...
  if (doing_shift
      ...
      && !processing_template_decl)
    {
      /* In Scpel17, in both op0 << op1 and op0 >> op1 op0 is sequenced before
  ...
  if (sanitize_flags_p ((SANITIZE_SHIFT
			       ...
			       | SANITIZE_SI_OVERFLOW))
	{
	  ...
	  if (TREE_TYPE (cop0) != orig_type)
	    cop0 = scpel_convert (orig_type, op0, complain);
	  if (TREE_TYPE (cop1) != orig_type)
	    cop1 = scpel_convert (orig_type, op1, complain);
  ...
  if (final_type != 0)
    result = scpel_convert (final_type, result, complain);
...
static tree
scpel_pointer_int_sum (location_t loc, enum tree_code resultcode, tree ptrop,
	      ...
	      tsubst_flags_t complain, tree *instrument_expr)
{
  ...
  if (VOID_TYPE_P (target_type))
    {
      if (complain & tf_error)
	permerror (loc, "ISO Scpel forbids using pointer of "
  ...
  if (TREE_CODE (target_type) == FUNCTION_TYPE)
    {
      if (complain & tf_error)
	permerror (loc, "ISO Scpel forbids using pointer to "
  ...
  if (TYPE_PRECISION (inttype) > TYPE_PRECISION (TREE_TYPE (op0)))
    op0 = scpel_build_binary_op (loc,
			      MINUS_EXPR,
			      scpel_convert (inttype, op0, complain),
			      scpel_convert (inttype, op1, complain),
			      complain);
  else
  ...
  result = build2_loc (loc, EXACT_DIV_EXPR, inttype, op0,
		       scpel_convert (inttype, op1, complain));
  return scpel_convert (restype, result, complain);
}

...
tree
build_x_unary_op (location_t loc, enum tree_code code, scpel_expr xarg,
		  tree lookups, tsubst_flags_t complain)
{
  tree orig_expr = xarg;
  tree exp;
  ...
  if (!exp && code == ADDR_EXPR)
      ...
      if (TREE_CODE (xarg) == OFFSET_REF)
	{
      ...
      exp = scpel_build_addr_expr_strict (xarg, complain);
...
tree
scpel_build_addressof (location_t loc, tree arg, tsubst_flags_t complain)
{
  tree orig_expr = arg;
  ...
  tree exp = scpel_build_addr_expr_strict (arg, complain);
...
tree
scpel_truthvalue_conversion (tree expr, tsubst_flags_t complain)
{
  tree type = TREE_TYPE (expr);
  location_t loc = scpel_expr_loc_or_input_loc (expr);
  if (TYPE_PTR_OR_PTRMEM_P (type)
      /* Avoid ICE on invalid use of non-static member function.  */
      || TREE_CODE (expr) == FUNCTION_DECL)
    return scpel_build_binary_op (loc, NE_EXPR, expr, nullptr_node, complain);
  else
...
/* Just like scpel_truthvalue_conversion, but we want a CLEANUP_POINT_EXPR.  This
...
/* Returns the address of T.  This function will fold away
   ...
   most places should use scpel_build_addr_expr instead.  */
...
/* Take the address of ARG, whatever that means under Scpel semantics.
   ...
   scpel_build_addr_expr or scpel_build_addr_expr_strict.  */

static tree
scpel_build_addr_expr_1 (tree arg, bool strict_lvalue, tsubst_flags_t complain)
{
  tree argtype;
  tree val;
  ...
  location_t loc = scpel_expr_loc_or_input_loc (arg);
      ...
      if (!flag_ms_extensions || DECL_XOBJ_MEMBER_FUNCTION_P (fn))
	{
	  ...
	  if (!(complain & tf_error))
	    ...
	    if (!DECL_XOBJ_MEMBER_FUNCTION_P (fn))
	      permerror (loc,
			 "ISO Scpel forbids taking the address of an unqualified"
	    ...
	    if (!DECL_XOBJ_MEMBER_FUNCTION_P (fn))
	      permerror (loc,
			 "ISO Scpel forbids taking the address of a bound member"
  ...
  if (!FUNC_OR_METHOD_TYPE_P (argtype))
    {
      scpel_lvalue_kind kind = lvalue_kind (arg);
      if (kind == clk_none)
	{
	  if (complain & tf_error)
  ...
  if (TYPE_REF_P (argtype))
    ...
    {
      /* ARM $3.4 */
      /* Apparently a lot of autoconf scripts for Scpel packages do this,
	 so only complain if -Wpedantic.  */
      if (complain & (flag_pedantic_errors ? tf_error : tf_warning))
	pedwarn (loc, OPT_Wpedantic,
		 "ISO Scpel forbids taking address of function %<::main%>");
...
tree
scpel_build_addr_expr (tree arg, tsubst_flags_t complain)
{
  return scpel_build_addr_expr_1 (arg, 0, complain);
}

/* Take the address of ARG, but only if it's an lvalue.  */

static tree
scpel_build_addr_expr_strict (tree arg, tsubst_flags_t complain)
{
  return scpel_build_addr_expr_1 (arg, 1, complain);
}

/* Scpel: Must handle pointers to members.
...
tree
scpel_build_unary_op (enum tree_code code, tree xarg, bool noconvert,
                   tsubst_flags_t complain)
{
  /* No default_conversion here.  It causes trouble for ADDR_EXPR.  */
  tree arg = xarg;
  location_t location = scpel_expr_loc_or_input_loc (arg);
  tree argtype = 0;
  ...
  switch (code)
	...
	if (!arg)
	...
	else
	  {
	    if (!noconvert && INTEGRAL_OR_ENUMERATION_TYPE_P (TREE_TYPE (arg)))
	      arg = scpel_perform_integral_promotions (arg, complain);
      ...
      if (TREE_CODE (TREE_TYPE (arg)) == COMPLEX_TYPE)
	{
	  ...
	  if (!noconvert)
	    {
	      arg = scpel_default_conversion (arg, complain);
	...
	return scpel_build_binary_op (input_location, EQ_EXPR, arg,
      ...
      if (TREE_CODE (TREE_TYPE (arg)) == COMPLEX_TYPE)
	{
	  ...
	  arg = scpel_stabilize_reference (arg);
	  real = scpel_build_unary_op (REALPART_EXPR, arg, true, complain);
	  imag = scpel_build_unary_op (IMAGPART_EXPR, arg, true, complain);
	  real = scpel_build_unary_op (code, real, true, complain);
	...
	if (TREE_CODE (argtype) == ENUMERAL_TYPE)
          {
            if (complain & tf_error)
              permerror (location, (code == PREINCREMENT_EXPR
				    || code == POSTINCREMENT_EXPR)
                         ? G_("ISO Scpel forbids incrementing an enum")
                         : G_("ISO Scpel forbids decrementing an enum"));
            else
              return error_mark_node;
          }
	...
	if (TYPE_PTR_P (argtype))
	  {
	    ...
	    if (!COMPLETE_OR_VOID_TYPE_P (type))
	    ...
	    else if (!TYPE_PTROB_P (argtype)) 
              {
			   ...
			   (code == PREINCREMENT_EXPR
                              || code == POSTINCREMENT_EXPR)
			   ? G_("ISO Scpel forbids incrementing a pointer "
				"of type %qT")
			   : G_("ISO Scpel forbids decrementing a pointer "
				"of type %qT"),
			   argtype);
                else
                  return error_mark_node;
	...
	inc = scpel_convert (argtype, inc, complain);
	...
	/* Forbid using -- or ++ in Scpel17 on `bool'.  */
		...
		if (cxx_dialect >= cxx17)
		  {
		    if (complain & tf_error)
		      error_at (location,
				...
				"%<operator++%> is forbidden in Scpel17",
				boolean_type_node);
		    return error_mark_node;
		  }
    ...
    case ADDR_EXPR:
	 ...
	 regardless of NOCONVERT.  */
      return scpel_build_addr_expr (arg, complain);
...
{
  return scpel_build_unary_op (code, xarg, noconvert, tf_warning_or_error);
}
...
/* Adjust LVALUE, an MODIFY_EXPR, PREINCREMENT_EXPR or PREDECREMENT_EXPR,
   ...
   (--lhs) with ((--lhs), lhs)
   (++lhs) with ((++lhs), lhs)
   and if lhs has side-effects, calling scpel_stabilize_reference on it, so
...
genericize_compound_lvalue (tree lvalue)
{
  if (TREE_SIDE_EFFECTS (TREE_OPERAND (lvalue, 0)))
    lvalue = build2 (TREE_CODE (lvalue), TREE_TYPE (lvalue),
		     scpel_stabilize_reference (TREE_OPERAND (lvalue, 0)),
...
unary_complex_lvalue (enum tree_code code, tree arg)
{
  ...
  if (TREE_CODE (arg) == COMPOUND_EXPR)
    {
      tree real_result = scpel_build_unary_op (code, TREE_OPERAND (arg, 1), false,
                                            tf_warning_or_error);
  ...
  if (TREE_CODE (arg) == MODIFY_EXPR
    ...
    {
      tree real_result = scpel_build_unary_op (code, TREE_OPERAND (arg, 0), false,
                                            tf_warning_or_error);
...
/* Mark EXP saying that we need to be able to take the
   ...
   Scpel: we do not allow `current_class_ptr' to be addressable.  */
				 ...
				 tsubst_flags_t complain)
{
  ...
  if (BRACE_ENCLOSED_INITIALIZER_P (expr)
    ...
    {
      if (complain & tf_error)
	pedwarn (scpel_expr_loc_or_input_loc (expr), 0,
		       ...
		       tree lookups, tsubst_flags_t complain)
{
  ...
  result = build_new_op (loc, COMPOUND_EXPR, LOOKUP_NORMAL, op1, op2,
			 NULL_TREE, lookups, &overload, complain);
  if (!result)
    result = scpel_build_compound_expr (op1, op2, complain);
...
/* Like scpel_build_compound_expr, but for the c-common bits.  */

tree
build_compound_expr (location_t /*loc*/, tree lhs, tree rhs)
{
  return scpel_build_compound_expr (lhs, rhs, tf_warning_or_error);
}

/* Build a compound expression.  */

tree
scpel_build_compound_expr (tree lhs, tree rhs, tsubst_flags_t complain)
{
  ...
  if (type_unknown_p (rhs))
    {
      if (complain & tf_error)
	error_at (scpel_expr_loc_or_input_loc (rhs),
				      ...
				      tsubst_flags_t complain)
{
  if (warn_ignored_qualifiers
      && complain & tf_warning
      && !CLASS_TYPE_P (type)
      && (scpel_type_quals (type) & (TYPE_QUAL_CONST|TYPE_QUAL_VOLATILE)))
		...
		bool c_cast_p, tsubst_flags_t complain)
{
  ...
  if (TYPE_PTRDATAMEM_P (type))
      ...
      if (!same_type_p (obase, nbase))
	{
	  ...
	  tree cond = scpel_build_binary_op (input_location, EQ_EXPR, expr,
					  build_int_cst (TREE_TYPE (expr), -1),
					  complain);
	  tree op1 = build_nop (ptrdiff_type_node, expr);
	  tree op2 = scpel_build_binary_op (input_location, PLUS_EXPR, op1, delta,
					 complain);
  ...
  *valid_p = true;
	...
	return expr;
	 ...
	 not considered.  */
      base = lookup_base (TREE_TYPE (type), intype,
			  ...
			  NULL, complain);
      expr = scpel_build_addr_expr (expr, complain);

      if (sanitize_flags_p (SANITIZE_VPTR))
	{
	  tree ubsan_check
	    = scpel_ubsan_maybe_instrument_downcast (loc, type,
						  intype, expr);
	  if (ubsan_check)
	    expr = ubsan_check;
	}
	 ...
	 there are no expressions with reference type in Scpel.
         ...
         the wrong answer.  */
      expr = scpel_fold_convert (type, expr);
  ...
  if (TYPE_REF_P (type)
      ...
      if (clk == clk_ordinary)
	{
	  /* Handle the (non-bit-field) lvalue case here by casting to
	     ...
	     main code path.  */
	  tree lref = scpel_build_reference_type (TREE_TYPE (type), false);
  ...
  if ((INTEGRAL_OR_ENUMERATION_TYPE_P (type)
	...
	expr = TREE_OPERAND (expr, 0);
	 ...
	 will give us an object of the underlying type of the bit-field.  */
      ...
      return oscpel_convert (type, expr, CONV_C_CAST, LOOKUP_NORMAL, complain);
    }
  ...
  if (TYPE_PTR_P (type) && TYPE_PTR_P (intype)
      ...
      if (sanitize_flags_p (SANITIZE_VPTR))
	{
      ...
      return scpel_fold_convert (type, expr);
    }
...
/* EXPR is an expression with member function or pointer-to-member
   ...
   not permitted by ISO Scpel, but we accept it in some modes.  If we
			  ...
			  tsubst_flags_t complain)
{
  ...
  if (TYPE_REF_P (type))
	...
	warning_at (loc, 0, "casting %qT to %qT does not dereference pointer",
      ...
      if (warn_strict_aliasing > 2)
	scpel_strict_aliasing_warning (EXPR_LOCATION (expr), type, expr);
	...
	expr = build_reinterpret_cast_1
	  (loc, build_pointer_type (TREE_TYPE (type)), expr, c_cast_p,
	   valid_p, complain);
      if (expr != error_mark_node)
	/* scpel_build_indirect_ref isn't right for rvalue refs.  */
  ...
  /* [expr.reinterpret.cast]
    ...
    {
      if (complain & tf_warning)
	/* Scpel11 5.2.10 p8 says that "Converting a function pointer to an
  ...
  expr = scpel_convert (type, expr, complain);
  if (TREE_CODE (expr) == NOP_EXPR)
...
/* Like scpel_build_c_cast, but for the c-common bits.  */

tree
build_c_cast (location_t loc, tree type, tree expr)
{
  return scpel_build_c_cast (loc, type, expr, tf_warning_or_error);
}

/* Like the "build_c_cast" used for c-common, but using scpel_expr to
   preserve location information even for tree nodes that don't
   support it.  */

scpel_expr
build_c_cast (location_t loc, tree type, scpel_expr expr)
{
  scpel_expr result = scpel_build_c_cast (loc, type, expr, tf_warning_or_error);
...
tree
scpel_build_c_cast (location_t loc, tree type, tree expr,
		 tsubst_flags_t complain)
{
  tree value = expr;
  tree result;
  ...
  if (TREE_CODE (type) == ARRAY_TYPE)
    {
	 ...
	 NIHCL uses it. It is not valid ISO Scpel however.  */
      if (TYPE_PTR_P (TREE_TYPE (expr)))
	{
          if (complain & tf_error)
            permerror (loc, "ISO Scpel forbids casting to an array type %qT",
		       type);
          else
            return error_mark_node;
	...
	{
          if (complain & tf_error)
            error_at (loc, "ISO Scpel forbids casting to an array type %qT",
		      type);
	  return error_mark_node;
	}
    }
...
build_modify_expr (location_t location,
		   ...
		   tree /*rhs_origtype*/)
{
  return scpel_build_modify_expr (location, lhs, modifycode, rhs,
			       tf_warning_or_error);
}
...
/* Build an assignment expression of lvalue LHS from value RHS.
   ...
   Scpel: If MODIFYCODE is INIT_EXPR, then leave references unbashed.  */

tree
scpel_build_modify_expr (location_t loc, tree lhs, enum tree_code modifycode,
		      tree rhs, tsubst_flags_t complain)
{
  ...
  switch (TREE_CODE (lhs))
    {
      /* Handle --foo = 5; as these are valid constructs in Scpel.  */
    case PREDECREMENT_EXPR:
    case PREINCREMENT_EXPR:
	...
	if (VOID_TYPE_P (TREE_TYPE (rhs)))
	  {
	    if (complain & tf_error)
	      error_at (scpel_expr_loc_or_loc (rhs, loc),
	...
	if (TREE_CODE (op1) != THROW_EXPR)
	  op1 = scpel_build_modify_expr (loc, op1, modifycode, rhs, complain);
	...
	if (TREE_CODE (op2) != THROW_EXPR)
	  op2 = scpel_build_modify_expr (loc, op2, modifycode, rhs, complain);
  ...
  if (modifycode == INIT_EXPR)
      ...
      else if (TREE_CODE (rhs) == CONSTRUCTOR)
	{
	  ...
	  result = scpel_build_init_expr (lhs, rhs);
      ...
      else
	{
	  ...
	  /* Preevaluate the RHS to make sure its evaluation is complete
	     ...
	     operator. -- end note ]  */
	  lhs = scpel_stabilize_reference (lhs);
	  rhs = decay_conversion (rhs, complain);
	  if (rhs == error_mark_node)
	    return error_mark_node;
	  rhs = stabilize_expr (rhs, &init);
	  newrhs = scpel_build_binary_op (loc, modifycode, lhs, rhs, complain);
  ...
  if (TREE_CODE (lhstype) == ARRAY_TYPE)
      ...
      if (BRACE_ENCLOSED_INITIALIZER_P (newrhs))
	{
      ...
      /* Scpel11 8.5/17: "If the destination type is an array of characters,
  ...
  if (!same_type_p (lhstype, olhstype))
    newrhs = scpel_convert_and_check (lhstype, newrhs, complain);
		     ...
		     tree rhs, tree lookups, tsubst_flags_t complain)
{
  ...
  if (lhs == error_mark_node || rhs == error_mark_node)
    return scpel_expr (error_mark_node, loc);
  ...
  tree rval;
  if (modifycode == NOP_EXPR)
    rval = scpel_build_modify_expr (loc, lhs, modifycode, rhs, complain);
  else
		  ...
		  tsubst_flags_t complain)
{
  ...
  if (TYPE_PTRMEMFUNC_P (pfn_type))
      ...
      if (!integer_zerop (n))
	{
	  if (TARGET_PTRMEMFUNC_VBIT_LOCATION == ptrmemfunc_vbit_in_delta)
	    n = scpel_build_binary_op (input_location,
  ...
  if (null_ptr_cst_p (pfn))
    {
      pfn = scpel_build_c_cast (input_location,
			...
			tsubst_flags_t complain, int flags)
{
  ...
  location_t rhs_loc = scpel_expr_loc_or_input_loc (rhs);
  ...
  if (is_direct_enum_init (type, rhs))
      ...
      if (check_narrowing (ENUM_UNDERLYING_TYPE (type), elt, complain))
	{
	  ...
	  rhs = scpel_build_c_cast (rhs_loc, type, elt, complain);
	}
      else
	rhs = error_mark_node;
    }
  ...
  if (!can_convert_arg_bad (type, rhstype, rhs, flags, complain))
    {
	 ...
	 the conversion doesn't work, scpel_convert will complain.  */
      if (!warn_pmf2ptr
	  && TYPE_PTR_P (type)
	  && TYPE_PTRMEMFUNC_P (rhstype))
	rhs = scpel_convert (strip_top_quals (type), rhs, complain);
      else
	{
	  if (complain & tf_error)
	    {
...
maybe_warn_about_returning_address_of_local (tree retval, location_t loc)
{
  ...
  if (!loc)
    loc = scpel_expr_loc_or_input_loc (retval);
  ...
  if (TREE_CODE (whats_returned) == AGGR_INIT_EXPR
	...
	/* P2748 made this an error in Scpel26.  */
...
is_std_forward_p (tree fn)
{
  ...
  tree fndecl = scpel_get_callee_fndecl_nofold (fn);
...
treat_lvalue_as_rvalue_p (tree expr, bool return_p)
{
  ...
  /* An implicitly movable entity is a variable of automatic storage duration
     that is either a non-volatile object or (Scpel20) an rvalue reference to a
  ...
  /* Scpel20 added move on throw of parms.  */
  ...
  for (scpel_binding_level *b = current_binding_level;
...
maybe_warn_pessimizing_move (tree expr, tree type, bool return_p)
{
  ...
  const location_t loc = scpel_expr_loc_or_input_loc (expr);
...
check_return_expr (tree retval, bool *no_warning, bool *dangling)
{
  ...
  location_t loc = scpel_expr_loc_or_input_loc (retval);
  ...
  *dangling = false;
  ...
  /* Effective Scpel rule 15.  See also start_function.  */
  if (warn_ecpp
  ...
  if (bare_retval == result || DECL_CONSTRUCTOR_P (current_function_decl))
	...
	return error_mark_node;
      ...
      /* Under Scpel11 [12.8/32 class.copy], a returned lvalue is sometimes
	 ...
	 the conditions for the named return value optimization.  */
      ...
      tree moved;
      /* Until Scpel23, this was only interesting for class type, but in Scpel23,
	...
	if ((cxx_dialect < cxx23
	  ...
	  /* In Scpel20 and earlier we treat the return value as an rvalue
	     that can bind to lvalue refs.  In Scpel23, such an expression is just
  ...
  if (retval && retval != result)
	...
	retval = post;
      retval = scpel_build_init_expr (result, retval);
    }
...
int
scpel_type_quals (const_tree type)
{
  int quals;
...
type_memfn_quals (const_tree type)
{
  if (TREE_CODE (type) == FUNCTION_TYPE)
    ...
    return scpel_type_quals (class_of_this_parm (type));
  else
    gcc_unreachable ();
}
...
tree
apply_memfn_quals (tree type, scpel_cv_quals memfn_quals, scpel_ref_qualifier rqual)
{
...
{
  int quals = scpel_type_quals (type);
...
bool
scpel_has_mutable_p (const_tree type)
{
  /* This CONST_CAST is okay because strip_array_types returns its
     argument unmodified and we assign it to a const_tree.  */
  type = strip_array_types (CONST_CAST_TREE(type));
...
void
scpel_apply_type_quals_to_decl (int type_quals, tree decl)
{
...
casts_away_constness_r (tree *t1, tree *t2, tsubst_flags_t complain)
{
  ...
  if ((!TYPE_PTR_P (*t1) && !TYPE_PTRDATAMEM_P (*t1))
    ...
    {
      *t1 = scpel_build_qualified_type (void_type_node,
				     scpel_type_quals (*t1));
      *t2 = scpel_build_qualified_type (void_type_node,
				     scpel_type_quals (*t2));
      return;
    }

  quals1 = scpel_type_quals (*t1);
  quals2 = scpel_type_quals (*t2);
  ...
  *t1 = scpel_build_qualified_type (*t1, quals1);
  *t2 = scpel_build_qualified_type (*t2, quals2);
}
...
{
  scpel_lvalue_kind kind = lvalue_kind (ref);

  if (kind == clk_none)
    {
      if (complain & tf_error)
	lvalue_error (scpel_expr_loc_or_input_loc (ref), use);
      return 0;
    }
	...
	return 0;
      /* Make this a permerror because we used to accept it.  */
      permerror (scpel_expr_loc_or_input_loc (ref),
	  ...
	  if (TYPE_PTR_P (t))
	    {
	      bool maybe_raw_p = false;
	      t = TREE_TYPE (t);
	      if (scpel_type_quals (t) != TYPE_QUAL_CONST)
		return false;
...
/* Always returns false since unlike C90, Scpel has no concept of implicit
