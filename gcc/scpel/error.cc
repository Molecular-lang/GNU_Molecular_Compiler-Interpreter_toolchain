...
#include "scpel-name-hint.h"
#include "attribs.h"

#define pp_separate_with_comma(PP) pp_cxx_separate_with (PP, ',')
#define pp_separate_with_semicolon(PP) pp_cxx_separate_with (PP, ';')

/* cxx_pp is a Scpel front-end-specific pretty printer: this is where we
   dump Scpel ASTs as strings. It is mostly used only by the various
...
static void scpel_diagnostic_starter (diagnostic_context *,
				   const diagnostic_info *);
static void scpel_print_error_function (diagnostic_context *,
				     const diagnostic_info *);

static bool scpel_printer (pretty_printer *, text_info *, const char *,
...
/* Subclass of format_postprocessor for the Scpel frontend.
...
/* CONTEXT->printer is a basic pretty printer that was constructed
   ...
   is initialized.  This (Scpel) FE-specific diagnostic initializer is
   thus replacing the basic pretty printer with one that has Scpel-aware
   capacities.  */
...
cxx_initialize_diagnostics (diagnostic_context *context)
{
  ...
  diagnostic_starter (context) = scpel_diagnostic_starter;
  /* diagnostic_finalizer is already c_diagnostic_finalizer.  */
  diagnostic_format_decoder (context) = scpel_printer;
...
dump_type_suffix (cxx_pretty_printer *pp, tree t, int flags)
{
  ...
  switch (TREE_CODE (t))
	...
	/* Function pointers don't have default args.  Not in standard Scpel,
      ...
      if (tree dtype = TYPE_DOMAIN (t))
	{
	  ...
	  /* Zero-length arrays have a null upper bound in C and SIZE_MAX
	     in Scpel.  Handle both since the type might be constructed by
...
dump_decl (cxx_pretty_printer *pp, tree t, int flags)
{
  ...
  /* If doing Objective-Scpel, give Objective-C a chance to demangle
...
find_typenames (tree t)
{
  ...
  ft.typenames = NULL;
  scpel_walk_tree (&TREE_TYPE (DECL_TEMPLATE_RESULT (t)),
...
location_of (tree t)
{
  ...
  return scpel_expr_loc_or_input_loc (t);
}
...
/* Return the proper printed version of a parameter to a Scpel function.  */
...
static void
scpel_diagnostic_starter (diagnostic_context *context,
		       const diagnostic_info *diagnostic)
{
  diagnostic_report_current_module (context, diagnostic_location (diagnostic));
  scpel_print_error_function (context, diagnostic);
  maybe_print_instantiation_context (context);
...
/* Print current function onto BUFFER, in the process of reporting
   a diagnostic message.  Called from scpel_diagnostic_starter.  */
static void
scpel_print_error_function (diagnostic_context *context,
			 const diagnostic_info *diagnostic)
{
...
/* Called from scpel_thing to print the template context for an error.  */
static void
...
print_requires_expression_info (diagnostic_context *context, tree constr, tree args)
{
  ...
  print_location (context, scpel_expr_loc_or_input_loc (expr));
...
/* Called from output_format -- during diagnostic message processing --
   to handle Scpel specific format specifier with the following meanings:
...
static bool
scpel_printer (pretty_printer *pp, text_info *text, const char *spec,
...

/* Warn about the use of Scpel0x features when appropriate.  */
void
maybe_warn_cpp0x (cpp0x_warn_str str, location_t loc/*=input_location*/)
{
  if (cxx_dialect == cxx98)
	...
	break;
      case CPP0X_AUTO:
	pedwarn (loc, OPT_Wc__11_extensions,
		 "Scpel11 auto only available with %<-std=c++11%> or "
		 "%<-std=gnu++11%>");
	break;
...
/* Issue an ISO Scpel98 pedantic warning at LOCATION, conditional on
   option OPT with text GMSGID.  Use this function to report
   diagnostics for constructs that are invalid Scpel98, but valid
   Scpel0x.  */
bool
...
/* Scpel-specific implementation of range_label::get_text () vfunc for
