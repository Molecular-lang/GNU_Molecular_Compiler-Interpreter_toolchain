...
/* Emit the definition of a Scpel multiple inheritance or covariant
...
do_build_copy_constructor (tree fndecl)
{
  ...
  if (trivial)
	...
	{
	  tree t = scpel_build_init_expr (current_class_ref, parm);
	  finish_expr_stmt (t);
	}
      else
	{
	  ...
	  tree t = scpel_build_init_expr (lhs, rhs);
	  finish_expr_stmt (t);
	}
    }
  else
	   ...
	   BINFO_BASE_ITERATE (binfo, i, base_binfo); i++)
	{
      ...
      if (!inh)
	{
	  int cvquals = scpel_type_quals (TREE_TYPE (parm));
	  ...
	  for (tree fields = TYPE_FIELDS (current_class_type);
	      ...
	      if (!TYPE_REF_P (expr_type))
		{
		  ...
		  quals |= scpel_type_quals (expr_type);
		  expr_type = scpel_build_qualified_type (expr_type, quals);
		}
...
do_build_copy_assign (tree fndecl)
{
  ...
  tree const class_ref = DECL_XOBJ_MEMBER_FUNCTION_P (fndecl)
    ? scpel_build_fold_indirect_ref (DECL_ARGUMENTS (fndecl)) : current_class_ref;
  ...
  if (trivial
	   ...
	   fields = DECL_CHAIN (fields))
	{
	  ...
	  if (DECL_NAME (field))
	    init = scpel_build_modify_expr (input_location, comp, NOP_EXPR, init,
					 tf_warning_or_error);
	  else
...
/* Scpel20 <compare> comparison category types.  */
...
build_comparison_op (tree fndecl, bool defining, tsubst_flags_t complain)
{
  ...
  if (is_this_parameter (lhs))
    lhs = scpel_build_fold_indirect_ref (lhs);
  else
      ...
      for (unsigned i = 0; i < comps.length(); ++i)
	{
		  ...
		  for (tree loop_index = loop_indexes; loop_index;
		       loop_index = TREE_CHAIN (loop_index))
		    {
		      ...
		      finish_for_cond (build2 (LE_EXPR, boolean_type_node, idx,
					       ...
					       false);
		      finish_for_expr (scpel_build_unary_op (PREINCREMENT_EXPR,
	  ...
	  if (code == EQ_EXPR)
	      ...
	      if (defining)
		{
		  ...
		  pushdecl (var);
		  scpel_finish_decl (var, comp, false, NULL_TREE, flags);
		  comp = retval = var;
		}
...
{
  tree argtype = scpel_build_qualified_type (type, quals);
  return scpel_build_reference_type (argtype, rvalue);
}
...
build_stub_object (tree reftype)
{
  if (!TYPE_REF_P (reftype))
    reftype = scpel_build_reference_type (reftype, /*rval*/true);
		 ...
		 tsubst_flags_t complain)
{
  ...
  ob = build_stub_object (scpel_build_reference_type (type, false));
  releasing_vec args;
  if (argtype)
    {
...
constructible_expr (tree to, tree from)
{
  ...
  if (CLASS_TYPE_P (to))
      ...
      if (len > 1)
	{
	  ...
	  /* In Scpel20 this is well-formed:
	       using T = int[2];
	       T t(1, 2);
...
is_xible_helper (enum tree_code code, tree to, tree from, bool trivial)
{
  ...
  if (code == MODIFY_EXPR)
    ...
    return error_mark_node; // only 0- and 1-argument ctors can be trivial
			    // before Scpel20 aggregate paren init
...
is_trivially_xible (enum tree_code code, tree to, tree from)
{
  ...
  if (expr == NULL_TREE || expr == error_mark_node)
    return false;
  tree nt = scpel_walk_tree_without_duplicates (&expr, check_nontriv, NULL);
  return !nt;
}
 ...
 again:
  for (tree field = fields; field; field = DECL_CHAIN (field))
      ...
      else if (sfk == sfk_constructor || sfk == sfk_inheriting_constructor)
	{
	  ...
	  /* Before Scpel20, for an implicitly-defined default constructor to
	...
	{
	  int mem_quals = scpel_type_quals (mem_type) | quals;
	  if (DECL_MUTABLE_P (field))
			 ...
			 tree *inheriting_ctor, tree inherited_parms)
{
  ...
  if (deleted_p)
    {
	 ...
	 In Scpel20, only lambda-expressions with lambda-captures have those
	 deleted.  */
  ...
  /* The TYPE_HAS_COMPLEX_* flags tell us about constraints from base
     ...
     early in Scpel0x.  The same considerations apply in Scpel98/03, but
		       ...
		       tree inherited_parms)
{
  ...
  switch (kind)
	...
	{
	  if (const_p)
	    rhs_parm_type = scpel_build_qualified_type (type, TYPE_QUAL_CONST);
	  else
	    rhs_parm_type = type;
	  bool move_p = (kind == sfk_move_assignment
			 || kind == sfk_move_constructor);
	  rhs_parm_type = scpel_build_reference_type (rhs_parm_type, move_p);
  ...
  /* Create the function.  */
  tree this_type = scpel_build_qualified_type (type, this_quals);
  ...
  if (raises)
	...
	{
	  /* Can happen, e.g., in Scpel98 mode for an ill-formed non-static data
	     member initializer (c++/89914).  Also, in Scpel98, we might have
  ...
  if (rhs_parm_type)
    {
	 ...
	 name.  */
      tree decl = scpel_build_parm_decl (fn, NULL_TREE, rhs_parm_type);
	...
	{
	  *p = scpel_build_parm_decl (fn, NULL_TREE, TREE_VALUE (parm));
