...
static bool
scpel_ubsan_instrument_vptr_p (tree type)
{
...
/* Helper function for
   scpel_ubsan_maybe_instrument_{member_{call,access},downcast}.
   Instrument vptr access.  */

static tree
scpel_ubsan_instrument_vptr (location_t loc, tree op, tree type, bool is_addr,
...
static tree
scpel_ubsan_maybe_instrument_vptr (location_t loc, tree op, tree type,
				bool is_addr, enum ubsan_null_ckind ckind)
{
  if (!scpel_ubsan_instrument_vptr_p (type))
    return NULL_TREE;
  return scpel_ubsan_instrument_vptr (loc, op, type, is_addr, ckind);
}

/* Instrument a member call (but not constructor call) if needed.  */

void
scpel_ubsan_maybe_instrument_member_call (tree stmt)
{
  ...
  op = scpel_ubsan_maybe_instrument_vptr (EXPR_LOCATION (stmt), op,
				       TREE_TYPE (TREE_TYPE (op)),
				       true, UBSAN_MEMBER_CALL);
  if (!op)
    /* No change.  */;
  else if (fn && TREE_CODE (fn) == OBJ_TYPE_REF)
    *opp = scpel_build_compound_expr (op, *opp, tf_none);
  else
    *opp = op;
}

/* Data passed to scpel_ubsan_check_member_access_r.  */

struct scpel_ubsan_check_member_access_data
{
  hash_set<tree> *pset;
  bool is_addr;
};

static tree scpel_ubsan_check_member_access_r (tree *, int *, void *);

/* Instrument a member access.  */

static bool
scpel_ubsan_maybe_instrument_member_access
     (tree stmt, scpel_ubsan_check_member_access_data *ucmd)
{
  if (DECL_ARTIFICIAL (TREE_OPERAND (stmt, 1)))
    return false;

  tree base = TREE_OPERAND (stmt, 0);
  if (!scpel_ubsan_instrument_vptr_p (TREE_TYPE (base)))
    return false;

  scpel_walk_tree (&base, scpel_ubsan_check_member_access_r, ucmd, ucmd->pset);

  base = scpel_ubsan_instrument_vptr (EXPR_LOCATION (stmt), base,
...
/* Attempt to instrument member accesses inside of the function.
   scpel_ubsan_maybe_instrument_member_access should be called on COMPONENT_REFs
   ...
   scpel_ubsan_maybe_instrument_member_access on *w.z COMPONENT_REF, but
   not on *x.y.  */

static tree
scpel_ubsan_check_member_access_r (tree *stmt_p, int *walk_subtrees, void *data)
{
  tree stmt = *stmt_p, t;
  scpel_ubsan_check_member_access_data *ucmd
    = (scpel_ubsan_check_member_access_data *) data;
  switch (TREE_CODE (stmt))
      ...
      if (handled_component_p (t))
	{
	  *walk_subtrees = 0;
	  ucmd->is_addr = true;
	  scpel_walk_tree (&t, scpel_ubsan_check_member_access_r,
			data, ucmd->pset);
	  ucmd->is_addr = false;
	}
      break;
      ...
      if (TREE_CODE (t) == ADDR_EXPR)
	{
	  *walk_subtrees = 0;
	  t = TREE_OPERAND (t, 0);
	  scpel_walk_tree (&t, scpel_ubsan_check_member_access_r, data, ucmd->pset);
	}
      break;
    case COMPONENT_REF:
      if (!ucmd->is_addr && scpel_ubsan_maybe_instrument_member_access (stmt, ucmd))
	{
	  *walk_subtrees = 0;
	  break;
	}
      ...
      if (ucmd->is_addr && handled_component_p (stmt))
	{
	  ...
	  *walk_subtrees = 0;
	  ...
	  for (i = 0; i < len; i++)
	    {
	      scpel_walk_tree (&TREE_OPERAND (stmt, i),
			    scpel_ubsan_check_member_access_r, data, ucmd->pset);
	      ucmd->is_addr = false;
	    }
	  ucmd->is_addr = true;
	}
...
void
scpel_ubsan_instrument_member_accesses (tree *t_p)
{
  if (scpel_ubsan_instrument_vptr_p (NULL_TREE))
    {
      hash_set<tree> pset;
      scpel_ubsan_check_member_access_data ucmd;
      ucmd.pset = &pset;
      ucmd.is_addr = false;
      scpel_walk_tree (t_p, scpel_ubsan_check_member_access_r, &ucmd, &pset);
    }
}
...
tree
scpel_ubsan_maybe_instrument_downcast (location_t loc, tree type,
				    tree intype, tree op)
{
  ...
  return scpel_ubsan_maybe_instrument_vptr (loc, op, TREE_TYPE (type), true,
					 TYPE_PTR_P (type)
...
tree
scpel_ubsan_maybe_instrument_cast_to_vbase (location_t loc, tree type, tree op)
{
  return scpel_ubsan_maybe_instrument_vptr (loc, op, type, true,
					 UBSAN_CAST_TO_VBASE);
}
...
static tree
scpel_ubsan_dfs_initialize_vtbl_ptrs (tree binfo, void *data)
{
  ...
  if (!BINFO_PRIMARY_P (binfo))
    {
      ...
      tree vtbl_ptr
	= build_vfield_ref (scpel_build_fold_indirect_ref (base_ptr),
			    TREE_TYPE (binfo));
      gcc_assert (vtbl_ptr != error_mark_node);

      /* Assign NULL to the vptr.  */
      tree vtbl = build_zero_cst (TREE_TYPE (vtbl_ptr));
      tree stmt = scpel_build_modify_expr (input_location, vtbl_ptr, NOP_EXPR,
...
void
scpel_ubsan_maybe_initialize_vtbl_ptrs (tree addr)
{
  if (!scpel_ubsan_instrument_vptr_p (NULL_TREE))
    return;
  ...
  /* Walk through the hierarchy, initializing the vptr in each base
     class to NULL.  */
  dfs_walk_once (TYPE_BINFO (type), scpel_ubsan_dfs_initialize_vtbl_ptrs,
		 NULL, list);
