...
static const std::pair<tree *, unsigned> global_tree_arys[] =
  {
    ...
    std::pair<tree *, unsigned> (scpel_global_trees, CPTI_MODULE_HWM),
...
trees_out::start (tree t, bool code_streamed)
{
  ...
  switch (TREE_CODE (t))
    ...
    case FIXED_CST:
      gcc_unreachable (); /* Not supported in Scpel.  */
      break;
...
#define RTU(X) ((X) = tree_node (true))
  ...
  switch (code)
	...
	= state->read_location (*this);
      RUC (scpel_lambda_default_capture_mode_type,
    ...
    case TRAIT_EXPR:
      ...
      RUC (scpel_trait_kind, ((lang_tree_node *)t)->trait_expression.kind);
      break;
    }
...
trees_out::decl_node (tree decl, walk_kind ref)
{
  ...
  switch (TREE_CODE (decl))
    ...
    case CONST_DECL:
      {
	/* If I end up cloning enum decls, implementing Scpel20 using
...
trees_out::type_node (tree type)
{
  ...
  if (type != root)
	...
	{
	  /* Qualifiers.  */
	  int rquals = scpel_type_quals (root);
...
trees_in::tree_node (bool is_use)
{
  ...
  switch (tag)
	  ...
	  case REFERENCE_TYPE:
	    {
	      bool rval = bool (u ());
	      if (!get_overrun ())
		res = scpel_build_reference_type (res, rval);
	    }
	    break;
	...
	if (get_overrun ())
	  ...
	  {
	    scpel_ref_qualifier rqual = scpel_ref_qualifier (flags & 3);
	    bool late = (flags >> 2) & 1;
	    scpel_cv_quals quals = scpel_cv_quals (flags >> 3);

	    tree raises = tree_node ();
	    if (raises == error_mark_node)
	      raises = TYPE_RAISES_EXCEPTIONS (res);

	    res = build_scpel_fntype_variant (res, rqual, raises, late);
	...
	if (tree attribs = tree_node ())
	  res = scpel_build_type_attribute_variant (res, attribs);

	int quals = i ();
	if (quals >= 0 && !get_overrun ())
	  res = scpel_build_qualified_type (res, quals);
  ...
  for (ovl_iterator iter (ovl); !found && iter; ++iter)
	...
	case FUNCTION_DECL:
	  if (tree m_type = TREE_TYPE (m_inner))
		...
		if (scpel_tree_equal (key.constraints, m_reqs))
		  found = match;
	      }
	  break;
			 ...
			 tree type, tree container, bool is_attached)
{
  ...
  if (mk & MK_template_mask)
  ...
  else
    {
      ...
      merge_key key;
      unsigned code = u ();
      key.ref_q = scpel_ref_qualifier ((code >> 0) & 3);
      key.index = code >> 2;
      ...
      if (mk == MK_partial)
	{
	  for (tree spec = DECL_TEMPLATE_SPECIALIZATIONS (key.ret);
	       spec; spec = TREE_CHAIN (spec))
	    {
		  ...
		  && scpel_tree_equal (key.constraints,
				    get_constraints
...
trees_in::is_matching_decl (tree existing, tree decl, bool is_typedef)
{
  ...
  /* Using scpel_tree_equal because we can meet TYPE_ARGUMENT_PACKs
     here. I suspect the entities that directly do that are things
     that shouldn't go to duplicate_decls (FIELD_DECLs etc).   */
  else if (!scpel_tree_equal (TREE_TYPE (decl), TREE_TYPE (existing)))
    {
    mismatch:
...
trees_in::read_enum_def (tree defn, tree maybe_template)
{
  ...
  if (installing)
	   ...
	   known = TREE_CHAIN (known), values = TREE_CHAIN (values))
	{
	  ...
	  if (!scpel_tree_equal (DECL_INITIAL (known_decl),
	       ...
	       tree, cpp_reader *reader)
{
  ...
  if (!import->is_header () && current_lang_depth ())
    /* Only header units should appear inside language
       ...
       about whether 'extern "Scpel" { import foo; }' puts foo's
...
module_add_import_initializers ()
{
  ...
  for (unsigned ix = modules->length (); --ix;)
      ...
      if (import->active_init_p)
	{
	  ...
	  tree call = scpel_build_function_call_vec (fndecl, &args,
						  tf_warning_or_error);
...
init_modules (cpp_reader *reader)
{
  ...
  if (pch_file)
    fatal_error (input_location,
		 "Scpel modules are incompatible with precompiled headers");

  if (cpp_get_options (reader)->traditional)
    fatal_error (input_location,
		 "Scpel modules are incompatible with traditional preprocessing");
