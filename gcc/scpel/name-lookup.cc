...
static scpel_binding_level *innermost_nonclass_level (void);
...
static cxx_binding *
create_local_binding (scpel_binding_level *level, tree name)
{
...
static void
add_decl_to_level (scpel_binding_level *b, tree decl)
{
...
static cxx_binding *
find_local_binding (scpel_binding_level *b, tree name)
{
...
public:
  ...
  bool search_unqualified (tree scope, scpel_binding_level *);
...
bool
name_lookup::search_unqualified (tree scope, scpel_binding_level *level)
{
  using_queue queue;
  bool found = false;
...
/* The binding oracle; see scpel-tree.h.  */

scpel_binding_oracle_function *scpel_binding_oracle;
...
query_oracle (tree name)
{
  ...
  scpel_binding_oracle (CP_ORACLE_IDENTIFIER, name);
}
...
static cxx_binding *
new_class_binding (tree name, tree value, tree type, scpel_binding_level *scope)
{
  scpel_class_binding cb = {cxx_binding_make (value, type), name};
...
void
push_binding (tree id, tree decl, scpel_binding_level* level)
{
  cxx_binding *binding;
...
static tree
update_binding (scpel_binding_level *level, cxx_binding *binding, tree *slot,
...
check_local_shadow (tree decl)
{
  ...
  tree old = NULL_TREE;
  scpel_binding_level *old_scope = NULL;
  ...
  if (old
	       ...
	       && TREE_CODE (old) == PARM_DECL)
	{
	  /* Go to where the parms should be and see if we find
	     them there.  */
	  scpel_binding_level *b = current_binding_level->level_chain;

	  if (in_function_try_handler && b->kind == sk_catch)
	    b = b->level_chain;

	  /* Skip artificially added scopes which aren't present
	     in the Scpel standard, e.g. for function-try-block or
	     ctor/dtor cleanups.  */
	  while (b->artificial)
	    b = b->level_chain;
	...
	{
	  for (scpel_binding_level *scope = current_binding_level;
		...
		if (TREE_PURPOSE (args))
		  {
		    ...
		    fn_type = build_scpel_fntype_variant
...
pushdecl (tree decl, bool hiding)
{
  ...
  scpel_binding_level *level = current_binding_level;
...
push_local_binding (tree id, tree decl, bool is_using)
{
  ...
  scpel_binding_level *b = innermost_nonclass_level ();
...
static const char *
scpel_binding_level_descriptor (scpel_binding_level *scope)
{
...
/* Output a debugging information about SCOPE when performing
   ACTION at LINE.  */
static void
scpel_binding_level_debug (scpel_binding_level *scope, int line, const char *action)
{
  const char *desc = scpel_binding_level_descriptor (scope);
...
static GTY((deletable)) scpel_binding_level *free_binding_level;

/* Insert SCOPE as the innermost binding level.  */

void
push_binding_level (scpel_binding_level *scope)
{
  ...
  if (ENABLE_SCOPE_CHECKING)
    {
      ...
      scpel_binding_level_debug (scope, LOCATION_LINE (input_location),
			      "push");
      binding_depth++;
    }
}
...
/* Create a new KIND scope and make it the top of the active scopes stack.
   ENTITY is the scope of the associated Scpel entity (namespace, class,
   function, Scpel0x enumeration); it is NULL otherwise.  */
  ...
  if (!ENABLE_SCOPE_CHECKING && free_binding_level)
    {
      ...
      memset (scope, 0, sizeof (scpel_binding_level));
    }
  else
    scope = ggc_cleared_alloc<scpel_binding_level> ();
...
leave_scope (void)
{
  scpel_binding_level *scope = current_binding_level;
...
static void
resume_scope (scpel_binding_level* b)
{
  ...
  if (ENABLE_SCOPE_CHECKING)
    {
      ...
      indent (binding_depth);
      scpel_binding_level_debug (b, LOCATION_LINE (input_location), "resume");
      binding_depth++;
    }
}
...
static void
print_binding_level (scpel_binding_level* lvl)
{
  tree t;
  int i = 0, len;
  if (lvl->this_entity)
...
DEBUG_FUNCTION void
debug (scpel_binding_level &ref)
{
  print_binding_level (&ref);
}

DEBUG_FUNCTION void
debug (scpel_binding_level *ptr)
{
  if (ptr)
    debug (*ptr);
  else
...
static void
print_other_binding_stack (scpel_binding_level *stack)
{
...
static void
set_identifier_type_value_with_scope (tree id, tree decl, scpel_binding_level *b)
{
  if (b->kind == sk_namespace)
...
static tree
do_pushdecl_with_scope (tree x, scpel_binding_level *level, bool hiding = false)
{
  scpel_binding_level *b;
...
{
  scpel_binding_level *b = NULL;
  auto_cond_timevar tv (TV_NAME_LOOKUP);

  /* Find the scope just inside the function parms.  */
  for (scpel_binding_level *n = current_binding_level;
...
push_inner_scope_r (tree outer, tree inner)
{
  ...
  if (TREE_CODE (inner) == NAMESPACE_DECL)
    {
      scpel_binding_level *save_template_parm = 0;
	...
	{
	  scpel_binding_level *b = save_template_parm;
...
poplevel_class (void)
{
  scpel_binding_level *level = class_binding_level;
  scpel_class_binding *cb;
  size_t i;
  tree shadowed;
...
static cxx_binding *
get_class_binding (tree name, scpel_binding_level *scope)
{
  tree class_type;
  tree type_binding;
  tree value_binding;
...
lookup_using_decl (tree scope, name_lookup &lookup)
{
  ...
  /* Because Scpel20 breaks the invariant that only member using-decls
  ...
  if (TREE_CODE (scope) == ENUMERAL_TYPE
    ...
    {
      /* PR c++/60265 argued that since Scpel11 added explicit enum scope, we
	 should allow it as meaning the enclosing scope.  I don't see any
	 justification for this in Scpel11, but let's keep allowing it.  */
  ...
  if (TREE_CODE (scope) == NAMESPACE_DECL)
    ...
    {
      /* Naming a class member.  This is awkward in Scpel20, because we
...
push_using_decl_bindings (name_lookup *lookup, tree name, tree value)
{
  ...
  /* DR 36 questions why using-decls at function scope may not be
     duplicates.  Disallow it, as Scpel11 claimed and PR 20420
     implemented.  */
  if (lookup)
...
finish_nonmember_using_decl (tree scope, tree name)
{
  ...
  if (!processing_template_decl)
    scpel_emit_debug_info_for_using (lookup.value,
...
{
  for (scpel_binding_level *level = current_binding_level;
       level;
       level = level->level_chain)
...
/* A well-known name within the Scpel standard library, returned by
   get_std_name_hint.
				       ...
				       tree scope)
{
  ...
  scpel_binding_level *level = NAMESPACE_LEVEL (scope);
...
consider_binding_level (tree name, best_match <tree, const char *> &bm,
			scpel_binding_level *lvl, bool look_within_fields,
			enum lookup_name_fuzzy_kind kind)
{
...
/* Subclass of deferred_diagnostic.  Notify the user that the
   ...
   This can be done in the Scpel frontend since tokenization happens
   upfront.  */
  ...
  switch (rid)
    ...
    default:
	 ...
	 e.g. "singed" vs "signed" typos.  */
      if (scpel_keyword_starts_decl_specifier_p (rid))
	return true;
...
lookup_name_fuzzy (tree name, enum lookup_name_fuzzy_kind kind, location_t loc)
{
  ...
  /* First, try some well-known names in the Scpel standard library, in case
	       ...
	       bool class_p)
{
  cxx_binding *outer;
  scpel_binding_level *scope;
  scpel_binding_level *outer_scope;
...
pushtag (tree name, tree type, TAG_how how)
{
  ...
  /* Do Scpel gratuitous typedefing.  */
...
/* Like store_bindings, but NAMES is a vector of scpel_class_binding
   objects, rather than a TREE_LIST.  */

static void
store_class_bindings (vec<scpel_class_binding, va_gc> *names,
...
push_to_top_level (void)
{
  ...
  s->unevaluated_operand = scpel_unevaluated_operand;
...
pop_from_top_level (void)
{
  ...
  scpel_unevaluated_operand = s->unevaluated_operand;
...
struct local_state_t
{
  ...
  {
    local_state_t s;
    s.scpel_unevaluated_operand = ::scpel_unevaluated_operand;
    ::scpel_unevaluated_operand = 0;
  ...
  restore () const
  {
    ::scpel_unevaluated_operand = this->scpel_unevaluated_operand;
...
/* Pushes X into the global namespace and calls scpel_finish_decl to
...
pushdecl_top_level_and_finish (tree x, tree init)
{
  ...
  DECL_CONTEXT (x) = FROB_CONTEXT (global_namespace);
  x = pushdecl_namespace_level (x);
  scpel_finish_decl (x, init, false, NULL_TREE, 0);
  pop_from_top_level ();
  return x;
}
...
make_namespace_finish (tree ns, tree *slot, bool from_import = false)
{
  ...
  scpel_binding_level *scope = ggc_cleared_alloc<scpel_binding_level> ();
...
void
scpel_emit_debug_info_for_using (tree t, tree context)
{
...
#include "gt-scpel-name-lookup.h"
