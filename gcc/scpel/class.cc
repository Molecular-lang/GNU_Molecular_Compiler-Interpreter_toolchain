		 ...
		 tsubst_flags_t complain)
{
  ...
  target_type = scpel_build_qualified_type
    (target_type, scpel_type_quals (TREE_TYPE (TREE_TYPE (expr))));
  ...
  if (code == PLUS_EXPR && !virtual_access
	 ...
	 interesting to the optimizers anyway.  */
      && !has_empty)
    {
      expr = scpel_build_fold_indirect_ref (expr);
  ...
  if (virtual_access)
      ...
      if (fixed_type_p < 0 && in_base_initializer)
	{
	  ...
	  v_offset = scpel_build_fold_indirect_ref (v_offset);
	}
      else
	{
	  tree t = expr;
	  if (sanitize_flags_p (SANITIZE_VPTR)
	      && fixed_type_p == 0)
	    {
	      t = scpel_ubsan_maybe_instrument_cast_to_vbase (input_location,
							   probe, expr);
	      if (t == NULL_TREE)
		t = expr;
	    }
	  v_offset = build_vfield_ref (scpel_build_fold_indirect_ref (t),
...
build_simple_base_path (tree expr, tree binfo)
{
  ...
  if (d_binfo == NULL_TREE)
    {
	 ...
	 in the back end.  */
      ...
      if (temp)
	expr = scpel_build_fold_indirect_ref (temp);
  ...
  for (field = TYPE_FIELDS (BINFO_TYPE (d_binfo));
	...
	int type_quals = scpel_type_quals (TREE_TYPE (expr));

	expr = build3 (COMPONENT_REF,
		       scpel_build_qualified_type (type, type_quals),
...
convert_to_base_statically (tree expr, tree base)
{
  ...
  if (!SAME_BINFO_TYPE_P (BINFO_TYPE (base), expr_type))
	...
	return build_simple_base_path (expr, base);
	 ...
	 when processing a template because they do not handle Scpel-specific
	 trees.  */
      gcc_assert (!processing_template_decl);
      expr = scpel_build_addr_expr (expr, tf_warning_or_error);
...
build_vfn_ref (tree instance_ptr, tree idx)
{
  ...
  aref = build_vtbl_ref (scpel_build_fold_indirect_ref (instance_ptr), idx);

  /* When using function descriptors, the address of the
     vtable entry is treated as a function pointer.  */
  if (TARGET_VTABLE_USES_DESCRIPTORS)
    aref = build1 (NOP_EXPR, TREE_TYPE (aref),
		   scpel_build_addr_expr (aref, tf_warning_or_error));
...
iobj_parm_corresponds_to (tree iobj_fn, tree xobj_param, tree context)
{
  ...
  scpel_ref_qualifier const iobj_ref_qual = type_memfn_rqual (iobj_fn_type);
  /* We only care about cv qualifiers when determining correspondence.  */
  static constexpr scpel_cv_quals cv_bits = TYPE_QUAL_VOLATILE
				       | TYPE_QUAL_CONST;
  scpel_cv_quals const iobj_cv_quals = type_memfn_quals (iobj_fn_type) & cv_bits;
  ...
  if (!TYPE_REF_P (xobj_param))
  ...
  else
    {
	 ...
	 to [basic.scope.scope.3] we need to ignore its ref qual.  */
      scpel_ref_qualifier const xobj_ref_qual = [&](){
	 ...
	 account.  */
      scpel_cv_quals const xobj_cv_quals
	= scpel_type_quals (TREE_TYPE (xobj_param)) & cv_bits;
...
handle_using_decl (tree using_decl, tree t)
{
  ...
  scpel_emit_debug_info_for_using (decl, t);
  ...
  if (USING_DECL_UNRELATED_P (using_decl))
    {
      /* Scpel20 using enum can import non-inherited enumerators into class
...
find_abi_tags_r (tree *tp, int *walk_subtrees, void *data)
{
  if (TYPE_P (*tp) && *walk_subtrees == 1 && flag_abi_version != 14)
    /* Tell scpel_walk_subtrees to look though typedefs. [PR98481] */
    *walk_subtrees = 2;
...
{
  if (TYPE_P (*tp) && *walk_subtrees == 1 && flag_abi_version != 14)
    /* Tell scpel_walk_subtrees to look though typedefs.  */
    *walk_subtrees = 2;
...
mark_abi_tags (tree t, bool val)
{
  ...
  if (DECL_P (t))
	  ...
	  for (int j = 0; j < TREE_VEC_LENGTH (level); ++j)
	    {
	      ...
	      scpel_walk_tree_without_duplicates (&arg, mark_abi_tags_r, &val);
	    }
	}
	...
	for (tree arg = FUNCTION_FIRST_USER_PARMTYPE (t); arg;
	     arg = TREE_CHAIN (arg))
	  scpel_walk_tree_without_duplicates (&TREE_VALUE (arg),
					   mark_abi_tags_r, &val);
    }
}
...
check_abi_tags (tree t, tree subob, bool just_checking = false)
{
  ...
  scpel_walk_tree_without_duplicates (&subtype, find_abi_tags_r, &data);
...
inherit_targ_abi_tags (tree t)
{
  ...
  for (int i = 0; i < TMPL_ARGS_DEPTH (args); ++i)
      ...
      for (int j = 0; j < TREE_VEC_LENGTH (level); ++j)
	{
	  tree arg = TREE_VEC_ELT (level, j);
	  data.subob = arg;
	  scpel_walk_tree_without_duplicates (&arg, find_abi_tags_r, &data);
	}
    }
		  ...
		  int* no_const_asn_ref)
{
  ...
  /* In Scpel98 an anonymous union cannot contain any fields which would change
  ...
  *access_decls = NULL_TREE;
  ...
  for (tree field = TYPE_FIELDS (t); field; field = DECL_CHAIN (field))
	...
	case VAR_DECL:
	  if (TREE_CODE (t) == UNION_TYPE
	      && cxx_dialect < cxx11)
	    {
		 ...
		 (Scpel98) If a union contains a static data member,
		 ... the program is ill-formed.  */
	      if (cxx_dialect < cxx11)
		error ("in Scpel98 %q+D may not be static because it is "
			    ...
			    cant_have_const_ctor_p, no_const_asn_ref_p))
	{
	 ...
	 non-aggregate in Scpel11, and non-layout-POD always.  */
  ...
  /* Effective Scpel rule 11: if a class has dynamic memory held by pointers,
...
{
  /* Base fields are actually potentially overlapping, but Scpel bases go through
     a different code path based on binfos, and ObjScpel base fields are laid out
		  ...
		  splay_tree offsets, tree *next_field)
{
  ...
  if (!is_empty_class (basetype))
      ...
      if (!BINFO_VIRTUAL_P (binfo) && CLASSTYPE_NEARLY_EMPTY_P (t))
	{
	 ...
	 be a problem anymore.  We need them to handle initialization of Scpel17
	 aggregate bases.  */
		       ...
		       bool need_vtt_parm_p, bool omit_inherited_parms_p)
{
  ...
  if (DECL_HAS_IN_CHARGE_PARM_P (clone))
	...
	= build_method_type_directly (basetype,
				      ...
				      parmtypes);
      TREE_TYPE (clone)
	= scpel_build_type_attribute_variant (TREE_TYPE (clone),
  ...
  for (clone = DECL_CHAIN (decl); clone && DECL_CLONED_FUNCTION_P (clone);
	     ...
	     clone_parms = TREE_CHAIN (clone_parms))
	{
	  ...
	  if (TREE_PURPOSE (decl_parms) && !TREE_PURPOSE (clone_parms))
		...
		type = scpel_build_type_attribute_variant (type, attrs);
...
trivial_default_constructor_is_constexpr (tree t)
{
  ...
  /* A class with a vptr doesn't have a trivial default ctor.
     In Scpel20, a class can have transient uninitialized members, e.g.:
...
{
  /* Until Scpel20, only trivial destruction is constexpr.  */
...
/* Check the validity of the bases and members declared in T.  Add any
   ...
   CLASSTYPE_NON_LAYOUT_POD_T) for T.  This routine works purely at the Scpel
...
check_bases_and_members (tree t)
{
  ...
  /* This is the Scpel98/03 definition of POD; it changed in Scpel0x, but we
  ...
  /* Is this class non-layout-POD because it wasn't an aggregate in Scpel98?  */
...
/* Warn as appropriate about the change in whether we pack into the tail
   padding of FIELD, a base field which has a Scpel14 aggregate type with default
   member initializers.  */
  ...
  for (field = non_static_data_members; field; field = DECL_CHAIN (field))
	  ...
	  && tree_int_cst_lt (TYPE_SIZE (type), DECL_SIZE (field)))
	{
	  ...
	  /* An unnamed bitfield does not normally affect the
	     ...
	     PCC_BITFIELD_TYPE_MATTERS.  But, the Scpel ABI does not
	...
	layout_nonempty_base_or_field (rli, field, NULL_TREE,
	 ...
	 comply with the ABI.  */
	  ...
	  /* The TREE_NO_WARNING flag gets set by Objective-C when
	     ...
	     from the Scpel ABI, and so we do not want a warning
	     here.  */
	...
	warning_at (DECL_SOURCE_LOCATION (field), OPT_Wabi,
	 ...
	 Because Scpel does not have integer types of arbitrary width,
...
finish_struct_1 (tree t)
{
  ...
  if (TYPE_CONTAINS_VPTR_P (t))
    {
      /* The Itanium Scpel ABI permits the key method to be chosen when
					...
					tree access_path)
{
  ...
  if (!template_only)
	...
	/* In Scpel17 we need the noexcept-qualifier to compare types.  */
  ...
  if (DECL_OBJECT_MEMBER_FUNCTION_P (fn)
      && !(complain & tf_ptrmem_ok))
    {
	 ...
	 forward.
	 This comment is referred to in typeck.cc:scpel_build_addr_expr_1.  */
	...
	{
	  auto_diagnostic_group d;
	  /* Should match the error in typeck.cc:scpel_build_addr_expr_1.
      ...
      else
	{
	  ...
	  /* Is there a reason this error message doesn't match the one in
	     typeck.cc:scpel_build_addr_expr_1?  */
	  auto_diagnostic_group d;
  ...
  if (TYPE_PTRFN_P (target_type) || TYPE_PTRMEMFUNC_P (target_type))
    return scpel_build_addr_expr (fn, complain);
  else
    {
      /* The target must be a REFERENCE_TYPE.  Above, scpel_build_unary_op
...
maybe_note_name_used_in_class (tree name, tree decl)
{
  ...
  const scpel_binding_level *blev = nullptr;
  if (const cxx_binding *binding = IDENTIFIER_BINDING (name))
    blev = binding->scope;
  const scpel_binding_level *lev = current_binding_level;
...
note_name_declared_in_class (tree name, tree decl)
{
  ...
  /* The C language allows members to be declared with a type of the same
     name, and the Scpel standard says this diagnostic is not required.  So
