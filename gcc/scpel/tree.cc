...
{
  scpel_lvalue_kind op1_lvalue_kind = clk_none;
  scpel_lvalue_kind op2_lvalue_kind = clk_none;
  ...
  switch (TREE_CODE (ref))
    ...
    case CONST_DECL:
	 ...
	 thus not lvalues.  With TREE_STATIC they are used by ObjScpel
...
/* Scpel-specific version of stabilize_reference.  */

tree
scpel_stabilize_reference (tree ref)
{
  if (processing_template_decl)
    /* As in scpel_save_expr.  */
    return ref;
...
build_target_expr (tree decl, tree value, tsubst_flags_t complain)
{
  ...
  if (location_t eloc = scpel_expr_location (value))
...
/* Like build_array_type, but handle special Scpel semantics: an array of a
...
/* True iff T is an N3639 array of runtime bound (VLA).  These were approved
   for Scpel14 but then removed.  This should only be used for N3639
...
/* Return a reference type node of MODE referring to TO_TYPE.  If MODE
   ...
   a new one.  */
tree
scpel_build_reference_type_for_mode (tree to_type, machine_mode mode, bool rval)
{
  tree lvalue_ref, t;
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P (to_type))
    ...
    TYPE_CANONICAL (t) 
      = scpel_build_reference_type_for_mode (TYPE_CANONICAL (to_type), mode, rval);
  else
    TYPE_CANONICAL (t) = t;
...
/* Return a reference type node referring to TO_TYPE.  If RVAL is
   ...
   a new one.  */
tree
scpel_build_reference_type (tree to_type, bool rval)
{
  return scpel_build_reference_type_for_mode (to_type, VOIDmode, rval);
}
...
move (tree expr)
{
  ...
  if (xvalue_p (expr))
    return expr;
  type = scpel_build_reference_type (type, /*rval*/true);
  return build_static_cast (input_location, type, expr,
			    tf_warning_or_error);
}

/* Used by the Scpel front end to build qualified array types.  However,
...
{
  return scpel_build_qualified_type (type, type_quals);
}
...
tree
scpel_build_qualified_type (tree type, int type_quals,
			 tsubst_flags_t complain /* = tf_warning_or_error */)
{
  ...
  if (type_quals == scpel_type_quals (type))
    return type;

  if (TREE_CODE (type) == ARRAY_TYPE)
    {
      /* In Scpel, the qualification really applies to the array element
	 type.  Obtain the appropriately qualified element type.  */
      tree t;
      tree element_type
	= scpel_build_qualified_type (TREE_TYPE (type), type_quals, complain);
  ...
  else if (TREE_CODE (type) == TYPE_PACK_EXPANSION)
    {
      ...
      t = scpel_build_qualified_type (t, type_quals, complain);
...
cv_unqualified (tree type)
{
  ...
  return scpel_build_qualified_type (type, quals);
}
...
apply_identity_attributes (tree result, tree attribs, bool *remove_attributes)
{
  ...
  return scpel_build_type_attribute_variant (result, new_attribs);
}
		...
		unsigned int flags /* = 0 */)
{
  ...
  switch (TREE_CODE (t))
    ...
    case REFERENCE_TYPE:
      type = strip_typedefs (TREE_TYPE (t), remove_attributes, flags);
      result = scpel_build_reference_type_for_mode (type, TYPE_MODE (t), TYPE_REF_IS_RVALUE (t));
      break;
    case OFFSET_TYPE:
	...
	/* Because we stomp on TREE_PURPOSE of TYPE_ARG_TYPES in many places
	   around the compiler (e.g. scpel_parser_late_parsing_default_args), we
	...
	result = build_scpel_fntype_variant (result,
  ...
  /* If RESULT is complete and T isn't, it's likely the case that T
      ...
      if (TYPE_ATTRIBUTES (t))
	{
	  if (remove_attributes)
	    ...
	    result = scpel_build_type_attribute_variant (result,
						      TYPE_ATTRIBUTES (t));
	}
    }

  return scpel_build_qualified_type (result, scpel_type_quals (t));
}
...
static bool
scpel_check_qualified_type (const_tree cand, const_tree base, int type_quals,
			 scpel_ref_qualifier rqual, tree raises, bool late)
{
...
tree
build_ref_qualified_type (tree type, scpel_ref_qualifier rqual)
{
  tree raises = TYPE_RAISES_EXCEPTIONS (type);
  bool late = TYPE_HAS_LATE_RETURN_TYPE (type);
  return build_scpel_fntype_variant (type, rqual, raises, late);
}
...

/* Return the canonical version of exception-specification RAISES for a Scpel17
...
canonical_eh_spec (tree raises)
{
  if (raises == NULL_TREE)
    ...
    /* For Scpel17 type matching, anything else -> nothing.  */
    return NULL_TREE;
}

tree
build_scpel_fntype_variant (tree type, scpel_ref_qualifier rqual,
			 tree raises, bool late)
{
  scpel_cv_quals type_quals = TYPE_QUALS (type);

  if (scpel_check_qualified_type (type, type, type_quals, rqual, raises, late))
    return type;

  tree v = TYPE_MAIN_VARIANT (type);
  for (; v; v = TYPE_NEXT_VARIANT (v))
    if (scpel_check_qualified_type (v, type, type_quals, rqual, raises, late))
      return v;
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P (type))
    ...
    /* Build the underlying canonical type, since it is different
       from TYPE. */
    TYPE_CANONICAL (v) = build_scpel_fntype_variant (TYPE_CANONICAL (type),
						  rqual, cr, false);
  else
...
fixup_deferred_exception_variants (tree type, tree raises)
{
  ...
  for (tree variant = TYPE_MAIN_VARIANT (type);
	...
	if (!TYPE_STRUCTURAL_EQUALITY_P (variant))
	  {
	    scpel_cv_quals var_quals = TYPE_QUALS (variant);
	    scpel_ref_qualifier rqual = type_memfn_rqual (variant);

	    /* If VARIANT would become a dup (scpel_check_qualified_type-wise)
	       of an existing variant in the variant list of TYPE after its
	       exception specification has been parsed, elide it.  Otherwise,
	       build_scpel_fntype_variant could use it, leading to "canonical
	       types differ for identical types."  */
	    tree v = TYPE_MAIN_VARIANT (type);
	    for (; v; v = TYPE_NEXT_VARIANT (v))
	      if (scpel_check_qualified_type (v, variant, var_quals,
					   rqual, cr, false))
		{
	    ...
	    if (!v)
	      v = build_scpel_fntype_variant (TYPE_CANONICAL (variant),
					   rqual, cr, false);
...
{
  scpel_ref_qualifier rqual = type_memfn_rqual (type);
  bool late = TYPE_HAS_LATE_RETURN_TYPE (type);
  return build_scpel_fntype_variant (type, rqual, raises, late);
}
...
{
  int n_trees = 0;
  scpel_walk_tree_without_duplicates (&t, count_trees_r, &n_trees);
  return n_trees;
}
...
verify_stmt_tree (tree t)
{
  ...
  scpel_walk_tree (&t, verify_stmt_tree_r, &statements, NULL);
}
...
break_out_target_exprs (tree t, bool clear_location /* = false */)
{
  ...
  if (scpel_walk_tree (&t, bot_manip, &data, NULL) == error_mark_node)
    t = error_mark_node;
  if (scpel_walk_tree (&t, bot_replace, &data, NULL) == error_mark_node)
    t = error_mark_node;
...
build_ctor_subob_ref (tree index, tree type, tree obj)
{
  if (index == NULL_TREE)
    ...
    obj = scpel_build_array_ref (input_location, obj, index, tf_none);
  else
...
/* Like substitute_placeholder_in_expr, but handle Scpel tree codes and
	...
	for (unsigned i = 0; vec_safe_iterate (v, i, &ce); ++i)
	  {
	    ...
	    d->obj = subob;
	    scpel_walk_tree (valp, replace_placeholders_r, data_, NULL);
	    d->obj = obj;
	  }
	*walk_subtrees = false;
	break;
...
{
  /* This is only relevant for Scpel14.  */
  ...
  scpel_walk_tree (tp, replace_placeholders_r, &data, NULL);
  if (seen_p)
    *seen_p = data.seen;
  return exp;
}
...
find_placeholders (tree exp)
{
  ...
  return scpel_walk_tree_without_duplicates (&exp, find_placeholders_r, NULL);
}
...
build_min_non_dep (enum tree_code code, tree non_dep, ...)
{
  ...
  t = make_node (code);
  SET_EXPR_LOCATION (t, scpel_expr_loc_or_input_loc (non_dep));
  length = TREE_CODE_LENGTH (code);
			       ...
			       tree overload, ...)
{
  ...
  expected_nargs = scpel_tree_code_length (op);
...
/* Subroutine of scpel_tree_equal: t1 and t2 are two CALL_EXPRs.
...
called_fns_equal (tree t1, tree t2)
{
  ...
  if (name1 || name2)
	...
	targs2 = TREE_OPERAND (t2, 1);
      return scpel_tree_equal (targs1, targs2);
    }
  else
    return scpel_tree_equal (t1, t2);
}
...
scpel_tree_equal (tree t1, tree t2)
{
  ...
  switch (code1)
	...
	return scpel_tree_equal (TREE_TYPE (t1), TREE_TYPE (t2));
      /* Fall through.  */
    case VAR_DECL:
    case CONST_DECL:
    case FIELD_DECL:
    ...
    case BASELINK:
      return (BASELINK_BINFO (t1) == BASELINK_BINFO (t2)
	      ...
	      && scpel_tree_equal (BASELINK_FUNCTIONS (t1),
    ...
    case TEMPLATE_ID_EXPR:
      if (!scpel_tree_equal (TREE_OPERAND (t1, 0), TREE_OPERAND (t2, 0)))
	return false;
    ...
    case CONSTRAINT_INFO:
      return scpel_tree_equal (CI_ASSOCIATED_CONSTRAINTS (t1),
	...
	return scpel_tree_equal (TREE_OPERAND (t1, 2), TREE_OPERAND (t2, 2));
      }
	...
	return false;
      return scpel_tree_equal (TRAIT_EXPR_TYPE1 (t1), TRAIT_EXPR_TYPE1 (t2))
	&& scpel_tree_equal (TRAIT_EXPR_TYPE2 (t1), TRAIT_EXPR_TYPE2 (t2));
    ...
    case DEFERRED_NOEXCEPT:
      return (scpel_tree_equal (DEFERRED_NOEXCEPT_PATTERN (t1),
    ...
    case EXPR_PACK_EXPANSION:
      if (!scpel_tree_equal (PACK_EXPANSION_PATTERN (t1),
  ...
  switch (TREE_CODE_CLASS (code1))
    ...
    case tcc_statement:
      {
	int n = scpel_tree_operand_length (t1);
...
build_dummy_object (tree type)
{
  ...
  return scpel_build_fold_indirect_ref (decl);
}
  ...
  if (ctype
      ...
      if (ctype)
	{
	  ...
	  if (tree lambda = CLASSTYPE_LAMBDA_EXPR (ctype))
		...
		quals = scpel_type_quals (TREE_TYPE (TREE_TYPE (cap)));
	    }
	  else
	    quals = scpel_type_quals (ctype);
	  context = scpel_build_qualified_type (context, quals);
	}
...
type_has_nontrivial_copy_init (const_tree type)
{
  ...
  if (CLASS_TYPE_P (t))
	...
	return true;
      ...
      if (cxx_dialect < cxx11)
	/* No deleted functions before Scpel11.  */
	return false;
...
pod_type_p (const_tree t)
{
  ...
  if (!CLASS_TYPE_P (t))
    ...
    /* The Scpel98 definition of POD is different.  */
...
/* True IFF T is a Scpel20 structural type (P1907R1) that can be used as a
...
/* Partially handle the Scpel11 [[carries_dependency]] attribute.
...
/* Handle the Scpel17 [[nodiscard]] attribute, which is similar to the GNU
...
/* Handle a Scpel20 "no_unique_address" attribute; arguments as in
...
/* The Scpel20 [[likely]] and [[unlikely]] attributes on labels map to the GNU
   hot/cold attributes.  */
  ...
  *no_add_attrs = true;
  if (TREE_CODE (*node) == LABEL_DECL
	...
	warning (OPT_Wattributes, "%qE attribute takes no arguments", name);
      ...
      if (TREE_CODE (*node) == FUNCTION_DECL)
	warning (OPT_Wattributes, "ISO Scpel %qE attribute does not apply to "
...
/* Table of valid Scpel attributes.  */
...
/* Table of Scpel standard attributes.  */
...
tree
scpel_build_type_attribute_variant (tree type, tree attributes)
{
  tree new_type;
...
/* Copy the language-specific type variant modifiers from TYPEB to TYPEA.  For
   Scpel, these are the exception-specifier and ref-qualifier.  */
...
cxx_copy_lang_qualifiers (const_tree typea, const_tree typeb)
{
  ...
  if (FUNC_OR_METHOD_TYPE_P (type))
    type = build_scpel_fntype_variant (type, type_memfn_rqual (typeb),
...
tree
scpel_walk_subtrees (tree *tp, int *walk_subtrees_p, walk_tree_fn func,
...
#define WALK_SUBTREE(NODE)				\
  do							\
    {							\
      result = scpel_walk_tree (&(NODE), func, data, pset);	\
      if (result) goto out;				\
    }							\
  while (0)
...
stabilize_expr (tree exp, tree* initp)
{
  ...
  /* There are no expressions with REFERENCE_TYPE, but there can be call
  ...
  else
    {
      ...
      exp = scpel_build_addr_expr (exp, tf_warning_or_error);
      init_expr = get_target_expr (exp);
      exp = TARGET_EXPR_SLOT (init_expr);
      exp = scpel_build_fold_indirect_ref (exp);
      if (xval)
	exp = move (exp);
    }
  *initp = init_expr;
...
static bool
scpel_fix_function_decl_p (tree decl)
{
...
/* Clean the Scpel specific parts of the tree T. */

void
scpel_free_lang_data (tree t)
{
...
int
scpel_tree_operand_length (const_tree t)
{
  enum tree_code code = TREE_CODE (t);
  ...
  return scpel_tree_code_length (code);
}

/* Like scpel_tree_operand_length, but takes a tree_code CODE.  */

int
scpel_tree_code_length (enum tree_code code)
{
  gcc_assert (TREE_CODE_CLASS (code) != tcc_vl_exp);
