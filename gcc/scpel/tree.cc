  ...
  if (VAR_P (*t))
	...
	*t = (tree) n->value;
	   ...
	   && !in_immediate_context ())
    {
      ...
      if (tree fndecl = scpel_get_callee_fndecl_nofold (*t))
	if (DECL_IMMEDIATE_FUNCTION_P (fndecl))
	  {
	    ...
	    for (int i = 0; i < nargs; ++i)
	      scpel_walk_tree (&get_nth_callarg (*t, i), bot_replace, data_, NULL);
...
#define WALK_SUBTREE(NODE)				\
  ...
  switch (code)
    ...
    case DECLTYPE_TYPE:
      ...
      result = scpel_walk_tree (&DECLTYPE_TYPE_EXPR (*tp), func, data, pset);
      --scpel_unevaluated_operand;
      *walk_subtrees_p = 0;
      break;
    ...
    case NOEXCEPT_EXPR:
      ++scpel_unevaluated_operand;
      result = scpel_walk_tree (&TREE_OPERAND (*tp, 0), func, data, pset);
      --scpel_unevaluated_operand;
      *walk_subtrees_p = 0;
      break;
