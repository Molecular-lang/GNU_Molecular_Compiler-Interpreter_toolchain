...
/* Scpel returns type information to the user in struct type_info
...
build_headof (tree exp)
{
  ...
  offset = build_vtbl_ref (scpel_build_fold_indirect_ref (exp),
                           index);

  scpel_build_qualified_type (ptr_type_node,
			   scpel_type_quals (TREE_TYPE (exp)));
...
get_tinfo_decl_dynamic (tree exp, tsubst_flags_t complain)
{
  ...
  return scpel_build_fold_indirect_ref (t);
}
...
typeid_ok_p (void)
{
  ...
  if (!const_type_info_type_node)
	...
	= scpel_build_qualified_type (TREE_TYPE (decl), TYPE_QUAL_CONST);
    }
...
build_typeid (tree exp, tsubst_flags_t complain)
{
  ...
  if (TYPE_POLYMORPHIC_P (TREE_TYPE (exp))
      ...
      && ! nonnull)
    {
      ...
      exp = scpel_build_addr_expr (exp, complain);
      exp = save_expr (exp);
      cond = scpel_convert (boolean_type_node, exp, complain);
...
get_typeid (tree type, tsubst_flags_t complain)
{
  ...
  return scpel_build_fold_indirect_ref (get_tinfo_ptr (type));
}
...
{
  tree null_ptr = scpel_convert (TREE_TYPE (test), nullptr_node, complain);
  ...
  null_ptr = scpel_convert (TREE_TYPE (result), nullptr_node, complain);
		      ...
		      tsubst_flags_t complain)
{
  ...
  if (tc == POINTER_TYPE)
      ...
      if (!COMPLETE_TYPE_P (complete_type (exprtype)))
	{
      ...
      exprtype = scpel_build_reference_type (exprtype, !lvalue_p (expr));
    }
  ...
  if (TYPE_POLYMORPHIC_P (TREE_TYPE (exprtype)))
      ...
      else
	{
	  ...
	  if (!mark_used (td2, complain) && !(complain & tf_error))
	    return error_mark_node;
	  td2 = scpel_build_addr_expr (td2, complain);
	  td3 = get_tinfo_decl (static_type);
	  if (!mark_used (td3, complain) && !(complain & tf_error))
	    return error_mark_node;
	  td3 = scpel_build_addr_expr (td3, complain);
	  ...
	  expr1 = expr;
	  if (tc == REFERENCE_TYPE)
	    expr1 = scpel_build_addr_expr (expr1, complain);
	  ...
	  if (!dcast_fn)
	    {
	      ...
	      tinfo_ptr = scpel_build_qualified_type (tinfo_ptr, TYPE_QUAL_CONST);
	  ...
	  if (tc == REFERENCE_TYPE)
	    {
	      ...
	      neq = scpel_truthvalue_conversion (result, complain);
	      return scpel_convert (type,
				 build3 (COND_EXPR, TREE_TYPE (result),
					 neq, result, bad), complain);
	    }

	  /* Now back to the type we want from a void*.  */
	  result = scpel_convert (type, result, complain);
...
tinfo_base_init (tinfo_s *ti, tree target)
{
...

  {
    ...
    tree name_type = build_cplus_array_type
		     (scpel_build_qualified_type (char_type_node, TYPE_QUAL_CONST),
		     NULL_TREE);
  ...
  if (!vtable_ptr)
      ...
      if (!COMPLETE_TYPE_P (real_type))
	{
      ...
      vtable_ptr = scpel_build_addr_expr (vtable_ptr, tf_warning_or_error);
...
typeinfo_in_lib_p (tree type)
{
  ...
  if (TYPE_PTR_P (type)
      && (scpel_type_quals (TREE_TYPE (type)) == TYPE_QUAL_CONST
	  || scpel_type_quals (TREE_TYPE (type)) == TYPE_UNQUALIFIED))
    type = TREE_TYPE (type);
...
get_tinfo_desc (unsigned ix)
{
  ...
  res->type = scpel_build_qualified_type (pseudo_type, TYPE_QUAL_CONST);
...
emit_support_tinfo_1 (tree bltn)
{
  ...
  types[2] = build_pointer_type (scpel_build_qualified_type (bltn,
							  TYPE_QUAL_CONST));
  ...
  for (int i = 0; i < 3; ++i)
    {
      ...
      TREE_USED (tinfo) = 1;
      mark_needed (tinfo);
      /* The Scpel ABI requires that these objects be COMDAT.  But,
...
emit_tinfo_decl (tree decl)
{
  ...
  if (DECL_NOT_REALLY_EXTERN (decl) && decl_needed_p (decl))
      ...
      if (TREE_TYPE (decl) != ti->type)
	{
      ...
      mark_used (decl);
      scpel_finish_decl (decl, init, false, NULL_TREE, 0);
