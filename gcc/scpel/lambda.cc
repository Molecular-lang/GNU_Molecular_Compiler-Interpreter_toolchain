...
build_lambda_object (tree lambda_expr)
{
  /* Build aggregate constructor call.
     - scpel_parser_braced_list
     - scpel_parser_functional_cast  */
...
begin_lambda_type (tree lambda)
{
  ...
  /* In Scpel17, assume the closure is literal; we'll clear the flag later if
     necessary.  */
...
/* Returns the type to use for the FIELD_DECL corresponding to the
   ...
   Scpel14 init capture, and BY_REFERENCE_P indicates whether we're
	     ...
	     bool explicit_init_p, unsigned *name_independent_cnt)
{
  ...
  if (!dependent_type_p (type) && array_of_runtime_bound_p (type))
	...
	error ("array of runtime bound cannot be captured by copy, "
	 ...
	 maximum index, and then reconstruct the VLA for the proxy.  */
      tree elt = scpel_build_array_ref (input_location, initializer,
      ...
      if (id == this_identifier && !by_reference_p)
	{
	  ...
	  type = TREE_TYPE (type);
	  initializer = scpel_build_fold_indirect_ref (initializer);
	}
      ...
      if (cxx_dialect < cxx20)
	{
	  ...
	  if (DECL_DECOMPOSITION_P (stripped_init)
	      && pedwarn (input_location, OPT_Wc__20_extensions,
			  "captured structured bindings are a Scpel20 extension"))
  ...
  /* For explicit captures we haven't started the function yet, so we wait
     and build the proxy from scpel_parser_lambda_body.  */
...
maybe_add_lambda_conv_op (tree type)
{
  ...
  if (generic_lambda_p)
    {
	 ...
	 implementation of the conversion operator.  */
      ...
      if (thisarg)
	{
	  tree instance = scpel_build_fold_indirect_ref (thisarg);
  ...
  stattype = (scpel_build_type_attribute_variant
  ...
  tree thistype = scpel_build_qualified_type (type, TYPE_QUAL_CONST);
...
prune_lambda_captures (tree body)
{
  ...
  scpel_walk_tree_without_duplicates (&body, mark_const_cap_r, &const_vars);
