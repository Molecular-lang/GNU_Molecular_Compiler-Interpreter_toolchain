...
friend_accessible_p (tree scope, tree decl, tree type, tree otype)
{
  ...
  /* Nested classes have the same access as their enclosing types, as
     per DR 45 (this is a change from Scpel98).  */
  if (TYPE_P (scope))
...
field_accessor_p (tree fn, tree field_decl, bool const_p)
{
  ...
  /* Failing that, determine if it is an accessor of the form:
       T& get_field () { return m_field; }.  */
  tree field_reference_type = scpel_build_reference_type (field_type, false);
...
check_final_overrider (tree overrider, tree basefn)
{
  ...
  if (same_type_p (base_return, over_return))
	       ...
	       && INDIRECT_TYPE_P (base_return)))
    {
      ...
      if (!fail)
	{
	  if (scpel_type_quals (base_return) != scpel_type_quals (over_return))
	    fail = 1;
      ...
      base_quals = scpel_type_quals (base_return);
      over_quals = scpel_type_quals (over_return);
...

/* Debug info for Scpel classes can get very large; try to avoid
...
maybe_suppress_debug_info (tree t)
{
  ...
  /* We might have set this earlier in scpel_finish_decl.  */
