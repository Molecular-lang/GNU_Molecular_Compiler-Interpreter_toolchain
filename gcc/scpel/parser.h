...
typedef struct scpel_token *scpel_token_position;

/* The scpel_lexer structure represents the Scpel lexer.  It is responsible
   for managing the token stream from the preprocessor and supplying
   it to the parser.  Tokens are never added to the scpel_lexer after
   it is created.  */

struct GTY (()) scpel_lexer {
  /* The memory allocated for the buffer.  NULL if this lexer does not
     own the token buffer.  */
  vec<scpel_token, va_gc> *buffer;

  /* A pointer just past the last available token.  The tokens
     in this lexer are [buffer, last_token).  */
  scpel_token_position GTY ((skip)) last_token;

  /* The next available token.  If NEXT_TOKEN is &eof_token, then there are
     no more available tokens.  */
  scpel_token_position GTY ((skip)) next_token;

  /* A stack indicating positions at which scpel_lexer_save_tokens was
     called.  The top entry is the most recent position at which we
     began saving tokens.  If the stack is non-empty, we are saving
     tokens.  */
  vec<scpel_token_position> GTY ((skip)) saved_tokens;
  ...
  /* True if we're in the context of OpenMP directives written as Scpel11
...
/* scpel_token_cache is a range of tokens.  There is no need to represent
   ...
   a scpel_token_cache, since everything in here is referenced through
   a lexer.  */

struct GTY(()) scpel_token_cache {
  /* The beginning of the token range.  */
  scpel_token * GTY((skip)) first;

  /* Points immediately after the last token in the range.  */
  scpel_token * GTY ((skip)) last;
};

typedef scpel_token_cache *scpel_token_cache_ptr;

struct scpel_token_ident
{
...
struct GTY(()) scpel_default_arg_entry {
...
struct GTY (()) scpel_parser_context {
  /* If this is a tentative parsing context, the status of the
     tentative parse.  */
  enum scpel_parser_status_kind status;
  ...
  struct scpel_parser_context *next;
};


/* Helper data structure for parsing #pragma omp declare {simd,variant}.  */
struct scpel_omp_declare_simd_data {
  bool error_seen; /* Set if error has been reported.  */
  bool fndecl_seen; /* Set if one fn decl/definition has been seen already.  */
  bool variant_p; /* Set for #pragma omp declare variant.  */
  location_t loc;
  vec<scpel_token_cache_ptr> tokens;
  tree *attribs[2];
};

/* Helper data structure for parsing #pragma acc routine.  */
struct scpel_oacc_routine_data : scpel_omp_declare_simd_data {
  tree clauses;
};

/* The scpel_parser structure represents the Scpel parser.  */
...
struct GTY(()) scpel_parser {
  ...
  /* TRUE if the `>' token should be interpreted as the greater-than
     ...
     template-parameter-list. In Scpel0x mode, this flag also applies to
...
#define IN_IF_STMT             16
  ...
  vec<scpel_unparsed_functions_entry, va_gc> *unparsed_queues;
  ...
  scpel_omp_declare_simd_data * GTY((skip)) omp_declare_simd;

  /* When parsing #pragma acc routine, this is a pointer to a helper data
     structure.  */
  scpel_oacc_routine_data * GTY((skip)) oacc_routine;
  ...
  /* Tracks the function's template parameter list when declaring a function
     ...
     scpel_parser_parameter_declaration.  */
  ...
  scpel_binding_level* implicit_template_scope;
  ...
  /* Pointer to state for parsing omp_loops.  Managed by
     scpel_parser_omp_for_loop in parser.cc and not used outside that file.  */
...
extern void debug (vec<scpel_token, va_gc> &ref);
extern void debug (vec<scpel_token, va_gc> *ptr);
extern void scpel_debug_parser (FILE *, scpel_parser *);
extern void debug (scpel_parser &ref);
extern void debug (scpel_parser *ptr);
extern bool scpel_keyword_starts_decl_specifier_p (enum rid keyword);
