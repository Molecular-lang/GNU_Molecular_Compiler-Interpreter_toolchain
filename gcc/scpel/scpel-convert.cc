...
static tree
scpel_convert (tree type, tree expr, bool init_const)
{
  tree e = expr;
  ...
  switch (code)
	...
	{
	  expr = save_expr (expr);
	  expr = scpel_fully_fold (expr, init_const, NULL);
	...
	(loc, type, scpel_objc_common_truthvalue_conversion (input_location, expr));
      ...
      if (NULLPTR_TYPE_P (TREE_TYPE (e)))
	{
	  /* To make sure that (void *)nullptr is not a null pointer constant,
	     build_scpel_cast will create an additional NOP_EXPR around the result
	     of this conversion.  */
    ...
    case NULLPTR_TYPE:
	 ...
	 handled.  build_scpel_cast will create an additional NOP_EXPR to
...
{
  return scpel_convert (type, expr, false);
}
...
{
  return scpel_convert (type, expr, true);
}
