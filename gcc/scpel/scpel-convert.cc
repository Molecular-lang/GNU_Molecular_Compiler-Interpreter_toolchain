...
static tree c_convert(tree type, tree expr, bool init_const) {
	tree e = expr;
	...
	switch (code) {
		...
		case ENUMERAL_TYPE:
			if (ENUM_UNDERLYING_TYPE (type) != NULL_TREE && TREE_CODE (ENUM_UNDERLYING_TYPE (type)) == BOOLEAN_TYPE)
		...
		case BOOLEAN_TYPE:
convert_to_boolean:
			return fold_convert_loc(loc, type, c_objc_common_truthvalue_conversion (input_location, expr));

		case POINTER_TYPE:
			/* The type nullptr_t may be converted to a pointer type. The result is a null pointer value. */
...
tree convert_init(tree type, tree expr) {
