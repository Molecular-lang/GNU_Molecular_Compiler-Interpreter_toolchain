...
static char *affix_data_type(const char *param) {
...
static const char *gen_type(const char *ret_val, tree t, formals_style style) {
	...
	if (TYPE_NAME (t) && TREE_CODE (TYPE_NAME (t)) == TYPE_DECL)
		...
		switch (TREE_CODE (t)) {
			...
			case FUNCTION_TYPE:
				ret_val = gen_type(concat(ret_val, gen_formal_list_for_type (t, style), NULL),
					TREE_TYPE (t), style);
				break;
...
void gen_aux_info_record(tree fndecl, int is_definition, int is_implicit, int is_prototyped) {
	if (flag_gen_aux_info) {
		...
		/* Each output .X file must have a header line. Write one now if we have not yet done so. */
