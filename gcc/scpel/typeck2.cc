...
#define ERROR_FOR_ASSIGNMENT(LOC, AS, ASM, IN, DE, ARG)			\
  ...
  /* Handle Scpel-specific things first.  */
  ...
  switch (TREE_CODE (type))
	  ...
	  || vla_type_p (type))
	{
	  if (!TYPE_DOMAIN (type)
	    ...
	    {
	      /* Flexible array.  */
	      scpel_complete_array_type (&type, init, /*default*/true);
	  ...
	  if (TREE_CODE (value) == CONSTRUCTOR)
	      ...
	      if (TREE_CODE (field_index) == RANGE_EXPR)
		{
		  ...
		  sub = build4 (ARRAY_REF, inner_type, dest, low,
				NULL_TREE, NULL_TREE);
		  sub = scpel_build_addr_expr (sub, tf_warning_or_error);
	      ...
	      else
		{
		  ...
		  if (unsafe_return_slot_p (sub))
		    ...
		    {
		    build_init:
		      code = scpel_build_init_expr (sub, value);
		    }
  ...
  if (TREE_CODE (init) == CONSTRUCTOR)
    {
      ...
      init = scpel_fully_fold_init (init);
      code = push_stmt_list ();
	...
	{
	  tree ie = scpel_build_init_expr (dest, init);
    ...
    code = scpel_build_init_expr (dest, init);
...
poison_mutable_constructors (tree t)
{
  ...
  if (scpel_has_mutable_p (TREE_TYPE (t)))
    {
...
/* Perform appropriate conversions on the initial value of a variable,
   ...
   Scpel: Note that INIT might be a TREE_LIST, which would mean that it is
...
store_init_value (tree decl, tree init, vec<tree, va_gc>** cleanups, int flags)
{
  ...
  /* In Scpel11 constant expression is a semantic, not syntactic, property.
     In Scpel98, make sure that what we thought was a constant expression at
  ...
  if (!current_function_decl
      ...
      || TREE_STATIC (decl))
    value = scpel_fully_fold_init (value);
  ...
  /* [dcl.init.list]#7.2: "from long double to double or float, or from
	   ...
	   && SCALAR_FLOAT_TYPE_P (type))
    {
	  ...
	  ? /* "from a floating-point type T to another floating-point type
	       ...
	       So, it is ok if
	       scpel_compare_floating_point_conversion_ranks (ftype, type)
	       ...
	       backend types).  */
	    scpel_compare_floating_point_conversion_ranks (ftype, type) >= 2
    ...
    /* Scpel20 P1957R2: converting from a pointer type or a pointer-to-member
       type to bool should be considered narrowing.  This is a DR so is not
       limited to Scpel20 only.  */
    ok = false;

  bool almost_ok = ok;
  if (!ok && !CONSTANT_CLASS_P (init) && (complain & tf_warning_or_error))
    {
      tree folded = scpel_fully_fold (init);
      ...
      if (cxx_dialect == cxx98)
	{
	  if (complain & tf_warning)
	    warning_at (loc, OPT_Wnarrowing, "narrowing conversion of %qE "
			"from %qH to %qI is ill-formed in Scpel11",
			init, ftype, type);
	  ok = true;
	}
...
/* True iff TYPE is a Scpel20 "ordinary" character type.  */
...
array_string_literal_compatible_p (tree type, tree init)
{
  ...
  /* P2513 (Scpel20/Scpel23): "an array of char or unsigned char may
	       ...
	       tsubst_flags_t complain)
{
  ...
  if (code == ARRAY_TYPE)
	...
	/* Scpel flexible array members have a null domain.  */
	{
	  ...
	  if (TYPE_DOMAIN (type) && TREE_CONSTANT (TYPE_SIZE (type)))
	    {
		 ...
		 counted in the length of the constant, but in Scpel this would
		 be invalid.  */
  ...
  if (cxx_dialect >= cxx11
	...
	{
	  /* In Scpel17, aggregates can have bases, thus participate in
...
digest_nsdmi_init (tree decl, tree init, tsubst_flags_t complain)
{
  ...
  hash_set<tree> pset;
  scpel_walk_tree (&init, replace_placeholders_for_class_temp_r, &pset, nullptr);
				...
				tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE)
	...
	{
	  if (complain & tf_error)
	    error_at (scpel_expr_loc_or_input_loc (init),
				 ...
				 tsubst_flags_t complain)
{
 ...
 restart:
  ...
  for (field = TYPE_FIELDS (type); field; field = DECL_CHAIN (field))
	...
	next = scpel_convert_and_check (TREE_TYPE (field), next, complain);
      picflags |= picflag_from_initializer (next);
			  ...
			  tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE && TYPE_DOMAIN (type) == NULL_TREE)
    scpel_complete_array_type (&TREE_TYPE (init), init, /*do_default=*/0);
...
/* Build a reference to an object specified by the Scpel `->' operator.
...
build_x_arrow (location_t loc, tree expr, tsubst_flags_t complain)
{
  ...
  if (TYPE_PTR_P (TREE_TYPE (last_rval)))
      ...
      if (processing_template_decl)
	{
      ...
      return scpel_build_indirect_ref (loc, last_rval, RO_ARROW, complain);
    }
...
build_m_component_ref (tree datum, tree component, tsubst_flags_t complain)
{
  ...
  if (TYPE_PTRDATAMEM_P (ptrmem_type))
	 ...
	 non-static data members.  */
      type = scpel_build_qualified_type (type,
				      (scpel_type_quals (type)
				       | scpel_type_quals (TREE_TYPE (datum))));
	...
	datum = build2 (POINTER_PLUS_EXPR, ptype,
      ...
      datum = scpel_fully_fold (datum);
      datum = scpel_build_fold_indirect_ref (datum);
      if (datum == error_mark_node)
	return error_mark_node;
  ...
  else
    {
	 ...
	 function with ref-qualifier & (for Scpel20: unless its cv-qualifier-seq
	      ...
	      else if (cxx_dialect < cxx20)
		{
		  if (complain & tf_warning_or_error)
		    pedwarn (input_location, OPT_Wpedantic,
			     ...
			     "an lvalue before Scpel20", ptrmem_type);
		  else
		    return error_mark_node;
		}
	    }
			 ...
			 tsubst_flags_t complain)
{
  /* This is either a call to a constructor,
     or a C cast in Scpel's `functional' notation.  */
  ...
  if (! MAYBE_CLASS_TYPE_P (type))
      ...
      if (parms == NULL_TREE)
	{
      ...
      return scpel_build_c_cast (loc, type, parms, complain);
    }
  ...
  if (parms && TREE_CHAIN (parms) == NULL_TREE)
    return scpel_build_c_cast (loc, type, TREE_VALUE (parms), complain);
...
merge_exception_specifiers (tree list, tree add)
{
  ...
  gcc_checking_assert (!TREE_PURPOSE (add)
		       ...
		       || scpel_tree_equal (noex, TREE_PURPOSE (add)));
...
/* Record that any TARGET_EXPR in T are going to be elided in
   scpel_gimplify_init_expr (or sooner).  */
...
tree
scpel_build_init_expr (location_t loc, tree target, tree init)
{
  set_target_expr_eliding (init);
