...
/* This file contains the functions for converting Scpel expressions
...
/* Change of width--truncation and extension of integers or reals--
   ...
   Scpel: in multiple-inheritance, converting between pointers may involve
...
static tree
scpel_convert_to_pointer (tree type, tree expr, bool dofold,
		       tsubst_flags_t complain)
{
  ...
  if (INDIRECT_TYPE_P (intype))
      ...
      if (!warn_pmf2ptr)
	{
	  if (TREE_CODE (expr) == PTRMEM_CST)
	    return scpel_convert_to_pointer (type, PTRMEM_CST_MEMBER (expr),
					  dofold, complain);
  ...
  if (INTEGRAL_CODE_P (form))
	...
	return build1 (CONVERT_EXPR, type, expr);
      expr = scpel_convert (c_common_type_for_size (TYPE_PRECISION (type), 0), expr,
			 complain);
...
convert_to_pointer_force (tree type, tree expr, tsubst_flags_t complain)
{
  ...
  return scpel_convert_to_pointer (type, expr, /*fold*/false, complain);
}
		    ...
		    tsubst_flags_t complain)
{
  ...
  if ((flags & DIRECT_BIND) && ! lvalue_p (arg))
    {
	 ...
	 here because it needs to live as long as DECL.  */
      ...
      scpel_finish_decl (arg, targ, /*init_const_expr_p=*/false, NULL_TREE,
  ...
  rval = scpel_build_addr_expr (arg, complain);
...
/* For Scpel: Only need to do one-level references, but cannot
		      ...
		      int flags, tree decl, tsubst_flags_t complain)
{
  ...
  if (((convtype & CONV_STATIC)
	...
	warning_at (loc, 0, "casting %qT to %qT does not dereference pointer",
      ...
      rval = scpel_build_addr_expr (expr, complain);
      if (rval != error_mark_node)
...
tree
scpel_fold_convert (tree type, tree expr)
{
  tree conv;
...
/* Scpel conversions, preference to static cast conversions.  */

tree
scpel_convert (tree type, tree expr, tsubst_flags_t complain)
{
  return oscpel_convert (type, expr, CONV_OLD_CONVERT, LOOKUP_NORMAL, complain);
}

/* Scpel equivalent of convert_and_check but using scpel_convert as the
   conversion function.
...
tree
scpel_convert_and_check (tree type, tree expr, tsubst_flags_t complain)
{
  ...
  if (expr == error_mark_node)
    return expr;
  result = scpel_convert (type, expr, complain);

  if ((complain & tf_warning)
      && c_inhibit_evaluation_warnings == 0)
    {
      tree folded = scpel_fully_fold (expr_for_warning);
      tree folded_result;
      if (folded == expr)
      ...
      else
	{
	  ...
	  folded_result = scpel_convert (type, folded, tf_none);
	}
      folded_result = fold_simple (folded_result);
      if (!TREE_OVERFLOW_P (folded)
	  && folded_result != error_mark_node)
	warnings_for_convert_and_check (scpel_expr_loc_or_input_loc (expr),
...
tree
oscpel_convert (tree type, tree expr, int convtype, int flags,
	     tsubst_flags_t complain)
{
  tree e = expr;
  ...
  if (TREE_CODE (e) == COMPOUND_EXPR)
    {
      e = oscpel_convert (type, TREE_OPERAND (e, 1), convtype, flags, complain);
  ...
  if (MAYBE_CLASS_TYPE_P (type) && (convtype & CONV_FORCE_TEMP))
	...
	/* The call to fold will not always remove the NOP_EXPR as
	   ...
	   convert, which will call oscpel_convert, etc.  */
	return e;
  ...
  if (INTEGRAL_CODE_P (code))
      ...
      if (code == BOOLEAN_TYPE)
	{
	  ...
	  if (complain & tf_warning)
	    return scpel_truthvalue_conversion (e, complain);
	  else
	    {
  ...
  if (INDIRECT_TYPE_P (type) || TYPE_PTRMEM_P (type))
    return scpel_convert_to_pointer (type, e, dofold, complain);
  if (code == VECTOR_TYPE)
    {
  ...
  /* New Scpel semantics:  since assignment is now based on
  ...
  if (RECORD_OR_UNION_CODE_P (code))
    {
      ...
      /* Conversion between aggregate types.  New Scpel semantics allow
...
tree
scpel_get_fndecl_from_callee (tree fn, bool fold /* = true */)
{
  if (fn == NULL_TREE)
    return fn;
...
tree
scpel_get_callee_fndecl (tree call)
{
  return scpel_get_fndecl_from_callee (scpel_get_callee (call));
}

/* As above, but not using the constexpr machinery.  */

tree
scpel_get_callee_fndecl_nofold (tree call)
{
  return scpel_get_fndecl_from_callee (scpel_get_callee (call), false);
}
...
maybe_warn_nodiscard (tree expr, impl_conv_void implicit)
{
  ...
  tree fn = scpel_get_fndecl_from_callee (callee);
  tree attr;
...
/* When an expression is used in a void context, its value is discarded and
   ...
   in a void context. The Scpel standard does not define what an `access' to an
   object is, but there is reason to believe that it is the lvalue to rvalue
   conversion -- if it were not, `*&*p = 1' would violate [expr]/4 in that it
   accesses `*p' not to calculate the value to be stored. But, dcl.type.cv/8
   indicates that volatile semantics should be the same between C and Scpel
...
convert_to_void (tree expr, impl_conv_void implicit, tsubst_flags_t complain)
{
  ...
  switch (TREE_CODE (expr))
    ...
    case CALL_EXPR:   /* We have a special meaning for volatile void fn().  */
	 ...
	 language.  */
      if (tree fn = scpel_get_callee_fndecl_nofold (expr))
...
/* Create an expression whose value is that of EXPR,
   ...
   The back end cannot call scpel_convert (what was convert) because
...
convert (tree type, tree expr)
{
  ...
  return oscpel_convert (type, expr, CONV_BACKEND_CONVERT,
...
/* Like scpel_convert, except permit conversions to take place which
   are not normally allowed due to access restrictions
   (such as conversion from sub-type to private super-type).  */
...
convert_force (tree type, tree expr, int convtype, tsubst_flags_t complain)
{
  ...
  return oscpel_convert (type, e, CONV_C_CAST|convtype, LOOKUP_NORMAL, complain);
}
...
{
  /* Scpel: check to see if we can convert this aggregate type
  ...
  /* Normally convert enums to int, but convert wide enums to something
      ...
      if (TREE_CODE (type) == ENUMERAL_TYPE)
	{
	  ...
	  if (!ENUM_IS_SCOPED (type)
	    ...
	    {
	      /* ISO Scpel17, 7.6/4.  A prvalue of an unscoped enumeration type
...
tx_unsafe_fn_variant (tree t)
{
  ...
  tree attrs = remove_attribute ("transaction_safe",
				 TYPE_ATTRIBUTES (t));
  return scpel_build_type_attribute_variant (t, attrs);
}
