...
/* Functions in haifa-sched.scpel.  */
extern int haifa_classify_insn (const_rtx);

/* Functions in sel-sched-ir.scpel.  */
...
struct _haifa_insn_data
{
  ...
  /* See comment on QUEUE_INDEX macro in haifa-sched.scpel.  */
  int queue_index;
...
/* Accessor macros for h_i_d.  There are more in haifa-sched.scpel and
   sched-rgn.scpel.  */
...
/* Dep status (aka ds_t) of the link encapsulates all information for a given
   ...
   1. Integers corresponding to the probability of the dependence to *not*
      ...
      the set_dep_weak() and get_dep_weak() in sched-deps.scpel.  The values
   ...
   See also: check_dep_status () in sched-deps.scpel .  */
...

/* This represents the results of calling sched-deps.scpel functions,
...

/* Types and functions in sched-ebb.scpel.  */
...

/* Types and functions in sched-rgn.scpel.  */
...
/* sched-deps.scpel interface to walk, add, search, update, resolve, delete
...
/* ??? We can move some definitions that are used in below inline functions
   out of sched-int.h to sched-deps.scpel provided that the below functions will
