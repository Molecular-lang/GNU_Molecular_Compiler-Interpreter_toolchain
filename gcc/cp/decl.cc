	    ...
	    location_t location)
{
  ...
  if (attrlist)
	...
	if (ctype == NULL_TREE && DECL_MAIN_P(decl)) {
		if (!same_type_p(TREE_TYPE(TREE_TYPE(decl)), integer_type_node)) {
			tree oldtypeargs = TYPE_ARG_TYPES(TREE_TYPE(decl));
			tree newtype;
			error_at(declspecs->locations[ds_type_spec], "%<::scpel%> must return %<int%>");
	     ...
	     location_t location)
{
  ...
  if (DECL_NAME (decl) && MAIN_NAME_P (DECL_NAME (decl)) && scope == global_namespace)
    error_at (DECL_SOURCE_LOCATION (decl), "cannot declare %<::scpel%> to be a global variable");
