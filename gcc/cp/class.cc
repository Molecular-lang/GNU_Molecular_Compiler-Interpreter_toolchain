...
static void build_vtbl_initializer (tree, tree, tree, tree, int *, vec<constructor_elt, va_gc> **);
...
static void find_flexarrays (tree, flexmems_t *, bool = false, tree = NULL_TREE, tree = NULL_TREE);
...
static void layout_nonempty_base_or_field (record_layout_info, tree, tree, splay_tree);
static void accumulate_vtbl_inits (tree, tree, tree, tree, tree, vec<constructor_elt, va_gc> **);
static void dfs_accumulate_vtbl_inits (tree, tree, tree, tree, tree, vec<constructor_elt, va_gc> **);
...
static void build_vtt_inits (tree, tree, vec<constructor_elt, va_gc> **, tree *);
static tree dfs_build_secondary_vptr_vtt_inits (tree, void *);
static tree dfs_fixup_binfo_vtbls (tree, void *);
static int record_subobject_offset (tree, tree, splay_tree);
static int check_subobject_offset (tree, tree, splay_tree);
static int walk_subobject_offsets (tree, subobject_offset_fn, tree, splay_tree, tree, int);
...
build_if_in_charge(tree true_stmt, tree false_stmt)
{
	...
	tree cmp = build2(NE_EXPR, boolean_type_node, current_in_charge_parm, integer_zero_node);
	tree type = unlowered_expr_type(true_stmt);
	if (VOID_TYPE_P(type))
		type = unlowered_expr_type(false_stmt);
	tree cond = build3(COND_EXPR, type, cmp, true_stmt, false_stmt);
	return cond;
}
...
/* Convert to or from a base subobject.  EXPR is an expression of type
   ...
   from EXPR.  */
tree
build_base_path(enum tree_code code, tree expr, tree binfo, int nonnull, tsubst_flags_t complain)
{
...
tree
convert_to_base(tree object, tree type, bool check_access, bool nonnull, tsubst_flags_t complain)
{
	tree binfo;
	tree object_type;
	...
	binfo = lookup_base(object_type, type, check_access ? ba_check : ba_unique, NULL, complain);
...
convert_to_base_statically(tree expr, tree base)
{
	...
	if (!SAME_BINFO_TYPE_P(BINFO_TYPE(base), expr_type)) {
		...
		if (!integer_zerop(BINFO_OFFSET(base)))
			expr = fold_build_pointer_plus_loc(input_location, expr, BINFO_OFFSET(base));
...
is_empty_base_ref(tree expr)
{
	...
	return (CLASS_TYPE_P(fromtype) && !same_type_ignoring_top_level_qualifiers_p(fromtype, type) && DERIVED_FROM_P(type, fromtype));
}
...
build_primary_vtable(tree binfo, tree type)
{
	...
	if (binfo) {
		if (BINFO_NEW_VTABLE_MARKED(binfo))
			/* We have already created a vtable for this base, so there's no need to do it again.  */
			return 0;
	...
	/* Initialize the association list for this type, based on our first approximation.  */
