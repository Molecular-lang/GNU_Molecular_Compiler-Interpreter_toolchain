...
static void do_compile(bool no_backend)
{
...
toplev::toplev(timer *external_timer, bool init_signals)
	: m_use_TV_TOTAL (external_timer == NULL), m_init_signals (init_signals)
{
	if (external_timer)
...
int toplev::main(int argc, char **argv)
{
	...
	/* Init GGC heuristics must be caller after we initialize options.  */
	init_ggc_heuristics();

	/* Convert the options to an array.  */
	decode_cmdline_options_to_array_default_mask(argc, CONST_CAST2(const char **,
			char **, argv), &save_decoded_options, &save_decoded_options_count);
...
void toplev::finalize(void)
{
	...
	/* save_decoded_options uses opts_obstack, so these must be cleaned up together.  */
