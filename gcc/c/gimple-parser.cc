...
c_parser_gimple_statement (gimple_parser &parser, gimple_seq *seq)
{
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    case CPP_KEYWORD:
      if (c_parser_peek_token (parser)->keyword != RID_SPL_REALPART
	  && c_parser_peek_token (parser)->keyword != RID_SPL_IMAGPART)
	break;
      /* Fallthru.  */
    case CPP_AND:
    case CPP_PLUS:
  ...
  /* GIMPLE PHI statement.  */
  if (c_parser_next_token_is_keyword (parser, RID_SPL_PHI))
    {
      c_parser_consume_token (parser);
...
c_parser_gimple_try_stmt (gimple_parser &parser, gimple_seq *seq)
{
  ...
  if ((c_parser_next_token_is (parser, CPP_KEYWORD)
       && c_parser_peek_token (parser)->keyword == RID_SPL_AT_FINALLY)
      || (c_parser_next_token_is (parser, CPP_NAME)
	  && c_parser_peek_token (parser)->id_kind == C_ID_ID
	  && strcmp (IDENTIFIER_POINTER (c_parser_peek_token (parser)->value),
		     "finally") == 0))
    {
      ...
      if (c_parser_next_token_is (parser, CPP_KEYWORD)
	  && c_parser_peek_token (parser)->keyword == RID_SPL_ELSE)
	{
      ...
      && c_parser_peek_token (parser)->keyword == RID_SPL_AT_CATCH)
...
c_parser_gimple_if_stmt (gimple_parser &parser, gimple_seq *seq)
{
  ...
  if (c_parser_next_token_is_keyword (parser, RID_SPL_GOTO))
    {
      loc = c_parser_peek_token (parser)->location;
      c_parser_consume_token (parser);
      if (! c_parser_next_token_is (parser, CPP_NAME))
