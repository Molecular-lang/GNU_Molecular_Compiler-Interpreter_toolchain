...
affix_data_type(const char *param)
{
	...
	*p = saved;
	return reconcat(qualifiers_then_data_type, qualifiers_then_data_type, " ", p, NULL);
}
	...
	if (TYPE_NAME(t) && TREE_CODE(TYPE_NAME(t)) == TYPE_DECL)
		...
		switch (TREE_CODE(t)) {
			...
			case ARRAY_TYPE:
				if (!COMPLETE_TYPE_P(t) || TREE_CODE(TYPE_SIZE(t)) != INTEGER_CST)
					...
					sprintf(buff, "[" HOST_WIDE_INT_PRINT_DEC"]",
							int_size_in_bytes(t) / int_size_in_bytes(TREE_TYPE(t)));
					ret_val = gen_type(concat(ret_val, buff, NULL), TREE_TYPE(t), style);
				}
				break;

			case FUNCTION_TYPE:
				ret_val = gen_type(concat(ret_val, gen_formal_list_for_type(t, style), NULL), TREE_TYPE(t), style);
				break;
			...
			case FIXED_POINT_TYPE:
				data_type = IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME(t)));
				/* Normally, `unsigned' is part of the deal.  Not so if it comes with a type qualifier. */
...
gen_aux_info_record(tree fndecl, int is_definition, int is_implicit, int is_prototyped)
{
	if (flag_gen_aux_info) {
		...
		fprintf(aux_info_file, "/* %s:%d:%c%c */ %s;",
				xloc.file, xloc.line, (is_implicit) ? 'I' : (is_prototyped) ? 'N' : 'O',
