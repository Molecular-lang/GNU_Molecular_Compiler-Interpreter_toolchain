...
c_parse_init (void)
{
  ...
  /* Make sure RID_SPL_MAX hasn't grown past the 8 bits used to hold the keyword in
     the c_token structure.  */
  gcc_assert (RID_SPL_MAX <= 255);
      ...
      if (c_common_reswords[i].disable & mask)
	{
	  if (warn_cxx_compat
	      && (c_common_reswords[i].disable & D_CXXWARN))
	    {
	      ...
	      C_SET_RID_SPL_CODE (id, RID_SPL_CXX_COMPAT_WARN);
...
c_lex_one_token (c_parser *parser, c_token *token, bool raw = false)
{
  ...
  switch (token->type)
	...
	if (C_IS_RESERVED_WORD (token->value))
	  {
	    enum rid rid_code = C_RID_SPL_CODE (token->value);

	    if (rid_code == RID_SPL_CXX_COMPAT_WARN)
	      {
	    ...
	    else if (rid_code >= RID_SPL_FIRST_ADDR_SPACE
		     && rid_code <= RID_SPL_LAST_ADDR_SPACE)
	      {
		addr_space_t as;
		as = (addr_space_t) (rid_code - RID_SPL_FIRST_ADDR_SPACE);
    ...
    case CPP_AT_NAME:
      ...
      token->type = CPP_KEYWORD;
      switch (C_RID_SPL_CODE (token->value))
	{
	  /* Replace 'class' with '@class', 'private' with '@private',
	     ...
	     reported as RID_SPL_AT_CLASS which is consistent with
	     '@synchronized', which is reported as
	     RID_SPL_AT_SYNCHRONIZED.
	  */
	case RID_SPL_CLASS:     token->keyword = RID_SPL_AT_CLASS; break;
	case RID_SPL_PRIVATE:   token->keyword = RID_SPL_AT_PRIVATE; break;
	case RID_SPL_PROTECTED: token->keyword = RID_SPL_AT_PROTECTED; break;
	case RID_SPL_PUBLIC:    token->keyword = RID_SPL_AT_PUBLIC; break;
	case RID_SPL_THROW:     token->keyword = RID_SPL_AT_THROW; break;
	case RID_SPL_TRY:       token->keyword = RID_SPL_AT_TRY; break;
	case RID_SPL_CATCH:     token->keyword = RID_SPL_AT_CATCH; break;
	case RID_SPL_SYNCHRONIZED: token->keyword = RID_SPL_AT_SYNCHRONIZED; break;
	default:            token->keyword = C_RID_SPL_CODE (token->value);
	}
      break;
    case CPP_COLON:
    case CPP_COMMA:
...
c_keyword_starts_typename (enum rid keyword)
{
  switch (keyword)
    ...
    case RID_SPL_UNION:
    case RID_SPL_TYPEOF:
    case RID_SPL_TYPEOF_UNQUAL:
    case RID_SPL_CONST:
    case RID_SPL_ATOMIC:
    case RID_SPL_VOLATILE:
    case RID_SPL_RESTRICT:
    case RID_SPL_ATTRIBUTE:
    case RID_SPL_FRACT:
    case RID_SPL_ACCUM:
    case RID_SPL_SAT:
    case RID_SPL_AUTO_TYPE:
    case RID_SPL_ALIGNAS:
      return true;
    default:
...
c_token_starts_declspecs (c_token *token)
{
  switch (token->type)
	...
	case RID_SPL_CONSTEXPR:
	  return true;
	default:
	  if (token->keyword >= RID_SPL_FIRST_INT_N
	      && token->keyword < RID_SPL_FIRST_INT_N + NUM_INT_N_ENTS
	      && int_n_enabled_p[token->keyword - RID_SPL_FIRST_INT_N])
	    return true;
	  return false;
	}
    case CPP_LESS:
...
c_token_starts_declaration (c_token *token)
{
  if (c_token_starts_declspecs (token)
      || token->keyword == RID_SPL_STATIC_ASSERT)
    return true;
  else
    return false;
}
...
c_parser_external_declaration (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_SPL_ASM:
	  ...
	  break;
	case RID_SPL_AT_INTERFACE:
	case RID_SPL_AT_IMPLEMENTATION:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_class_definition (parser, NULL_TREE);
	  break;
	case RID_SPL_AT_CLASS:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_class_declaration (parser);
	  break;
	case RID_SPL_AT_ALIAS:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_alias_declaration (parser);
	  break;
	case RID_SPL_AT_PROTOCOL:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_protocol_definition (parser, NULL_TREE);
	  break;
	case RID_SPL_AT_PROPERTY:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_at_property_declaration (parser);
	  break;
	case RID_SPL_AT_SYNTHESIZE:
	  gcc_assert (c_dialect_objc ());
	  c_parser_objc_at_synthesize_declaration (parser);
	  break;
	case RID_SPL_AT_DYNAMIC:
	  gcc_assert (c_dialect_objc ());
			       ...
			       bool *fallthru_attr_p /* = NULL */)
{
  ...
  if (static_assert_ok
      && c_parser_next_token_is_keyword (parser, RID_SPL_STATIC_ASSERT))
    {
  ...
  if (c_parser_peek_token (parser)->type == CPP_NAME
      ...
      else
	{
      ...
      c_parser_peek_token (parser)->keyword = RID_SPL_VOID;
  ...
  while (true)
      ...
      else if (specs->constexpr_p)
	{
	  ...
	  || c_parser_next_token_is_keyword (parser, RID_SPL_ASM)
	  || c_parser_next_token_is_keyword (parser, RID_SPL_ATTRIBUTE)
	  || c_parser_next_token_is_keyword (parser, RID_SPL_IN))
	{
	  ...
	  /* Having seen a data definition, there cannot now be a
	     function definition.  */
	  fndef_ok = false;
	  if (c_parser_next_token_is_keyword (parser, RID_SPL_ASM))
	    asm_name = c_parser_simple_asm_expr (parser);
	  if (c_parser_next_token_is_keyword (parser, RID_SPL_ATTRIBUTE))
	    {
	  ...
	  if (c_parser_next_token_is (parser, CPP_EQ))
		...
		finish_decl (d, UNKNOWN_LOCATION, NULL_TREE,
	      ...
	      if (c_parser_next_token_is_keyword (parser, RID_SPL_IN))
		{
		  if (d)
	  ...
	  else if (c_parser_next_token_is_keyword (parser, RID_SPL_IN))
	    {
      ...
      if (!start_function (specs, declarator, all_prefix_attrs))
	{
	  /* At this point we've consumed:
	     ...
	     RID_SPL_ASM, RID_SPL_ATTRIBUTE, or RID_SPL_IN,
	     but the
	       declaration-specifiers declarator
...
c_parser_static_assert_declaration_no_semi (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_STATIC_ASSERT));
  ...
  while (c_parser_next_token_is (parser, CPP_NAME)
	 ...
	 || (c_dialect_objc () && c_parser_next_token_is (parser, CPP_LESS)))
    {
	 ...
	 has simply forgotten a semicolon, so we exit.  */
	  ...
	  && !c_parser_next_token_is_keyword (parser, RID_SPL_ALIGNAS))
	break;
      ...
      if (c_parser_next_token_is (parser, CPP_NAME))
	{
	  ...
	  if (kind == C_ID_ADDRSPACE)
	    {
	      addr_space_t as
		= name_token->keyword - RID_SPL_FIRST_ADDR_SPACE;
...
c_parser_enum_specifier (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_ENUM));
  c_parser_consume_token (parser);
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
    {
	 ...
	 e.g. foo(sizeof(struct{ @defs(ClassName) }));.  */
      if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_DEFS))
	{
	  tree name;
	  gcc_assert (c_dialect_objc ());
	  c_parser_consume_token (parser);
...
c_parser_struct_declaration (c_parser *parser)
{
  ...
  tree decls;
  location_t decl_loc;
  if (c_parser_next_token_is_keyword (parser, RID_SPL_EXTENSION))
    {
      int ext;
      tree decl;
      ext = disable_extension_diagnostics ();
  ...
  if (c_parser_next_token_is_keyword (parser, RID_SPL_STATIC_ASSERT))
    {
      c_parser_static_assert_declaration_no_semi (parser);
      return NULL_TREE;
    }
...
c_parser_typeof_specifier (c_parser *parser)
{
  ...
  ret.expr_const_operands = true;
  ret.has_enum_type_specifier = false;
  if (c_parser_next_token_is_keyword (parser, RID_SPL_TYPEOF))
    {
      is_unqual = false;
      tree spelling = c_parser_peek_token (parser)->value;
      is_std = (flag_isoc2x
    ...
    {
      gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_TYPEOF_UNQUAL));
      is_unqual = true;
      is_std = true;
    }
  c_parser_consume_token (parser);
...
c_parser_alignas_specifier (c_parser * parser)
{
  ...
  location_t loc = c_parser_peek_token (parser)->location;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_ALIGNAS));
  tree spelling = c_parser_peek_token (parser)->value;
  c_parser_consume_token (parser);
  if (flag_isoc99)
    pedwarn_c99 (loc, OPT_Wpedantic,
				  ...
				  struct c_declarator *inner)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE)
      && !c_parser_nth_token_starts_std_attributes (parser, 1))
    {
      ...
      static_seen = c_parser_next_token_is_keyword (parser, RID_SPL_STATIC);
      if (static_seen)
	c_parser_consume_token (parser);
...
{
  tree str;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_ASM));
  c_parser_consume_token (parser);
  matching_parens parens;
  if (!parens.require_open (parser))
    return NULL_TREE;
...
c_parser_gnu_attribute_any_word (c_parser *parser)
{
  ...
  if (c_parser_next_token_is (parser, CPP_KEYWORD))
	...
	case RID_SPL_ACCUM:
	case RID_SPL_SAT:
	case RID_SPL_TRANSACTION_ATOMIC:
	case RID_SPL_TRANSACTION_CANCEL:
	case RID_SPL_ATOMIC:
	case RID_SPL_AUTO_TYPE:
	case RID_SPL_CONSTEXPR:
	case RID_SPL_INT_N_0:
...
{
  tree attrs = NULL_TREE;
  while (c_parser_next_token_is_keyword (parser, RID_SPL_ATTRIBUTE))
    {
...
c_parser_compound_statement_nostart (c_parser *parser)
{
  ...
  mark_valid_location_for_stdc_pragma (true);
  if (c_parser_next_token_is_keyword (parser, RID_SPL_LABEL))
    {
      /* Read zero or more forward-declarations for labels that nested
	 functions can jump to.  */
      mark_valid_location_for_stdc_pragma (false);
      while (c_parser_next_token_is_keyword (parser, RID_SPL_LABEL))
	{
  ...
  while (c_parser_next_token_is_not (parser, CPP_CLOSE_BRACE))
	...
	= c_parser_nth_token_starts_std_attributes (parser, 1);
      ...
      if (have_std_attrs)
	std_attrs = c_parser_std_attribute_specifier_sequence (parser);
      if (c_parser_next_token_is_keyword (parser, RID_SPL_CASE)
	  || c_parser_next_token_is_keyword (parser, RID_SPL_DEFAULT)
	  || (c_parser_next_token_is (parser, CPP_NAME)
	      && c_parser_peek_2nd_token (parser)->type == CPP_COLON))
	{
	  if (c_parser_next_token_is_keyword (parser, RID_SPL_CASE))
		   ...
		   && c_parser_next_token_is (parser, CPP_SEMICOLON)))
	{
      ...
      else if (c_parser_next_token_is_keyword (parser, RID_SPL_EXTENSION))
	{
      ...
      else if (c_parser_next_token_is (parser, CPP_EOF))
	{
      ...
      else if (c_parser_next_token_is_keyword (parser, RID_SPL_ELSE))
        {
...
c_parser_all_labels (c_parser *parser)
{
  ...
  while (c_parser_next_token_is_keyword (parser, RID_SPL_CASE)
	 || c_parser_next_token_is_keyword (parser, RID_SPL_DEFAULT)
	 || (c_parser_next_token_is (parser, CPP_NAME)
	     && c_parser_peek_2nd_token (parser)->type == CPP_COLON))
    {
...
c_parser_label (c_parser *parser, tree std_attrs)
{
  ...
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_DEFAULT))
    {
      c_parser_consume_token (parser);
      if (c_parser_require (parser, CPP_COLON, "expected %<:%>"))
 ...
 restart:
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_SPL_ASM:
	  ...
	  break;
	case RID_SPL_TRANSACTION_ATOMIC:
	case RID_SPL_TRANSACTION_RELAXED:
  ...
  if (c_parser_next_token_is (parser, CPP_SEMICOLON))
    {
      ...
      c_parser_consume_token (parser);
      if (!c_parser_next_token_is_keyword (parser, RID_SPL_ELSE))
		    ...
		    vec<tree> *chain)
{
  ...
  if (body_loc_after_labels != UNKNOWN_LOCATION
      && next_tinfo.type != CPP_SEMICOLON)
    warn_for_multistatement_macros (body_loc_after_labels, next_tinfo.location,
				    else_tinfo.location, RID_SPL_ELSE);
...
c_parser_if_statement (c_parser *parser, bool *if_p, vec<tree> *chain)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_IF));
  ...
  if (c_parser_next_token_is_keyword (parser, RID_SPL_ELSE))
	...
	= get_token_indent_info (c_parser_peek_token (parser));
      ...
      if (warn_duplicated_cond)
	{
	  if (c_parser_next_token_is_keyword (parser, RID_SPL_IF)
	      && chain == NULL)
	    {
	  ...
	  else if (!c_parser_next_token_is_keyword (parser, RID_SPL_IF))
...
c_parser_switch_statement (c_parser *parser, bool *if_p)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_SWITCH));
  c_parser_consume_token (parser);
  block = c_begin_compound_stmt (flag_isoc99);
			  ...
			  bool *if_p)
{
  ...
  unsigned char save_in_statement;
  location_t loc;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_WHILE));
  ...
  if (next_tinfo.type != CPP_SEMICOLON && !open_brace)
    warn_for_multistatement_macros (loc_after_labels, next_tinfo.location,
				    while_tinfo.location, RID_SPL_WHILE);
...
c_parser_do_statement (c_parser *parser, bool ivdep, unsigned short unroll)
{
  ...
  unsigned char save_in_statement;
  location_t loc;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_DO));
  c_parser_consume_token (parser);
  if (c_parser_next_token_is (parser, CPP_SEMICOLON))
    warning_at (c_parser_peek_token (parser)->location,
		OPT_Wempty_body,
  ...
  c_parser_require_keyword (parser, RID_SPL_WHILE, "expected %<while%>");
  in_statement = save_in_statement;
  cond = c_parser_paren_condition (parser);
  if (ivdep && cond != error_mark_node)
    cond = build3 (ANNOTATE_EXPR, TREE_TYPE (cond), cond,
			...
			bool *if_p)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_FOR));
  ...
  if (next_tinfo.type != CPP_SEMICOLON && !open_brace)
    warn_for_multistatement_macros (loc_after_labels, next_tinfo.location,
				    for_tinfo.location, RID_SPL_FOR);
...
#define POP								      \
  do {									      \
	...
	&& (omp_atomic_lhs == void_list_node				      \
	    ...
	    || (stack[sp].op == EQ_EXPR					      \
		&& c_parser_peek_2nd_token (parser)->keyword == RID_SPL_IF)))     \
      {									      \
...
c_parser_unary_expression (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_SPL_EXTENSION:
	  ...
	  restore_extension_diagnostics (ext);
	  return ret;
	case RID_SPL_REALPART:
	  c_parser_consume_token (parser);
	  exp_loc = c_parser_peek_token (parser)->location;
	  op = c_parser_cast_expression (parser, NULL);
	  op = default_function_array_conversion (exp_loc, op);
	  return parser_build_unary_op (op_loc, REALPART_EXPR, op);
	case RID_SPL_IMAGPART:
	  c_parser_consume_token (parser);
	  exp_loc = c_parser_peek_token (parser)->location;
	  op = c_parser_cast_expression (parser, NULL);
	  op = default_function_array_conversion (exp_loc, op);
...
c_parser_sizeof_expression (c_parser *parser)
{
  ...
  location_t expr_loc;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_SIZEOF));
...
c_parser_alignof_expression (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_ALIGNOF));
...
c_parser_generic_selection (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_GENERIC));
  ...
  while (1)
    {
      ...
      if (token->type == CPP_KEYWORD && token->keyword == RID_SPL_DEFAULT)
	{
	  c_parser_consume_token (parser);
...
c_parser_predefined_identifier (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->keyword)
    {
    case RID_SPL_FUNCTION_NAME:
      pedwarn (loc, OPT_Wpedantic, "ISO C does not support %qs predefined "
	       "identifier", "__FUNCTION__");
      break;
    case RID_SPL_PRETTY_FUNCTION_NAME:
      pedwarn (loc, OPT_Wpedantic, "ISO C does not support %qs predefined "
	       "identifier", "__PRETTY_FUNCTION__");
      break;
    case RID_SPL_C99_FUNCTION_NAME:
...
c_parser_postfix_expression (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_SPL_BUILTIN_CALL_WITH_STATIC_CHAIN:
	  {
	    vec<c_expr_t, va_gc> *cexpr_list;
	    c_expr_t *e2_p;
	    tree chain_value;
	...
	case RID_SPL_AT_ENCODE:
	  ...
	  break;
	case RID_SPL_GENERIC:
	  expr = c_parser_generic_selection (parser);
	  break;
	case RID_SPL_OMP_ALL_MEMORY:
...
c_parser_objc_class_definition (c_parser *parser, tree attributes)
{
  ...
  tree id1;
  tree superclass;
  if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_INTERFACE))
    iface_p = true;
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_IMPLEMENTATION))
    iface_p = false;
  else
    gcc_unreachable ();
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN))
	...
	proto = c_parser_objc_protocol_refs (parser);
      ...
      c_parser_require_keyword (parser, RID_SPL_AT_END, "expected %<@end%>");
...
c_parser_objc_class_instance_variables (c_parser *parser)
{
  ...
  while (c_parser_next_token_is_not (parser, CPP_EOF))
      ...
      if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
	{
      ...
      if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PRIVATE))
	{
	  c_parser_consume_token (parser);
	  objc_set_visibility (OBJC_IVAR_VIS_PRIVATE);
	  continue;
	}
      else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PROTECTED))
	{
	  c_parser_consume_token (parser);
	  objc_set_visibility (OBJC_IVAR_VIS_PROTECTED);
	  continue;
	}
      else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PUBLIC))
	{
	  c_parser_consume_token (parser);
	  objc_set_visibility (OBJC_IVAR_VIS_PUBLIC);
	  continue;
	}
      else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PACKAGE))
	{
	  c_parser_consume_token (parser);
...
{
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_CLASS));
  c_parser_consume_token (parser);
...
{
  tree id1, id2;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_ALIAS));
  c_parser_consume_token (parser);
  if (c_parser_next_token_is_not (parser, CPP_NAME))
    {
      c_parser_error (parser, "expected identifier");
...
{
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PROTOCOL));
...
c_parser_objc_methodprotolist (c_parser *parser)
{
  while (true)
	...
	case CPP_EOF:
	  return;
	default:
	  if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_END))
	    return;
	  else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PROPERTY))
	    c_parser_objc_at_property_declaration (parser);
	  else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_OPTIONAL))
	    {
	      objc_set_method_opt (true);
	      c_parser_consume_token (parser);
	    }
	  else if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_REQUIRED))
	    {
  ...
  while (true)
    {
	  ...
	  && (token->keyword == RID_SPL_IN
	      || token->keyword == RID_SPL_OUT
	      || token->keyword == RID_SPL_INOUT
	      || token->keyword == RID_SPL_BYCOPY
	      || token->keyword == RID_SPL_BYREF
	      || token->keyword == RID_SPL_ONEWAY))
	{
...
c_parser_objc_try_catch_finally_statement (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_TRY));
  c_parser_consume_token (parser);
  ...
  while (c_parser_next_token_is_keyword (parser, RID_SPL_AT_CATCH))
    {
      struct c_parm *parm;
  ...
  if (c_parser_next_token_is_keyword (parser, RID_SPL_AT_FINALLY))
    {
      c_parser_consume_token (parser);
      location = c_parser_peek_token (parser)->location;
      stmt = c_parser_compound_statement (parser);
...
c_parser_objc_synchronized_statement (c_parser *parser)
{
  location_t loc;
  tree expr, stmt;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_SYNCHRONIZED));
  c_parser_consume_token (parser);
  loc = c_parser_peek_token (parser)->location;
...
{
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_PROPERTY));
  location_t loc = c_parser_peek_token (parser)->location;
	...
	while (true)
	  {
	    ...
	    tree attr_name = NULL_TREE;
	    enum rid keyword = RID_SPL_MAX; /* Not a valid property attribute.  */
	    bool add_at = false;
	    if (token->type == CPP_KEYWORD)
	      {
...
c_parser_objc_at_synthesize_declaration (c_parser *parser)
{
  tree list = NULL_TREE;
  location_t loc;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_SYNTHESIZE));
  loc = c_parser_peek_token (parser)->location;
...
c_parser_objc_at_dynamic_declaration (c_parser *parser)
{
  tree list = NULL_TREE;
  location_t loc;
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_AT_DYNAMIC));
  loc = c_parser_peek_token (parser)->location;
...
c_parser_pragma (c_parser *parser, enum pragma_context context, bool *if_p)
{
  ...
  switch (id)
	...
	if (c_parser_peek_token (parser)->pragma_kind == PRAGMA_UNROLL)
	  ...
	  unroll = 0;
	if (!c_parser_next_token_is_keyword (parser, RID_SPL_FOR)
	    && !c_parser_next_token_is_keyword (parser, RID_SPL_WHILE)
	    && !c_parser_next_token_is_keyword (parser, RID_SPL_DO))
	  {
	...
	else if (c_parser_next_token_is_keyword (parser, RID_SPL_WHILE))
...
c_parser_omp_clause_name (c_parser *parser)
{
  ...
  if (c_parser_next_token_is_keyword (parser, RID_SPL_AUTO))
    result = PRAGMA_OACC_CLAUSE_AUTO;
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_IF))
    result = PRAGMA_OMP_CLAUSE_IF;
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_DEFAULT))
    result = PRAGMA_OMP_CLAUSE_DEFAULT;
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_FOR))
  ...
  while (1)
	  ...
	  if (c_parser_next_token_is_not (parser, CPP_NAME)
	      || c_parser_peek_token (parser)->id_kind != C_ID_ID)
	    {
		  ...
		  && (c_parser_peek_2nd_token (parser)->type == CPP_COMMA
			  ...
			  == CPP_CLOSE_PAREN)))
		{
		  expr.value = ridpointers[RID_SPL_OMP_ALL_MEMORY];
		  c_parser_consume_token (parser);
		}
	      else
		expr = c_parser_expr_no_commas (parser, NULL);
	  ...
	  && c_parser_peek_token (parser)->id_kind == C_ID_ID)
	{
	  ...
	  c_parser_consume_token (parser);
	       ...
	       && (c_parser_peek_token (parser)->keyword == RID_SPL_FUNCTION_NAME
		   || (c_parser_peek_token (parser)->keyword
		       == RID_SPL_PRETTY_FUNCTION_NAME)
		   || (c_parser_peek_token (parser)->keyword
		       == RID_SPL_C99_FUNCTION_NAME)))
...
c_parser_omp_clause_defaultmap (c_parser *parser, tree list)
{
  ...
  if (!parens.require_open (parser))
    return list;
  if (c_parser_next_token_is_keyword (parser, RID_SPL_DEFAULT))
    p = "default";
  else if (!c_parser_next_token_is (parser, CPP_NAME))
    {
    invalid_behavior:
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN))
      ...
      do
	{
	  ...
	  if (kind == OMP_CLAUSE_GANG
	      && c_parser_next_token_is_keyword (parser, RID_SPL_STATIC))
	    {
	      c_parser_consume_token (parser);
...
c_parser_omp_clause_schedule (c_parser *parser, tree list)
{
  ...
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_STATIC))
    OMP_CLAUSE_SCHEDULE_KIND (c) = OMP_CLAUSE_SCHEDULE_STATIC;
  else if (c_parser_next_token_is_keyword (parser, RID_SPL_AUTO))
...
c_parser_omp_clause_dist_schedule (c_parser *parser, tree list)
{
  ...
  if (!c_parser_next_token_is_keyword (parser, RID_SPL_STATIC))
    {
...
c_parser_oacc_routine (c_parser *parser, enum pragma_context context)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_PAREN))
	  ...
	  || c_parser_next_token_is_keyword (parser, RID_SPL_STATIC_ASSERT))
	{
      ...
      if (c_parser_next_token_is (parser, CPP_KEYWORD)
	  && c_parser_peek_token (parser)->keyword == RID_SPL_EXTENSION)
	{
  ...
  switch (code)
      ...
      if (c_parser_next_token_is (parser, CPP_OPEN_BRACE))
	{
	  ...
	  structured_block = true;
	       ...
	       && c_parser_next_token_is_keyword (parser, RID_SPL_IF))
	break;
      else
	{
	  v = c_parser_cast_expression (parser, NULL).value;
	  ...
	  if (!c_parser_require (parser, CPP_EQ, "expected %<=%>"))
	    goto saw_error;
	  if (compare && c_parser_next_token_is_keyword (parser, RID_SPL_IF))
	    {
	      eloc = c_parser_peek_token (parser)->location;
...
restart:
  if (compare && c_parser_next_token_is_keyword (parser, RID_SPL_IF))
	...
	{
	  c_parser_require_keyword (parser, RID_SPL_ELSE, "expected %<else%>");
	  goto saw_error;
	}
...
c_parser_transaction (c_parser *parser, enum rid keyword)
{
  ...
  gcc_assert ((keyword == RID_SPL_TRANSACTION_ATOMIC
      || keyword == RID_SPL_TRANSACTION_RELAXED)
      && c_parser_next_token_is_keyword (parser, keyword));
  c_parser_consume_token (parser);

  if (keyword == RID_SPL_TRANSACTION_RELAXED)
  ...
  if (flag_tm)
    ...
    error_at (loc, (keyword == RID_SPL_TRANSACTION_ATOMIC ?
...
c_parser_transaction_cancel (c_parser *parser)
{
  ...
  gcc_assert (c_parser_next_token_is_keyword (parser, RID_SPL_TRANSACTION_CANCEL));
  c_parser_consume_token (parser);
