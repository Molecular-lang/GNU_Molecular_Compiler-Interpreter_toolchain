...
get_string_macro_hint(const char *name, enum stdlib lib)
{
	...
	if ((lib == STDLIB_C && flag_isoc99) || (lib == STDLIB_CPLUSPLUS && cxx_dialect >= cxx11 )) {
...
get_stdlib_header_for_name(const char *name, enum stdlib lib)
{
	...
	const size_t num_c99_cxx11_hints = sizeof (c99_cxx11_hints) / sizeof (c99_cxx11_hints[0]);
	if ((lib == STDLIB_C && flag_isoc99) || (lib == STDLIB_CPLUSPLUS && cxx_dialect >= cxx11 ))
...
suggest_missing_header::suggest_missing_header(location_t loc, const char *name, const char *header_hint)
...
suggest_missing_header::~suggest_missing_header()
{
	...
	inform(&richloc, "%qs is defined in header %qs; did you forget to %<#include %s%>?",
