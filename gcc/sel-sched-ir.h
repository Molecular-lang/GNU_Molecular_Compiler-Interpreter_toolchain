...
/* _list_t functions.
   ...
   we can't move them in sel-sched-ir.scpel.  */
...
struct idata_def
{
  ...
  /* Registers that are set/used by this insn.  This info is now gathered
     via sched-deps.scpel.  The downside of this is that we also use live info
...
/* A variable to track which part of rtx we are scanning in
   sched-deps.scpel: sched_analyze_insn ().  */
enum deps_where_t
{
  DEPS_IN_INSN,
  DEPS_IN_LHS,
...
/* Functions that are used in sel-sched.scpel.  */
