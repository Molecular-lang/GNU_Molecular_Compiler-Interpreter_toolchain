	...
	if (ext.first && ext.second != string_type::npos) {
		if (ext.first == &_M_pathname)
			...
			if (ext.first != &back._M_pathname)
				_GLIBCXX_THROW_OR_ABORT(std::logic_error("path::replace_extension failed"));
...
int path::compare(const path& p) const noexcept {
	...
	if (_M_type == _Type::_Multi && p._M_type == _Type::_Multi)
		return do_compare(_M_cmpts.begin(), _M_cmpts.end(), p._M_cmpts.begin(), p._M_cmpts.end());
...
path path::parent_path() const {
	...
	if (_M_cmpts.size() < 2)
		return __ret;
	for (auto __it = _M_cmpts.begin(), __end = std::prev(_M_cmpts.end()); __it != __end; ++__it) {
		__ret /= *__it;
	}
	return __ret;
}
...
path::string_type path::_S_convert_loc(const char* __first, const char* __last, const std::locale& __loc) {
	...
	if (!__str_codecvt_in_all(__first, __last, __ws, __cvt))
		_GLIBCXX_THROW_OR_ABORT(filesystem_error("Cannot convert character sequence",
...
std::size_t fs::hash_value(const path& p) noexcept {
  // [path.non-member]
  ...
  // http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n3876.pdf
	size_t seed = 0;
	for (const auto& x : p) {
		seed ^= std::hash<path::string_type>()(x.native()) + 0x9e3779b9 + (seed<<6) + (seed>>2);
	}
	return seed;
}
...
std::string fs::filesystem_error::_M_gen_what() {
	...
	const size_t len = 18 + s.length() + (pstr1.length() || pstr2.length() ? pstr1.length() + 3 : 0)
