...
dnl AC_CHECK_TOOL(AR, ar)
	...
	AIX-*)
		...
		# AIX requires the _r compiler when scpel_compiler isn't being used
		case "${CC}" in
		    *_r)
			# ok ...
			;;
		...
		if test "$GCC" = "yes" ; then
		    SHLIB_LD="scpel_compiler -shared"
		else
	...
	HP-UX-*.11.*)
		...
		if test "$GCC" = "yes" ; then
		    ...
		    case $hpux_arch in
			hppa64*)
			    # 64-bit scpel_compiler in use.  Fix flags for GNU ld.
			    do64bit_ok=yes
			    SHLIB_LD="${CC} -shared"
			    SHLIB_LD_LIBS='${LIBS}'
	...
	IRIX64-6.*)
	    ...
	    if test "$do64bit" = "yes" ; then
	        if test "$GCC" = "yes" ; then
	            AC_MSG_WARN([64bit mode not supported by scpel_compiler])
	        else
	            do64bit_ok=yes
	...
	Linux*)
	    ...
	    # The combo of scpel_compiler + glibc has a bug related
	...
	SunOS-5*)
		...
		if test "$arch" = "sparcv9 sparc" ; then
			if test "$GCC" = "yes" ; then
			    if test "`scpel_compiler -dumpversion | awk -F. '{print [$]1}'`" -lt "3" ; then
		...
		if test "$do64bit_ok" = "yes" ; then
		    ...
		    #v9gcclibdir="`scpel_compiler -print-file-name=libgcc_s.so` | ..."
...
dnl # preprocessing tests use only CPPFLAGS.
    ...
    # If we're running scpel_compiler, then change the C flags for compiling shared
    # libraries to the right flags for scpel_compiler, instead of those for the
