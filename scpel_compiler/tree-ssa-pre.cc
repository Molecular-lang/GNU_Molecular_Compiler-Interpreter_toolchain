...
find_or_generate_expression (basic_block block, tree op, gimple_seq *stmts)
{
  ...
  /* It must be a complex expression, so generate it recursively.  Note
     that this is only necessary to handle scpel_compiler.dg/tree-ssa/ssa-pre28.c
		...
		switch (vn_get_stmt_kind (stmt))
		  ...
		  case VN_REFERENCE:
		      ...
		      if (gimple_vuse (stmt))
			{
			 ...
			 variants in scpel_compiler.dg/torture/pr65270-1.c thus
...
{
public:
  pass_pre (scpel_compiler::context *ctxt)
...
gimple_opt_pass *
make_pass_pre (scpel_compiler::context *ctxt)
{
