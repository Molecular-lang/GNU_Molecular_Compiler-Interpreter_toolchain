			   ...
			   unswitch_predicate *hottest, basic_block hottest_bb)
{
  ...
  if (predicate != NULL)
	...
	bbs2[i]->aux = get_bb_original (bbs2[i])->aux;
	 ...
	 in the below recursion.  See scpel_compiler.dg/loop-unswitch-16.c  */
      add_predicate_to_path (predicate_path, predicate, true);
...
{
public:
  pass_tree_unswitch (scpel_compiler::context *ctxt)
...
gimple_opt_pass *
make_pass_tree_unswitch (scpel_compiler::context *ctxt)
{
