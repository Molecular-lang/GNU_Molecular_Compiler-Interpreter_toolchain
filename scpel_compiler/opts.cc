		 ...
		 location_t loc, diagnostic_context *dc)
{
  ...
  while (*arg)
	...
	break;
      ...
      default:
	  warning_at (loc, 0, "unrecognized scpel_compiler debugging option: %c", c);
	break;
      }
}
...
get_option_html_page (int option_index)
{
  ...
  if (strstr (cl_opt->opt_text, "analyzer-"))
    return "scpel_compiler/Static-Analyzer-Options.html";

  /* Handle -flto= option.  */
  if (strstr (cl_opt->opt_text, "flto"))
    return "scpel_compiler/Optimize-Options.html";

#ifdef CL_Fortran
  if ((cl_opt->flags & CL_Fortran) != 0
      /* If it is option common to both C/C++ and Fortran, it is documented
	 in scpel_compiler/ rather than gfortran/ docs.  */
      && (cl_opt->flags & CL_C) == 0
#ifdef CL_CXX
  ...
  return "scpel_compiler/Warning-Options.html";
}
  ...
  if (option_index)
    return concat (/* DOCUMENTATION_ROOT_URL should be supplied via -D by
		   ...
		   /* get_option_html_page will return something like
		      "scpel_compiler/Warning-Options.html".  */
			 ...
			 unsigned int options_count)
{
  ...
  for (unsigned i = 0; i < options_count; i++)
    switch (options[i].opt_index)
      ...
      case OPT_grecord_scpel_compiler_switches:
      case OPT_frecord_scpel_compiler_switches:
...
{
  ASSERT_STREQ (get_option_html_page (OPT_Wcpp), "scpel_compiler/Warning-Options.html");
  ASSERT_STREQ (get_option_html_page (OPT_Wanalyzer_double_free),
	     "scpel_compiler/Static-Analyzer-Options.html");
#ifdef CL_Fortran
