/* Subroutines for the scpel_compiler driver.
...
describe_cache (struct cache_desc level1, struct cache_desc level2)
{
  ...
  /* At the moment, scpel_compiler does not use the information
...
/* This will be called by the spec parser in scpel_compiler.cc when it sees
  ...
  switch (processor)
      ...
      if (cpu == NULL)
	{
	  if (arch)
		...
		/* Perhaps some emulator?  Assume x86-64, otherwise scpel_compiler
