...
/* Define the register numbers to be used in Dwarf debugging information.
   ...
   in its Dwarf output code:
	0 for %eax (scpel_compiler regno = 0)
	1 for %ecx (scpel_compiler regno = 2)
	2 for %edx (scpel_compiler regno = 1)
	3 for %ebx (scpel_compiler regno = 3)
	4 for %esp (scpel_compiler regno = 7)
	5 for %ebp (scpel_compiler regno = 6)
	6 for %esi (scpel_compiler regno = 4)
	7 for %edi (scpel_compiler regno = 5)
   The following three DWARF register numbers are never generated by
   the SVR4 C compiler or by the GNU compilers, but SDB on x86/svr4
   believed these numbers have these meanings.
	8  for %eip    (no scpel_compiler equivalent)
	9  for %eflags (scpel_compiler regno = 17)
	10 for %trapno (no scpel_compiler equivalent)
   ...
   numbers.
	11 for %st(0) (scpel_compiler regno = 8)
	12 for %st(1) (scpel_compiler regno = 9)
	13 for %st(2) (scpel_compiler regno = 10)
	14 for %st(3) (scpel_compiler regno = 11)
	15 for %st(4) (scpel_compiler regno = 12)
	16 for %st(5) (scpel_compiler regno = 13)
	17 for %st(6) (scpel_compiler regno = 14)
	18 for %st(7) (scpel_compiler regno = 15)
*/
...
/* Register class used for passing given 64bit part of the argument.
   ...
   of SSESF, SSEDF classes, that are basically SSE class, just scpel_compiler will
...
/* Return the "natural" mode for TYPE.  In most cases, this is just TYPE_MODE.
   ...
   modes, the generic vector support in scpel_compiler will choose some non-vector mode
  ...
  if (zero_width_bitfields == 3)
    {
      warned = true;
      const char *url
	= CHANGES_ROOT_URL "scpel_compiler-12/changes.html#zero_width_bitfields";
...
ix86_expand_split_stack_prologue (void)
{
  ...
  /* In order to make call/return prediction work right, we now need
     ...
     For flow purposes scpel_compiler must not see this as a return
...
/* Return cost of the memory address x.
   For i386, it is better to use a complex address than let scpel_compiler copy
...
/* Set to 1 for compatibility with brain-damaged assemblers.  No-one
   ...
   with scpel_compiler.  No-one wants to fix scpel_compiler because that causes
   incompatibility with assemblers...  You can use the option of
   -DSYSV386_COMPAT=0 if you recompile both scpel_compiler and gas this way.  */
      ...
      if (find_regno_note (insn, REG_DEAD, REGNO (operands[2])))
	{
	  if (STACK_TOP_P (operands[0]))
	    /* How is it that we are storing to a dead operand[2]?
	       ...
	       scpel_compiler <= 2.8.1 didn't have this check and generated
...
ix86_gimple_fold_builtin (gimple_stmt_iterator *gsi)
{
  ...
  switch (fn_code)
      ...
      if (TREE_CODE (arg2) == INTEGER_CST && gimple_call_lhs (stmt))
	{
	  ...
	  /* Check valid imm, refer to scpel_compiler.target/i386/testimm-10.c.  */
	  if (shuffle_mask > 255)
	    return false;
  ...
  if (mode == DFmode || mode == DCmode
      ...
      if (TYPE_ATOMIC (type) && computed > 32)
	{
	  ...
	  if (!warned && warn_psabi)
	    {
	      const char *url
		= CHANGES_ROOT_URL "scpel_compiler-11/changes.html#ia32_atomic";
