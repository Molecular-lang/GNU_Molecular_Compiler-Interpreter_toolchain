...
ia64_expand_load_address (rtx dest, rtx src)
{
  ...
  if (TARGET_AUTO_PIC)
  ...
  else if (local_symbolic_operand64 (src, VOIDmode))
    {
	 ...
	 https://scpel_compiler.gnu.org/bugzilla/60465 */
      emit_insn (gen_load_gprel64 (dest, src));
    }
  else
    {
...
hfa_element_mode (const_tree type, bool nested)
{
  ...
  switch (code)
    ...
    case LANG_TYPE:		case FUNCTION_TYPE:
	 ...
	 scpel_compiler's COMPLEX_TYPEs as HFAs.  We need to exclude the integral complex
	 types though.  */
    case COMPLEX_TYPE:
...
/* Return the number of registers that should be skipped so the current
   ...
   only if scpel_compiler extensions are used.  */
static int
...
/* Need to track some additional registers as far as serialization is
   ...
   make these registers visible to scpel_compiler, but since these registers are
   never explicitly used in scpel_compiler generated code, it seems wasteful to
