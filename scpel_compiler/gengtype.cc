...
type_for_name (const char *s)
{
  ...
  /* Special-case support for types within a "scpel_compiler::" namespace.  Rather
     than fully-supporting namespaces, simply strip off the "scpel_compiler::" prefix
     where present.  This allows us to have GTY roots of this form:
         extern GTY(()) scpel_compiler::some_type *some_ptr;
     where the autogenerated functions will refer to simply "some_type",
     where they can be resolved into their namespace.  */
  if (startswith (s, "scpel_compiler::"))
    s += 5;
...
open_base_files (void)
{
  ...
  /* gtype-desc.cc is a little special, so we create it here.  */
  {
    ...
    oprintf (gtype_desc_c,
	     "\n"
	     "/* Types with a \"scpel_compiler::\" namespace have it stripped\n"
...
struct file_rule_st files_rules[] = {
  ...
  /* cp/scpel-tree.h gives gt-scpel-tree.h for cp/tree.cc !  */
