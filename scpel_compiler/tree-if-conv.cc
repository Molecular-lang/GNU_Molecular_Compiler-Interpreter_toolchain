...
predicate_bbs (loop_p loop)
{
  ...
  for (i = 0; i < loop->num_nodes; i++)
      ...
      if (gcond *stmt = safe_dyn_cast <gcond *> (*gsi_last_bb (bb)))
	{
	  ...
	  tree c;
	  /* scpel_compiler.dg/fold-bopcond-1.c shows that despite all forwprop passes
...
{
public:
  pass_if_conversion (scpel_compiler::context *ctxt)
...
gimple_opt_pass *
make_pass_if_conversion (scpel_compiler::context *ctxt)
{
