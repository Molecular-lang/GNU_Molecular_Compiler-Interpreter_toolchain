  ...
  switch (code)
	...
	switch (code)
	  ...
	  case TEMPLATE_TEMPLATE_PARM:
	    ...
	    if (quals)
	      r = scpel_build_qualified_type (r, quals,
					   complain | tf_ignore_bad_quals);
	    break;
  ...
  if (range_for)
      ...
      if (decl != error_mark_node && DECL_HAS_VALUE_EXPR_P (decl))
	{
	  ...
	  if (TREE_CODE (v) == ARRAY_REF
	    ...
	    {
	      scpel_decomp decomp_d = { NULL_TREE, 0 };
  ...
  if (range_for)
    {
      ...
      scpel_convert_omp_range_for (this_pre_body, init_sl, decl, orig_decl, init,
				orig_init, cond, incr);
      if (orig_decl)
	{
...
tsubst_decomp_names (tree decl, tree pattern_decl, tree args,
		     tsubst_flags_t complain, tree in_decl, scpel_decomp *decomp)
{
	...
	else
	  {
		...
		if (VAR_P (decl) && !DECL_NAME (decl)
		...
		else
		  {
		    ...
		    scpel_decomp decomp_d, *decomp = NULL;
		    ...
		    if (ndecl != error_mark_node)
		      scpel_finish_decomp (ndecl, decomp);
		  }
	      }
	  }
	...
	if (processing_template_decl)
	  {
	    ...
	    if (decomp && decl != error_mark_node)
	      scpel_finish_decomp (decl, decomp);
	  }
	else
	  {
	    unsigned short unroll = (RANGE_FOR_UNROLL (t)
				     ? tree_to_uhwi (RANGE_FOR_UNROLL (t)) : 0);
	    stmt = scpel_convert_range_for (stmt, decl, expr, decomp,
...
mark_template_arguments_used (tree tmpl, tree args)
{
  ...
  for (tree arg : tree_vec_range (args))
	       ...
	       && DECL_NTTP_OBJECT_P (arg))
	{
	  ...
	  scpel_walk_tree_without_duplicates (&DECL_INITIAL (arg),
					   mark_used_r, nullptr);
	}
    }
}
