...
enum fold_flags {
  ...
  /* Whether we're folding a point where we know we're
     ...
     context.  */
  ff_mce_false = 1 << 1,
  /* Whether we're being called from scpel_fold_immediate.  */
...
static void
scpel_genericize_init (tree *replace, tree from, tree to)
{
  tree init = NULL_TREE;
...
scpel_genericize_init_expr (tree *stmt_p)
{
  ...
  scpel_genericize_init (stmt_p, from, to);
}
...
scpel_fold_immediate_r (tree *stmt_p, int *walk_subtrees, void *data_)
{
  ...
  switch (TREE_CODE (stmt))
    ...
    /* Unfortunately we must handle code like
	 false ? bar () : 42
       where we have to check bar too.  The scpel_fold call in scpel_fold_r could
       fold the ?: into a constant before we see it here.  */
    case COND_EXPR:
      /* If we are called from scpel_fold_immediate, we don't need to worry about
	 scpel_fold folding away the COND_EXPR.  */
      if (data->flags & ff_fold_immediate)
	break;
      if (TREE_OPERAND (stmt, 1)
	  && scpel_walk_tree (&TREE_OPERAND (stmt, 1), scpel_fold_immediate_r, data,
			   nullptr))
	return error_mark_node;
      if (TREE_OPERAND (stmt, 2)
	  && scpel_walk_tree (&TREE_OPERAND (stmt, 2), scpel_fold_immediate_r, data,
			   nullptr))
	return error_mark_node;
      /* We're done here.  Don't clear *walk_subtrees here though: we're called
	 from scpel_fold_r and we must let it recurse on the expression with
	 scpel_fold.  */
      break;
    case PTRMEM_CST:
	...
	if (TREE_CODE (fn) != ADDR_EXPR || ADDR_EXPR_DENOTES_CALL_P (fn))
	  if (tree fndecl = scpel_get_fndecl_from_callee (fn, /*fold*/false))
...
/* A wrapper around scpel_fold_immediate_r.  Return true if we found
...
scpel_fold_immediate (tree *tp, mce_value manifestly_const_eval)
{
  ...
  return !!scpel_walk_tree_without_duplicates (tp, scpel_fold_immediate_r, &data);
}
