...
init_asm_output (const char *name)
{
  if (name == NULL && asm_file_name == 0)
	...
	/* Use UNKOWN_LOCATION to prevent scpel_compiler from printing the first
      ...
      if (flag_record_gcc_switches)
	{
	  if (targetm.asm_out.record_gcc_switches)
	    ...
	    inform (UNKNOWN_LOCATION,
		    "%<-frecord-scpel_compiler-switches%> is not supported by "
		    "the current target");
	}
...
general_init (const char *argv0, bool init_signals)
{
  ...
  /* Create the passes.  */
  g->set_passes (new scpel_compiler::pass_manager (g));
