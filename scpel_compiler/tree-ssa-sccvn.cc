  ...
  do
      ...
      else
	{
	  ...
	  if (TREE_CODE (ptr) != SSA_NAME
		 ...
		 See scpel_compiler.dg/tree-ssa/20040408-1.c for an example.  Can easily
	...
	mem_op->off = -1;
	 ...
	 scpel_compiler.c-torture/execute/strcmp-1.c  */
  ...
  if (*slot)
    {
	 ...
	 only though.  See scpel_compiler.dg/tree-ssa/pr87126.c for example.  */
      if (dump_file && (dump_flags & TDF_DETAILS)
  ...
  if (to == VN_TOP)
    {
	 ...
	 preserve that to optimize for example scpel_compiler.dg/tree-ssa/ssa-sccvn-2.c.
...
visit_phi (gimple *phi, bool *inserted, bool backedges_varying_p)
{
  ...
  /* If the value we want to use is flowing over the backedge and we
     ...
     See scpel_compiler.dg/torture/pr87176.c.  If the value is the same on a
...
eliminate_dom_walker::eliminate_stmt (basic_block b, gimple_stmt_iterator *gsi)
{
  ...
  if (lhs && TREE_CODE (lhs) == SSA_NAME
	 ...
	 variables since scpel_compiler does not guarantee a local variable will
  ...
  if (gimple_assign_single_p (stmt)
	  ...
	  || is_gimple_min_invariant (gimple_assign_rhs1 (stmt))))
    {
      ...
      /* ???  scpel_compiler.dg/torture/pr91445.c shows that we lookup a boolean
  ...
  while (!to_remove.is_empty ())
    {
	 ...
	 dead copies to be generated here (scpel_compiler.c-torture/execute/20060905-1.c
	    ...
	    bool do_region, bitmap exit_bbs, bool skip_phis)
{
  ...
  if (!skip_phis)
	...
	/* Eliminate */
	/* The interesting case is scpel_compiler.dg/tree-ssa/pr22230.c for correctness
	   how we handle backedges and availability.
	   And scpel_compiler.dg/tree-ssa/ssa-sccvn-2.c for optimization.  */
...
{
public:
  pass_fre (scpel_compiler::context *ctxt)
...
gimple_opt_pass *
make_pass_fre (scpel_compiler::context *ctxt)
{
