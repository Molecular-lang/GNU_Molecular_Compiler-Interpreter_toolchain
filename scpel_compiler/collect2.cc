...
const char *c_file_name;		/* pathname of scpel_compiler */
...
# endif
  ...
  if (c_file_name == 0)
    {
#ifdef CROSS_DIRECTORY_STRUCTURE
      c_file_name = concat (target_machine, "-scpel_compiler", NULL);
#else
      c_file_name = "scpel_compiler";
#endif
    }
  ...
  if (helpflag)
    {
      ...
      printf ("\n");
      printf ("Overview: https://scpel_compiler.gnu.org/onlinedocs/gccint/Collect2.html\n");
  ...
  *ld2 = (char*) 0;
  ...
  fork_execute ("scpel_compiler",  c_argv, at_file_supplied, "gcc_args");
#ifdef COLLECT_EXPORT_LIST
		...
		scanfilter filter)
{
  ...
  /* LTO objects must be in a known format.  This check prevents
     ...
     scpel_compiler cannot currently handle.  */
