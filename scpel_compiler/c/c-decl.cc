...
i_symbol_binding (tree node)
{
  ...
  if (lid->symbol_binding == NULL
      ...
      && !I_SYMBOL_CHECKED (node))
    {
	 ...
	 when the binding oracle calls back into scpel_compiler.  */
  ...
  for (parm = parmids; parm; parm = TREE_CHAIN (parm))
      ...
      else
	{
	  /* FIXME diagnostics: This should be the location of the argument,
	     ...
	     See scpel_compiler.dg/Wshadow-3.c for an example of this problem. */
	  ...
	  if (parm == NULL_TREE
	      ...
	      else
		{
		  /* FIXME diagnostics: This should be the location of
		     ...
		     outside of it (e.g., scpel_compiler.dg/pr15698-2.c).  In
