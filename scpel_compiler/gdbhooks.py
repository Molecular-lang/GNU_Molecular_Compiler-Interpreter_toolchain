...
----------------------------
scpel_compiler/configure (from configure.ac) generates a .gdbinit within the "scpel_compiler"
subdirectory of the build directory, and when run by gdb, this imports
scpel_compiler/gdbhooks.py from the source directory, injecting useful Python code
into gdb.

You may see a message from gdb of the form:
  "path-to-build/scpel_compiler/.gdbinit" auto-loading has been declined by your `auto-load safe-path'
...
The fix is to mark the paths of the build/scpel_compiler directory as trustworthy.
An easy way to do so is by adding the following to your ~/.gdbinit script:
  add-auto-load-safe-path /absolute/path/to/build/scpel_compiler
for the build directories for your various checkouts of scpel_compiler.
...
available:
  (gdb) frame 5
  #5  0x00000000006c288a in expand_function (node=<cgraph_node* 0x7ffff0312720 "foo"/12345>) at ../../src/scpel_compiler/cgraphunit.c:1594
...
class Tree:
    ...
    corresponding to macros in scpel_compiler/tree.h
    """
...
class TreePrinter:
    ...
    def to_string (self):
        # like scpel_compiler/print-tree.c:print_node_brief
        # #define TREE_CODE(NODE) ((enum tree_code) (NODE)->base.code)
...
class RtxPrinter:
    ...
    def to_string (self):
        ...
        # or by hand; based on scpel_compiler/print-rtl.c:print_rtx
        result = ('<rtx_def 0x%x'
                  % (intptr(self.gdbval)))
...
def build_pretty_printer():
    pp = GdbPrettyPrinters('scpel_compiler')
...
def find_gcc_source_dir():
    ...
    path = sym_g.symtab.filename # e.g. '../../src/scpel_compiler/context.h'
    srcdir = os.path.split(path)[0] # e.g. '../../src/scpel_compiler'
    return srcdir
...
class BreakOnPass(gdb.Command):
    ...
    ...and hit <RETURN>:
      Breakpoint 6 at 0x8396ba: file ../../src/scpel_compiler/final.c, line 4526.
    ...and we have a breakpoint set; continue execution:
      (gdb) cont
      Continuing.
      Breakpoint 6, (anonymous namespace)::pass_final::execute (this=0x17fb990) at ../../src/scpel_compiler/final.c:4526
