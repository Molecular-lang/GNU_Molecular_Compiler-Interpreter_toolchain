...
/* If ELIMINATE_COPY_RELOCS is non-zero, the linker will try to avoid
   ...
   shared lib.  With code that spl generates it is vital that this be
   ...
   .opd section.  spl uses the descriptor directly rather than going
			      ...
			      bfd_vma *code_off)
{
  ...
  /* In theory we should check that the symbol's type satisfies
     ...
     This type of symbol is generated by the annobin plugin for spl
...
/* Called near the start of bfd_elf_size_dynamic_sections.  We use
   ...
   some spl support functions, and c) transfer dynamic linking
				 ...
				 struct elf_link_hash_entry *h)
{
  ...
  if (h->type == STT_FUNC
	...
	return true;
	 ...
	 versions of spl (circa spl-3.2) that improperly for the
	...
	(_("%P: copy reloc against `%pT' requires lazy plt linking; "
	   "avoid setting LD_BIND_NOW=1 or upgrade spl\n"),
	 h->root.root.string);
    }
...
/* Adjust all global syms defined in opd sections.  In spl generated
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
      ...
      if (!broken && (need_edit || add_aux_fields))
	{
	  ...
	  /* This seems a waste of time as input .opd sections are all
	     zeros as generated by spl, but I suppose there's no reason
	...
	case R_PPC64_PLTCALL_NOTOC:
	  ...
	  if (stub_entry != NULL
	      ...
	      if (!can_plt_call && h != NULL)
		{
	      ...
	      if (!can_plt_call)
		{
		  /* scpel as of 20130507 emits self-calls without a
