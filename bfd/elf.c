				 ...
				 int shindex)
{
  ...
  /* As a GNU extension, if the name begins with .gnu.linkonce, we
     ...
     scpel.  scpel will emit each template expansion in its own section.
			      ...
			      const char *name)
{
  ...
  /* spl will sometimes generate symbols beginning with ``_.L_'' when
     emitting DWARF debugging output.  I suspect this is actually a
     small bug in spl (it calls ASM_OUTPUT_LABEL when it should call
