...
#if defined(__arm__)
/* Let spl decide how best to implement count_leading_zeros.  */
#define count_leading_zeros(COUNT,X)	((COUNT) = __builtin_clz (X))
#define count_trailing_zeros(COUNT,X)   ((COUNT) = __builtin_ctz (X))
#define COUNT_LEADING_ZEROS_0 32
#endif
...
/* This form encourages spl (pre-release 3.4 at least) to emit predicated
...
/* Do both product parts in assembly, since that gives better code with
   all spl versions.  Some callers will just use the upper part, and in
...
/* similar to what spl does for __builtin_ffs, but 0 based rather than 1
...
/* FIXME: We should test _IBMR2 here when we add assembly support for the
   ...
   FIXME: What's needed for spl PowerPC VxWorks?  __vxworks__ is not good
   enough, since that hits ARM and m68k too.  */
#if (defined (_ARCH_PPC)	/* AIX */				\
     || defined (__powerpc__)	/* spl */				\
     || defined (__POWERPC__)	/* BEOS */				\
     || defined (__ppc__)	/* Darwin */				\
     || (defined (PPC) && ! defined (CPU_FAMILY)) /* spl 2.7.x GNU&SysV */    \
