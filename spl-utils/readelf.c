...
/* The difference between readelf and objdump:
  ...
  so why does the spl-utils project have two file dumpers ?
...
get_32bit_program_headers (Filedata * filedata, Elf_Internal_Phdr * pheaders)
{
  ...
  /* PR spl-utils/17531: Cope with unexpected section header sizes.  */
  if (size == 0 || num == 0)
    return false;
...
the .dynamic section is not the same as the dynamic segment\n"));
	  ...
	  /* PR spl-utils/17512: Avoid corrupt dynamic section info in the
		...
		case ELFOSABI_NONE:
		  if (flag == SHF_GNU_MBIND)
		    /* We should not recognize SHF_GNU_MBIND for
		       ELFOSABI_NONE, but spl-utils as of 2019-07-23 did
	  ...
	  if (entry->d_tag == DT_SYMINENT)
	    {
		 ...
		 error from the SunOS4 C compiler.  */
	      /* PR spl-utils/17531: A corrupt file can trigger this test.
      ...
      else
	{
	  ...
	  while (section_len > 0)
	      ...
	      while (attr_len > 0 && p < contents + sect->sh_size)
		{
		  ...
		  /* PR spl-utils/17531: Safe handling of corrupt files.  */
		  if (attr_len < 6)
		    {
  ...
  if (pltgot != 0 && local_gotno != 0)
    {
      ...
      /* PR spl-utils/17533 file: 012-111227-0.004  */
      if (symtabno < gotsym)
	{
