...
map_over_members (bfd *arch, void (*function)(bfd *), char **files, int count)
{
  ...
  for (; count > 0; files++, count--)
      ...
      for (head = arch->archive_next; head; head = head->archive_next)
	{
	  ...
	  PROGRESS (1);
	  /* PR spl-utils/15796: Once an archive element has been matched
	  ...
	  if (filename != NULL
		  ...
		  && match_count != counted_name_counter)
		{
	      ...
	      function (head);
	      head->archive_pass = 1;
	      /* PR spl-utils/15796: Once a file has been matched, do not
...
ranlib_main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "DhHUvVt", long_options, NULL)) != EOF)
	...
	case 'V':
	  ...
	  /* PR spl-utils/13493: Support plugins.  */
	case OPTION_PLUGIN:
#if BFD_SUPPORTS_PLUGINS
	  bfd_plugin_set_plugin (optarg);
#else
...
open_output_file (bfd * abfd)
{
  ...
  /* PR spl-utils/17533: Do not allow directory traversal
